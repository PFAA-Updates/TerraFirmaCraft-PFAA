...
public class GuiKnapping extends GuiContainerTFC {
    ...
    public static ResourceLocation texture = new ResourceLocation(
        Reference.MOD_ID,
        Reference.ASSET_PATH_GUI + "gui_knapping.png");
    private final Field _selectedButton = ReflectionHelper
        .findField(GuiScreen.class, "selectedButton", "field_146290_a");
    ...
    public void onGuiClosed() {
        PlayerManagerTFC.getInstance()
            .getClientPlayer().knappingInterface = new boolean[25];
        super.onGuiClosed();
    }
    ...
    public void initGui() {
        ...
        for (int y = 0; y < 5; y++) {
            for (int x = 0; x < 5; x++) {
                buttonList
                    .add(new GuiKnappingButton(x + (y * 5), guiLeft + (x * 16) + 10, guiTop + (y * 16) + 12, 16, 16));
                // Normal Behavior
                if (!previouslyLoaded) {
                    if (PlayerManagerTFC.getInstance()
                        .getClientPlayer().knappingInterface[y * 5 + x]) {
                        resetButton(y * 5 + x);
                    }
                }
                // GUI has been reloaded, usually caused by looking up a recipe in NEI while having the interface open.
                    ...
                    /*
                     * For whatever reason all my attempts at implementing this for all crafting types just wouldn't
                     * work for the clay ones.
                     * Types that completely remove pieces (rocks, leather) work properly to save states when reloaded
                     * with this.
                     */
                    if (PlayerManagerTFC.getInstance()
                        .getClientPlayer().specialCraftingType.getItem() != TFCItems.flatClay
                        && ((ContainerSpecialCrafting) this.inventorySlots).craftMatrix.getStackInSlot(y * 5 + x)
                            == null) {
                        resetButton(y * 5 + x);
                    }
                }
    ...
    public void resetButton(int id) {
        if (PlayerManagerTFC.getInstance()
            .getClientPlayer().specialCraftingTypeAlternate == null) {
            ((GuiKnappingButton) this.buttonList.get(id)).visible = false;
        }
        PlayerManagerTFC.getInstance()
            .getClientPlayer().knappingInterface[id] = true;
                ...
                if (this.buttonList.get(i) instanceof GuiButton) {
                    ...
                    if (guiButton.mousePressed(this.mc, mouseX, mouseY)) {
                        try {
                            ...
                            else if (_selectedButton.get(this) == event.button) continue;
                            else this.mouseMovedOrUp(mouseX, mouseY, 0);
