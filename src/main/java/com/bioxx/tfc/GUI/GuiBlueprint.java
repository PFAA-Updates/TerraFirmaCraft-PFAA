...
public class GuiBlueprint extends GuiScreen {
    ...
    public void initGui() {
        ...
        if (!stack.hasTagCompound() || stack.stackTagCompound.getString(ItemBlueprint.TAG_ITEM_NAME)
            .isEmpty()) {
            this.nameTextField.setText("name_it");
            this.nameTextField.setCursorPosition(0);
            this.nameTextField.setSelectionPos(
        ...
        this.buttonList.add(
            new GuiButton(
                DONE_BUTTON,
                (width + xSize) / 2 - 10 - doneWidth,
                (height + ySize) / 2 - 10 - buttonsA,
                doneWidth,
                buttonsA,
        ...
        this.buttonList.add(
            new GuiButton(
                CANCEL_BUTTON,
                (width + xSize) / 2 - 10 - doneWidth - cancelWidth - 4,
                (height + ySize) / 2 - 10 - buttonsA,
                cancelWidth,
                buttonsA,
    ...
    protected void keyTyped(char par1, int par2) {
        ...
        ((GuiButton) this.buttonList.get(DONE_BUTTON)).enabled = this.nameTextField.getText()
            .trim()
            .length() > 0;
        if (par1 == 13) this.actionPerformed((GuiButton) this.buttonList.get(DONE_BUTTON));
    }
    ...
    protected void actionPerformed(GuiButton guibutton) {
        ...
        if (guibutton.id == X_MINUS_BUTTON) xAngle = (xAngle == 0 ? 270 : xAngle - 90);
        else if (guibutton.id == X_PLUS_BUTTON) xAngle = (xAngle == 270 ? 0 : xAngle + 90);
        else if (guibutton.id == Y_MINUS_BUTTON) yAngle = (yAngle == 0 ? 270 : yAngle - 90);
        else if (guibutton.id == Y_PLUS_BUTTON) yAngle = (yAngle == 270 ? 0 : yAngle + 90);
        else if (guibutton.id == Z_MINUS_BUTTON) zAngle = (zAngle == 0 ? 270 : zAngle - 90);
        else if (guibutton.id == Z_PLUS_BUTTON) zAngle = (zAngle == 270 ? 0 : zAngle + 90);
            ...
            ((ItemNBTPacket) pkt).addAcceptedTag(ItemBlueprint.TAG_COMPLETED)
            ...
            if (!stack.stackTagCompound.getBoolean(ItemBlueprint.TAG_COMPLETED)) {
                ...
                AbstractPacket pkt = new ItemNBTPacket(stack.stackTagCompound);
                ((ItemNBTPacket) pkt).addRemoveTag(ItemBlueprint.TAG_COMPLETED)
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        if (!stack.hasTagCompound() || stack.stackTagCompound.getString(ItemBlueprint.TAG_ITEM_NAME)
            ...
            drawCenteredString(
                fontRendererObj,
                TFC_Core.translate("gui.Blueprint"),
                this.width / 2,
                top + 10,
                0x000000);
        else drawCenteredString(fontRendererObj, TFC_Core.translate("gui.Rotate"), this.width / 2, top + 10, 0x000000);

        int axesNameLeft = left + 10;
        int axesAngleLeft = axesNameLeft + fontRendererObj.getStringWidth("X: ")
            + 4
            + 20
            + fontRendererObj.getStringWidth("360") / 2;
