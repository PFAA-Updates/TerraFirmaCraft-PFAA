...
public class GuiCrucible extends GuiContainerTFC {
    ...
    public static ResourceLocation texture = new ResourceLocation(
        Reference.MOD_ID,
        Reference.ASSET_PATH_GUI + "gui_crucible.png");
        ...
        if (crucibleTE.currentAlloy != null) {
            if (crucibleTE.currentAlloy.outputAmount == 0) {
                this.fontRendererObj
                    .drawString(EnumChatFormatting.UNDERLINE + TFC_Core.translate("gui.empty"), 7, 7, 0x000000);
                return;
            } else if (crucibleTE.currentAlloy.outputType != null) {
                this.fontRendererObj.drawString(
                    EnumChatFormatting.UNDERLINE
                        + TFC_Core.translate("gui.metal." + crucibleTE.currentAlloy.outputType.name.replace(" ", "")),
                    7,
                    7,
                    0x000000);
            } else {
                this.fontRendererObj
                    .drawString(EnumChatFormatting.UNDERLINE + TFC_Core.translate("gui.metal.Unknown"), 7, 7, 0x000000);
            }
                    ...
                    this.fontRendererObj
                        .drawString(
                            EnumChatFormatting.DARK_GRAY
                                + TFC_Core.translate(
                                    "gui.metal."
                                        + crucibleTE.currentAlloy.alloyIngred.get(c).metalType.name.replace(" ", ""))
                                + ": "
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        if (crucibleTE.currentAlloy != null) {
            ...
            if (par1 >= 129 + w && par2 >= 6 + h && par1 <= 137 + w && par2 <= 106 + h) {
                String[] text = { String.format("%2.0f", crucibleTE.currentAlloy.outputAmount) };
