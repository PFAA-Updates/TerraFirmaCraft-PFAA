...
public class GuiCustomNametag extends GuiScreen {
    ...
    protected void keyTyped(char par1, int par2) {
        ...
        ((GuiButton) this.buttonList.get(0)).enabled = this.theGuiTextField.getText()
            .trim()
            .length() > 0;
        if (par1 == 13) this.actionPerformed((GuiButton) this.buttonList.get(0));
    }
    ...
    protected void actionPerformed(GuiButton guibutton) {
        if (guibutton.id == 0 && world.isRemote) {
            ...
            Minecraft.getMinecraft()
                .displayGuiScreen(null);// player.closeScreen();
