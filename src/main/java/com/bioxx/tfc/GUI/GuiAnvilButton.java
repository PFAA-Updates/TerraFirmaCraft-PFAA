...
public class GuiAnvilButton extends GuiButton {
    ...
    // private static ResourceLocation texture = new ResourceLocation(Reference.ModID, Reference.AssetPathGui +
    // "anvilicons.png");
    public IIcon icon;
    public int bX;
    public int bY;
    ...
    public GuiAnvilButton(int index, int xPos, int yPos, int width, int height, IIcon ico, int buttonX, int buttonY,
        int buttonW, int buttonH, GuiAnvil gui, String s) {
    ...
    public GuiAnvilButton(int index, int xPos, int yPos, int width, int height, int buttonX, int buttonY, int buttonW,
        int buttonH, GuiAnvil gui, int i, byte r, byte g, byte b) {
            ...
            if (icon == null) {
                ...
                if (screen.anvilTE != null && screen.anvilTE.workRecipe != null) {
                    PlanRecipe p = AnvilManager.getInstance()
                        .getPlan(screen.anvilTE.craftingPlan);
                    if (p == null) return;
     ...
     * mouseX -= k1;
     * mouseY -= l1;
     * return mouseX >= xPosition - 1 && mouseX < xPosition + width + 1 && mouseY >= yPosition - 1 && mouseY < yPosition
     * + height + 1;
     * }
     */
}
