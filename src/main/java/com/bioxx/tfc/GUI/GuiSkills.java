...
public class GuiSkills extends GuiContainerTFC {
    ...
    public static ResourceLocation texture = new ResourceLocation(
        Reference.MOD_ID,
        Reference.ASSET_PATH_GUI + "gui_skills.png");
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        fontRendererObj.drawString(
            TFC_Core.translate("gui.skillpage"),
            this.xSize / 2 - fontRendererObj.getStringWidth(TFC_Core.translate("gui.skillpage")) / 2,
            4,
            4210752,
            false);
        ...
        for (Skill o : SkillsManager.instance.getSkillsArray()) {
            ...
            if (count < (SKILLS_PER_PAGE * skillsPage) + SKILLS_PER_PAGE && count >= (SKILLS_PER_PAGE * skillsPage)) {
                ...
                fontRendererObj.drawString(
                    TFC_Core.translate(o.skillName) + ": "
                        + EnumChatFormatting.DARK_BLUE
                        + ss.getSkillRank(o.skillName)
                            .getLocalizedName(),
                    6,
                    y - 9,
                    0,
    ...
    protected void drawGui(ResourceLocation rl) {
        ...
        drawTexturedModalRect(guiLeft, guiTop, 0, 0, xSize, ySize); // No inventory drawn, so shifted ySize is not
                                                                    // necessary
    }
    ...
    protected void actionPerformed(GuiButton guibutton) {
        if (guibutton.id == 0) Minecraft.getMinecraft()
            .displayGuiScreen(new GuiInventoryTFC(Minecraft.getMinecraft().thePlayer));
        else if (guibutton.id == 2) Minecraft.getMinecraft()
            .displayGuiScreen(new GuiCalendar(Minecraft.getMinecraft().thePlayer));
        else if (guibutton.id == 3) Minecraft.getMinecraft()
            .displayGuiScreen(new GuiHealth(Minecraft.getMinecraft().thePlayer));
        else if (guibutton.id == 4) {
            if (skillsPage > 0) skillsPage--;
        } else if (guibutton.id == 5) {
            if (9 + (skillsPage * SKILLS_PER_PAGE) < SkillsManager.instance.getSkillsArray()
                .size()) skillsPage++;
        }
    }
    ...
    public void updateScreen() {
        super.updateScreen();
        if (skillsPage == 0) ((GuiButton) buttonList.get(4)).enabled = false;
        else((GuiButton) buttonList.get(4)).enabled = true;

        if (9 + (skillsPage * SKILLS_PER_PAGE) < SkillsManager.instance.getSkillsArray()
            .size()) ((GuiButton) buttonList.get(5)).enabled = true;
    ...
    public class GuiButtonPage extends GuiButton {
        ...
        public void drawButton(Minecraft par1Minecraft, int xPos, int yPos) {
            if (this.visible) {
                ...
                // this.drawTexturedModalRect(this.xPosition + this.width / 2, this.yPosition, 200 - this.width / 2, 46
                // + k * 20, this.width / 2, this.height);
                this.mouseDragged(par1Minecraft, xPos, yPos);

                // this.drawCenteredString(fontrenderer,
                // barrel.mode==0?TFC_Core.translate("gui.Barrel.ToggleOn"):TFC_Core.translate("gui.Barrel.ToggleOff"),
