...
public class GuiLargeVessel extends GuiContainerTFC {
    ...
    public static final ResourceLocation TEXTURE = new ResourceLocation(
        Reference.MOD_ID,
        Reference.ASSET_PATH_GUI + "gui_largevessel.png");
    ...
    public void createButtons() {
        buttonList.clear();
        if (guiTab == 0) {
            if (!vesselTE.getSealed()) buttonList
            ...
            if (vesselTE.mode == TEBarrel.MODE_IN) buttonList.add(
                new GuiBarrelTabButton(
                    ...
                    16,
                    16));
            else if (vesselTE.mode == TEBarrel.MODE_OUT) buttonList.add(
                new GuiBarrelTabButton(
                    2,
                    guiLeft + 39,
                    guiTop + 29,
        ...
        public void drawButton(Minecraft par1Minecraft, int xPos, int yPos) {
            if (this.visible) {
                ...
                this.drawCenteredString(
                    fontrenderer,
                    vesselTE.mode == 0 ? TFC_Core.translate("gui.Barrel.ToggleOn")
                        : TFC_Core.translate("gui.Barrel.ToggleOff"),
                    this.xPosition + this.width / 2,
    ...
    public class GuiBarrelTabButton extends GuiButton {
        ...
        public GuiBarrelTabButton(int index, int xPos, int yPos, int width, int height, GuiLargeVessel gui, IIcon icon,
            String s) {
        ...
        public GuiBarrelTabButton(int index, int xPos, int yPos, int width, int height, GuiLargeVessel gui, String s,
    ...
    protected void actionPerformed(GuiButton guibutton) {
        if (guiTab == 0) {
            if (guibutton.id == 0) {
                if (!vesselTE.getSealed()) vesselTE.actionSeal(0, player);
                else vesselTE.actionUnSeal(0, player);
            } else if (guibutton.id == 1) vesselTE.actionEmpty();
            ...
            if (guibutton.id == 1 && vesselTE.getInvCount() == 0) vesselTE.actionSwitchTab(0, player);
            else if (guibutton.id == 2) {
                if (!vesselTE.getSealed()) vesselTE.actionSeal(1, player);
                else vesselTE.actionUnSeal(1, player);
                createButtons();
            }
        }
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int mouseX, int mouseY) {
        ...
        if (guiTab == 0) {
            ...
            if (vesselTE != null && vesselTE.fluid != null) {
                ...
                IIcon liquidIcon = vesselTE.fluid.getFluid()
                    .getIcon(vesselTE.fluid);
                TFC_Core.bindTexture(TextureMap.locationBlocksTexture);
                int color = vesselTE.fluid.getFluid()
                    .getColor(vesselTE.fluid);
                GL11.glColor4ub(
                    (byte) ((color >> 16) & 255),
                    (byte) ((color >> 8) & 255),
                    (byte) (color & 255),
            ...
            // Draw Fluid Name
            if (vesselTE.getFluidStack() != null) drawCenteredString(
                this.fontRendererObj,
                vesselTE.fluid.getFluid()
                    .getLocalizedName(vesselTE.getFluidStack()),
                guiLeft + 88,
                guiTop + 7,
                0x555555);
            ...
            if (vesselTE.sealtime != 0) {
                drawCenteredString(
                    this.fontRendererObj,
                    TFC_Time.getDateStringFromHours(vesselTE.sealtime),
                    guiLeft + 88,
                    guiTop + 17,
                    0x555555);
            }
            ...
            if (vesselTE.recipe != null) {
                if (!(vesselTE.recipe instanceof BarrelBriningRecipe)) {
                    drawCenteredString(
                        this.fontRendererObj,
                        TFC_Core.translate("gui.Output") + ": " + vesselTE.recipe.getRecipeName(),
                        guiLeft + 88,
                        guiTop + 72,
                        0x555555);
                } else if (vesselTE.getSealed() && vesselTE.getFluidStack() != null
                    && vesselTE.getFluidStack()
            ...
            } else if (vesselTE.recipe == null && vesselTE.getSealed()
                && vesselTE.getFluidStack() != null
                && inStack != null
                && inStack.getItem() instanceof IFood
                && vesselTE.getFluidStack()
                    .getFluid() == TFCFluids.VINEGAR
                && !Food.isPickled(inStack)
                && Food.getWeight(inStack) / vesselTE.getFluidStack().amount
                    <= Global.FOOD_MAX_WEIGHT / vesselTE.getMaxLiquid()) {
