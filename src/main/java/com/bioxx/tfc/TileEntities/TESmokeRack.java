...
public class TESmokeRack extends NetworkTileEntity implements IInventory {
    ...
    public void updateEntity() {
        ...
        if (!TFC_Core.isExposedToRain(worldObj, xCoord, yCoord, zCoord)
            && TFC_Time.getTotalHours() > this.lastSmokedTime + 1)
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        if (driedCounter.length == 0) driedCounter = new int[] { 0, 0 };
    }
    ...
    public void setInventorySlotContents(int i, ItemStack itemstack) {
        ...
        if (itemstack != null && !ItemStack.areItemStacksEqual(itemstack, storage[i])) {
            ...
            else driedCounter[i] = (int) TFC_Time.getTotalHours();// Reset the counter if its a new item
            flag = true;
        }
        if (flag) {
            storage[i] = itemstack;
