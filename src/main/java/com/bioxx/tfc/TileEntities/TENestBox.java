...
public class TENestBox extends TileEntity implements IInventory {
    ...
    public EntityAnimal getBird() {
        List list = worldObj.getEntitiesWithinAABB(
            ...
            AxisAlignedBB.getBoundingBox(xCoord + 0.1, yCoord, zCoord + 0.1, xCoord + 0.9, yCoord + 1.1, zCoord + 0.9));
        // Code that returns the closest chicken instead of the first one found, which afaik would be the one found
        // first when the AABB was checked,
        // which might be left to right? either way, takes longer and doesn't neccesarilly produce better results.
        // Chickens do swap nests humourously though.
        /*
         * if(list.size()!=0){
         * EntityAnimal ea = (EntityAnimal)list.get(0);
         * for(Object entity : list){
         * ea = ((EntityAnimal)entity).getDistanceSq(xCoord+0.5,yCoord,zCoord+0.5) <
         * ea.getDistanceSq(xCoord+0.5,yCoord,zCoord+0.5)?(EntityAnimal)entity:ea;
         * }
    ...
    public EntityAnimal getRooster() {
        List list = worldObj.getEntitiesWithinAABB(
            EntityChickenTFC.class,
            AxisAlignedBB.getBoundingBox(xCoord - 5, yCoord, zCoord - 5, xCoord + 5, yCoord + 2, zCoord + 5));
    ...
    public void updateEntity() {
        if (!worldObj.isRemote) {
                        ...
                        if (father != null) {
                            ...
                            nbt.setLong(
                                ...
                                TFC_Time.getTotalTicks()
                                    + (long) (TFCOptions.animalTimeMultiplier * TFC_Time.ticksInMonth * 0.75f));
            ...
            for (int i = 0; i < this.getSizeInventory(); i++) {
                if (inventory[i] != null) {
                    if (inventory[i].getTagCompound() != null && inventory[i].getTagCompound()
                        .hasKey("Fertilized")) {
                        long time = inventory[i].getTagCompound()
                            .getLong("Fertilized");
                        if (time <= TFC_Time.getTotalTicks()) {
                            EntityChickenTFC chick = new EntityChickenTFC(
                                worldObj,
                                xCoord + 0.5,
                                yCoord + 1,
                                zCoord + 0.5,
                                (NBTTagCompound) inventory[i].getTagCompound()
                                    .getTag("Genes"));
