...
public class TEFarmland extends NetworkTileEntity {
    ...
    public void updateEntity() {
        if (!worldObj.isRemote) {
            if (nutrientTimer <= 0) nutrientTimer = TFC_Time.getTotalHours();
            ...
            if (nutrientTimer < TFC_Time.getTotalHours()) {
                ...
                if (worldObj.getBlock(xCoord, yCoord + 1, zCoord) == TFCBlocks.crops) {
                    crop = CropManager.getInstance()
                        .getCropFromId(((TECrop) worldObj.getTileEntity(xCoord, yCoord + 1, zCoord)).cropId);

                    if (crop.cycleType != 0) {
                        if (nutrients[0] < soilMax) nutrients[0] += restoreAmount + crop.nutrientExtraRestore[0];
                    }

                    if (crop.cycleType != 1) {
                        if (nutrients[1] < soilMax) nutrients[1] += restoreAmount + crop.nutrientExtraRestore[1];
                    }

                    if (crop.cycleType != 2) {
                        if (nutrients[2] < soilMax) nutrients[2] += restoreAmount + crop.nutrientExtraRestore[2];
                    }
                } else {
                    if (nutrients[0] < soilMax) nutrients[0] += restoreAmount;
                    if (nutrients[1] < soilMax) nutrients[1] += restoreAmount;
                    if (nutrients[2] < soilMax) nutrients[2] += restoreAmount;
                }

                if (nutrients[0] > soilMax) nutrients[0] = soilMax;
                if (nutrients[1] > soilMax) nutrients[1] = soilMax;
                if (nutrients[2] > soilMax) nutrients[2] = soilMax;
                ...
                if (isInfested) {
                    float temp = TFC_Climate.getHeightAdjustedTempSpecificDay(
                        worldObj,
                        TFC_Time.getDayFromTotalHours(this.nutrientTimer),
                        xCoord,
                        yCoord,
                        zCoord);
                    if (temp > 10 && worldObj.rand.nextInt(10) == 0) {
                        ...
                        if (worldObj.rand.nextInt(5) == 0) uninfest();
                    }
                }
    ...
    public void infest() {
        ...
        if (cd != null && cd.cropInfestation == 0) cd.infest();
    }
    ...
    public void uninfest() {
        ...
        ChunkData cd = TFC_Core.getCDM(worldObj)
            .getData(xCoord >> 4, zCoord >> 4);
        if (cd != null && cd.cropInfestation > 0) cd.uninfest();
    }
    ...
    public void drainNutrients(int type, float multiplier) {
        ...
        if (nutrients[type] < 0) nutrients[type] = 0;
    }
    ...
    public void handleDataPacket(NBTTagCompound nbt) {
        if (worldObj.isRemote) nutrients = nbt.getIntArray("nutrients");
        else broadcastPacketInRange();
    }
