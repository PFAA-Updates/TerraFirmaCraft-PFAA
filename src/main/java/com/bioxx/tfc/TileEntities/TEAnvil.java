...
public class TEAnvil extends NetworkTileEntity implements IInventory {
    ...
    public void updateEntity() {
        ...
        // Disabled auto plan selection until we can solve the issue of correctly determining the lastworker outside of
        ...
        if (!worldObj.isRemote) {
            ...
            if (getItemCraftingValue() > 150) this.setInventorySlotContents(INPUT1_SLOT, null);

            if (workedRecently > 0) workedRecently--;
            ...
            if (workRecipe != null && getItemCraftingValue() != itemCraftingValue) {
                ...
                if (result != null) {
                    AnvilCraftEvent eventCraft = new AnvilCraftEvent(
                        lastWorker,
                        this,
                        anvilItemStacks[INPUT1_SLOT],
                        anvilItemStacks[INPUT2_SLOT],
                        result);
                    ...
                    if (!eventCraft.isCanceled()) {
                        ...
                        // If the lastWorker is not null, then we attempt to apply some crafting buffs to items based on
                        // the players skills
                        ...
                        // We need to call this after the NBT is set, since this call sets lastWorker to null if the
                        // output has no further recipes.
    ...
    public void increaseSkills(AnvilRecipe recipe) {
        if (lastWorker != null) {
            for (String s : recipe.skillsList) {
                TFC_Core.getSkillStats(lastWorker)
                    .increaseSkill(s, recipe.craftingXP);
            }
        }
    }
    ...
    public Object[] getRecipe(AnvilManager manager) {
        ...
        if (itemCraftingValue == workRecipe.getCraftingValue()) {
            out = manager.findCompleteRecipe(
                new AnvilRecipe(
                    anvilItemStacks[INPUT1_SLOT],
                    anvilItemStacks[INPUT2_SLOT],
                    craftingPlan,
                    workRecipe.getCraftingValue(),
                    anvilItemStacks[FLUX_SLOT] != null ? true : false,
                    anvilTier,
                    null),
                getItemRules());
        }
    ...
    public void onSlotChanged(int slot) {
        if (slot == INPUT1_SLOT || slot == INPUT2_SLOT || slot == FLUX_SLOT) updateRecipe();
    }

    public void updateRecipe() {
        AnvilManager manager = AnvilManager.getInstance();
        Object[] plans = manager.getPlans()
            .keySet()
            .toArray();
        ...
        for (Object p : plans) {
            AnvilRecipe ar = manager.findMatchingRecipe(
                new AnvilRecipe(
                    anvilItemStacks[INPUT1_SLOT],
                    anvilItemStacks[INPUT2_SLOT],
                    (String) p,
                    anvilItemStacks[FLUX_SLOT] != null,
                    anvilTier));

            if (ar != null) planList.put((String) p, ar);
        }
        ...
        // This is the core of the process. If the plan list for the input items contains our crafting plan, then we've
        // found our recipe.
        if (planList.containsKey(craftingPlan)) workRecipe = planList.get(craftingPlan);
        else {
            workRecipe = null;
            return;
        }

        // We don't want to allow refined blooms of < 100 units to be refined into wrought iron ingots so we null the
        // recipe unless the amount == 100
        if (anvilItemStacks[INPUT1_SLOT] != null && anvilItemStacks[INPUT1_SLOT].getItem() == TFCItems.bloom
            && workRecipe.getCraftingResult()
                .getItem() == TFCItems.bloom) {
    ...
    private boolean canBeWorked() {
        return isTemperatureWorkable(INPUT1_SLOT) && anvilItemStacks[HAMMER_SLOT] != null
            && (anvilItemStacks[INPUT1_SLOT].getItemDamage() == 0 || anvilItemStacks[INPUT1_SLOT].getItem()
                .getHasSubtypes())
    ...
    public void actionHeavyHammer() {
        ...
        } else this.sendAnvilUsePacket(0);
    }
    ...
    public void actionLightHammer() {
        ...
        } else this.sendAnvilUsePacket(-1);
    }
    ...
    public void actionHammer() {
        ...
        } else this.sendAnvilUsePacket(2);
    }
    ...
    public void actionPunch() {
        ...
        } else this.sendAnvilUsePacket(3);
    }
    ...
    public void actionBend() {
        ...
        } else this.sendAnvilUsePacket(4);
    }
    ...
    public void actionUpset() {
        ...
        } else this.sendAnvilUsePacket(5);
    }
    ...
    public void actionShrink() {
        ...
        } else this.sendAnvilUsePacket(6);
    }

    public void actionWeld() {
        if (!worldObj.isRemote) {
            if (isTemperatureWeldable(WELD1_SLOT) && isTemperatureWeldable(WELD2_SLOT)
                && anvilItemStacks[HAMMER_SLOT] != null
                && (anvilItemStacks[WELD1_SLOT].getItemDamage() == 0 || anvilItemStacks[WELD1_SLOT].getItem()
                    .getHasSubtypes())
                && (anvilItemStacks[WELD2_SLOT].getItemDamage() == 0 || anvilItemStacks[WELD2_SLOT].getItem()
                    .getHasSubtypes())
                && workedRecently == 0
                && anvilItemStacks[WELDOUT_SLOT] == null) {
                AnvilManager manager = AnvilManager.getInstance();
                // new Random(worldObj.getSeed()); // Why is this here?
                AnvilRecipe recipe = new AnvilRecipe(
                    anvilItemStacks[WELD1_SLOT],
                    anvilItemStacks[WELD2_SLOT],
                    "",
                    0,
                    anvilItemStacks[FLUX_SLOT] != null ? true : false,
                    anvilTier,
                ...
                if (result == null) result = manager.findCompleteWeldRecipe(recipe2);

                if (result != null) {
                    TFC_ItemHeat.setTemp(
                        result,
                        (TFC_ItemHeat.getTemp(anvilItemStacks[2]) + TFC_ItemHeat.getTemp(anvilItemStacks[3])) / 2);
    ...
    public ItemStack decrStackSize(int i, int j) {
        if (anvilItemStacks[i] != null) {
            ...
            if (anvilItemStacks[i].stackSize == 0) anvilItemStacks[i] = null;
            return itemstack1;
        } else return null;
    ...
    public void ejectContents() {
        ...
        for (int i = 0; i < getSizeInventory(); i++) {
            if (anvilItemStacks[i] != null) {
                entityitem = new EntityItem(worldObj, xCoord + f, yCoord + f1, zCoord + f2, anvilItemStacks[i]);
    ...
    public int getItemCraftingValue() {
        ...
        if (input != null && input.hasTagCompound()
            && input.getTagCompound()
                .hasKey(ITEM_CRAFTING_VALUE_TAG)) {
            return input.getTagCompound()
                .getShort(ITEM_CRAFTING_VALUE_TAG);
        }
    ...
    public void setPlan(String s) {
        if (worldObj.isRemote) this.sendPlanPacket(s);
        this.craftingPlan = s;
    ...
    public void handleDataPacket(NBTTagCompound nbt) {
        ...
        if (soundFlag) {
            worldObj.playSoundEffect(
                ...
                yCoord,
                zCoord,
                TFC_Sounds.METALIMPACT,
                0.1F,
                0.1F + (worldObj.rand.nextFloat() / 4));
        }
    }
     ...
     * }
     * }
     * worldObj.playSoundEffect(xCoord,yCoord,zCoord, TFC_Sounds.METALIMPACT, 0.1F, 0.1F +
     * (worldObj.rand.nextFloat()/4));
     * }
     * @Override
     ...
     * outStream.writeInt(anvilItemStacks[this.HAMMER_SLOT]!= null ?
     * Item.getIdFromItem(anvilItemStacks[this.HAMMER_SLOT].getItem()) : 0);
     * outStream.writeInt(anvilItemStacks[this.INPUT1_SLOT]!= null ?
     * Item.getIdFromItem(anvilItemStacks[this.INPUT1_SLOT].getItem()) : 0);
     * }
     ...
     * }
     * this.lastWorker = worldObj.getPlayerEntityByName(inStream.readUTF());
     * worldObj.playSoundEffect(xCoord, yCoord, zCoord, TFC_Sounds.METALIMPACT, 0.1F, 0.5F + (worldObj.rand.nextFloat()
     * / 2));
     * }
     */
}
