...
public class TEBlastFurnace extends TEFireEntity implements IInventory {
    ...
    private Boolean checkValidity() {
        int y = yCoord + 1;
        if (this.isStackValid(xCoord, y, zCoord)) return true;
        return false;
    }
    ...
    public void cookItem(int i) {
         ...
         */
        if (cookingItemStack != null && crucibleTE != null
            && crucibleTE.getTotalMetal() < TECrucible.MAX_UNITS
            ...
            if (index != null && TFC_ItemHeat.getTemp(cookingItemStack) >= index.meltTemp) {
                ...
                if (cookingItem instanceof ISmeltable) {
                    ...
                    if (!crucibleTE.addMetal(((ISmeltable) cookingItem).getMetalType(cookingItemStack), output)) return; // Do
                                                                                                                         // not
                                                                                                                         // decrease
                                                                                                                         // fuel
                                                                                                                         // or
                    ...
                    output = index.getOutput(cookingItemStack, r)
                        .getItemDamage();
                    if (m != null) {
                        // Attempt to add metal to crucible.
                        if (!crucibleTE.addMetal(m, (short) (100 - output))) return; // Do not decrease fuel or ore if
                                                                                     // the crucible is too full.
                    }
                }
    ...
    public void receiveAirFromBellows() {
        if (storage[1] != null) super.receiveAirFromBellows();
    }
    ...
    public boolean isStackValid(int i, int j, int k) {
        ...
        for (int num = 0; num < 5; num++) {
            if (!((BlockBlastFurnace) TFCBlocks.blastFurnace).checkStackAt(worldObj, i, j + num, k)) break;
            maxValidStackSize++;
        }
                            ...
                            for (; c > 0; c--) {
                                if (getTotalCount() < 40 && oreCount < (this.maxValidStackSize * 4)) {
                                    if (foundFlux(moltenCount)
                                        && addOreToFire(new ItemStack(item, 1, itemstack.getItemDamage())))
                                        oreCount += 1;
                                    else nonConsumedOre++;
                                } else {
                                    nonConsumedOre++;
    ...
    private boolean foundFlux(int moltenCount) {
        ...
        for (Iterator iterator = list.iterator(); iterator.hasNext() && !found;) {
            ...
            if (!entity.isDead && is.getItemDamage() == 0 && is.getItem() == TFCItems.powder) {
                ...
                else entity.setEntityItemStack(is);
                found = true;
            }
        }
        return found;
    ...
    public void onDataPacket(NetworkManager net, S35PacketUpdateTileEntity pkt) {
        ...
        GuiScreen gui = FMLClientHandler.instance()
            .getClient().currentScreen;
        if (gui instanceof GuiBlastFurnace) ((GuiBlastFurnace) gui).updateScreen();
    }
