    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            if (crop != null && growthTimer < time && sunLevel > 0) {
                ...
                if (te instanceof TEFarmland) tef = (TEFarmland) te;

                float ambientTemp = TFC_Climate.getHeightAdjustedTempSpecificDay(
                    worldObj,
                    TFC_Time.getDayOfYearFromTick(growthTimer),
                    xCoord,
                    yCoord,
                    zCoord);
                float tempAdded = 0;
                ...
                if (!crop.dormantInFrost && ambientTemp < crop.minGrowthTemp)
                    ...
                    if (growth > 1) tempAdded = -0.03f * (crop.minGrowthTemp - ambientTemp);
                    isDormant = true;
                } else if (ambientTemp < 28) tempAdded = ambientTemp * 0.00035f;
                else if (ambientTemp < 37) tempAdded = (28 - (ambientTemp - 28)) * 0.0003f;

                if (!crop.dormantInFrost && ambientTemp < crop.minAliveTemp) {
                    int baseKillChance = 6;
                    if (this.worldObj.rand.nextInt(baseKillChance - this.killLevel) == 0) killCrop(crop);
                    else {
                        if (killLevel < baseKillChance - 1) this.killLevel++;
                    }
                ...
                if (tef != null && !isDormant) {
                    if (tef.nutrients[nutriType] > 0) tef.drainNutrients(nutriType, crop.nutrientUsageMult);
                    // Drain Fertilizer
                    if (tef.nutrients[3] > 0) tef.drainNutrients(3, crop.nutrientUsageMult);
                }

                float growthRate = Math.max(
                    0.0f,
                    (((crop.numGrowthStages / (crop.growthTime * TFC_Time.timeRatio96) + tempAdded) * nutriMult)
                        * timeMultiplier) * TFCOptions.cropGrowthMultiplier);
                if (tef != null && tef.isInfested) growthRate /= 2;
                int oldGrowth = (int) Math.floor(growth);

                if (!isDormant) growth += growthRate;
                ...
                if ((TFCOptions.enableCropsDie || !TFC_Core.isFarmland(worldObj.getBlock(xCoord, yCoord - 1, zCoord)))
                    && (crop.maxLifespan == -1 && growth > crop.numGrowthStages + ((float) crop.numGrowthStages / 2))
                    || growth < 0) {
                    killCrop(crop);
                }
            ...
            if (TFC_Core.isExposedToRain(worldObj, xCoord, yCoord, zCoord)
                && TFC_Climate.getHeightAdjustedTemp(worldObj, xCoord, yCoord, zCoord) < 0) {
    ...
    public void onHarvest(World world, EntityPlayer player, boolean isBreaking) {
        if (!world.isRemote) {
            ...
            if (crop != null && growth >= crop.numGrowthStages - 1) {
                ...
                int skill = 20 - (int) (20 * TFC_Core.getSkillStats(player)
                    .getSkillMultiplier(Global.SKILL_AGRICULTURE));

                seedStack.stackSize = 1 + (world.rand.nextInt(1 + skill) == 0 ? 1 : 0);
                if (isBreaking) world
                    .spawnEntityInWorld(new EntityItem(world, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, seedStack));

                TFC_Core.getSkillStats(player)
                    .increaseSkill(Global.SKILL_AGRICULTURE, 1);
