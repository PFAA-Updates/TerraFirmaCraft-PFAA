...
public class TEBarrel extends NetworkTileEntity implements IInventory {
    ...
    public void setUnsealed(String reason) {
        if ("killing fuse".equals(reason)) sealed = false;
    }
    ...
    public int getInvCount() {
        ...
        if (storage[INPUT_SLOT] != null && count == 1) return 0;
        return count;
    }
    ...
    public int getGunPowderCount() {
        int count = 0;
        for (ItemStack is : storage) {
            if (is != null && is.getItem() == Items.gunpowder) count += is.stackSize;
        }
        return count;
    }
    ...
    public boolean addLiquid(FluidStack inFS) {
        if (inFS != null) {
            ...
            if (inFS.getFluid() != null && inFS.getFluid()
                .getTemperature(inFS) > Global.HOT_LIQUID_TEMP) return false;
    ...
    public void drainLiquid(int amount) {
        if (!getSealed() && this.getFluidStack() != null) {
            ...
            if (this.getFluidStack().amount <= 0) this.actionEmpty();
            else worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
        }
    }

    public int getLiquidScaled(int i) {
        if (fluid != null) return fluid.amount * i / getMaxLiquid();
        return 0;
    }
    ...
    public void handleDataPacket(NBTTagCompound nbt) {
        ...
        if (!worldObj.isRemote) {
            ...
            if (nbt.hasKey("SealTime")) sealtime = nbt.getInteger("SealTime");
        }
    }

    protected void switchTab(EntityPlayer player, int tab) {
        if (player != null)
            if (tab == 0) player.openGui(TerraFirmaCraft.instance, 35, worldObj, xCoord, yCoord, zCoord);
            else if (tab == 1) player.openGui(TerraFirmaCraft.instance, 36, worldObj, xCoord, yCoord, zCoord);
    }
    ...
    public void updateEntity() {
        if (!worldObj.isRemote) {
            ...
            // If the stack is > its own max stack size then we split it and add it to the invisible solid storage area
            // or
                ...
                if (container != null && container.getItem() instanceof IFluidContainerItem) {
                    ...
                    if (isfs != null && addLiquid(isfs)) {
                        ((IFluidContainerItem) container.getItem())
                            .drain(container, ((IFluidContainerItem) container.getItem()).getCapacity(container), true);
                    }
    ...
    public void processItems() {
        if (this.getInvCount() == 0) {
            // Before we handle standard barrel processing we have to see if we are handling cheese and run that code
            // first
            // since it has to be handled specially.
            boolean isCheese = handleCheese();

            if (getFluidStack() != null && !isCheese) {
                recipe = BarrelManager.getInstance()
                ...
                if (recipe != null && !worldObj.isRemote) {
                    ...
                    if (!resultStacks.isEmpty()) {
                        ...
                        while (!resultStacks.isEmpty()) worldObj
                            .spawnEntityInWorld(new EntityItem(worldObj, xCoord, yCoord, zCoord, resultStacks.pop()));
    ...
    private boolean handleCheese() {
        ...
        if (this.getSealed() && this.fluid != null
            && this.fluid.getFluid() == TFCFluids.MILKCURDLED
            && (inIS == null
                || inIS.getItem() instanceof IFood && ((IFood) inIS.getItem()).getFoodGroup() != EnumFoodGroup.Dairy
            ...
            if (!worldObj.isRemote) {
                ...
                if (inIS != null && inIS.getItem() instanceof IFood) {
                    ...
                    float ratio = Math.min(
                        (Food.getWeight(getInputStack()) - Food.getDecay(inIS)) / (Global.FOOD_MAX_WEIGHT / 8),
                        1.0f);
    ...
    public static ItemStack createFullBarrel(FluidStack f, ItemStack is) {
        ...
        is.getTagCompound()
            .setBoolean("Sealed", true);
        // nbt.setInteger("mode", mode);
        NBTTagCompound fluidNBT = new NBTTagCompound();
        if (f != null) f.writeToNBT(fluidNBT);
        is.getTagCompound()
            .setTag("fluidNBT", fluidNBT);
    ...
    public static void registerRecipes() {
        ...
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelRecipe(
                    ...
                    null,
                    new FluidStack(TFCFluids.MILKCURDLED, 10000)).setMinTechLevel(0)
                        .setRemovesLiquid(false));
        // BarrelManager.getInstance().addRecipe(new BarrelRecipeNoItem(new FluidStack(TFCFluid.MILKCURDLED, 10000),
        // ItemFoodTFC.createTag(new ItemStack(TFCItems.Cheese), 160), null).setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelRecipe(
                    ...
                    null,
                    new FluidStack(TFCFluids.TANNIN, 1000)).setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelRecipe(
        ...
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelMultiItemRecipe(
                    ...
                    new FluidStack(TFCFluids.FRESHWATER, 300)).setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelMultiItemRecipe(
                    ...
                    new FluidStack(TFCFluids.FRESHWATER, 400)).setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelMultiItemRecipe(
                    ...
                    new FluidStack(TFCFluids.FRESHWATER, 500)).setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelMultiItemRecipe(
                    ...
                    new FluidStack(TFCFluids.LIMEWATER, 300)).setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelMultiItemRecipe(
                    ...
                    new FluidStack(TFCFluids.LIMEWATER, 400)).setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelMultiItemRecipe(
                    ...
                    new FluidStack(TFCFluids.LIMEWATER, 500)).setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelMultiItemRecipe(
                    new ItemStack(TFCItems.prepHide, 1, 0),
                    ...
                    new FluidStack(TFCFluids.TANNIN, 300)).setKeepStackSize(false)
                        .setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelMultiItemRecipe(
                    ...
                    new FluidStack(TFCFluids.TANNIN, 400)).setKeepStackSize(false)
                        .setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelMultiItemRecipe(
                    ...
                    new FluidStack(TFCFluids.TANNIN, 500)).setKeepStackSize(false)
                        .setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelRecipe(
                    ...
                    new FluidStack(TFCFluids.LIMEWATER, 100)).setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelRecipe(
        ...
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelVinegarRecipe(
                    new FluidStack(TFCFluids.WHISKEY, 100),
                    new FluidStack(TFCFluids.VINEGAR, 100)));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelVinegarRecipe(
                    new FluidStack(TFCFluids.RYEWHISKEY, 100),
                    new FluidStack(TFCFluids.VINEGAR, 100)));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelVinegarRecipe(new FluidStack(TFCFluids.BEER, 100), new FluidStack(TFCFluids.VINEGAR, 100)));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelVinegarRecipe(new FluidStack(TFCFluids.SAKE, 100), new FluidStack(TFCFluids.VINEGAR, 100)));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelVinegarRecipe(new FluidStack(TFCFluids.RUM, 100), new FluidStack(TFCFluids.VINEGAR, 100)));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelVinegarRecipe(
        ...
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelLiquidToLiquidRecipe(
                    ...
                    new FluidStack(TFCFluids.BRINE, 10000)).setSealedRecipe(false)
                        .setMinTechLevel(0)
                        .setRemovesLiquid(false));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelBriningRecipe(new FluidStack(TFCFluids.BRINE, 60), new FluidStack(TFCFluids.BRINE, 60))
                    .setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelMultiItemRecipe(
                    ...
                    new FluidStack(TFCFluids.FRESHWATER, 60)).setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelMultiItemRecipe(
                    ...
                    new FluidStack(TFCFluids.FRESHWATER, 200)).setMinTechLevel(0));
        BarrelManager.getInstance()
            .addRecipe(
                new BarrelLiquidToLiquidRecipe(
                    new FluidStack(TFCFluids.MILK, 9000),
                    new FluidStack(TFCFluids.VINEGAR, 1000),
                    new FluidStack(TFCFluids.MILKVINEGAR, 10000)).setSealedRecipe(false)
                        .setMinTechLevel(0)
                        .setRemovesLiquid(false));
        BarrelManager.getInstance()
            .addRecipe(
        ...
        BarrelPreservativeRecipe picklePreservative = new BarrelPreservativeRecipe(
            new FluidStack(TFCFluids.VINEGAR, 31),
            "gui.barrel.preserving").setAllowGrains(false)
                .setRequiresPickled(true)
                .setEnvironmentalDecayFactor(0.25f)
                .setBaseDecayModifier(0.1f)
                .setRequiresSealed(true);
        BarrelPreservativeRecipe brineInBrinePreservative = new BarrelPreservativeRecipe(
            new FluidStack(TFCFluids.BRINE, 62),
            "gui.barrel.preserving").setAllowGrains(false)
                .setRequiresBrined(true)
                .setEnvironmentalDecayFactor(0.75f)
                .setRequiresSealed(true);
        BarrelPreservativeRecipe brineInVinegarPreservative = new BarrelPreservativeRecipe(
            new FluidStack(TFCFluids.VINEGAR, 62),
            "gui.barrel.preserving").setAllowGrains(false)
                .setRequiresBrined(true)
                .setEnvironmentalDecayFactor(0.75f)
                .setRequiresSealed(true);
        BarrelManager.getInstance()
            .addPreservative(picklePreservative);
        BarrelManager.getInstance()
            .addPreservative(brineInBrinePreservative);
        BarrelManager.getInstance()
            .addPreservative(brineInVinegarPreservative);
    }
}
