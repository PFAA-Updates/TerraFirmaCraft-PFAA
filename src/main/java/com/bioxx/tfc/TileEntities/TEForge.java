...
public class TEForge extends TEFireEntity implements IInventory {
    ...
    private boolean validateSmokeStack() {
        if (!TFC_Core.isExposedToRain(worldObj, xCoord, yCoord, zCoord)) return true;
        else if (checkChimney(xCoord + 1, yCoord + 1, zCoord)) return true;
        else if (checkChimney(xCoord - 1, yCoord + 1, zCoord)) return true;
        else if (checkChimney(xCoord, yCoord + 1, zCoord + 1)) return true;
        else if (checkChimney(xCoord, yCoord + 1, zCoord - 1)) return true;
        else if (notOpaque(xCoord + 1, yCoord + 1, zCoord) && checkChimney(xCoord + 2, yCoord + 1, zCoord)) return true;
        else if (notOpaque(xCoord - 1, yCoord + 1, zCoord) && checkChimney(xCoord - 2, yCoord + 1, zCoord)) return true;
        else if (notOpaque(xCoord, yCoord + 1, zCoord + 1) && checkChimney(xCoord, yCoord + 1, zCoord + 2)) return true;
        else return notOpaque(xCoord, yCoord + 1, zCoord - 1) && checkChimney(xCoord, yCoord + 1, zCoord - 2);
    }
    ...
    private boolean notOpaque(int x, int y, int z) {
        return worldObj.blockExists(x, y, z) && !worldObj.getBlock(x, y, z)
            .isOpaqueCube();
    }

    private void genSmokeRoot(int x, int y, int z) {
        if (fuelTimeLeft >= 0) {
            if (worldObj.getBlock(x, y, z) != TFCBlocks.smoke) worldObj.setBlock(x, y, z, TFCBlocks.smoke);
        } else {
    ...
    public void cookItem(int i) {
        ...
        if (fireItemStacks[i] != null) {
            ...
            if (index != null && TFC_ItemHeat.getTemp(fireItemStacks[i]) > index.meltTemp) {
                ...
                // If not unshaped metal, morph the input to the output. If not an input with direct morph (sand,
                // sticks, etc) this deletes the input item from the slot.
                if (!(fireItemStacks[i].getItem() instanceof ItemMeltedMetal)) fireItemStacks[i] = index.getMorph();
                ...
                if (fireItemStacks[i] != null) {
                    ...
                    if (inputCopy.getItem() instanceof ISmeltable) {
                        ...
                        if (inputCopy.getItem() instanceof ItemBloom) units = Math.min(100, units);
    ...
    public ItemStack decrStackSize(int i, int j) {
        if (fireItemStacks[i] != null) {
            ...
            if (fireItemStacks[i].stackSize == 0) fireItemStacks[i] = null;
            return isSplit;
        } else return null;
    }
    ...
    public int getMoldIndex() {
        if (fireItemStacks[10] != null && fireItemStacks[10].getItem() == TFCItems.ceramicMold) return 10;
        if (fireItemStacks[11] != null && fireItemStacks[11].getItem() == TFCItems.ceramicMold) return 11;
        if (fireItemStacks[12] != null && fireItemStacks[12].getItem() == TFCItems.ceramicMold) return 12;
        if (fireItemStacks[13] != null && fireItemStacks[13].getItem() == TFCItems.ceramicMold) return 13;
        return -1;
    }
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            // Play the fire sound
            Random r = new Random();
            if (r.nextInt(10) == 0 && fireTemp > 20) worldObj
                .playSoundEffect(xCoord, yCoord, zCoord, "fire.fire", 0.4F + (r.nextFloat() / 2), 0.7F + r.nextFloat());
            ...
            for (int c = 0; c < 5; c++) {
                if (fireItemStacks[c] != null) {
                    if (fireItemStacks[c].stackSize <= 0) fireItemStacks[c].stackSize = 1;
                }
            }
        }
    }
    ...
    private void createSmoke() {
        if (!TFCOptions.generateSmoke) return;

        if (checkChimney(xCoord + 1, yCoord + 1, zCoord)) genSmokeRoot(xCoord + 1, yCoord + 1, zCoord);
        else if (checkChimney(xCoord - 1, yCoord + 1, zCoord)) genSmokeRoot(xCoord - 1, yCoord + 1, zCoord);
        else if (checkChimney(xCoord, yCoord + 1, zCoord + 1)) genSmokeRoot(xCoord, yCoord + 1, zCoord + 1);
        else if (checkChimney(xCoord, yCoord + 1, zCoord - 1)) genSmokeRoot(xCoord, yCoord + 1, zCoord - 1);
    ...
    public void removeSmoke() {
        if (isSmoke(xCoord, yCoord + 1, zCoord)) worldObj.setBlockToAir(xCoord, yCoord + 1, zCoord);
        else if (isSmoke(xCoord + 1, yCoord + 1, zCoord)) worldObj.setBlockToAir(xCoord + 1, yCoord + 1, zCoord);
        else if (isSmoke(xCoord - 1, yCoord + 1, zCoord)) worldObj.setBlockToAir(xCoord - 1, yCoord + 1, zCoord);
        else if (isSmoke(xCoord, yCoord + 1, zCoord + 1)) worldObj.setBlockToAir(xCoord, yCoord + 1, zCoord + 1);
        else if (isSmoke(xCoord, yCoord + 1, zCoord - 1)) worldObj.setBlockToAir(xCoord, yCoord + 1, zCoord - 1);
        else if (isSmoke(xCoord + 2, yCoord + 1, zCoord)) worldObj.setBlockToAir(xCoord + 2, yCoord + 1, zCoord);
        else if (isSmoke(xCoord - 2, yCoord + 1, zCoord)) worldObj.setBlockToAir(xCoord - 2, yCoord + 1, zCoord);
        else if (isSmoke(xCoord, yCoord + 1, zCoord + 2)) worldObj.setBlockToAir(xCoord, yCoord + 1, zCoord + 2);
        else if (isSmoke(xCoord, yCoord + 1, zCoord - 2)) worldObj.setBlockToAir(xCoord, yCoord + 1, zCoord - 2);
    }
