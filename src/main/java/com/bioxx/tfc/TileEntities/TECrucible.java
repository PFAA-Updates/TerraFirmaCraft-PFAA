...
public class TECrucible extends NetworkTileEntity implements IInventory {
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        ...
        Iterator<MetalPair> iter = metals.values()
            .iterator();
        while (iter.hasNext()) {
    ...
    public void updateEntity() {
        if (!worldObj.isRemote) {
            ...
            if (worldObj.getBlock(xCoord, yCoord - 1, zCoord) == TFCBlocks.forge) {
                ...
                if (te.fireTemp >= 1 && TFCOptions.enableDebugMode) temperature = 2000;
                else if (te.fireTemp > temperature) temperature++;
            }
            if (tempTick > 22) {
                tempTick = 0;
                if (temperature > TFC_Climate.getHeightAdjustedTemp(worldObj, xCoord, yCoord, zCoord)) temperature--;
            }
            ...
            if (stackToSmelt != null) {
                ...
                int maxDamage = stackToSmelt.getMaxDamage() - 1; // Subtract one so we never have an unshaped metal with
                                                                 // 0/100 units
                ...
                if (itemToSmelt instanceof ItemMeltedMetal && TFC_ItemHeat.getIsLiquid(storage[0])) {
                    if (inputTick > 10) {
                        ...
                        if (currentAlloy != null && currentAlloy.outputType != null
                            && itemToSmelt == currentAlloy.outputType.meltedItem) {
                ...
                } else if (itemToSmelt instanceof ISmeltable && ((ISmeltable) itemToSmelt).isSmeltable(stackToSmelt)
                    ...
                    && cookDelay == 0) {
                        ...
                        if (addMetal(mType, ((ISmeltable) itemToSmelt).getMetalReturnAmount(stackToSmelt))) {
                            temperature *= 0.9f;
                            cookDelay = 40;
                            if (stackToSmelt.stackSize <= 1) storage[0] = null;
                            else storage[0].stackSize--;
                            updateGui((byte) 0);
                        }
                    }
            }
            // Metal Output handling
            if (currentAlloy != null && storage[1] != null
                ...
                if (storage[1].getItem() == TFCItems.ceramicMold) {
                    ...
                    if (storage[1].getItem() == currentAlloy.outputType.meltedItem && storage[1].getItemDamage() > 0) {
            ...
            if (storage[1] != null && storage[1].stackSize <= 0) storage[1].stackSize = 1;
            if (inputTick > 10) inputTick = 0;
            if (outputTick >= 2) outputTick = 0;
        }
    }

    public boolean drainOutput(float amount) {
        if (metals != null && metals.values()
            .size() > 0) {
    ...
    public boolean addMetal(Metal m, float amt) {
        if (getTotalMetal() + amt <= MAX_UNITS && m.name != null && !"Unknown".equals(m.name)) {
            if (metals.containsKey(m.name)) metals.get(m.name).amount += amt;
            else metals.put(m.name, new MetalPair(m, amt));
    ...
    public float getTotalMetal() {
        ...
        while (iter.hasNext()) {
            MetalPair m = iter.next();
            if (m != null) totalAmount += m.amount;
        }
        return totalAmount;
    }
    ...
    public int getOutCountScaled(int length) {
        if (currentAlloy != null) return ((int) this.currentAlloy.outputAmount * length) / MAX_UNITS;
        else return 0;
    }
    ...
    public void handleDataPacket(NBTTagCompound nbt) {
        ...
        if (action == 0) this.currentAlloy = new Alloy().fromNBT(nbt);
        else if (action == 1 && currentAlloy != null) {
            currentAlloy.outputAmount = nbt.getFloat("outputAmount");
        } else if (action == 2) currentAlloy = null;
