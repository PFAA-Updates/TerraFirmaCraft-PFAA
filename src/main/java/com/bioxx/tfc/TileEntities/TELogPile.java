...
public class TELogPile extends TileEntity implements IInventory {
    ...
    public ItemStack takeLog(int slot) {
        if (storage[slot] == null) return null;
            ...
            if (storage[slot].stackSize == 0) storage[slot] = null;
            if (this.getNumberOfLogs() == 0) worldObj.setBlockToAir(xCoord, yCoord, zCoord);
            return is;
        }
    }
    ...
    public boolean contentsMatch(int index, ItemStack is) {
        return storage[index] != null && storage[index].getItem() == is.getItem()
    ...
    public void ejectContents() {
        for (int i = 0; i < getSizeInventory(); i++) {
            if (storage[i] != null) {
                worldObj
                    .spawnEntityInWorld(new EntityItem(worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, storage[i]));
            }
        }
        extinguishFire();
    }
    ...
    public void lightNeighbors() {
        ...
        if (!TFC_Core.isValidCharcoalPitCover(block)) blocksToBeSetOnFire.add(new Vector3f(xCoord + 1, yCoord, zCoord));

        block = worldObj.getBlock(xCoord - 1, yCoord, zCoord);
        if (!TFC_Core.isValidCharcoalPitCover(block)) blocksToBeSetOnFire.add(new Vector3f(xCoord - 1, yCoord, zCoord));

        block = worldObj.getBlock(xCoord, yCoord, zCoord + 1);
        if (!TFC_Core.isValidCharcoalPitCover(block)) blocksToBeSetOnFire.add(new Vector3f(xCoord, yCoord, zCoord + 1));

        block = worldObj.getBlock(xCoord, yCoord, zCoord - 1);
        if (!TFC_Core.isValidCharcoalPitCover(block)) blocksToBeSetOnFire.add(new Vector3f(xCoord, yCoord, zCoord - 1));

        block = worldObj.getBlock(xCoord, yCoord + 1, zCoord);
        if (!TFC_Core.isValidCharcoalPitCover(block)) blocksToBeSetOnFire.add(new Vector3f(xCoord, yCoord + 1, zCoord));
    ...
    public void createCharcoal(int x, int y, int z, boolean forceComplete) {
        if (worldObj.getBlock(x, y, z) == TFCBlocks.logPile) {
            TELogPile te = (TELogPile) worldObj.getTileEntity(x, y, z);
            if (te.isOnFire
                && (te.fireTimer + TFCOptions.charcoalPitBurnTime < TFC_Time.getTotalHours() || forceComplete)) {
