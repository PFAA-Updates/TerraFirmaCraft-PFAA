...
public class TEIngotPile extends NetworkTileEntity implements IInventory {
    ...
    public String type;
    public static final Item[] INGOTS = new Item[] { TFCItems.bismuthIngot, TFCItems.bismuthBronzeIngot,
        TFCItems.blackBronzeIngot, TFCItems.blackSteelIngot, TFCItems.blueSteelIngot, TFCItems.brassIngot,
    ...
    public void addContents(int index, ItemStack is) {
        if (storage[index] == null) storage[index] = is;
        updateNeighbours();
    }
    ...
    public boolean contentsMatch(int index, ItemStack is) {
        if (storage[index] != null) {
            if (storage[index].stackSize == 0) return true;
        }

        return storage[index].getItem() == is.getItem() && storage[index].getItem() == is.getItem() &&
        /* storage[index].stackSize < storage[index].getMaxStackSize() && */ storage[index].stackSize + 1
    ...
    public void updateNeighbours() {
        if (worldObj.blockExists(xCoord, yCoord + 1, zCoord) && !worldObj.isAirBlock(xCoord, yCoord + 1, zCoord))
            worldObj.getBlock(xCoord, yCoord + 1, zCoord)
                .onNeighborBlockChange(worldObj, xCoord, yCoord + 1, zCoord, TFCBlocks.ingotPile);
    }
    ...
    public void createInitNBT(NBTTagCompound nbt) {
        if (this.type != null) nbt.setString("type", this.type);
        if (storage[0] != null) {
