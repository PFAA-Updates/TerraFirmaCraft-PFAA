...
public class TEBloomery extends NetworkTileEntity {
    ...
    public boolean isStackValid(int i, int j, int k) {
        ...
        if (yNegBlock != TFCBlocks.molten && worldObj.getBlock(i, j - 1, k)
            .getMaterial() != Material.rock
            && !worldObj.getBlock(i, j - 1, k)
                .isNormalCube()
            && yNegBlock != TFCBlocks.charcoal) {
            return false;
        }
        return ((BlockEarlyBloomery) TFCBlocks.bloomery)
            .checkStack(worldObj, xCoord, j, zCoord, worldObj.getBlockMetadata(xCoord, yCoord, zCoord) & 3);
    }
    ...
    public boolean canLight() {
        if (!worldObj.isRemote) {
            if (this.charcoalCount < this.oreCount || oreCount == 0) return false;
            ...
            if (bid == TFCBlocks.charcoal && worldObj.getBlockMetadata(x, yCoord, z) >= 7 && !bloomeryLit) {
                bloomeryLit = true;
                this.fuelTimeLeft = (long) (TFC_Time.getTotalTicks()
                    + (TFC_Time.HOUR_LENGTH * TFCOptions.bloomeryBurnTime));
                if ((meta & 4) == 0) worldObj.setBlockMetadataWithNotify(xCoord, yCoord, zCoord, meta + 4, 3);
                return true;
            }
        }
        return false;
    ...
    public void updateEntity() {
        if (!worldObj.isRemote) {
            ...
            if (worldObj.blockExists(x, yCoord, z)) {
                if (bloomeryLit && TFC_Time.getTotalTicks() > fuelTimeLeft) {
                    if (worldObj.getBlock(x, yCoord, z) == TFCBlocks.molten) {
                        if (worldObj.setBlock(x, yCoord, z, TFCBlocks.bloom)) {
                            ...
                            /*
                             * Was causing any blocks above the chimney to be removed in weird places depending on
                             * molten count.
                             * Bloomery appears to be working just fine and removing the molten blocks without line. Not
                             * sure why it was here.
                             * -Kitty
                             */
                ...
                for (int i = bloomeryLit ? 0 : 1, j = bloomeryLit ? moltenHeight + 7 : moltenHeight; j
                    > 0; i++, j -= 8) {
                    ...
                    if ((bid.isAir(worldObj, x, yCoord + i, z) || bid == TFCBlocks.molten || bid == TFCBlocks.charcoal)
                        && worldObj.getBlock(x, yCoord - 1, z)
                            .getMaterial() == Material.rock) {
                        // Make sure that the Stack is surrounded by rock
                        if (isStackValid(x, yCoord + i, z)) validCount++;

                        if (i <= validCount) {
                            int mMeta = worldObj.getBlockMetadata(xCoord, yCoord + i, zCoord);
                            int m = j > 7 ? 7 : j;
                            if (this.bloomeryLit) {
                                if (bid == TFCBlocks.molten && (mMeta & 8) == 0 || bid.isAir(worldObj, x, yCoord + i, z)
                                    || bid == TFCBlocks.charcoal) {
                                    m += 8;
                                    worldObj.setBlock(x, yCoord + i, z, TFCBlocks.molten, m, 2);
                                }
                            } else {
                                if (count > 0) worldObj.setBlock(x, yCoord + i, z, TFCBlocks.molten, m, 2);
                                else worldObj.setBlockToAir(x, yCoord + i, z);
                            }
                        } else {
                ...
                List list = worldObj.getEntitiesWithinAABB(
                    EntityItem.class,
                    AxisAlignedBB.getBoundingBox(x, yCoord, z, x + 1, yCoord + (maxCount / 8) + 1.1, z + 1));
                ...
                /*
                 * Make sure the list isn't null or empty, that the stack is valid 1 layer above the Molten Ore, and
                 * there are no players inside
                 */
                if (list != null && !list.isEmpty() && !bloomeryLit && (playerList == null || playerList.isEmpty())) {
                    ...
                    for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                        ...
                        if (entity.getEntityItem()
                            .getItem() == TFCItems.coal
                            && entity.getEntityItem()
                                .getItemDamage() == 1 /* || entity.getEntityItem().getItem() == TFCItems.Coke */) {
                        ...
                        /*
                         * If the item that's been tossed in is a type of Ore and it can melt down into something then
                         * add the ore to the list of items in the fire.
                         */
                        else if (entity.getEntityItem()
                            .getItem() instanceof ItemOre
                            && ((ItemOre) entity.getEntityItem()
                                .getItem()).isSmeltable(entity.getEntityItem())) {
                                    int c = entity.getEntityItem().stackSize;
                                    while (c > 0) {
                                        if (charcoalCount + oreCount < (2 * maxCount) && oreCount < maxCount
                                            && outCount < 1000) {
                                            if (addOreToFire(
                                                new ItemStack(
                                                    entity.getEntityItem()
                                    ...
                                    if (c == 0) entity.setDead();
                                    else entity.getEntityItem().stackSize = c;
                                } else
                            if (entity.getEntityItem()
                                .getItem() instanceof ISmeltable
                                && ((ISmeltable) entity.getEntityItem()
                                    .getItem()).isSmeltable(entity.getEntityItem())) {
                                        int c = entity.getEntityItem().stackSize;
                                        while (c > 0) {
                                            if (((ISmeltable) entity.getEntityItem()
                                                .getItem()).getMetalReturnAmount(entity.getEntityItem()) < 100
                                                && oreCount < maxCount
                                                && outCount < 1000) {
                                                if (addOreToFire(
                                                    new ItemStack(
                ...
                if (this.validationCheck <= 0) {
                    if (((BlockEarlyBloomery) worldObj.getBlock(xCoord, yCoord, zCoord))
                        .canBlockStay(worldObj, xCoord, yCoord, zCoord)) validationCheck = 600;
                    else {
                        worldObj.setBlockToAir(xCoord, yCoord, zCoord);
                        worldObj.spawnEntityInWorld(
                            new EntityItem(worldObj, xCoord, yCoord, zCoord, new ItemStack(TFCBlocks.bloomery, 1)));
                    }
