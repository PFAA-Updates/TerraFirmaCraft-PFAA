...
public class TEFoodPrep extends NetworkTileEntity implements IInventory {
    ...
    public int getFoodIdFromItemStack(ItemStack is) {
        if (is != null) {
            if (is.getItem() instanceof IFood) return ((IFood) is.getItem()).getFoodID();
            else if (is.getItem() instanceof IItemFoodBlock) return ((IItemFoodBlock) is.getItem()).getFoodId(is);
        }
        return 1;
    }
    ...
    public int getHealAmountFromItemStack(ItemStack is) {
        if (is != null) {
            if (is.getItem() instanceof IFood) return ((IFood) is.getItem()).getFoodID();
            else if (is.getItem() instanceof IItemFoodBlock) return ((IItemFoodBlock) is.getItem()).getHealAmount(is);
        }
        return 1;
    }
    ...
    public void actionCreate(EntityPlayer player) {
        if (!worldObj.isRemote) {
            if (lastTab == 0) createSandwich(player);
            else if (lastTab == 1) createSalad(player);
        } else {
    ...
    private void createSandwich(EntityPlayer player) {
        if (validateSandwich())// Bread
        {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                if (f != null && Food.getWeight(f) >= sandwichWeights[i]) w += sandwichWeights[i];
            }
    ...
    private void setSandwichIcon(ItemStack is) {
        if (getStackInSlot(0).getItem() == TFCItems.wheatBread) is.setItemDamage(0);
        else if (getStackInSlot(0).getItem() == TFCItems.oatBread) is.setItemDamage(1);
        else if (getStackInSlot(0).getItem() == TFCItems.barleyBread) is.setItemDamage(2);
        else if (getStackInSlot(0).getItem() == TFCItems.ryeBread) is.setItemDamage(3);
        else if (getStackInSlot(0).getItem() == TFCItems.cornBread) is.setItemDamage(4);
        else if (getStackInSlot(0).getItem() == TFCItems.riceBread) is.setItemDamage(5);
    }
    ...
    private void createSalad(EntityPlayer player) {
        if (validateSalad())// Bread
        {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (f != null && Food.getWeight(f) >= saladWeights[i]) w += saladWeights[i];
            }
    ...
    public boolean validateSandwich() {
        if (lastTab == 0) {
            ...
            if (!validateIngreds(storage[1], storage[2], storage[3], storage[4])) return false;

            float weight = 0;
            for (int i = 0; i < 5; i++) {
                ItemStack f = getStackInSlot(i);
                if (f != null && f.getItem() instanceof IFood
                    && Food.getWeight(f) - Food.getDecay(f) >= sandwichWeights[i]) {
                    weight += sandwichWeights[i];
                }
            }

            if (weight < 7) return false;
        }
        return true;
    }
    ...
    public boolean validateIngreds(ItemStack... is) {
        for (int i = 0; i < is.length; i++) {
            if (is[i] != null && !((IFood) is[i].getItem()).isUsable(is[i])) return false;
            for (int j = 0; j < is.length; j++) {

                if (j != i && !compareIngred(is[i], is[j])) return false;
            }
        }
        return true;
    }
    ...
    public void openGui(EntityPlayer player) {
        if (lastTab == 0) player.openGui(TerraFirmaCraft.instance, 44, worldObj, xCoord, yCoord, zCoord);
        else if (lastTab == 1) player.openGui(TerraFirmaCraft.instance, 45, worldObj, xCoord, yCoord, zCoord);
    }
    ...
    private long getIconSeed() {
        ...
        for (int i = 1; i < 5; i++) {
            ItemStack is = getStackInSlot(i);
            if (is != null) seed += ((ItemFoodTFC) is.getItem()).getFoodID();
        }
        return seed + worldObj.getSeed();
    }
    ...
    public void consumeFoodWeight(float[] weights, ItemStack... isArray) {
        for (int i = 0; i < isArray.length; i++) {
            ...
            if (is != null) {
                ...
                if (newW <= 0 || newW <= Food.getDecay(is)) is.stackSize = 0;
            }
        }
    }
    ...
    public void setInventorySlotContents(int i, ItemStack itemstack) {
        if (!TFC_Core.areItemsEqual(storage[i], itemstack)) worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
        storage[i] = itemstack;
    }
    ...
    public void closeInventory() {
        ...
        /*
         * else if(storage[0] == null && storage[1] == null && storage[2] == null && storage[3] == null && storage[4] ==
         * null && storage[5] == null
         * && storage[6] == null && storage[7] == null && storage[8] == null && storage[9] == null && storage[10] ==
         * null)
         * {
