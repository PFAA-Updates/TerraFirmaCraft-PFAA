...
public class TEFirepit extends TEFireEntity implements IInventory {
        ...
        if (fireItemStacks[1] != null) {
            ...
            if (index != null && TFC_ItemHeat.getTemp(fireItemStacks[1]) > index.meltTemp) {
                ...
                if (output != null) {
                    ...
                    if (output.getItem() == fireItemStacks[1].getItem()) damage = fireItemStacks[1].getItemDamage();
                    ...
                    if (fireItemStacks[1].getItem() instanceof ItemMeltedMetal) {
                        ...
                        // Otherwise if the first output has an item that doesnt match the input item then put the item
                        // in the second output slot
                ...
                if (output != null && output.getItem() instanceof ItemMeltedMetal) {
                    ...
                    if (fireItemStacks[7] != null && output.getItem() == fireItemStacks[7].getItem()
                        ...
                        && fireItemStacks[8].getItemDamage() > 0) {
                            int amt1 = 100 - damage;// the percentage of the output
                            int amt2 = 100 - fireItemStacks[8].getItemDamage();// the percentage currently in the out
                                                                               // slot
                            int amt3 = amt1 + amt2;// combined amount
                            leftover = amt3 - 100;// assign the leftover so that we can add to the other slot if
                                                  // applicable
                            if (leftover > 0) addLeftover = true;
                            fromSide = 1;
                            int amt4 = 100 - amt3;// convert the percent back to mc damage
                    ...
                    if (addLeftover) {
                        ...
                        if (fireItemStacks[dest] != null && output.getItem() == fireItemStacks[dest].getItem()
                            ...
                            int amt2 = 100 - fireItemStacks[dest].getItemDamage();// the percentage currently in the out
                                                                                  // slot
                            int amt3 = amt1 + amt2;// combined amount
                            int amt4 = 100 - amt3;// convert the percent back to mc damage
                            if (amt4 < 0) amt4 = 0;// stop the infinite glitch
    ...
    public ItemStack decrStackSize(int slot, int amount) {
        if (fireItemStacks[slot] != null) {
            ...
            if (fireItemStacks[slot].stackSize == 0) fireItemStacks[slot] = null;
            return itemstack1;
        } else return null;
    }
            ...
            if (index != null) {
                ...
                if (fuelTimeLeft > 0 && is.getItem() instanceof ICookableFood) {
                    ...
                    if (Food.isCooked(is)) {
                        ...
                        Random r = new Random(
                            ((ICookableFood) is.getItem()).getFoodID() + (((int) Food.getCooked(is) - 600) / 120));
    ...
    public void updateEntity() {
        if (!worldObj.isRemote) {
            ...
            if (list != null && !list.isEmpty() && fireItemStacks[0] == null) // Only go through the list if more fuel
                                                                              // can fit.
            {
                ...
                for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                    ...
                    if (item == TFCItems.logs || item == Item.getItemFromBlock(TFCBlocks.peat)) {
                        for (int c = 0; c < is.stackSize; c++) {
                            ...
                            {
                                /**
                                 * Place a copy of only one of the logs into the fuel slot, due to the stack limitation
                                 * of the fuel slots.
                                 * Do not change to fireItemStacks[0] = is;
                                 */
                                setInventorySlotContents(0, new ItemStack(item, 1, is.getItemDamage()));
                                is.stackSize--;
                                handleFuelStack(); // Attempt to shift the fuel down so more fuel can be added within
                                                   // the same for loop.
                            }
                        }
            ...
            } else if (fuelTimeLeft <= 0 && fireTemp >= 1
    ...
    private void smokeBlock(int x, int y, int z) {
        if (worldObj.blockExists(x, y, z) && worldObj.getBlock(x, y, z) == TFCBlocks.smokeRack
            ...
            if (broadcast) te.broadcastPacketInRange();
        }
    }
    ...
    public void generateSmoke() {
         ...
         * if(worldObj.rand.nextInt(10) == 0) worldObj.spawnParticle("largesmoke", f+f4 - 0.2F, f1, f2 + f5 + 0.2F,
         * 0.0D, 0.0D, 0.0D);
         * }
         */
    }
