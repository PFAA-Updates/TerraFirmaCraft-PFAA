...
public class TEDetailed extends NetworkTileEntity {
        ...
        for (int subX = 0; subX < 8; subX++) {
            for (int subZ = 0; subZ < 8; subZ++) {
                for (int subY = 0; subY < 8; subY++) {
                    if (!getBlockExists(subX, subY, subZ)) setQuad(subX, subY, subZ);
                }
            }
        }
    ...
    public void handleDataPacket(NBTTagCompound nbt) {
        ...
        if (packetType == TEDetailed.PACKET_UPDATE) {
            ...
            PlayerManagerTFC.getInstance()
                .getPlayerInfoFromPlayer(entityplayer).chiselMode = nbt.getByte("chiselMode");
            ...
            ((BlockDetailed) TFCBlocks.detailed)
                .onBlockActivatedServer(worldObj, xCoord, yCoord, zCoord, this.entityplayer, 0, 0, 0, 0);
    ...
    public static BitSet turnCube(byte[] bytes, int xAngle, int yAngle, int zAngle) {
        if (xAngle == 0 && yAngle == 0 && zAngle == 0) return fromByteArray(bytes, 512);

        BitSet data = fromByteArray(bytes, 512);
        BitSet turnedData = new BitSet(512);

        int xCoord, yCoord, zCoord;
        for (int x = 0; x < 8; ++x) for (int z = 0; z < 8; ++z) for (int y = 0; y < 8; ++y) {
            xCoord = x;
            yCoord = y;
            zCoord = z;
