...
public class TEQuern extends NetworkTileEntity implements IInventory {
    ...
    public boolean processItem() {
        if (storage[0] != null) {
            QuernRecipe qr = QuernManager.getInstance()
                .findMatchingRecipe(storage[0]);
            if (qr == null) {
                TerraFirmaCraft.LOG.warn(
                    "QUERN RECIPE NOT FOUND! This is a BUG! -- " + storage[0].getItem()
                        .getUnlocalizedName());
                return false; // If this happens, it's a bug!
            }

            // If the output item can not be combined or is different from what is being made, eject and make room for
            // the new output
            if (storage[1] != null && !(storage[1].getItem() == qr.getResult()
                .getItem() && storage[1].getItemDamage()
                    == qr.getResult()
                        .getItemDamage())) {
                ejectItem(storage[1]);
                storage[1] = null;
            ...
            if (qr.getInItem()
                ...
                if (storage[1] == null) storage[1] = qr.getResult()
                    .copy();
                    ...
                    if ((qr.getResult().stackSize + storage[1].stackSize) > storage[1].getMaxStackSize()) {
                        int amountleft = qr.getResult().stackSize
                            - (storage[1].getMaxStackSize() - storage[1].stackSize);
                        ItemStack tossStack = qr.getResult()
                            .copy();
                        tossStack.stackSize = tossStack.getMaxStackSize();
                        ejectItem(tossStack);
                        ItemStack remainStack = qr.getResult()
                            .copy();
    ...
    public ItemStack decrStackSize(int slot, int amount) {
        if (storage[slot] != null) {
            ...
            if (storage[slot].stackSize == 0) setInventorySlotContents(slot, null);
            return itemstack1;
        } else return null;
    }
