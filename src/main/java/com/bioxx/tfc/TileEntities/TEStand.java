...
public class TEStand extends TileEntity implements IInventory {
    ...
    public void destroy() {
        if (!isTop && entity != null) entity.setDead();
    }
    ...
    public void setInventorySlotContents(int i, ItemStack itemstack) {
        if (!isTop) {
            if (items[i] == null || itemstack == null) items[i] = itemstack;
    ...
    public void updateEntity() {
        if (!worldObj.isRemote && !isTop) {
            ...
            if (hasEntity && entity == null) {
                ...
                if (!list.isEmpty()) entity = (EntityStand) list.get(0);
    ...
    public void readFromNBT(NBTTagCompound nbttagcompound) {
        ...
        for (int i = 0; i < nbttaglist.tagCount(); i++) {
            ...
            if (byte0 >= 0 && byte0 < items.length) items[byte0] = ItemStack.loadItemStackFromNBT(nbttagcompound1);
        }
    }
