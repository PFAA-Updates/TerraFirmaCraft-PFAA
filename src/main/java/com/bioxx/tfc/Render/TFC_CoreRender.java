...
public class TFC_CoreRender {
    ...
    public static boolean renderBlockSlab(Block block, int x, int y, int z, RenderBlocks renderblocks) {
        ...
        if (!breaking) renderblocks.overrideBlockTexture = tex;
    ...
    public static boolean renderBlockStairs(Block block, int x, int y, int z, RenderBlocks renderblocks) {
        if (renderer == null) renderer = new RenderBlocksLightCache(renderblocks);
        else renderer.update(renderblocks);
        ...
        IIcon myTexture = renderblocks.overrideBlockTexture == null ? Block.getBlockById(type)
            .getIcon(0, temeta) : renderblocks.overrideBlockTexture;
    ...
    public static boolean renderWoodTrunk(Block block, int i, int j, int k, RenderBlocks renderblocks) {
        ...
        /*
         * if(blockAccess.getBlockMaterial(i, j+1, k) == Material.leaves || blockAccess.getBlockMaterial(i, j-1, k) ==
         * Material.leaves ||
         * blockAccess.getBlock(i, j+1, k) == mod_TFC_Core.fruitTreeWood || blockAccess.getBlock(i, j-1, k) ==
         * mod_TFC_Core.fruitTreeWood)
         */
        if (blockAccess.getTileEntity(i, j, k) != null
            && (blockAccess.getBlock(i, j - 1, k) == TFCBlocks.fruitTreeWood || blockAccess.getBlock(i, j - 1, k)
                .isOpaqueCube())) {
            renderblocks.setRenderBounds(0.3F, 0.0F, 0.3F, 0.7F, 1.0F, 0.7F);
            renderblocks.renderStandardBlock(block, i, j, k);
        }
        if (blockAccess.getBlock(i - 1, j, k)
            .getMaterial() == Material.leaves || blockAccess.getBlock(i - 1, j, k) == TFCBlocks.fruitTreeWood) {
        ...
        if (blockAccess.getBlock(i, j, k - 1)
            .getMaterial() == Material.leaves || blockAccess.getBlock(i, j, k - 1) == TFCBlocks.fruitTreeWood) {
        ...
        if (!((TEFruitTreeWood) blockAccess.getTileEntity(i, j, k)).isTrunk
            && blockAccess.getBlock(i, j - 1, k) != TFCBlocks.fruitTreeWood
            && !blockAccess.getBlock(i, j - 1, k)
                .isOpaqueCube()) {
            renderblocks.setRenderBounds(0.0F, 0.4F, 0.4F, 0.5F, 0.6F, 0.6F);
            renderblocks.renderStandardBlock(block, i, j, k);
    ...
    public static boolean renderLooseRock(Block block, int i, int j, int k, RenderBlocks renderblocks) {
        ...
        DataLayer rockLayer1 = TFC_Climate.getCacheManager(w)
            .getRockLayerAt(i, k, 0);
    ...
    public static boolean renderOre(Block block, int xCoord, int yCoord, int zCoord, float par5, float par6, float par7,
         ...
         * renderblocks.overrideBlockTexture = getRockTexture(ModLoader.getMinecraftInstance().theWorld, xCoord, yCoord,
         * zCoord);
    ...
    public static IIcon getRockTexture(World world, int xCoord, int yCoord, int zCoord) {
        ...
        DataLayer rockLayer1 = TFC_Climate.getCacheManager(world)
            .getRockLayerAt(xCoord, zCoord, 0);
        DataLayer rockLayer2 = TFC_Climate.getCacheManager(world)
            .getRockLayerAt(xCoord, zCoord, 1);
        DataLayer rockLayer3 = TFC_Climate.getCacheManager(world)
            .getRockLayerAt(xCoord, zCoord, 2);

        if (yCoord <= TFCOptions.rockLayer3Height) var27 = rockLayer3.block.getIcon(5, rockLayer3.data2);
        else if (yCoord <= TFCOptions.rockLayer2Height) var27 = rockLayer2.block.getIcon(5, rockLayer2.data2);
        else var27 = rockLayer1.block.getIcon(5, rockLayer1.data2);
        return var27;
    }
    ...
    public static boolean renderSluice(Block block, int i, int j, int k, RenderBlocks renderblocks) {
        ...
        if (!BlockSluice.isBlockFootOfBed(meta)) {
            if (dir == 0) {
                ...
                if ((meta & 4) != 0) renderblocks.uvRotateTop = 1;
                for (int count = 0; count < 4; count++) {
                    // ribs
                    renderblocks.setRenderBounds(
                        0.95F - (0.25 * count),
                ...
                if ((meta & 4) != 0) renderblocks.uvRotateTop = 3;
                for (int count = 0; count < 4; count++) {
                    // ribs
                    renderblocks.setRenderBounds(
                        0.0F,
                ...
                if ((meta & 4) != 0) renderblocks.uvRotateTop = 2;
                for (int count = 0; count < 4; count++) {
                    // ribs
                    renderblocks.setRenderBounds(
                        0.0F + (0.25 * count),
            ...
            if (dir == 0) {
                for (int count = 0; count < 4; count++) {
                    ...
                    renderblocks.setRenderBounds(
                        ...
                        0.05F + (0.25 * count),
                        1,
                        Math.max(0.3125 - (0.125 * count), 0.01),
                        0.25F + (0.25 * count));
                    renderblocks.renderStandardBlock(block, i, j, k);
                }
            }
    ...
    public static boolean renderFruitLeaves(Block block, int xCoord, int yCoord, int zCoord,
        ...
        renderblocks.renderStandardBlock(block, xCoord, yCoord, zCoord);
        if (index != null && (index.inBloom(TFC_Time.getSeasonAdjustedMonth(zCoord))
            || index.inHarvest(TFC_Time.getSeasonAdjustedMonth(zCoord)))) {
    ...
    public static boolean renderSeaPlant(Block par1Block, int par2, int par3, int par4, RenderBlocks renderblocks) {
        ...
        if (te instanceof TEWaterPlant) {
            ...
            if (wp.getBlockFromType() != null) {
                substrateRender = renderblocks
                    .renderStandardBlockWithColorMultiplier(wp.getBlockFromType(), par2, par3, par4, 1, 1, 1);
