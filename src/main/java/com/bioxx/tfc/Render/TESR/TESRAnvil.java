...
public class TESRAnvil extends TESRBase {
    ...
    public void renderAt(TEAnvil te, double d, double d1, double d2, float f) {
        if (te.getWorldObj() != null) {
            int dir = BlockAnvil.getDirectionFromMetadata(
                te.getWorldObj()
    ...
    private void drawItem(TEAnvil te, double d, double d1, double d2, int dir, EntityItem customitem, float blockScale,
        int i) {
        if (te.getStackInSlot(i) != null) {
            float[] pos = getLocation(dir, i, Block.getBlockById(te.stonePair[0]) != null);
            if (Block.getBlockById(te.stonePair[0]) != Blocks.air) pos[1] += 0.3f;
            GL11.glPushMatrix(); // start
            {
                GL11.glTranslatef((float) d + pos[0], (float) d1 + pos[1], (float) d2 + pos[2]);
                if (RenderManager.instance.options.fancyGraphics) GL11.glRotatef(pos[3], pos[4], pos[5], pos[6]);
