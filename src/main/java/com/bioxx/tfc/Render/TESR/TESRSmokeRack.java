...
public class TESRSmokeRack extends TESRBase {
    ...
    public void drawItem(TESmokeRack te, int index, double minX, double maxX, double minZ, double maxZ) {
        if (te.storage[index] != null /* && te.storage[index].getItem() instanceof Item */) // That will always ben an
                                                                                            // instanceof Item, perhaps
                                                                                            // this is supposed to check
                                                                                            // somethign else?
        {
            float minU = te.storage[index].getIconIndex()
                .getMinU();
            float maxU = te.storage[index].getIconIndex()
                .getMaxU();
            float minV = te.storage[index].getIconIndex()
                .getMinV();
            float maxV = te.storage[index].getIconIndex()
                .getMaxV();
