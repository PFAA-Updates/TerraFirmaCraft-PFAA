...
public class ModelFrond extends ModelBox {
    ...
    public ModelFrond(ModelRenderer renderer, int textureOffsetX, int textureOffsetY, float originX, float originY,
        ...
        for (int i = 0; i < 18; i++) {
            // blades[i+6] =
            // createBlade(originX,originY,originZ,i<9?((i/2f)-((i/9f)*3))+(float)Math.pow((9-i)/9f,1.1):((18-i)/2f)-(((18-i)/9f)*3),1.2f*(i+6));
            blades[i + 6] = createBlade(
                originX,
                originY,
                originZ,
                (3 - ((float) Math.pow(i - 12, 2) / 60f)) * lengthMod,
                0,
                1.2f * (i + 6));
        }
        for (int i = 0; i < 6; i++) {
            blades[i + 24] = createBlade(
                originX,
                originY,
                originZ,
                (2 - (float) Math.pow(i, 2) / 4) * lengthMod,
                0,
                1.2f * (i + 24));
        }
        ...
        for (int i = 0; i < 30; i++) {
            quadList[i] = new TexturedQuad(
                (PositionTextureVertex[]) blades[i],
    ...
    private PositionTextureVertex[] createBlade(float originX, float originY, float originZ, float modifierX,
        float modifierY, float modifierZ) {

        PositionTextureVertex vert0 = new PositionTextureVertex(originX, originY, originZ + modifierZ, 0.0F, 0.0F);
        PositionTextureVertex vert1 = new PositionTextureVertex(
            originX + length + modifierX,
            originY + modifierY,
            originZ + modifierZ + (modifierZ / 15f),
            0.0F,
            8.0F);
        PositionTextureVertex vert2 = new PositionTextureVertex(originX, originY, originZ + 1 + modifierZ, 8.0F, 8.0F);
        PositionTextureVertex vert3 = new PositionTextureVertex(
            originX + length + modifierX,
            originY + modifierY,
            originZ + 1 + modifierZ + (modifierZ / 15f),
            8.0F,
            0.0F);
