...
public class RenderMetalTrapDoor implements ISimpleBlockRenderingHandler {
    ...
    public static boolean render(Block block, int i, int j, int k, RenderBlocks renderer) {
        ...
        if (BlockMetalTrapDoor.isTrapdoorOpen(access.getBlockMetadata(i, j, k))) {
            ...
            renderer
                .setRenderBounds(fx + 0.0001f, fy + 0.0001f, fz + 0.0001f, fx2 - 0.0001f, fy2 - 0.0001f, fz2 - 0.0001f);
        ...
        int hingeID = te.sheetStack != null
            ? Math.min(((BlockMetalSheet) TFCBlocks.metalSheet).icons.length - 1, te.sheetStack.getItemDamage() >> 5)
            : 0;

        boolean breaking = renderer.overrideBlockTexture != null;

        if (!breaking) renderer.setOverrideBlockTexture(((BlockMetalSheet) TFCBlocks.metalSheet).icons[hingeID]);
