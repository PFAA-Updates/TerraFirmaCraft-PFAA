...
public class RenderPottery implements ISimpleBlockRenderingHandler {
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        ...
        if (te.straw > 0) {
            if (!breaking) renderer.overrideBlockTexture = TFCBlocks.thatch.getIcon(world, x, y, z, 0);
        ...
        if (te.wood > 0) {
            if (!breaking) renderer.overrideBlockTexture = TFCBlocks.logPile.getIcon(0, 0);
