...
public class RenderBloomery implements ISimpleBlockRenderingHandler {
    ...
    public static boolean render(Block block, int i, int j, int k, RenderBlocks renderblocks) {
        ...
        if (te != null) if (te.isFlipped) dir = BlockEarlyBloomery.flipDir(dir);
        float f = 0.125F;
        // float xMin = 0.0625f;
        // float yMin = 0.0625f;
        // float zMin = 0.0625f;
        ...
        if (!BlockEarlyBloomery.isOpen(meta)) {
            switch (dir) {
                case 0:
                    renderblocks.setRenderBounds(0.0F, 0.0F, 0.0f, 1.0f, 1.0F, f);
                    break;
                case 1:
                    renderblocks.setRenderBounds(1.0f - f, 0.0F, 0.0F, 1.0F, 1.0F, 1.0f);
                    break;
                case 2:
                    renderblocks.setRenderBounds(0.0f, 0.0F, 1.0f - f, 1.0F, 1.0F, 1.0F);
                    break;
                case 3:
                    renderblocks.setRenderBounds(0.0F, 0.0F, 0.0F, f, 1.0F, 1.0F);
                    break;
            }
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        ...
        if (modelId == TFCBlocks.bloomeryRenderId) return render(block, x, y, z, renderer);
        return false;
    }
