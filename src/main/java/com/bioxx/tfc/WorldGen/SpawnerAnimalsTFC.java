...
public final class SpawnerAnimalsTFC {
    ...
    public static boolean canCreatureTypeSpawnAtLocation(EnumCreatureType par0EnumCreatureType, World par1World,
        ...
        if (par0EnumCreatureType.getCreatureMaterial() == Material.water) {
            return par1World.getBlock(par2, par3, par4)
                .getMaterial()
                .isLiquid()
     ...
     */
    public static void performWorldGenSpawning(World world, TFCBiome biome, int par2, int par3, int par4, int par5,
        Random par6Random) {
        ...
        if (!list.isEmpty()) {
            while (par6Random.nextFloat() < biome.getSpawningChance()) {
                ...
                int i1 = spawnlistentry.minGroupCount
                    + par6Random.nextInt(1 + spawnlistentry.maxGroupCount - spawnlistentry.minGroupCount);
                ...
                for (int j2 = 0; j2 < i1; ++j2) {
                    ...
                    for (int k2 = 0; !flag && k2 < 4; ++k2) {
                        ...
                        if (canCreatureTypeSpawnAtLocation(EnumCreatureType.creature, world, j1, l2, k1)) {
                            ...
                            if (entityliving instanceof EntityFishTFC) {
                                if (entityliving.getRNG()
                                    .nextInt((int) ChunkData.FISH_POP_MAX)
                                    > TFC_Core.getCDM(world)
                                        .getFishPop(j1 >> 4, k1 >> 4)) {
                                    return;
                                }
                            }
                        ...
                        for (k1 += par6Random.nextInt(5) - par6Random.nextInt(5); j1 < par2 || j1 >= par2 + par4
                            || k1 < par3
