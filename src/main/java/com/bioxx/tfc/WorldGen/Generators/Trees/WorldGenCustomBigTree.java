...
public class WorldGenCustomBigTree extends WorldGenerator {
    ...
    private int checkBlockLine(int[] startCoords, int[] endCoords) {
        ...
        for (newAxis = 0; axis < 3; ++axis) {
            ...
            if (Math.abs(distance[axis]) > Math.abs(distance[newAxis])) newAxis = axis;
        }
        ...
        if (distance[newAxis] == 0) {
            ...
            if (distance[newAxis] > 0) direction = 1;
            else direction = -1;
            ...
            for (endPoint = distance[newAxis] + direction; result != endPoint; result += direction) {
                ...
                if (!(block.isAir(worldObj, coords[0], coords[1], coords[2]) || block == TFCBlocks.leaves
                    || block == TFCBlocks.leaves2)) break;
            }
    ...
    private void generateLeafNodeBases() {
        ...
        for (int[] location = new int[] { this.basePos[0], this.basePos[1], this.basePos[2] }; i
            < this.leafNodes.length; ++i) {
            ...
            if (this.leafNodeNeedsBase(baseHeight)) this.placeBlockLine(location, newLocation, TFCBlocks.logNatural);
        }
    }
            ...
            if (var8 < 0.0F) {
                ...
                for (double var9 = 0.5D; var7 < var1; ++var7) {
                    ...
                    if (this.checkBlockLine(var17, var18) == -1) {
                        ...
                        if (var17[1] - var22 > treeTop) var19[1] = treeTop;
        ...
        for (coords[axis] = startCoords[axis]; width1 <= range; ++width1) {
            ...
            while (width2 <= range) {
                double distance = Math
                    .sqrt(Math.pow(Math.abs(width1) + 0.5D, 2.0D) + Math.pow(Math.abs(width2) + 0.5D, 2.0D));
                ...
                if (distance > leafSizeAtHeight) {
                    ...
                    Block block = this.worldObj.getBlock(coords[0], coords[1], coords[2]);
                    if (worldObj.isAirBlock(coords[0], coords[1], coords[2]) || block == TFCBlocks.leaves
    ...
    private float layerSize(int height) {
        if (height < this.heightLimit * 0.3D) {
            ...
            if (leafBase == 0.0F) size = halfLimit;
            else if (Math.abs(leafBase) >= halfLimit) size = 0.0F;
            else size = (float) Math.sqrt(Math.pow(Math.abs(halfLimit), 2.0D) - Math.pow(Math.abs(leafBase), 2.0D));
    ...
    private float leafSize(int height) {
        return height >= 0 && height < this.leafDistanceLimit
            ? height != 0 && height != this.leafDistanceLimit - 1 ? 3.0F : 2.0F
            : -1.0F;
    }
    ...
    private void placeBlockLine(int[] bottom, int[] top, Block b) {
        ...
        for (newAxis = 0; axis < 3; ++axis) {
            ...
            if (Math.abs(location[axis]) > Math.abs(location[newAxis])) newAxis = axis;
        }
        ...
        if (location[newAxis] != 0) {
            ...
            if (location[newAxis] > 0) direction = 1;
            else direction = -1;
    ...
    public void setScale(double heightScale, double widthScale, double densityScale) {
        ...
        if (heightScale > 0.5D) this.leafDistanceLimit = 5;
