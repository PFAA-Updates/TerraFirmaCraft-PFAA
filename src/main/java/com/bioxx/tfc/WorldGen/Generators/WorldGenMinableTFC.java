...
public class WorldGenMinableTFC extends WorldGenerator {
    ...
    public WorldGenMinableTFC(Block block, int j, Block layerB, int layerMeta, int rarity, int veinSize, int veinAmount,
    ...
    public boolean betterOreDistribution(int xChunk, int zChunk, Block mPMinableBlock, int mPMinableBlockMeta, int min,
        int max, Random rand) {
    ...
    public boolean betterOreDistributionVein(int xChunk, int zChunk, Block mPMinableBlock, int mPMinableBlockMeta,
        int min, int max, Random rand) {
        if (rand.nextInt(rarity) == 0) {
            for (int loopCount = 0; loopCount < veinAm; loopCount++) {
                int temp1 = mPCalculateDensity(rand, diameter, hDens);
                int temp2 = mPCalculateDensityVert(rand, height, vDens, min, max);
    ...
    public boolean bODgenerateVein(World world, Random rand, int parX, int parY, int parZ, int xyz) {
        ...
        for (int blocksMade = 0; blocksMade <= blocksToUse;) // make veins
        {
            ...
            for (int blocksMade1 = 0; blocksMade1 <= blocksToUse2;) // make branch
            {
                ...
                boolean isCorrectRockType = world.getBlock(posX, posY, posZ) == layerBlock;
                boolean isCorrectMeta = m == layerMeta || layerMeta == -1;
                if (isCorrectRockType && isCorrectMeta) world.setBlock(posX, posY, posZ, mPBlock, mPBlockMeta, 0x2);
                blocksMade++;
                blocksMade1++;
            }
    ...
    public boolean generate(World world, Random random, int x, int z, int min, int max, String n)// absorb default
                                                                                                 // system
    {
        ...
        worldObj = world;
        // rand = random;
        if (mPChunkX != mPPrevX || mPChunkZ != mPPrevZ
            || mPBlock != mPPrevBlock
            || mPBlock == mPPrevBlock && mPBlockMeta != mPPrevMeta
            || mPlayerBlock != mPPrevLayerBlock
            || mPlayerMeta != mPPrevLayerMeta)// if it is a new x or y chunk or is a new ore, then generate
        {
            mPPrevX = mPChunkX;
            mPPrevZ = mPChunkZ;
            mPPrevBlock = mPBlock;
    ...
    public boolean generateVein(World world, Random random, int x, int z, int min, int max, String n)// absorb default
                                                                                                     // system
    {
        mPChunkX = x >> 4;// set output chunk x
        mPChunkZ = z >> 4;// set output chunk z
    ...
    public int mPCalculateDensity(Random rand, int oreDist, float oreDens) // returns the density value
    {
        ...
        dValPassInr = (int) (oreDist / oreDens + .5F); // distance devided by number of times it will loop, establishes
                                                       // the number for randomization
        dValPass = 0;
    ...
    // ======================================================================================
    public int mPCalculateDensityVert(Random rand, int oreDist, float oreDens, int min, int max) // returns the density
                                                                                                 // value
    {
        int lpCnt = 0;
        int dValPassInr = 0;
