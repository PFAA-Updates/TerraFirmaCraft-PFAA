...
public class WorldGenDouglasFir extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int par3, int par4, int par5) {
        ...
        if (rand.nextInt(20) == 0) tall = true;
        if (tall) i += rand.nextInt(10);

        if (par4 < 1 || par4 + i + 1 > 256) return false;
        ...
        if (!TFC_Core.isSoil(world.getBlock(par3, par4 - 1, par5)) || par4 >= 256 - i - 1) return false;
        ...
        for (int k1 = par4 + (i / 3) - 1; k1 <= par4 + i - 1; k1++) {
            ...
            if (i > 20) z = 20;
            int x = z / 10 + 1;
            if (k1 - par4 > i / 2 || k1 - par4 - (i / 3) + 2 < 3) x--;
            if (par4 + i - k1 < 4) x = 1;
