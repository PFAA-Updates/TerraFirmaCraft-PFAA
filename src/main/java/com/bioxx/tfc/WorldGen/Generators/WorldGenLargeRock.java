...
public class WorldGenLargeRock implements IWorldGenerator {
    ...
    public boolean generate(World world, Random rand, int i, int j, int k) {
        ...
        for (; yOffset > -2 && !isFlatEnough; yOffset--) {
            if (world.getBlock(i, j + yOffset, k)
                .isNormalCube()) {
                if (world.getBlock(i + 1, j + yOffset, k)
                    .isNormalCube()
                    && world.getBlock(i - 1, j + yOffset, k)
                        .isNormalCube()
                    && world.getBlock(i - 1, j + yOffset, k)
                        .isNormalCube()
                    && world.getBlock(i, j + yOffset, k + 1)
                        .isNormalCube())
                    isFlatEnough = true;
            }
        }
        ...
        for (int xCoord = i - xWidth; xCoord <= i + xWidth2; ++xCoord) {
            for (int zCoord = k - zWidth; zCoord <= k + zWidth2; ++zCoord) {
                for (int yCoord = j + yOffset - HEIGHT; yCoord <= j + yOffset + HEIGHT; ++yCoord) {
                    ...
                    if (xCoord < i && distance > xWidth * 4) canPlaceX = false;
                    if (xCoord > i && distance > xWidth2 * 4) canPlaceX = false;
                    if (zCoord < i && distance > zWidth * 4) canPlaceZ = false;
                    if (zCoord > i && distance > zWidth2 * 4) canPlaceZ = false;
