...
public class WorldGenKapokTrees extends WorldGenerator {
     ...
     */
    private static final byte[] OTHER_COORD_PAIRS = new byte[] { (byte) 2, (byte) 0, (byte) 0, (byte) 1, (byte) 2,
        (byte) 1 };
    ...
    public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5) {
        ...
        if (this.heightLimit == 0) this.heightLimit = 15 + this.rand.nextInt(this.heightLimitLimit / 2);
    ...
    private void generateBranchPoints() {
        double branchSlope = 0; // starts out horizontal and becomes more vertical. should be 0.2-1.5, trying out
                                // constant 0.5
            ...
            if (var8 < 0.0F) {
                ...
                for (double var9 = 0.5D; var7 < var1; ++var7) {
                    ...
                    if (this.checkBlockLine(var17, var18) == -1) {
                        ...
                        if (var17[1] - var22 > var5) var19[1] = var5;
                        else var19[1] = (int) (var17[1] - var22);
    ...
    private void genTreeLayer(int par1, int par2, int par3, float par4, byte par5, Block b) {
        ...
        for (var11[par5] = var10[par5]; var12 <= var7; ++var12) {
            ...
            while (var13 <= var7) {
                double var15 = Math
                    .sqrt(Math.pow(Math.abs(var12) + 0.5D, 2.0D) + Math.pow(Math.abs(var13) + 0.5D, 2.0D));
    ...
    private float layerSize(int par1) {
        if (par1 < this.heightLimit * 0.3D) {
            ...
            if (var3 == 0.0F) var4 = var2;
            else if (Math.abs(var3) >= var2) var4 = 0.0F;
            else var4 = (float) Math.sqrt(Math.pow(Math.abs(var2), 2.0D) - Math.pow(Math.abs(var3), 2.0D));
    ...
    private float leafSize(int par1) {
        return par1 >= 0 && par1 < this.leafDistanceLimit
            ? par1 != 0 && par1 != this.leafDistanceLimit - 1 ? 4.0F : 3.0F
            : -1.0F;
    }
    ...
    private void placeBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger, Block par3) {
        ...
        for (var6 = 0; var5 < 3; ++var5) {
            ...
            if (Math.abs(var4[var5]) > Math.abs(var4[var6])) var6 = var5;
        }
        ...
        if (var4[var6] != 0) {
            ...
            if (par1ArrayOfInteger[0] == par2ArrayOfInteger[0] && par1ArrayOfInteger[2] == par2ArrayOfInteger[2]
                && par1ArrayOfInteger[1] < par2ArrayOfInteger[1]) {
                for (int i = -1; i < 2; i++) {
                    for (int k = -1; k < 2; k++) {
                        if (rand.nextBoolean() && par1ArrayOfInteger[1] > 1
                            && Math.abs(i) != Math.abs(k)
                            && !worldObj.isAirBlock(
                                i + par1ArrayOfInteger[0],
                                par1ArrayOfInteger[1] - 1,
                                par1ArrayOfInteger[2] + k)) {
                            this.setBlockAndNotifyAdequately(
                                this.worldObj,
                                i + par1ArrayOfInteger[0],
                                par1ArrayOfInteger[1],
                                par1ArrayOfInteger[2] + k,
                                par3,
                                treeId);
                        }
    ...
    private void placeBlockLineCurved(int[] startingPos, int[] endingPos, Block par3) {
        int[] midPointRaised = { (endingPos[0] - startingPos[0]) / 2, (int) ((endingPos[1] - startingPos[1]) / 1.5D),
            (endingPos[1] - startingPos[1]) / 2 };
        int[] newDest = { startingPos[0] + midPointRaised[0], startingPos[1] + midPointRaised[1],
            startingPos[2] + midPointRaised[2] };
    ...
    public void setScale(double par1, double par3, double par5) {
        ...
        if (par1 > 0.5D) this.leafDistanceLimit = 5;
