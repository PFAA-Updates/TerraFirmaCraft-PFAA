...
public class WorldGenSoilPits implements IWorldGenerator {
    ...
    public boolean generatePeat(World world, Random random, int xCoord, int yCoord, int zCoord) {
        ...
        if (random.nextInt(50) == 0 && yCoord <= Global.SEA_LEVEL) {
            ...
            for (int xCoord = i - radius; xCoord <= i + radius; ++xCoord) {
                for (int zCoord = k - radius; zCoord <= k + radius; ++zCoord) {
                    ...
                    if (x * x + z * z <= radius * radius
                        && TFC_Climate.getRainfall(world, xCoord, Global.SEA_LEVEL, zCoord) >= 500) {
                        flag = false;
                            ...
                            if (TFC_Climate.getCacheManager(world) != null) {
                                ...
                                if (block == TFCBlocks.dirt || block == TFCBlocks.dirt2) {
                                    ...
                                    world.setBlock(
                                        ...
                                        yCoord,
                                        zCoord,
                                        TFC_Core.getTypeForClayGrass(block),
                                        TFC_Core.getSoilMetaFromStone(rockLayer1.block, rockLayer1.data2),
                                        0x2);
                                    flag = true;
                                }
                        ...
                        if (flag && rand.nextInt(15) == 0) {
                            ...
                            if (world.isAirBlock(xCoord, y, zCoord)
                                && TFC_Core.isSoil(world.getBlock(xCoord, y - 1, zCoord)))
