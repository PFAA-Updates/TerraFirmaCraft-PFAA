...
public class WorldGenCustomWillowTrees extends WorldGenerator {
        ...
        for (int y1 = 0; y1 < 2; y1++) {
            for (int x1 = x - 2 + y1; x1 < x + 2 - y1; x1++) {
                for (int z1 = z - 2 + y1; z1 < z + 2 - y1; z1++) {
                    if (world.getBlock(x1, y1 + y, z1)
                        .canBeReplacedByLeaves(world, x1, y1 + y, z1)) {
    ...
    public boolean generate(World world, Random random, int xCoord, int yCoord, int zCoord) {
        ...
        while (world.getBlock(xCoord, yCoord - 1, zCoord)
            .getMaterial() == Material.water) {
            yCoord--;
        }
        if (yCoord < 1 || yCoord + height + 5 > world.getHeight()) return false;

        boolean flag = true;
        for (int i1 = yCoord; i1 <= yCoord + 1 + height; i1++) {
            byte byte0 = 1;
            if (i1 == yCoord) byte0 = 0;
            if (i1 >= yCoord + 1 + height - 2) byte0 = 3;
            ...
            for (int j2 = xCoord - byte0; j2 <= xCoord + byte0 && flag; j2++) {
                for (int j3 = zCoord - byte0; j3 <= zCoord + byte0 && flag; j3++) {
                    if (i1 >= 0 && i1 < world.getHeight()) {
                        ...
                        if (i4.isAir(world, j2, i1, j3) || i4 == TFCBlocks.leaves || i4 == TFCBlocks.leaves2) continue;
                        if (i4 == TFCBlocks.freshWaterStationary) {
                            if (i1 > yCoord) flag = false;
                        } else {
                            flag = false;
                        }
                    } else {
        ...
        for (int n = 0; n < 2; n++) {
            ...
            for (int a = 0; a < length; a++) {
                if (world.isAirBlock(x2 * a / length + x, y2 * a / length + y, z2 * a / length + z)) world.setBlock(
                    x2 * a / length + x,
                    y2 * a / length + y,
                    z2 * a / length + z,
                    TFCBlocks.logNatural,
