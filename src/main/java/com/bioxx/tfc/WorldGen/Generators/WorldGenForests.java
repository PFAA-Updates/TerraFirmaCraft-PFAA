...
public class WorldGenForests implements IWorldGenerator {
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator,
        ...
        if (world.getBiomeGenForCoords(chunkX, chunkZ) instanceof TFCBiome) // Fixes ClassCastException
        {
            ...
            if (biome == TFCBiome.OCEAN || biome == TFCBiome.DEEP_OCEAN) return;
            ...
            if (!generateJungle(random, chunkX, chunkZ, world)) generateForest(random, chunkX, chunkZ, world);
        }
    }
    ...
    private void generateForest(Random random, int chunkX, int chunkZ, World world) {
        ...
        for (int var2 = 0; var2 < numTrees; ++var2) {
            ...
            if (numTrees > 30) numTrees = 30;

            temperature = TFC_Climate
                .getBioTemperatureHeight(world, xCoord, world.getHeightValue(xCoord, zCoord), zCoord);
            ...
            try {
                if (zCoord > 14500 || zCoord < -14500) gen2 = TFCBiome.getTreeGen(8, random.nextBoolean());
                ...
                if ((spawnParam0 & 1) > 0 || (spawnParam1 & 1) > 0 || (spawnParam2 & 1) > 0) {
                    ...
                    if (spawnParam0 > 0 && (spawnParam0 & 2) == 0
                        && spawnParam1 > 0
                        && (spawnParam1 & 2) == 0
                        && spawnParam2 > 0
                        && (spawnParam2 & 2) == 0) {
                        // there is a 1 in 10 chance for a tree otherwise no trees
                        if (random.nextInt(8) == 0) numTrees = 1;
                        else return;
                    }
                } else {
                    return;
    ...
    private int canTreeSpawn(int tree) {
        ...
        if (temperature >= treeTempMin && temperature <= treeTempMax) out += 1;
        if (evt >= treeEVTMin && evt <= treeEVTMax) out += 2;
        if (rainfall >= treeRainMin && rainfall <= treeRainMax) out += 4;
    ...
    public boolean generateJungle(Random random, int chunkX, int chunkZ, World world) {
        ...
        for (int var2 = 0; var2 < numTrees; ++var2) {
            ...
            // float temperature = TFC_Climate.getBioTemperatureHeight(world, xCoord, world.getHeightValue(xCoord,
            // zCoord), zCoord);
            float temperatureAvg = TFC_Climate.getBioTemperature(world, xCoord, zCoord);

            try {
                if (evt <= EnumTree.KAPOK.maxEVT && rainfall >= EnumTree.KAPOK.minRain
                    && rainfall <= EnumTree.KAPOK.maxRain
                    && temperatureAvg >= EnumTree.KAPOK.minTemp
                    && temperatureAvg <= EnumTree.KAPOK.maxTemp) {
                    WorldGenerator gen0;
                    if (random.nextInt(5) == 0) gen0 = new WorldGenKapokTrees(false, 15);
                    else if (random.nextInt(2) == 0) gen0 = new WorldGenCustomShortTrees(false, 15);
                    else gen0 = new WorldGenJungleShrub(15);

                    // gen0 = random.nextInt(2) == 0 ? new WorldGenJungleShrub(15) : random.nextInt(3) == 0 ? new
                    // WorldGenKapokTrees(false,15): new WorldGenCustomShortTrees(false, 15);
                    gen0.setScale(1.0D, 1.0D, 1.0D);
                    gen0.generate(world, random, xCoord, yCoord, zCoord);
                    completed = true;
                }

                if (evt <= EnumTree.KOA.maxEVT && rainfall >= EnumTree.KOA.minRain
                ...
                // TerraFirmaCraft.log.catching(e);//TerraFirmaCraft.log.info("Tree0 Type:
                // "+TreeType0);TerraFirmaCraft.log.info("Tree1 Type: "+TreeType1);TerraFirmaCraft.log.info("Tree2 Type:
                // "+TreeType2);
            }
        }
        if (completed) {
            ...
            for (int z1 = -4; z1 < 5; ++z1) {
                for (int y1 = -2; y1 < 1; ++y1) {
                    if (world.blockExists(x + x1, y + y1, z + z1)
                        && TFC_Core.isWater(world.getBlock(x + x1, y + y1, z + z1))) return true;
                }
            }
        }
        return false;
