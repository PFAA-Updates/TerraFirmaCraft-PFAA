...
public class WorldGenMinable extends WorldGenerator {
    ...
    // ==========================================mp mod
    // private static int[] aOreCheck = new int[256];// setup array to store oreIDs for this chunk // has to be static
    // to survive instance calls
    ...
    public boolean generateBeforeCheck() // takes a set of current global variables and checks to see if this ore has
                                         // spawned before in this chunk
    {
    ...
    private void createMine(World worldObj, Random rand, int x, int z) {
        for (int loopCount = 0; loopCount < veinAm; loopCount++) {
            ...
            int posY = temp2;
            int posZ = z + temp3;
            if (!useMarcoVeins) bODgenerate(worldObj, rand, posX, posY, posZ, veinSi); // generate based on values
            else bODgenerateVein(worldObj, rand, posX, posY, posZ, veinSi);
        }
    }
    ...
    public boolean generate(World world, Random random, int x, int z, int min, int max)// obsorb default system
    {
        ...
        if (mPChunkX != mPPrevX || mPChunkZ != mPPrevZ || mPPrevBlock != mPBlock || minableBlockMeta != mPPrevMeta) {
            if (!generateBeforeCheck()) {
                ...
                if (rarity == 1 || rarity > 0 && rand.nextInt(rarity) == 0) createMine(worldObj, rand, xChunk, zChunk);
            }
        }
        return true;
    }
    ...
    public int mPCalculateDensity(int oreDistance, float oreDensity) // returns the density value
    {
        ...
        densityValuePassInner = oreDistance / loopCount; // distance devided by number of times it will loop,
                                                         // establishes the number for randomization
            ...
            for (int blocksMade1 = 0; blocksMade1 <= blocksToUse2;) // make branch
            {
                ...
                if (rand.nextInt(3) == 0) // make sub-branch
                {
                    ...
                    for (int blocksMade2 = 0; blocksMade2 <= (1 + (blocksToUse2 / 5));) {
                        ...
                        ChunkData data = TFC_Core.getCDM(world)
                            .getData(posX >> 4, posZ >> 4);
    ...
    public boolean bODgenerate(World world, Random rand, int par3, int par4, int par5, int xyz) {
        ...
        // TerraFirmaCraft.log.info("a vein was placed " + minableBlockId + "." + minableBlockMeta+ " at " + par3 +"
        // "+par4+" "+par5); /// for debugging
        return true;
    }
