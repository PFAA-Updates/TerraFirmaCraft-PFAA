...
public class WorldGenFissure implements IWorldGenerator {
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator,
        ...
        if (rarity <= 0 || rand.nextInt(rarity) != 0
            ...
            || biome == TFCBiome.OCEAN
            || biome == TFCBiome.GRAVEL_BEACH
            || biome == TFCBiome.LAKE
            || biome == TFCBiome.RIVER
            || biome == TFCBiome.DEEP_OCEAN) return;

        int startY = world.getTopSolidOrLiquidBlock(startX, startZ) - 1;
        if (underground) startY = depth + rand.nextInt(60);
    ...
    public void generate(World world, Random rand, int x, int y, int z) {
        creviceDepth = 1;
        if (rand.nextInt(100) < 50) creviceDepth += 2 + rand.nextInt(8);
        poolDepth = 1 + rand.nextInt(Math.max(creviceDepth - 1, 1));

        for (int d = 1; d <= poolDepth; d++) {
            if (!world.getBlock(x, y - d, z)
                .isNormalCube()) return;
        }

        Block block = world.getBlock(x, y, z);

        if (block != null && block.getMaterial() == Material.water) return;
        if (underground) y -= 20 + rand.nextInt(depth);

        int stability = TFC_Climate.getStability(world, x, z);
        if (checkStability && stability == 0) return;
        if (stability == 1 && fillBlock != null && fillBlock.getMaterial() == Material.water)
            fillBlock = TFCBlocks.hotWater;
        if (!TFC_Core.isGround(block)) return;
        ...
        if (makeTunnel) makeTunnel(rand, world, x, z, y, rockLayer);
    }

    private void carve(World world, int x, int y, int z, Block block, int meta) {
        if (world.getBlock(x, y, z)
            .getMaterial() != Material.air && TFC_Core.isGround(world.getBlock(x, y, z)))
            world.setBlock(x, y, z, Blocks.air, 0, 3);
        if (world.getBlock(x - 1, y, z)
            .getMaterial() != Material.air && TFC_Core.isRawStone(world.getBlock(x - 1, y, z)))
            world.setBlock(x - 1, y, z, block, meta, 3);
        if (world.getBlock(x + 1, y, z)
            .getMaterial() != Material.air && TFC_Core.isRawStone(world.getBlock(x + 1, y, z)))
            world.setBlock(x + 1, y, z, block, meta, 3);
        if (world.getBlock(x, y, z - 1)
            .getMaterial() != Material.air && TFC_Core.isRawStone(world.getBlock(x, y, z - 1)))
    ...
    private void fill(World world, int x, int y, int z, Block block, int meta, Block fill) {
        ...
        if (world.getBlock(x - 1, y, z)
            .getMaterial() == Material.air) world.setBlock(x - 1, y, z, block, meta, 2);
        if (world.getBlock(x + 1, y, z)
            .getMaterial() == Material.air) world.setBlock(x + 1, y, z, block, meta, 2);
        if (world.getBlock(x, y, z - 1)
            .getMaterial() == Material.air) world.setBlock(x, y, z - 1, block, meta, 2);
        if (world.getBlock(x, y, z + 1)
            .getMaterial() == Material.air) world.setBlock(x, y, z + 1, block, meta, 2);
        if (world.getBlock(x, y - 1, z)
            .getMaterial() == Material.air) world.setBlock(x, y - 1, z, block, meta, 2);
    }
    ...
    private void makeTunnel(Random random, World world, int x, int z, int y, BlockMeta rockLayer) {
        ...
        while (yCoord > minTunnel) {
            if (world.getBlock(xCoord, yCoord, zCoord) == Blocks.bedrock) break;
            ...
            if (fillBlock != null && world.getBlock(xCoord + 1, yCoord, zCoord)
                .getMaterial() != fillBlock.getMaterial())
                world.setBlock(xCoord + 1, yCoord, zCoord, rockLayer.block, rockLayer.meta, 2);
            if (fillBlock != null && world.getBlock(xCoord - 1, yCoord, zCoord)
                .getMaterial() != fillBlock.getMaterial())
                world.setBlock(xCoord - 1, yCoord, zCoord, rockLayer.block, rockLayer.meta, 2);
            if (fillBlock != null && world.getBlock(xCoord, yCoord, zCoord + 1)
                .getMaterial() != fillBlock.getMaterial())
    ...
    public List<ByteCoord> getCollapseMap(World world, int i, int j, int k) {
        ...
        BlockMeta rockLayer = fillBlock != null && fillBlock.getMaterial() == Material.lava
            ? new BlockMeta(dl2.block, dl2.data2)
            : new BlockMeta(dl.block, dl.data2);
        ...
        while (checkQueue.peek() != null) {
            ...
            if (!checkedmap.contains(block) && TFC_Core.isGround(id)
                ...
                if (checkQueue.size() < 500) switch (block.direction) {
                    case NORTH: {
                        checkQueue.add(
                            checkedmap,
                            new CollapseData(
                    ...
                    default: {
                        ...
                        checkQueue.add(
                            ...
                            new CollapseData(
                                new ByteCoord(localX + 1, localY + 0, localZ + 1),
                                baseCollapse - incrementChanceDiag,
                                TFCDirection.NORTHEAST));
                        checkQueue.add(
                            checkedmap,
                            new CollapseData(
