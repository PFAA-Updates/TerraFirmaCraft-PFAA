...
public class WorldGenRedwoodXL extends WorldGenerator {
            ...
            for (int x1 = x - k1; x1 <= x + k1; x1++) {
                for (int z1 = z - k1; z1 <= z + k1; z1++) {
                    if (y1 >= 0 && y1 < 256) {
                        ...
                        if (id1 != null && id1.isLeaves(world, x1, y1, z1)) return false;
                    } else {
                        return false;
                    }
                }
            ...
            for (int x1 = x - l1; x1 <= x + l1; x1++) {
                ...
                for (int z1 = z - l1; z1 <= z + l1; z1++) {
                    ...
                    if (Math.abs(k4) != l1 || Math.abs(i5) != l1 || l1 <= 0) {
                        // Must use special method so that each block is checked for if it can be replaced. Using
                        // setBlockAndNotifyAdequately directly will remove the majority of the trunk. -K
    ...
    private void setLeaf(World world, int x, int y, int z) {
        ...
        if (b.canBeReplacedByLeaves(world, x, y, z)) setBlockAndNotifyAdequately(world, x, y, z, blockLeaf, metaLeaf);
    }
