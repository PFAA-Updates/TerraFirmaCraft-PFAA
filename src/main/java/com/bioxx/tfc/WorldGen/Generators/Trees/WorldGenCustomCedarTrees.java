...
public class WorldGenCustomCedarTrees extends WorldGenerator {
    ...
    public boolean generate(World world, Random random, int xCoord, int yCoord, int zCoord) {
        ...
        if (yCoord < 1 || yCoord + treeHeight + 1 > world.getHeight()) return false;

        boolean flag = true;
        for (int y = yCoord; y <= yCoord + 1 + treeHeight; y++) {
            byte byte0 = 1;
            if (y == yCoord) byte0 = 0;
            if (y >= yCoord + 1 + treeHeight - 2) byte0 = 2;
            ...
            for (int x = xCoord - byte0; x <= xCoord + byte0 && flag; x++) {
                for (int z = zCoord - byte0; z <= zCoord + byte0 && flag; z++) {
                    if (y >= 0 && y < world.getHeight()) {
                        ...
                        if (!j3.isAir(world, x, y, z) && !j3.canBeReplacedByLeaves(world, x, y, z)) flag = false;
                    } else {
                        flag = false;
                    }
                }
        ...
        if (!(TFC_Core.isSoil(var3)) || yCoord >= world.getHeight() - treeHeight - 1) return false;
        ...
        for (int treeHeightOffset = yCoord + 1; treeHeightOffset <= yCoord + treeHeight; treeHeightOffset++) {
            ...
            for (int xPos = xCoord - 1; xPos <= xCoord + 1; xPos++) {
                ...
                for (int zPos = zCoord - 1; zPos <= zCoord + 1; zPos++) {
                    int j4 = zPos - zCoord;
                    if ((Math.abs(l3) != treeRadius || Math.abs(j4) != treeRadius
                        || random.nextInt(2) != 0 && treeDiameter != 0) && world.getBlock(xPos, treeHeightOffset, zPos)
                            .canBeReplacedByLeaves(world, xPos, treeHeightOffset, zPos)) {
