...
public class WorldGenBerryBush extends WorldGenerator {
    ...
    public boolean generate(World world, Random random, int i, int j, int k) {
        ...
        float evt = TFC_Climate.getCacheManager(world)
            .getEVTLayerAt(i, k).floatdata1;

        FloraIndex index = FloraManager.getInstance()
            .findMatchingIndex(((BlockBerryBush) TFCBlocks.berryBush).getType(meta));
        if (world.isAirBlock(i, j, k) && j < 250
            && temp > index.minBioTemp
            && temp < index.maxBioTemp
            && rain >= index.minRain
            && rain <= index.maxRain
            && evt >= index.minEVT
            && evt <= index.maxEVT) {
            ...
            for (short realCount = 0; count < cluster && realCount < (spawnRadius * spawnRadius); realCount++) {
                ...
                if (createBush(
                    ...
                    random,
                    i - spawnRadius + x,
                    world.getHeightValue(i - spawnRadius + x, k - spawnRadius + z),
                    k - spawnRadius + z,
                    index)) count++;
            }
            // TerraFirmaCraft.log.info(_fi.type + ": " + count + " bushes spawned of " + _cluster + " expected. [" + i
            // +"]" + "[" + j +"]" + "[" + k +"]");
        }
        return true;
    }
    ...
    public boolean createBush(World world, Random random, int i, int j, int k, FloraIndex fi) {
        ...
        if ((world.canBlockSeeTheSky(i, j, k) || world.getBlockLightValue(i, j, k) > 8)
            ...
            for (int h = 0; h < bushHeight && random.nextBoolean(); h++) {
                ...
                if (TFC_Time.getSeasonAdjustedMonth(k) > fi.harvestStart
                    && TFC_Time.getSeasonAdjustedMonth(k) < fi.harvestFinish + fi.fruitHangTime) {
