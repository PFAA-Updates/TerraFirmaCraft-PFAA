...
public class WorldGenPlants implements IWorldGenerator {
    ...
    private static WorldGenCustomFruitTree bananaTree = new WorldGenCustomFruitTree(
        false,
        TFCBlocks.fruitTreeLeaves,
        1);
    private static WorldGenCustomFruitTree orangeTree = new WorldGenCustomFruitTree(
        false,
        TFCBlocks.fruitTreeLeaves,
        2);
    private static WorldGenCustomFruitTree grappleTree = new WorldGenCustomFruitTree(
        false,
        TFCBlocks.fruitTreeLeaves,
        3);
    ...
    private static WorldGenCustomFruitTree cherryTree = new WorldGenCustomFruitTree(
        false,
        TFCBlocks.fruitTreeLeaves,
        6);
    ...
    // private static WorldGenCustomFruitTree cacaoTree = new WorldGenCustomFruitTree(false, TFCBlocks.fruitTreeLeaves2,
    // 9);
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator,
        ...
        if (random.nextInt(70) == 0 && rain >= 500) {
            ...
            switch (new Random(world.getSeed() + ((chunkX >> 4) - (chunkZ >> 4)) * (chunkZ >> 4)).nextInt(9)) {
                default: {
                    if (bioTemperature > 8 && world.isAirBlock(xCoord, yCoord, zCoord)
                        && TFC_Core.isGrass(world.getBlock(xCoord, yCoord - 1, zCoord)))
                        appleTree.generate(world, random, xCoord, yCoord, zCoord);
                    break;
                }
                case 1: {
                    if (bioTemperature > 15 && world.isAirBlock(xCoord, yCoord, zCoord)
                        && TFC_Core.isGrass(world.getBlock(xCoord, yCoord - 1, zCoord)))
                        bananaTree.generate(world, random, xCoord, yCoord, zCoord);
                    break;
                }
                case 2: {
                    if (bioTemperature > 12 && world.isAirBlock(xCoord, yCoord, zCoord)
                        && TFC_Core.isGrass(world.getBlock(xCoord, yCoord - 1, zCoord)))
                ...
                case 4: {
                    if (bioTemperature > 10 && world.isAirBlock(xCoord, yCoord, zCoord)
                        && TFC_Core.isGrass(world.getBlock(xCoord, yCoord - 1, zCoord)))
                ...
                // case 9:
                // {
                // if(world.isAirBlock(var2, var3, var4) && (world.getBlock(var2, var3-1, var4) ==
                // mod_TFC_Core.terraGrass || world.getBlock(var2, var3-1, var4) == mod_TFC_Core.terraGrass2))
