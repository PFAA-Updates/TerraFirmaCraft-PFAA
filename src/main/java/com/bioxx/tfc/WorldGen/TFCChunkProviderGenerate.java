...
public class TFCChunkProviderGenerate extends ChunkProviderGenerate {
    ...
    public Chunk provideChunk(int chunkX, int chunkZ) {
        ...
        biomesForGeneration = this.worldObj.getWorldChunkManager()
            .loadBlockGeneratorData(biomesForGeneration, chunkX * 16 - 1, chunkZ * 16 - 1, 18, 18);
        if (TFC_Climate.getCacheManager(worldObj) != null) {
            ...
            stabilityLayer = TFC_Climate.getCacheManager(worldObj)
                .loadStabilityLayerGeneratorData(stabilityLayer, chunkX * 16, chunkZ * 16, 16, 16);
    ...
    public void populate(IChunkProvider chunkProvider, int chunkX, int chunkZ) {
        ...
        MinecraftForge.EVENT_BUS
            .post(new PopulateChunkEvent.Post(chunkProvider, worldObj, rand, chunkX, chunkZ, var11));
    ...
    public static List<SpawnListEntry> getCreatureSpawnsByChunk(World world, TFCBiome biome, int x, int z) {
        ...
        if (temp > 0 && temp < 21 && rain > 250) {
            ...
            spawnableCreatureList
                .add(new SpawnListEntry(EntityDeer.class, 2 + mountainousAreaModifier, 1, 3 + mountainousAreaModifier));
        ...
        if (temp > -20 && temp <= 0) {
            ...
            if (rain > 250) {
                ...
                spawnableCreatureList
                    .add(new SpawnListEntry(EntityPheasantTFC.class, 1 + mountainousAreaModifier, 1, 2));
                spawnableCreatureList.add(new SpawnListEntry(EntitySheepTFC.class, 2, 2, 4));
            }
    ...
    public void generateTerrainHigh(int chunkX, int chunkZ, Block[] idsTop) {
        ...
        this.biomesForGeneration = this.worldObj.getWorldChunkManager()
            .getBiomesForGeneration(this.biomesForGeneration, chunkX * 4 - 2, chunkZ * 4 - 2, xSize + 5, zSize + 5);
        this.noiseArray = this
            .initializeNoiseFieldHigh(this.noiseArray, chunkX * subDivXZ, 0, chunkZ * subDivXZ, xSize, ySize, zSize);
    ...
    private double[] initializeNoiseFieldHigh(double[] outArray, int xPos, int yPos, int zPos, int xSize, int ySize,
        ...
        this.noise1 = this.noiseGen1
            .generateNoiseOctaves(this.noise1, xPos, yPos, zPos, xSize, ySize, zSize, double1, double2, double1);
        this.noise2 = this.noiseGen2
            .generateNoiseOctaves(this.noise2, xPos, yPos, zPos, xSize, ySize, zSize, double1, double2, double1);
                            ...
                            for (int counter = 1; counter < var12Temp / 3; counter++) {
                                if (arrayIndexx >= 0
                                    ...
                                    && seaLevelOffsetMap[arrayIndex] - (3 * counter) > seaLevelOffsetMap[arrayIndexX]) {
                                        ...
                                        indexBig = (arrayIndex) * worldHeight + height + indexOffset;
                                        index = (arrayIndex) * 128 + height;
                                    } else if (arrayIndexz >= 0 && seaLevelOffsetMap[arrayIndex] - (3 * counter)
