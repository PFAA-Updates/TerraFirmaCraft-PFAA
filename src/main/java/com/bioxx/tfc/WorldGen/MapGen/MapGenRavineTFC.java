                    ...
                    for (x = xMin; !isBlocked && x < xMax; ++x) {
                        for (int z = zMin; !isBlocked && z < zMax; ++z) {
                            for (int y = maxY + 1; !isBlocked && y >= minY - 1; --y) {
                                ...
                                if (y >= 0 && y < 256) {
                                    if (blockArray[index] == TFCBlocks.saltWaterStationary
                                        || blockArray[index] == TFCBlocks.freshWaterStationary) isBlocked = true;
                                            ...
                                            if (TFC_Core.isGround(blockArray[index2])) {
                                                if (TFC_Core.isSoilOrGravel(blockArray[index2 + 1])) {
                                                    for (int upCount = 1; TFC_Core
                                                        .isSoilOrGravel(blockArray[index2 + upCount]); upCount++) {
