    ...
    public int[] getInts(int x, int z, int xSize, int zSize) {
        ...
        for (int zElement = 0; zElement < zSize; ++zElement) {
            for (int xElement = 0; xElement < xSize; ++xElement) {
                ...
                if (TFC_Core.isOceanicBiome(b) || TFC_Core.isMountainBiome(b)) layerOut[index] = b;
                else if (r > 0) {
                    layerOut[index] = r;
                    ...
                    if (TFC_Core.isBeachBiome(b)) {
                        ...
                        if (inBounds(zp, layerOut) && TFC_Core.isOceanicBiome(layerBiomes[zp])
                            && layerRivers[zp] == 0) {
                            layerOut[index] = b;
                        }
                        if (inBounds(zn, layerOut) && TFC_Core.isOceanicBiome(layerBiomes[zn])
                            && layerRivers[zn] == 0) {
                            layerOut[index] = b;
                        }
                        if (inBounds(xn, layerOut) && TFC_Core.isOceanicBiome(layerBiomes[xn])
                            && layerRivers[xn] == 0) {
                            layerOut[index] = b;
                        }
                        if (inBounds(xp, layerOut) && TFC_Core.isOceanicBiome(layerBiomes[xp])
                            && layerRivers[xp] == 0) {
                            layerOut[index] = b;
                        }
                    }
