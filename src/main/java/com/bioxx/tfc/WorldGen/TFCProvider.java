...
public class TFCProvider extends WorldProvider {
    ...
    public IChunkProvider createChunkGenerator() {
        return new TFCChunkProviderGenerate(
            worldObj,
            worldObj.getSeed(),
    ...
    public boolean canBlockFreeze(int x, int y, int z, boolean byWater) {
        ...
        if (temp <= 0 && biome != TFCBiome.DEEP_OCEAN) {
            if (worldObj.isAirBlock(x, y + 1, z) && TFC_Core.isWater(id)
                && worldObj.rand.nextInt(4) == 0
                && isNextToShoreOrIce(x, y, z)) {
                Material mat = worldObj.getBlock(x, y, z)
                    .getMaterial();
                boolean salty = TFC_Core.isSaltWaterIncludeIce(id, meta, mat);

                if (temp <= -2) salty = false;

                if ((mat == Material.water || mat == Material.ice) && !salty) {
                    if (id == TFCBlocks.freshWaterStationary
                        && meta == 0/* || id == TFCBlocks.FreshWaterFlowing.blockID */) {
                        worldObj.setBlock(x, y, z, TFCBlocks.ice, 1, 2);
                    } else
                        if (id == TFCBlocks.saltWaterStationary && meta == 0/* || id == Block.waterMoving.blockID */) {
            ...
            if (id == TFCBlocks.ice) {
                ...
                if (id == TFCBlocks.ice && worldObj.rand.nextInt(chance) == 0) {
                    if (worldObj.getBlock(x, y + 1, z) == Blocks.snow) {
                         ...
                         * if((b == TFCBiome.ocean || b == TFCBiome.lake || b == TFCBiome.river || b == TFCBiome.river)
                         * && y == 143)
                         * flag = 2;
                         */
    ...
    public boolean canSnowAt(int x, int y, int z, boolean checkLight) {
        if (TFC_Climate.getHeightAdjustedTemp(worldObj, x, y, z) > 0) return false;
        Material material = worldObj.getBlock(x, y, z)
            .getMaterial();
        if (material == Material.snow) // avoid vanilla MC to replace snow
            return false;
        else return TFCBlocks.snow.canPlaceBlockAt(worldObj, x, y, z) && material.isReplaceable();
    }
