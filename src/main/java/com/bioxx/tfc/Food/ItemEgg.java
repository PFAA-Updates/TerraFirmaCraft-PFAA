...
public class ItemEgg extends ItemFoodTFC implements IFood {
    ...
    public void addInformation(ItemStack is, EntityPlayer player, List arraylist, boolean flag) {
        ...
        if (is.hasTagCompound()) {
            if (is.getTagCompound()
                .hasKey("Fertilized")) arraylist.add(EnumChatFormatting.GOLD + TFC_Core.translate("gui.fertilized"));
            else addFoodInformation(is, player, arraylist);
        } else {
    ...
    public float getDecayRate(ItemStack is) {
        if (Food.isPickled(is)) return 0.3f;
        return 0.5f;
    }
}
