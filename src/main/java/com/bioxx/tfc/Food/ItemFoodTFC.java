...
public class ItemFoodTFC extends ItemTerra implements ISize, ICookableFood, IMergeableFood {
    ...
    public ItemFoodTFC(EnumFoodGroup fg, int sw, int so, int sa, int bi, int um) {
        ...
        tasteBitter = bi;
        tasteUmami = um;
        foodID = FoodRegistry.getInstance()
            .registerFood(fg, this);
    }
    ...
    public void registerIcons(IIconRegister registerer) {
        ...
        if (hasCookedIcon) {
            ...
            this.cookedIcon = registerer
                .registerIcon(Reference.MOD_ID + ":" + this.textureFolder + name.replace("item.", "") + " Cooked");
        }
    ...
    public IIcon getIcon(ItemStack stack, int pass) {
        if (Food.isCooked(stack) && cookedIcon != null) return cookedIcon;
    ...
    public float getDecayRate(ItemStack is) {
        ...
        if (Food.isCooked(is)) {
            mult *= 0.75f;
            if (Food.isPickled(is) || Food.isSalted(is)) mult *= 0.75f;
            if (Food.isSmoked(is)) mult *= 1f - (0.25f * this.getSmokeAbsorbMultiplier());
        } else {
            if (Food.isPickled(is) || Food.isSalted(is)) mult *= 0.5f;
            if (Food.isSmoked(is)) mult *= 1f - (0.25f * this.getSmokeAbsorbMultiplier());
            if (Food.isDried(is)) mult *= 0.25f;
        }
    ...
    public String getItemStackDisplayName(ItemStack is) {
        ...
        {
            if (Food.isPickled(is)) name.append(TFC_Core.translate("word.pickled"))
                .append(' ');
            else if (Food.isBrined(is) && !Food.isDried(is)) name.append(TFC_Core.translate("word.brined"))
                .append(' ');

            if (Food.isSalted(is)) name.append(TFC_Core.translate("word.salted"))
                .append(' ');
            if (Food.isCooked(is)) name.append(TFC_Core.translate("word.cooked"))
                .append(' ');
            else if (Food.isSmoked(is)) name.append(TFC_Core.translate("word.smoked"))
                .append(' ');

            if (Food.isDried(is) && !Food.isCooked(is)) name.append(TFC_Core.translate("word.dried"))
                .append(' ');
            if (Food.isInfused(is)) name.append(TFC_Core.translate(Food.getInfusion(is) + ".name"))
                .append(' ');
        }

        return name.append(TFC_Core.translate(this.getUnlocalizedName(is) + ".name"))
            .append(getCookedLevelString(is))
            .toString();
    }
    ...
    protected String getCookedLevelString(ItemStack is) {
        ...
        if (Food.isCooked(is)) {
            ...
            switch (cookedLevel) {
                case 0:
                    s += TFC_Core.translate("food.cooked.0");
                    break;
                case 1:
                    s += TFC_Core.translate("food.cooked.1");
                    break;
                case 2:
                    s += TFC_Core.translate("food.cooked.2");
                    break;
                case 3:
                    s += TFC_Core.translate("food.cooked.3");
                    break;
                default:
                    s += TFC_Core.translate("food.cooked.4");
                    break;
            }
            s += ")";
        }
    ...
    public static void addFoodHeatInformation(ItemStack is, List<String> arraylist) {
        if (TFC_ItemHeat.hasTemp(is)) {
            ...
            if (meltTemp != -1) arraylist.add(TFC_ItemHeat.getHeatColorFood(TFC_ItemHeat.getTemp(is), meltTemp));
        }
    }
    ...
    public void addInformation(ItemStack is, EntityPlayer player, List arraylist, boolean flag) {
        ...
        if (is.hasTagCompound()) {
            ...
            TerraFirmaCraft.LOG.error(
                TFC_Core.translate("error.error") + " "
                    + is.getUnlocalizedName()
                    + " "
                    + TFC_Core.translate("error.NBT")
                    + " "
                    + TFC_Core.translate("error.Contact"));
        }
    }
    ...
    public void addFoodInformation(ItemStack is, EntityPlayer player, List<String> arraylist) {
        ...
        if (ounces > 0 && ounces <= Global.FOOD_MAX_WEIGHT) arraylist
            .add(TFC_Core.translate("gui.food.amount") + " " + ounces + " oz / " + Global.FOOD_MAX_WEIGHT + " oz");

        float decay = Food.getDecay(is);
        if (decay > 0) arraylist.add(
            EnumChatFormatting.DARK_GRAY + TFC_Core.translate("gui.food.decay")
                + " "
                + Helper.roundNumber(decay / ounces * 100, 10)
                + "%");
        if (TFCOptions.enableDebugMode) {
            arraylist.add(EnumChatFormatting.DARK_GRAY + TFC_Core.translate("gui.food.decay") + ": " + decay);
            arraylist
                .add(EnumChatFormatting.DARK_GRAY + "Decay Rate: " + Helper.roundNumber(this.getDecayRate(is), 100));
        }

        if (TFC_Core.showCtrlInformation()) ItemFoodTFC.addTasteInformation(is, player, arraylist);
        else arraylist.add(TFC_Core.translate("gui.showtaste"));
    }
    ...
    public static void addTasteInformation(ItemStack is, EntityPlayer player, List<String> arraylist) {
        ...
        SkillRank rank = TFC_Core.getSkillStats(player)
            .getSkillRank(Global.SKILL_COOKING);
        if (Food.hasMealSkill(is)) rank = SkillRank.values()[Food.getMealSkill(is)];

        int[] prefs = TFC_Core.getPlayerFoodStats(player)
            .getPrefTaste();

        String sSweet = EnumChatFormatting.DARK_GRAY + TFC_Core.translate("gui.taste.sweet")
            + ": "
            + EnumChatFormatting.WHITE;
        String sSour = EnumChatFormatting.DARK_GRAY + TFC_Core.translate("gui.taste.sour")
            + ": "
            + EnumChatFormatting.WHITE;
        String sSalty = EnumChatFormatting.DARK_GRAY + TFC_Core.translate("gui.taste.salty")
            + ": "
            + EnumChatFormatting.WHITE;
        String sBitter = EnumChatFormatting.DARK_GRAY + TFC_Core.translate("gui.taste.bitter")
            + ": "
            + EnumChatFormatting.WHITE;
        String sSavory = EnumChatFormatting.DARK_GRAY + TFC_Core.translate("gui.taste.savory")
            + ": "
            + EnumChatFormatting.WHITE;

        if (rank == SkillRank.Novice) {
            sSweet += (sweet > prefs[0] ? TFC_Core.translate("gui.taste.novice.sweet1")
                : TFC_Core.translate("gui.taste.novice.sweet0"));
            sSour += (sour > prefs[1] ? TFC_Core.translate("gui.taste.novice.sour1")
                : TFC_Core.translate("gui.taste.novice.sour0"));
            sSalty += (salty > prefs[2] ? TFC_Core.translate("gui.taste.novice.salty1")
                : TFC_Core.translate("gui.taste.novice.salty0"));
            sBitter += (bitter > prefs[3] ? TFC_Core.translate("gui.taste.novice.bitter1")
                : TFC_Core.translate("gui.taste.novice.bitter0"));
            sSavory += (savory > prefs[4] ? TFC_Core.translate("gui.taste.novice.savory1")
                : TFC_Core.translate("gui.taste.novice.savory0"));
    ...
    private static String createExpertString(int val, int pref, String name) {
        ...
        if (abs <= 5) out += " (+-5)";
        else if (abs <= 10) out += " (+-10)";
        else if (abs <= 15) out += " (+-15)";
    ...
    private static String createBasicString(int val, int pref, String name) {
        ...
        else if (dif < -20) return TFC_Core.translate("gui.taste.0") + " " + TFC_Core.translate("gui.taste." + name);
        ...
        else if (dif > 20) return TFC_Core.translate("gui.taste.8") + " " + TFC_Core.translate("gui.taste." + name);

        return "";
    }

    public static String getFoodGroupName(EnumFoodGroup fg) {
        if (fg == EnumFoodGroup.Dairy) return (EnumChatFormatting.WHITE + TFC_Core.translate("gui.food.dairy"));
        ...
        else if (fg == EnumFoodGroup.Grain) return (EnumChatFormatting.YELLOW + TFC_Core.translate("gui.food.grain"));
        else return "N/A";
    }

    public static String getFoodGroupColor(EnumFoodGroup fg) {
        if (fg == EnumFoodGroup.Dairy) return EnumChatFormatting.WHITE.toString();
        else if (fg == EnumFoodGroup.Fruit) return EnumChatFormatting.DARK_PURPLE.toString();
        else if (fg == EnumFoodGroup.Vegetable) return EnumChatFormatting.DARK_GREEN.toString();
        else if (fg == EnumFoodGroup.Protein) return EnumChatFormatting.DARK_RED.toString();
        else if (fg == EnumFoodGroup.Grain) return EnumChatFormatting.YELLOW.toString();
        else return "N/A";
    }
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer player) {
        ...
        if (foodstats.needFood() && this.isEdible(is)) player.setItemInUse(is, 32);
    ...
    public ItemStack onEaten(ItemStack is, World world, EntityPlayer player) {
        ...
        if (!world.isRemote && this.isEdible(is)) {
            if (is.hasTagCompound()) {
                ...
                if (FoodStatsTFC.reduceFood(is, eatAmount)) is.stackSize = 0;
            } else {
                foodstats.addNutrition(((IFood) (is.getItem())).getFoodGroup(), 1f);

                String error = TFC_Core.translate("error.error") + " "
                    + is.getUnlocalizedName()
                    + " "
                    + TFC_Core.translate("error.NBT")
                    + " "
    ...
    public EnumSize getSize(ItemStack is) {
        float weight = Food.getWeight(is);
        if (weight <= 20) return EnumSize.TINY;
        else if (weight <= 40) return EnumSize.VERYSMALL;
        else if (weight <= 80) return EnumSize.SMALL;
        else return EnumSize.MEDIUM;
    }

    @Override
    public EnumWeight getWeight(ItemStack is) {
        float weight = Food.getWeight(is);
        if (weight < 80) return EnumWeight.LIGHT;
        else if (weight < 160) return EnumWeight.MEDIUM;
        else return EnumWeight.HEAVY;
    }
    ...
    public int getTasteSweet(ItemStack is) {
        ...
        if (is != null) {
            if (is.getTagCompound()
                .hasKey("tasteSweet"))
                base = is.getTagCompound()
                    .getInteger("tasteSweet");
