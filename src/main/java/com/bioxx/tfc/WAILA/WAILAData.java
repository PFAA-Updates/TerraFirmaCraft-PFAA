...
public class WAILAData implements IWailaDataProvider {
    ...
    public ItemStack getWailaStack(IWailaDataAccessor accessor, IWailaConfigHandler config) {
        ...
        if (block instanceof BlockAnvil) return anvilStack(accessor, config);

        else if (tileEntity instanceof TEBerryBush) return berryBushStack(accessor, config);

        else if (tileEntity instanceof TEBloom) return new ItemStack(TFCItems.rawBloom);

        else if (block instanceof BlockCharcoal) return new ItemStack(TFCItems.coal, accessor.getMetadata(), 1);

        else if (TFC_Core.isClay(block) || TFC_Core.isClayGrass(block)) return new ItemStack(TFCItems.clayBall);

        else if (block instanceof BlockCobble) return new ItemStack(block, 1, accessor.getMetadata() % 8);

        else if (tileEntity instanceof TECrop) return cropStack(accessor, config);

        else if (block instanceof BlockCustomDoor)
            return new ItemStack(Recipes.doors[((BlockCustomDoor) block).getWoodType() / 2]);

        else if (tileEntity instanceof TEFruitLeaves) return fruitLeavesStack(accessor, config);

        else if (tileEntity instanceof TEFruitTreeWood) return fruitTreeWoodStack(accessor, config);

        else if (tileEntity instanceof TEIngotPile) return ingotPileStack(accessor, config);

        else if (tileEntity instanceof TELoom) return loomStack(accessor, config);

        else if (tileEntity instanceof TEMetalSheet) return metalSheetStack(accessor, config);

        else if (tileEntity instanceof TEMetalTrapDoor) return metalTrapDoorStack(accessor, config);

        else if (tileEntity instanceof TEOilLamp) return oilLampStack(accessor, config);

        else if (tileEntity instanceof TEOre) return oreStack(accessor, config);

        else if (block instanceof BlockPartial) return partialStack(accessor, config);

        else if (block instanceof BlockWaterPlant
            && TFC_Core.isSaltWater(world.getBlock(pos.blockX, pos.blockY + 1, pos.blockZ))) // Sea Weed
            return ItemFoodTFC.createTag(new ItemStack(TFCItems.seaWeed, 1, 0));

        else if (tileEntity instanceof TEWorldItem) return worldItemStack(accessor, config);
    ...
    public List<String> getWailaHead(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (tileEntity instanceof TEBarrel) currenttip = barrelHead(itemStack, currenttip, accessor, config);
        ...
        else if (tileEntity instanceof TEOre) currenttip = oreHead(itemStack, currenttip, accessor, config);
        ...
        else if (block instanceof BlockWaterPlant) currenttip = waterPlantHead(itemStack, currenttip, accessor, config);
    ...
    public List<String> getWailaBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        Block block = accessor.getBlock();
        TileEntity tileEntity = accessor.getTileEntity();
        if (tileEntity instanceof TEAnvil) currenttip = anvilBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TEBarrel) currenttip = barrelBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TEBerryBush) currenttip = berryBushBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TEBlastFurnace)
            currenttip = blastFurnaceBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TEBloom) currenttip = bloomBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TEBloomery) currenttip = bloomeryBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TECrop) currenttip = cropBody(itemStack, currenttip, accessor, config);

        // I haven't decided if this is OP or not. Useful for debugging though, so uncomment when needing to check
        // farmland nutrient %.
        /*
        ...
        else if (tileEntity instanceof TEFirepit) currenttip = firepitBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TEForge) currenttip = forgeBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TEFruitLeaves)
            currenttip = fruitLeavesBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TELogPile) currenttip = logPileBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TELoom) currenttip = loomBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TEMetalTrapDoor)
            currenttip = metalTrapDoorBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TENestBox) currenttip = nestBoxBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TEOilLamp) currenttip = oilLampBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TEOre) currenttip = oreBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TEPottery) currenttip = potteryBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TESapling) currenttip = saplingBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TESluice) currenttip = sluiceBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TESmokeRack) currenttip = smokeRackBody(itemStack, currenttip, accessor, config);

        else if (TFC_Core.isSoilWAILA(block)) currenttip = soilBody(itemStack, currenttip, accessor, config);

        else if (tileEntity instanceof TESpawnMeter)
            currenttip = spawnMeterBody(itemStack, currenttip, accessor, config);

        else if (block == TFCBlocks.torch) currenttip = torchBody(itemStack, currenttip, accessor, config);
    ...
    public ItemStack berryBushStack(IWailaDataAccessor accessor, IWailaConfigHandler config) {
        ...
        if (hasFruit) return ItemFoodTFC.createTag(index.getOutput());
        else return null;
    }
    ...
    public ItemStack cropStack(IWailaDataAccessor accessor, IWailaConfigHandler config) {
        ...
        CropIndex crop = CropManager.getInstance()
            .getCropFromId(cropId);
        ItemStack itemstack;

        if (crop.output2 != null) itemstack = new ItemStack(crop.output2);
        else itemstack = new ItemStack(crop.output1);
    ...
    public ItemStack fruitLeavesStack(IWailaDataAccessor accessor, IWailaConfigHandler config) {
        FloraIndex index = FloraManager.getInstance()
            .findMatchingIndex(BlockFruitLeaves.getType(accessor.getBlock(), accessor.getMetadata() % 8));

        if (index != null) return ItemFoodTFC.createTag(index.getOutput());
        else return null;
    }

    public ItemStack fruitTreeWoodStack(IWailaDataAccessor accessor, IWailaConfigHandler config) {
        FloraIndex index = FloraManager.getInstance()
            .findMatchingIndex(BlockFruitWood.getType(accessor.getMetadata()));
    ...
    public ItemStack ingotPileStack(IWailaDataAccessor accessor, IWailaConfigHandler config) {
        ...
        if (storage[0] != null) return storage[0];
    ...
    public ItemStack oreStack(IWailaDataAccessor accessor, IWailaConfigHandler config) {
        ...
        if (accessor.getBlock() == TFCBlocks.ore) // Metals & Coal
        {
            ...
            else itemstack = new ItemStack(TFCItems.oreChunk, 1, meta); // All normal quality ores.
    ...
    public List<String> fruitLeavesHead(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (!hasFruit) currenttip.set(0, EnumChatFormatting.WHITE.toString() + TFC_Core.translate("gui." + type));
    ...
    public List<String> oreHead(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (accessor.getBlock() == TFCBlocks.ore) {
            if (meta == 14) {
                currenttip
                    .set(0, EnumChatFormatting.WHITE.toString() + TFC_Core.translate("item.Ore.Bituminous Coal.name"));
            } else if (meta == 15) {
    ...
    public List<String> waterPlantHead(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (TFC_Core.isFreshWater(blockDirectlyAbove)) {
            if (airTwoAbove) {
                currenttip
                    .set(0, EnumChatFormatting.WHITE.toString() + TFC_Core.translate("tile.Flora.Cat Tails.name"));
            } else {
                currenttip
                    .set(0, EnumChatFormatting.WHITE.toString() + TFC_Core.translate("tile.Flora.Pond Weed.name"));
            }
        }
    ...
    public List<String> barrelBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        FluidStack fluid = FluidStack.loadFluidStackFromNBT(tag.getCompoundTag("fluidNBT"));
        BarrelRecipe recipe = BarrelManager.getInstance()
            .findMatchingRecipe(inStack, fluid, sealed, te.getTechLevel());

        if (sealed && fluid != null
            && fluid.getFluid() == TFCFluids.MILKCURDLED
            && (inStack == null || inStack.getItem() instanceof IFood
                && ((IFood) inStack.getItem()).getFoodGroup() != EnumFoodGroup.Dairy
                && ((IFood) inStack.getItem()).isEdible(inStack)
                && Food.getWeight(inStack) <= 20.0f))
            recipe = new BarrelRecipe(
                null,
                new FluidStack(TFCFluids.MILKCURDLED, 10000),
                ItemFoodTFC.createTag(new ItemStack(TFCItems.cheese, 1), 160),
                null).setMinTechLevel(0);
        ...
        // Output
        BarrelPreservativeRecipe preservative = BarrelManager.getInstance()
            .findMatchingPreservativeRepice(te, inStack, fluid, sealed);
        ...
        if (recipe != null) {
            if (!(recipe instanceof BarrelBriningRecipe)) {
                ...
                if (inStack != null && inStack.getItem() instanceof IFood
        ...
        } else if (sealed && fluid != null
            && inStack != null
            && inStack.getItem() instanceof IFood
            && fluid.getFluid() == TFCFluids.VINEGAR) {
                if (!Food.isPickled(inStack)
                    && Food.getWeight(inStack) / fluid.amount <= Global.FOOD_MAX_WEIGHT / te.getMaxLiquid()) {
                    if ((((IFood) inStack.getItem()).getFoodGroup() == EnumFoodGroup.Fruit
                        || ((IFood) inStack.getItem()).getFoodGroup() == EnumFoodGroup.Vegetable
                        || ((IFood) inStack.getItem()).getFoodGroup() == EnumFoodGroup.Protein
                        || ((IFood) inStack.getItem()) == TFCItems.cheese) && Food.isBrined(inStack)) {
                        currenttip.add(TFC_Core.translate("gui.barrel.pickling"));
                    }
                } else if (Food.isPickled(inStack)
                    && Food.getWeight(inStack) / fluid.amount <= Global.FOOD_MAX_WEIGHT / te.getMaxLiquid() * 2) {
    ...
    public List<String> blastFurnaceBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        else currenttip.add(TFC_Core.translate("gui.plans.tuyere") + EnumChatFormatting.RED.toString() + " \u2718");
    ...
    public List<String> bloomeryBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (isLit) {
            ...
            if (hours > 0) {
                float percent = Helper
                    .roundNumber(Math.min(100 - ((hours / TFCOptions.bloomeryBurnTime) * 100), 100), 10);
    ...
    public List<String> cropBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (percentGrowth < 100) currenttip.add(TFC_Core.translate("gui.growth") + " : " + percentGrowth + "%");
        else currenttip.add(TFC_Core.translate("gui.growth") + " : " + TFC_Core.translate("gui.mature"));
    ...
    public List<String> farmlandBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        IWailaConfigHandler config) {
        SkillRank rank = TFC_Core.getSkillStats(accessor.getPlayer())
            .getSkillRank(Global.SKILL_AGRICULTURE);
        if (rank == SkillRank.Expert || rank == SkillRank.Master) {
            ...
            for (int i = 0; i < nutrients.length; i++) {
                ...
                else if (i == 1) currenttip
                    .add(EnumChatFormatting.GOLD + TFC_Core.translate("gui.Nutrient.B") + " : " + percent + "%");
                else if (i == 2) currenttip
                    .add(EnumChatFormatting.YELLOW + TFC_Core.translate("gui.Nutrient.C") + " : " + percent + "%");
                else if (i == 3 && percent != 0) currenttip
                    .add(EnumChatFormatting.WHITE + TFC_Core.translate("item.Fertilizer.name") + " : " + percent + "%");
            }
        }
        ...
        if (storage != null) {
            ...
            for (ItemStack is : storage) {
                if (is != null && is.getItem() != null
                    && (is.getItem() == TFCItems.logs || is.getItem() == Item.getItemFromBlock(TFCBlocks.peat)))
                    fuelCount++;
            }

            if (fuelCount > 0) currenttip.add(TFC_Core.translate("gui.fuel") + " : " + fuelCount + "/4");
        }
    ...
    public List<String> forgeBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (storage != null) {
            ...
            if (fuelCount > 0) currenttip.add(TFC_Core.translate("gui.fuel") + " : " + fuelCount + "/5");
            ...
            if (hasMold) currenttip.add(
                TFC_Core.translate("item.Mold.Ceramic Mold.name") + EnumChatFormatting.GREEN.toString() + " \u2714");
            else currenttip
                .add(TFC_Core.translate("item.Mold.Ceramic Mold.name") + EnumChatFormatting.RED.toString() + " \u2718");
        }
    ...
    public List<String> logPileBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (isOnFire) {
            ...
            currenttip.add(
                hours + " "
                    + TFC_Core.translate("gui.hoursRemaining")
                    + " ("
                    + Helper.roundNumber(100 - (hours / TFCOptions.charcoalPitBurnTime) * 100, 10)
                    + "%)");
        } else {
            ItemStack storage[] = getStorage(tag, accessor.getTileEntity());
            boolean counted[] = new boolean[] { false, false, false, false }; // Used to keep track of which slots have
                                                                              // already been combined into others.

            /**
             * Rather than just display the number of logs in each slot, I wanted to display the total number of each
             * type of log in the pile.
            ...
            {
                if (storage[j] != null && !counted[j]) // Make sure the slot is not empty, and has not already been
                                                       // accounted for
                {
                    ...
                    {
                        if (storage[k] != null && storage[j].isItemEqual(storage[k])) // Make sure the comparison slot
                                                                                      // isn't empty, and see if it's
    ...
    public List<String> loomBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (!finished && storage[0] != null) {
            LoomRecipe recipe = LoomManager.getInstance()
            ...
            {
                String name = recipe.getOutItemStack()
                    .getDisplayName() + " : ";
        ...
        for (ItemStack is : storage) {
            if (is != null && is.getItem() == TFCItems.egg) {
                if (is.hasTagCompound() && is.getTagCompound()
                    .hasKey("Fertilized")) fertEggCount++;
                else eggCount++;
            }
        }

        if (eggCount > 0) currenttip.add(TFC_Core.translate("gui.eggs") + " : " + eggCount);
        if (fertEggCount > 0) currenttip.add(TFC_Core.translate("gui.fertEggs") + " : " + fertEggCount);
    ...
    public List<String> oilLampBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (tag.hasKey("Fuel")) {
            ...
            if (fuel.getFluid() == TFCFluids.OLIVEOIL) currenttip.add(
                hours + " "
                    + TFC_Core.translate("gui.hoursRemaining")
                    + " ("
                    + Helper.roundNumber((hours / (250f * TFCOptions.oilLampFuelMult)) * 100f, 10)
                    + "%)");
    ...
    public List<String> oreBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (accessor.getBlock() == TFCBlocks.ore) {
            switch (meta) {
                ...
                case 2:
                    currenttip
                        .add(TFC_Core.translate("gui.metal.Platinum") + " - " + TFC_Core.translate("gui.useless"));
                    break;
                case 3:
                case 10:
                case 11:
            ...
            if (config.getConfig("tfc.oreQuality")) {
                ...
                if (units > 0) currenttip.add(TFC_Core.translate("gui.units") + " : " + units);
            }
    ...
    public List<String> potteryBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (straw > 0 && straw < 8) currenttip.add(TFC_Core.translate("item.Straw.name") + " : " + straw + "/8");
        else if (wood > 0 && wood < 8) currenttip.add(TFC_Core.translate("gui.logs") + " : " + wood + "/8");
        else if (burnStart > 0) {
            int hours = (int) (TFCOptions.pitKilnBurnTime
                - (TFC_Time.getTotalHours() - (burnStart / TFC_Time.HOUR_LENGTH)));
            currenttip.add(
                hours + " "
                    + TFC_Core.translate("gui.hoursRemaining")
                    + " ("
                    + Helper.roundNumber(100 - (hours / TFCOptions.pitKilnBurnTime) * 100, 10)
                    + "%)");
        }
    ...
    public List<String> sluiceBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (soilAmount == -1) currenttip.add(TFC_Core.translate("gui.Sluice.Overworked"));
        ...
        for (ItemStack is : storage) {
            if (is != null) {
                if (is.getItem() instanceof ItemGem) gemCount++;
                else if (is.getItem() instanceof ItemOre) oreCount++;
            }
        }

        if (gemCount > 0) currenttip.add(TFC_Core.translate("gui.gems") + " : " + gemCount);
        if (oreCount > 0) currenttip.add(TFC_Core.translate("gui.Bloomery.Ore") + " : " + oreCount);
        ...
        for (int i = 0; i < storage.length; i++) {
            ...
            if (is != null) {
                ...
                if (smokeHours < Food.SMOKEHOURS && !Food.isSmoked(is)) {
                    ...
                    currenttip.add(
                        "\u00B7 " + dryHours + " " + TFC_Core.translate("gui.hoursRemaining") + " (" + percent + "%)");
                } else currenttip.add(is.getDisplayName());
            }
        }
    ...
    public List<String> soilBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (dam < Global.STONE_ALL.length) currenttip.add(Global.STONE_ALL[dam]);
    ...
    public List<String> torchBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (accessor.getMetadata() < 8 && TFCOptions.torchBurnTime != 0) {
            ...
            if (hours > 0) currenttip.add(
                hours + " "
                    + TFC_Core.translate("gui.hoursRemaining")
                    + " ("
                    + Helper.roundNumber((100f * hours) / TFCOptions.torchBurnTime, 10)
                    + "%)");
        }
        return currenttip;
    }
    ...
    private ItemStack[] getStorage(NBTTagCompound tag, TileEntity te) {
        if (te instanceof IInventory) {
            ...
            for (int i = 0; i < tagList.tagCount(); i++) {
                ...
                if (slot >= 0 && slot < storage.length) storage[slot] = ItemStack.loadItemStackFromNBT(itemTag);
            }
