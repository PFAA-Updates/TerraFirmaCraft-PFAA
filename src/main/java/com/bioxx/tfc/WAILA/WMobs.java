...
public class WMobs implements IWailaEntityProvider {
    ...
    public List<String> getWailaHead(Entity entity, List<String> currenttip, IWailaEntityAccessor accessor,
        ...
        if (entity instanceof IAnimal) {
            ...
            if (config.getConfig("tfc.baby")) {
                if (!animal.isAdult()) head = EnumChatFormatting.WHITE + TFC_Core.translate("gui.baby") + " " + head;
            }

            if (config.getConfig("tfc.gender")) {
                if (animal.getGender() == GenderEnum.MALE) head += " \u2642";
                else if (animal.getGender() == GenderEnum.FEMALE) head += " \u2640";
            }
        }
    ...
    public List<String> getWailaBody(Entity entity, List<String> currenttip, IWailaEntityAccessor accessor,
        ...
        if (pregnant) {
            currenttip.add(
                TFC_Core.translate("entity.pregnant") + " : "
                    + TFC_Time.getDateStringFromHours(animal.getDueDay() * TFC_Time.HOURS_IN_DAY));
        }

        if (config.getConfig("tfc.familiarToday") && animal.canFamiliarize()) {
            if (familiarizedToday) currenttip
                .add(TFC_Core.translate("gui.familiarized") + EnumChatFormatting.GREEN.toString() + " \u2714");
            else currenttip.add(TFC_Core.translate("gui.familiarized") + EnumChatFormatting.RED.toString() + " \u2718");
        }
        ...
        if (animal instanceof EntityCowTFC && animal.getGender() == GenderEnum.FEMALE && animal.isAdult()) {
            ...
            else currenttip.add(TFC_Core.translate("fluid.milk") + EnumChatFormatting.RED.toString() + " \u2718");
        }
