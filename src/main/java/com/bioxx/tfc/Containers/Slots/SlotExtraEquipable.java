...
public class SlotExtraEquipable extends Slot {
    ...
    /**
     * Leave as-is for now. In the future, modify it to allow non-armor items to be in here. Set up a method to register
     * certain item
    ...
    public boolean isItemValid(ItemStack is) {
        if (is != null && is.getItem() instanceof IEquipable)
            if (((IEquipable) is.getItem()).getEquipType(is) == armorType) return true;
