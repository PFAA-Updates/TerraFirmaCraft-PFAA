...
public class SlotSize extends Slot {
    ...
    public boolean isItemValid(ItemStack itemstack) {
        ...
        if (itemstack.getItem() instanceof ISize
            ...
            && !except
            && include) return true;
        else if (!(itemstack.getItem() instanceof ISize) && !except && include) return true;
        return false;
    }
    ...
    public SlotSize addItemException(Item... ex) {
        for (int i = 0; i < ex.length; i++) excpetions.add(ex[i]);
        return this;
    }

    public SlotSize addItemInclusion(Item... ex) {
        for (int i = 0; i < ex.length; i++) inclusions.add(ex[i]);
        return this;
    }
}
