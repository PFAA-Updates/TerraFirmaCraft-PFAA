...
public class ContainerAnvil extends ContainerTFC {
    ...
    public ItemStack transferStackInSlotTFC(EntityPlayer player, int slotNum) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotNum < 7) {
                if (!this.mergeItemStack(slotStack, 7, inventorySlots.size(), true)) return null;
            }
            // Flux
            else if (slotStack.getItem() == TFCItems.powder && slotStack.getItemDamage() == 0) {
                if (!this.mergeItemStack(slotStack, 6, 7, false)) return null;
            }
            // Hammer
            else if (slotStack.getItem() instanceof ItemHammer) {
                if (slothammer.getHasStack()) return null;
    ...
    public void detectAndSendChanges() {
        ...
        for (int var1 = 0; var1 < this.crafters.size(); ++var1) {
            ...
            if (this.redIndicator != cv) var2.sendProgressBarUpdate(this, 0, cv);
            if (this.greenIndicator != icv) var2.sendProgressBarUpdate(this, 1, icv);
            if (this.tier != t) var2.sendProgressBarUpdate(this, 2, t);
        }
    ...
    public void updateProgressBar(int par1, int par2) {
        if (anvil != null) {
            if (par1 == 0) this.anvil.craftingValue = par2;
            else if (par1 == 1) this.anvil.itemCraftingValue = par2;
            else if (par1 == 2) this.anvil.anvilTier = par2;
        }
    }
