...
public class ContainerFirepit extends ContainerTFC {
    ...
    public ContainerFirepit(InventoryPlayer inventoryplayer, TEFirepit tileentityfirepit, World world, int x, int y,
        int z) {
    ...
    public ItemStack transferStackInSlotTFC(EntityPlayer player, int slotNum) {
        ...
        Slot[] slotfuel = { (Slot) inventorySlots.get(1), (Slot) inventorySlots.get(3), (Slot) inventorySlots.get(4),
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotNum < 11) {
                if (!this.mergeItemStack(slotStack, 11, this.inventorySlots.size(), true)) return null;
            } else {
                HeatRegistry manager = HeatRegistry.getInstance();

                // Fuel to the fuel input slot
                if (slotStack.getItem() == TFCItems.logs
                    || slotStack.getItem() == Item.getItemFromBlock(TFCBlocks.peat)) {
                    if (slotfuel[0].getHasStack()) return null;
                ...
                // No ores, but anything else with a heat index to the input slot
                    ...
                    if (slotinput.getHasStack()) return null;
                    ItemStack stack = slotStack.copy();
                    stack.stackSize = 1;
    ...
    public void updateProgressBar(int par1, int par2) {
        if (par1 == 0) this.firepit.fireTemp = par2;
    }
}
