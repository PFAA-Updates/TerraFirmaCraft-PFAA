...
public class ContainerPlayerTFC extends ContainerPlayer {
    ...
    public ContainerPlayerTFC(InventoryPlayer playerInv, boolean par2, EntityPlayer player) {
        ...
        for (x = 0; x < 2; ++x) {
            for (y = 0; y < 2; ++y) this.addSlotToContainer(new Slot(craftMatrix, y + x * 3, 82 + y * 18, 18 + x * 18));
        }
        ...
        // Manually built the remaining crafting slots because of an order issue. These have to be created after the
        // default slots
        if (player.getEntityData()
            .hasKey("craftingTable") || !player.worldObj.isRemote) {
            x = 2;
            y = 0;
            ...
            x = 2;
            y = 0;
            this.addSlotToContainer(new Slot(craftMatrix, y + x * 3, 82 + y * 18 - 50000, 18 + x * 18));
            x = 2;
            y = 1;
            this.addSlotToContainer(new Slot(craftMatrix, y + x * 3, 82 + y * 18 - 50000, 18 + x * 18));
            x = 0;
    ...
    public void onCraftMatrixChanged(IInventory iinventory) {
        ...
        if (craftOut != null && craftOut.getHasStack()) {
            ...
            if (craftResult != null) {
                ...
                else CraftingHandler.transferNBT(false, thePlayer, craftResult, craftMatrix);
            }
        }
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slotNum) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotNum == 0) {
                ...
                if (!this.mergeItemStack(slotStack, 9, 45, true, true)) return null;

                slot.onSlotChange(slotStack, origStack);
            }
            // From crafting grid input to inventory
            else if (slotNum >= 1 && slotNum < 5 || player.getEntityData()
                .hasKey("craftingTable") && slotNum >= 45
                && slotNum < 50) {
                    if (!this.mergeItemStack(slotStack, 9, 45, true, false)) return null;
                }
            ...
            // From inventory to armor slots
                ...
                if (origStack.getItem() instanceof ItemTFCArmor) {
                    ...
                    if (!((Slot) this.inventorySlots.get(armorSlotNum)).getHasStack()) {
                        if (!this.mergeItemStack(slotStack, armorSlotNum, armorSlotNum + 1, false, false)) return null;
                    }
            ...
            // From inventory to back slot
                ...
                if (equipment.getEquipType(origStack) == EquipType.BACK
                    && (equipment == TFCItems.quiver || equipment.getTooHeavyToCarry(origStack))) {
            ...
            else if (slotNum >= 9 && slotNum < 45
                && origStack.getItem() instanceof IFood
                && !(origStack.getItem() instanceof ItemMeal)
                && !isCraftingGridFull()) {
                    if (!this.mergeItemStack(slotStack, 1, 5, false, false) && slotStack.stackSize == 0) return null;
                    else if (slotStack.stackSize > 0 && player.getEntityData()
                        .hasKey("craftingTable") && !this.mergeItemStack(slotStack, 45, 50, false)) return null;
                    else if (slotStack.stackSize > 0 && slotNum >= 9 && slotNum < 36) {
                        if (!this.mergeItemStack(slotStack, 36, 45, false, false)) return null;
                    } else if (slotStack.stackSize > 0 && slotNum >= 36 && slotNum < 45) {
                        if (!this.mergeItemStack(slotStack, 9, 36, false, false)) return null;
                    }
                }
    ...
    public ItemStack slotClick(int sourceSlotID, int destSlotID, int clickType, EntityPlayer p) {
        if (sourceSlotID >= 0 && sourceSlotID < this.inventorySlots.size()) {
            ...
            // Couldn't figure out what was causing the food dupe with a full inventory, so we're just going to block
            // shift clicking for that case.
            else if (clickType == 1 && sourceSlotID == 0
                && isInventoryFull()
                && slotStack != null
                && slotStack.getItem() instanceof IFood) return null;
        }
    ...
    protected boolean isCraftingGridFull() {
        for (int i = 0; i < this.craftMatrix.getSizeInventory(); i++) {
            if (this.craftMatrix.getStackInSlot(i) == null) return false;
        }
        return true;
    }
    ...
    protected boolean isInventoryFull() {
        ...
        for (int i = 9; i < 45; i++) {
            if (((Slot) inventorySlots.get(i)).getStack() == null) return false;
        }
        return true;
    }
    ...
    protected boolean mergeItemStack(ItemStack is, int slotStart, int slotFinish, boolean reverseOrder,
        boolean craftingOutput) {
        boolean merged = false;
        int slotIndex = slotStart;

        if (reverseOrder) slotIndex = slotFinish - 1;
        ...
        if (is.isStackable()) {
            while (is.stackSize > 0
                && (!reverseOrder && slotIndex < slotFinish || reverseOrder && slotIndex >= slotStart)) {
                ...
                // && !is.getHasSubtypes()
                    ...
                    // First we check if we can add the two stacks together and the resulting stack is smaller than the
                    // maximum size for the slot or the stack
                    ...
                    // Do not attempt merge stacks resulting in greater than the max size for slot/stack if
                    // shift-clicking the crafting grid output
                    else if (!craftingOutput && slotstack.stackSize < is.getMaxStackSize()
                        && slotstack.stackSize < slot.getSlotStackLimit()) {
                            // Slot stack size is greater than or equal to the item's max stack size. Most containers
                            // are this case.
