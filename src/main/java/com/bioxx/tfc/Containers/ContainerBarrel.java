...
public class ContainerBarrel extends ContainerTFC {
    ...
    protected void buildLayout() {
        if (guiTab == 0) {
            // Input slot
            if (!barrel.getSealed()) addSlotToContainer(
                new SlotChest(barrel, 0, 80, 29).setSize(EnumSize.LARGE)
                    .addItemException(getExceptions()));
            else addSlotToContainer(new SlotForShowOnly(barrel, 0, 80, 29));
        } else if (guiTab == 1) {
            for (int i = 0; i < 4; i++) {
                for (int k = 0; k < 3; k++) {
                    if (!barrel.getSealed()) addSlotToContainer(
                        new SlotChest(barrel, k + (i * 3), 53 + (i * 18), 17 + (k * 18)).setSize(EnumSize.LARGE)
                            .addItemException(ContainerChestTFC.getExceptions()));
                    else addSlotToContainer(new SlotForShowOnly(barrel, k + (i * 3), 53 + (i * 18), 17 + (k * 18)));
                }
            }
        }
    }
    ...
    public ItemStack transferStackInSlotTFC(EntityPlayer player, int slotNum) {
        ...
        if (!barrel.getSealed() && slot != null && slot.getHasStack()) {
            ...
            // From solid storage slots to inventory
                ...
                if (!this.mergeItemStack(slotStack, 12, this.inventorySlots.size(), true)) return null;
            } else {
                // To solid storage
                if (guiTab == 1) {
                    if (!this.mergeItemStack(slotStack, 0, 12, false)) return null;
                }
    ...
    public void detectAndSendChanges() {
        ...
        for (int var1 = 0; var1 < this.crafters.size() && guiTab == 0; ++var1) {
            ...
            if (this.barrel.getFluidStack() != null && this.liquidID != this.barrel.getFluidStack()
                .getFluidID()) {
                liquidID = barrel.getFluidStack()
                    .getFluidID();
                var2.sendProgressBarUpdate(
                    this,
                    0,
                    this.barrel.getFluidStack()
                        .getFluidID());
            }
    ...
    public void updateProgressBar(int id, int val) {
        if (id == 0) {
            ...
            if (barrel.fluid != null) this.barrel.fluid.amount = val;
        }
    }
}
