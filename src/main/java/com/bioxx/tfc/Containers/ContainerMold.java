...
public class ContainerMold extends ContainerTFC {
    ...
    public ContainerMold(InventoryPlayer playerinv, World world, int x, int y, int z) {
        ...
        PlayerInfo pi = PlayerManagerTFC.getInstance()
            .getPlayerInfoFromPlayer(playerinv.player);
    ...
    public void onContainerClosed(EntityPlayer player) {
        ...
        if (!this.world.isRemote) {
            ...
            if (itemstack != null) player.entityDropItem(itemstack, 0);
            if (itemstack2 != null) player.entityDropItem(itemstack2, 0);
            if (itemstack3 != null) player.entityDropItem(itemstack3, 0);
        }
    }
    ...
    public void detectAndSendChanges() {
        ...
        if (craftResult.getStackInSlot(0) == null) {
            ...
            if (sourceStack != null && inputStack != null) {
                ...
                if (sourceItem instanceof ItemMeltedMetal && inputItem == TFCItems.ceramicMold
                    && inputDamage == 1
                    && TFC_ItemHeat.getIsLiquid(sourceStack)) {
                    ItemStack is = sourceStack.copy();
                    is.setItemDamage(100);
                    containerInv.setInventorySlotContents(1, is);
                    pi.moldTransferTimer = 100;
                } else if (sourceItem instanceof ItemMeltedMetal && inputItem instanceof ItemMeltedMetal
            ...
            if (inputStack != null && pi.moldTransferTimer < 100
                && CraftingManagerTFC.getInstance()
                    .findMatchingRecipe(this.containerInv, world) != null) {
            ...
            if (sourceStack != null && inputStack != null && pi.moldTransferTimer == 100) {
                ...
                int inputDamage = inputStack.getItemDamage();
                ItemStack recipeOutput = CraftingManagerTFC.getInstance()
                    .findMatchingRecipe(this.containerInv, world);
                ...
                if (sourceItem instanceof ItemMeltedMetal && inputItem instanceof ItemMeltedMetal) {
                    if (sourceItem == inputItem && inputDamage != 0) {
                        ...
                        if (newSourceDamage >= sourceStack.getMaxDamage() - 1) // Subtract one so we never have an
                                                                               // unshaped metal with 0/100 units
                        {
    ...
    public ItemStack transferStackInSlotTFC(EntityPlayer player, int slotNum) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            // To inventory
            if (slotNum < 3) {
                if (!this.mergeItemStack(slotStack, 3, inventorySlots.size(), true)) return null;
            } else {
                if (!this.mergeItemStack(slotStack, 0, 3, false)) return null;
            }
