...
public class ChunkEventHandler {
    ...
    public void onLoad(ChunkEvent.Load event) {
        if (!event.world.isRemote && TFC_Core.getCDM(event.world) != null && event.getChunk() != null) {
            ChunkData cd = TFC_Core.getCDM(event.world)
                .getData(event.getChunk().xPosition, event.getChunk().zPosition);
            if (cd == null) return;
            BiomeGenBase biome = event.world
                .getBiomeGenForCoords(event.getChunk().xPosition, event.getChunk().zPosition);
            ...
            if (TFC_Time.getYear() > cd.lastSpringGen && month > 1 && month < 6) {
                ...
                int cropid = rand.nextInt(
                    CropManager.getInstance()
                        .getTotalCrops());
            ...
            data.rainfallMap = TFC_Climate.getCacheManager(event.world)
                .loadRainfallLayerGeneratorData(
                    data.rainfallMap,
                    event.getChunk().xPosition * 16,
                    event.getChunk().zPosition * 16,
                    16,
                    16);
            TFC_Core.getCDM(event.world)
                .addData(chunk, data);
        }
    }
    ...
    public void onUnload(ChunkEvent.Unload event) {
        if (TFC_Core.getCDM(event.world) != null && TFC_Core.getCDM(event.world)
            .getData(event.getChunk().xPosition, event.getChunk().zPosition) != null)
            TFC_Core.getCDM(event.world)
                .getData(event.getChunk().xPosition, event.getChunk().zPosition).isUnloaded = true;
    }
    ...
    public void onUnloadWorld(WorldEvent.Unload event) {
        ...
        if (event.world.provider.dimensionId == 0) AnvilManager.getInstance()
            .clearRecipes();
    }

    @SubscribeEvent
    public void onLoadWorld(WorldEvent.Load event) {
        if (event.world.provider.dimensionId == 0 && event.world.getTotalWorldTime() < 100) createSpawn(event.world);
        if (!event.world.isRemote && event.world.provider.dimensionId == 0
            && AnvilManager.getInstance()
                .getRecipeList()
                .size() == 0) {
        ...
        if (!event.world.isRemote) {
            ...
            if (eventTag.hasKey("ChunkData")) {
                ...
                ChunkData data = new ChunkData(chunk)
                    .createNew(event.world, levelTag.getInteger("xPos"), levelTag.getInteger("zPos"));
                if (TFC_Core.getCDM(event.world) != null) TFC_Core.getCDM(event.world)
                    .addData(chunk, data);
            }
        }
    ...
    public void onDataSave(ChunkDataEvent.Save event) {
        if (!event.world.isRemote && TFC_Core.getCDM(event.world) != null) {
            ...
            if (data != null) {
                ...
                event.getData()
                    .setTag("ChunkData", spawnProtectionTag);
                if (data.isUnloaded) TFC_Core.getCDM(event.world)
                    .removeData(x, z);
            }
        }
    }
    ...
    private ChunkCoordinates createSpawn(World world) {
        List biomeList = world.getWorldChunkManager()
            .getBiomesToSpawnIn();
        long seed = world.getWorldInfo()
            .getSeed();
        ...
        while (chunkCoord == null) {
            chunkCoord = world.getWorldChunkManager()
                .findBiomePosition(xOffset, -startingZ, 64, biomeList, rand);
            if (chunkCoord != null) {
        ...
        if (!info.getNBTTagCompound()
            .hasKey("superseed"))
            info.getNBTTagCompound()
                .setLong("superseed", System.currentTimeMillis());
