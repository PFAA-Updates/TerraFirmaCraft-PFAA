    ...
    public void onEnterChunk(EnteringChunk event) {
        if (event.entity instanceof EntityPlayer) {
            ...
            if (!player.worldObj.isRemote) {
                ...
                if (TFC_Core.getCDM(event.entity.worldObj) != null
                    && (event.newChunkX != event.oldChunkX || event.newChunkZ != event.oldChunkZ)) {
                    TFC_Core.getCDM(event.entity.worldObj)
                        .setLastVisted(event.oldChunkX, event.oldChunkZ);
