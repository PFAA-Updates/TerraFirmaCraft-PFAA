...
public class EntitySpawnHandler {
    ...
    public void onCheckSpawn(LivingSpawnEvent.CheckSpawn event) {
        ...
        if (event.world
            .getBlock((int) Math.floor(entity.posX), (int) Math.floor(entity.posY), (int) Math.floor(entity.posZ))
            == TFCBlocks.thatch) {
        ...
        if (TFC_Core.getCDM(event.world) != null) {
            ChunkData data = TFC_Core.getCDM(event.world)
                .getData(chunkX, chunkZ);
            if (!(data == null || data.getSpawnProtectionWithUpdate() <= 0)) event.setResult(Result.DENY);
        }
    }

    @SubscribeEvent
    public void onJoinWorld(EntityJoinWorldEvent event) {
        if (event.entity instanceof EntityPlayer && !event.entity.getEntityData()
            .hasKey("hasSpawned")) {
            ...
            ((EntityPlayer) event.entity).getEntityAttribute(SharedMonsterAttributes.maxHealth)
                .setBaseValue(1000);
            ((EntityPlayer) event.entity).setHealth(1000);
            event.entity.getEntityData()
                .setBoolean("hasSpawned", true);
        }
        ...
        if (event.entity instanceof EntityPlayer) {
            ...
            ((EntityPlayer) event.entity).inventoryContainer = new ContainerPlayerTFC(
