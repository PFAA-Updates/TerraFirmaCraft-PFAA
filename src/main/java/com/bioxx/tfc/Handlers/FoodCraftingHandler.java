...
public class FoodCraftingHandler {
        ...
        if (craftingInv != null) {
            if (refiningGrain(craftResult, craftingInv)) {
                ...
                for (int i = 0; i < craftingInv.getSizeInventory(); i++) {
                    ...
                    if (inputStack == null || !(inputStack.getItem() instanceof IFood)) continue;

                    float foodWeight = Food.getWeight(inputStack);
                    int strawCount = 0;

                    for (int j = 0; j < foodWeight; j += 4) strawCount++;
                ...
                for (int i = 0; i < craftingInv.getSizeInventory(); i++) {
                    ...
                    if (grainWeight > 0) {
                        ...
                        if (inputStack.stackSize > 2) inputStack.stackSize = 2;
                    }
                }
            } else if (craftResult.hasTagCompound() && craftResult.getTagCompound()
                .hasKey(Food.WEIGHT_TAG)) {
    ...
    private static ItemStack processFoodInput(EntityPlayer player, ItemStack craftResult, IInventory craftingInv) {
        ...
        int foodCount = 0;
        int itemCount = 0;
        int foodSlot = 0; // This is used when cutting food to track where the food originally was since the merge code
                          // may remove the stack
        for (int i = 0; i < craftingInv.getSizeInventory(); i++) {
            ItemStack is = craftingInv.getStackInSlot(i);
            if (is == null) continue;
            itemCount++;
            if (is.getItem() instanceof ItemFoodTFC && is.hasTagCompound()
                && is.getTagCompound()
                    .hasKey(Food.WEIGHT_TAG)) {
                foodSlot = i;
                if (foodCount == 0) {
                ...
                if (inputWeight != oldInputWeight) {
                    if (inputWeight == 0) // The input is being completely combined
                    {
                        ...
                        } else finalDecay += inputDecay; // Add the decay from the input to the output
                    } else {
                        float decayChange = weightChange * inputDecayPercent;
                        inputDecay -= decayChange; // Remove the decay from the input
                        if (finalDecay < 0) // Still within the 24 hour grace period
                        {
                            if (decayChange > finalDecay) // The input is losing more decay than what the output
                                                          // currently has
                                finalDecay = decayChange; // Set the output's decay to the amount removed from the input
                        } else finalDecay += decayChange; // Add the decay to the output
                    }
                    foodCount++;
                }
                ...
                if (inputWeight > 0) // If we're leaving a piece of food in the crafting grid
                {
                    ...
                    is.stackSize++;
                    if (is.stackSize > 2) is.stackSize = 2;
                }
            }
        }

        if (craftResult.stackSize == 0) craftResult.stackSize = 1;
            ...
            for (int i = 0; i < craftingInv.getSizeInventory(); i++) {
                ...
                if (itemstack.getItem() instanceof ItemKnife && fullInv) {
                    if (!FoodCraftingHandler.preCrafted) {
                        ...
                        itemstack.stackSize++;
                        if (itemstack.stackSize > 2) itemstack.stackSize = 2;
                    }
                }
                ...
                if (itemstack.getItem() instanceof ItemKnife && (!fullInv || !FoodCraftingHandler.preCrafted)) {
                    ...
                    } else if (finalDecay <= 0) // Splitting food in half
                    {
                        if (finalWeight / 2f < 1) // Food is too small to split
                        {
                            ...
                            FoodCraftingHandler.damageItem(player, craftingInv, i, itemstack.getItem());
                            Food.setWeight(
                                craftingInv.getStackInSlot(foodSlot),
                                Helper.roundNumber(finalWeight / 2f, 100));
    ...
    public static void updateOutput(EntityPlayer player, ItemStack craftResult, IInventory craftingInv) {
        ...
        int itemCount = 0;
        for (int i = 0; i < craftingInv.getSizeInventory(); i++) {
            if (craftingInv.getStackInSlot(i) == null) continue;
            itemCount++;
            ItemStack is = craftingInv.getStackInSlot(i);
            if (is.getItem() instanceof ItemFoodTFC && is.hasTagCompound()
                && is.getTagCompound()
                ...
                if (sweetMod == -1) sweetMod = Food.getSweetMod(is);
                else if (sweetMod != Food.getSweetMod(is)) sweetMod = 0;

                if (sourMod == -1) sourMod = Food.getSourMod(is);
                else if (sourMod != Food.getSourMod(is)) sourMod = 0;

                if (saltyMod == -1) saltyMod = Food.getSaltyMod(is);
                else if (saltyMod != Food.getSaltyMod(is)) saltyMod = 0;

                if (bitterMod == -1) bitterMod = Food.getBitterMod(is);
                else if (bitterMod != Food.getBitterMod(is)) bitterMod = 0;

                if (umamiMod == -1) umamiMod = Food.getSavoryMod(is);
                else if (umamiMod != Food.getSavoryMod(is)) umamiMod = 0;
        ...
        if (itemCount == 1) // Trimming decay with a knife from the inventory, not the crafting grid.
        {
            ...
            for (int i = 0; i < craftingInv.getSizeInventory(); i++) {
                ...
                if (inputStack.getItem() == TFCItems.powder && inputStack.getItemDamage() == 9) {
                    ...
                    {
                        if (!refiningGrain(craftResult, craftingInv) && finalWeight / 2f >= 1) // Must be big enough to
                                                                                               // split
                        {
                            finalWeight /= 2f;
                        }
        ...
        craftResult = ItemFoodTFC
            .createTag(craftResult, Helper.roundNumber(finalWeight, 100), Helper.roundNumber(finalDecay, 100));
        if (sweetMod != 0) Food.setSweetMod(craftResult, sweetMod);
        if (sourMod != 0) Food.setSourMod(craftResult, sourMod);
        if (saltyMod != 0) Food.setSaltyMod(craftResult, saltyMod);
        if (bitterMod != 0) Food.setBitterMod(craftResult, bitterMod);
        if (umamiMod != 0) Food.setSavoryMod(craftResult, umamiMod);

        if (cookedTime > 0) Food.setCooked(craftResult, cookedTime);
        ...
        if (salted) Food.setSalted(craftResult, salted);
        if (pickled) Food.setPickled(craftResult, pickled);
        if (brined) Food.setBrined(craftResult, brined);
        ...
        if (infusion != null) Food.setInfusion(craftResult, infusion);
    ...
    public static boolean makingDough(ItemStack craftResult, IInventory iinventory) {
        return (craftResult.getItem() == TFCItems.wheatDough || craftResult.getItem() == TFCItems.ryeDough
            || craftResult.getItem() == TFCItems.oatDough
            || craftResult.getItem() == TFCItems.barleyDough
            || craftResult.getItem() == TFCItems.cornmealDough
            || craftResult.getItem() == TFCItems.riceDough)
            && (gridHasItem(iinventory, TFCItems.woodenBucketWater)
                || gridHasItem(iinventory, TFCItems.redSteelBucketWater));
    }

    public static boolean isInvFull(EntityPlayer player) {
        for (int i = 0; i < player.inventory.mainInventory.length; i++) {
            if (player.inventory.mainInventory[i] == null) return false;
        }
        return true;
    }

    /**
     * Should only make changes to the craftResult and should allow the normal crafting handler to change the
     * craftMatrix. This is
    ...
    public static void preCraft(EntityPlayer player, ItemStack craftResult, IInventory craftingInv) {
        ...
        if (refiningGrain(craftResult, craftingInv)) {
            ...
            for (int i = 0; i < craftingInv.getSizeInventory(); i++) {
                ...
                if (grainWeight > 0) inputStack.stackSize++;
            }
        } else if (craftResult.hasTagCompound() && craftResult.getTagCompound()
            .hasKey(Food.WEIGHT_TAG)) {
    ...
    public static boolean gridHasItem(IInventory iinventory, Item id) {
        for (int i = 0; i < iinventory.getSizeInventory(); i++) {
            ...
            if (iinventory.getStackInSlot(i)
    ...
    public static void handleItem(EntityPlayer entityplayer, IInventory iinventory, Item[] items) {
        for (int i = 0; i < iinventory.getSizeInventory(); i++) {
            if (iinventory.getStackInSlot(i) == null) continue;
            for (int j = 0; j < items.length; j++) damageItem(entityplayer, iinventory, i, items[j]);
        }
    }
    ...
    public static void handleItem(EntityPlayer entityplayer, IInventory iinventory, List<ItemStack> items) {
        for (int i = 0; i < iinventory.getSizeInventory(); i++) {
            if (iinventory.getStackInSlot(i) == null) continue;
            for (ItemStack is : items) damageItem(entityplayer, iinventory, i, is.getItem());
        }
    }
    ...
    public static void damageItem(EntityPlayer entityplayer, IInventory iinventory, int i, Item item) {
        if (iinventory.getStackInSlot(i)
            .getItem() == item) {
            int index = i;
            ItemStack is = iinventory.getStackInSlot(index)
                .copy();
            if (is != null) {
                ...
                if (is.getItemDamage() != 0 || entityplayer.capabilities.isCreativeMode) {
                    ...
                    if (iinventory.getStackInSlot(index).stackSize > 2) iinventory.getStackInSlot(index).stackSize = 2;
                }
            }
        }
    }
