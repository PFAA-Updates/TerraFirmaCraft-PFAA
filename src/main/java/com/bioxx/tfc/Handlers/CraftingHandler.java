...
public class CraftingHandler {
    ...
    public void onCrafting(ItemCraftedEvent e)// (EntityPlayer player, ItemStack itemstack, IInventory iinventory)
    {
        ...
        if (iinventory != null) {
            ...
            if (item == TFCItems.stoneBrick) {
                ...
                || item == Item.getItemFromBlock(TFCBlocks.woodSupportV2)) {
                    ...
                    for (int i = 0; i < iinventory.getSizeInventory(); i++) {
                        ...
                        if (iinventory.getStackInSlot(i)
                            .getItem() == TFCItems.sheepSkin)
                            size = iinventory.getStackInSlot(i)
                                .getItemDamage();
                    }
            ...
            if (!player.worldObj.isRemote && item instanceof ItemIngot) {
                for (int i = 0; i < iinventory.getSizeInventory(); i++) {
                    ...
                    if (is == null) continue;
                        ...
                        if (player.worldObj.rand.nextInt(20) == 0) player.worldObj.playSoundAtEntity(
                            player,
                            TFC_Sounds.CERAMICBREAK,
                            0.7f,
                            player.worldObj.rand.nextFloat() * 0.2F + 0.8F);
                        else TFC_Core.giveItemToPlayer(new ItemStack(TFCItems.ceramicMold, 1, 1), player);
    ...
    public static void triggerAchievements(EntityPlayer player, ItemStack itemstack, Item item, int itemDamage) {
        ...
        else if (item == TFCItems.clayBall && itemDamage == 1) player.triggerAchievement(TFC_Achievements.achFireClay);
        else if (item == TFCItems.unknownIngot) player.triggerAchievement(TFC_Achievements.achUnknown);
    }
    ...
    public static void transferNBT(boolean clearContents, EntityPlayer player, ItemStack itemstack,
        ...
        if (item instanceof ItemIngot) {
            ...
            for (int i = 0; i < iinventory.getSizeInventory(); i++) {
                if (iinventory.getStackInSlot(i) == null) continue;
                if (iinventory.getStackInSlot(i)
                    .getItem() instanceof ItemMeltedMetal) {
            ...
            for (int i = 0; i < iinventory.getSizeInventory(); i++) {
                if (iinventory.getStackInSlot(i) == null) continue;
                if (iinventory.getStackInSlot(i)
                    .getItem() instanceof ItemIngot) temperature = TFC_ItemHeat.getTemp(iinventory.getStackInSlot(i));
            }
            TFC_ItemHeat.setTemp(itemstack, temperature);
            ...
            if (color != -1) {
                ...
                if (itemstack.hasTagCompound()) nbt = itemstack.getTagCompound();
        ...
        for (int i = 0; i < iinventory.getSizeInventory(); i++) {
            ...
            // Only transfer the tag when making something out of a tool head. Don't transfer when crafting things with
            // the completed tool.
            // Note: If crafting recipes with armor or completed tools to further refine them are ever added, the
            // instanceof will need to be updated. -Kitty
            if (iinventory.getStackInSlot(i)
                .getItem() instanceof ItemMiscToolHead && iinventory.getStackInSlot(i)
                    .hasTagCompound()
