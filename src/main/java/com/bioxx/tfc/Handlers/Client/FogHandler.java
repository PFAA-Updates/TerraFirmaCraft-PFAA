...
public class FogHandler {
    ...
    public void renderFogHandler(RenderFogEvent event) {
        ...
        if (event.fogMode >= 0) {
            ...
            if (shouldLerp) {
                ...
                // TerraFirmaCraft.log.info(event.renderPartialTicks + " | " +(lerpTime - lerpTimer)/20 + ": " +
                // fogStartBegin + " of " + fogStartFinish + " | " + fogEndBegin + " of " + fogEndFinish);
                if (lerpTimer >= lerpTime) shouldLerp = false;
    ...
    private void calcFog(RenderFogEvent event) {
        if (renderRange < 6) return;
        boolean raining = event.entity.worldObj.isRaining();
        float temp = TFC_Climate.getHeightAdjustedTemp(
            event.entity.worldObj,
            (int) event.entity.posX,
            (int) event.entity.posY,
            (int) event.entity.posZ);
        float localFog = WeatherManager.getInstance()
            .getLocalFog(
                event.entity.worldObj,
                (int) event.entity.posX,
                (int) event.entity.posY,
                (int) event.entity.posZ);
        if (localFog != localWorldFog) localFog = WeatherManager.getInstance()
            .getLocalFog(
                event.entity.worldObj,
                (int) event.entity.posX,
                (int) event.entity.posY,
        ...
        if (raining && temp > 0) {
            ...
            if (!snowLast) {
                ...
                rainLast = true;
                snowLast = true;
                snowStrength = WeatherManager.getInstance()
                    .getSnowStrength();
            ...
            } else if (snowLast && WeatherManager.getInstance()
                .getSnowStrength() != snowStrength) {
                    startLerp(300);
                    snowStrength = WeatherManager.getInstance()
                        .getSnowStrength();
                    fogStartFinish = event.farPlaneDistance * 0.1f;
