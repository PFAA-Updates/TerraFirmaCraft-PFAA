...
public class ArmourStandHighlightHandler {
    ...
    public void drawBlockHighlightEvent(DrawBlockHighlightEvent evt) {
        ...
        if (evt.currentItem == null
            && TFCBlocks.isArmourStand(world.getBlock(evt.target.blockX, evt.target.blockY, evt.target.blockZ))
            && (Math.sqrt(evt.target.hitVec.squareDistanceTo(evt.player.posX, evt.player.posY, evt.player.posZ)))
                < 2.5) {
            ...
            vectorTerm = vectorTerm
                .addVector(evt.target.hitVec.xCoord, evt.target.hitVec.yCoord, evt.target.hitVec.zCoord);
            ...
            AxisAlignedBB head = AxisAlignedBB.getBoundingBox(
                evt.target.blockX + 0.5 - (0.3 * 1),
                evt.target.blockY + (1.35) + (isTop ? -1 : 0),
                evt.target.blockZ + 0.5 - (0.3 * 1),
                evt.target.blockX + 0.5 + (0.3 * 1),
                evt.target.blockY + (1.95 + (isTop ? -1 : 0)),
                evt.target.blockZ + 0.5 + (0.3 * 1));
            // Body box
            AxisAlignedBB body = AxisAlignedBB.getBoundingBox(
                evt.target.blockX + 0.5 - (0.55 * 1),
                evt.target.blockY + (0.75) + (isTop ? -1 : 0),
                evt.target.blockZ + 0.5 - (0.17 * 1),
                evt.target.blockX + 0.5 + (0.55 * 1),
                evt.target.blockY + (1.5) + (isTop ? -1 : 0),
                evt.target.blockZ + 0.5 + (0.17 * 1));
            if (!ns) body = AxisAlignedBB.getBoundingBox(
                evt.target.blockX + 0.5 - (0.17 * 1),
                evt.target.blockY + (0.75) + (isTop ? -1 : 0),
                evt.target.blockZ + 0.5 - (0.55 * 1),
                evt.target.blockX + 0.5 + (0.17 * 1),
                evt.target.blockY + (1.5) + (isTop ? -1 : 0),
                evt.target.blockZ + 0.5 + (0.55 * 1));
            // Legs box
            AxisAlignedBB legs = AxisAlignedBB.getBoundingBox(
                evt.target.blockX + 0.5 - (0.35 * 0.9),
                evt.target.blockY + (1.8 - (1.6)) + (isTop ? -1 : 0),
                evt.target.blockZ + 0.5 - (0.155 * 0.9),
                evt.target.blockX + 0.5 + (0.35 * 0.9),
                evt.target.blockY + (1.8 - (1 * 0.9)) + (isTop ? -1 : 0),
                evt.target.blockZ + 0.5 + (0.155 * 0.9));
            if (!ns) legs = AxisAlignedBB.getBoundingBox(
                evt.target.blockX + 0.5 - (0.155 * 0.9),
                evt.target.blockY + (1.8 - (1.6)) + (isTop ? -1 : 0),
                evt.target.blockZ + 0.5 - (0.35 * 0.9),
                evt.target.blockX + 0.5 + (0.155 * 0.9),
                evt.target.blockY + (1.8 - (1 * 0.9)) + (isTop ? -1 : 0),
                evt.target.blockZ + 0.5 + (0.35 * 0.9));
            // Feet box
            AxisAlignedBB feet = AxisAlignedBB.getBoundingBox(
                evt.target.blockX + 0.5 - (0.35 * 1),
                evt.target.blockY + 0.15 + (isTop ? -1 : 0),
                evt.target.blockZ + 0.5 - (0.17 * 1),
                evt.target.blockX + 0.5 + (0.35 * 1),
                evt.target.blockY + (1.8 - (1.4)) + (isTop ? -1 : 0),
                evt.target.blockZ + 0.5 + (0.17 * 1));
            if (!ns) feet = AxisAlignedBB.getBoundingBox(
                evt.target.blockX + 0.5 - (0.17 * 1),
                evt.target.blockY + 0.15 + (isTop ? -1 : 0),
                evt.target.blockZ + 0.5 - (0.35 * 1),
                evt.target.blockX + 0.5 + (0.17 * 1),
                evt.target.blockY + (1.8 - (1.4)) + (isTop ? -1 : 0),
                evt.target.blockZ + 0.5 + (0.35 * 1));
            ...
            if (boxToRender != null) {
                ...
                // drawOutlinedBoundingBox(AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(evt.target.blockX+depth,evt.target.blockY+depth,evt.target.blockZ+depth,evt.target.blockX+1-depth,evt.target.blockY+1-depth,evt.target.blockZ+1-depth).expand(0.002F,
                // 0.002F, 0.002F).getOffsetBoundingBox(-var8, -var10, -var12));
                ...
                drawOutlinedBoundingBox(
                    boxToRender.expand(0.02F, 0.02F, 0.02F)
    ...
    public boolean isVecInsideBox(AxisAlignedBB aabb, EntityPlayer player, Vec3 unit, double xOffset, double yOffset,
        double zOffset) {
        unit = player.getLookVec();
        aabb.minY += 0.1;
        aabb.maxY += 0.1;
        Vec3 playerVec = Vec3.createVectorHelper(player.posX, player.posY + player.eyeHeight, player.posZ);
        // Vec3 negPlayerVec = Vec3.createVectorHelper(-player.lastTickPosX, -(player.lastTickPosY + player.eyeHeight),
        // -player.lastTickPosZ);
        // Vec3 zeroVec = Vec3.createVectorHelper(0,0,0);
        Vec3 distBlockxyz = playerVec.subtract(Vec3.createVectorHelper(aabb.minX, aabb.minY, aabb.minZ))
            .normalize();
        Vec3 distBlockXYZ = playerVec.subtract(Vec3.createVectorHelper(aabb.maxX, aabb.maxY, aabb.maxZ))
            .normalize();

        Vec3 distBlockxyZ = playerVec.subtract(Vec3.createVectorHelper(aabb.minX, aabb.minY, aabb.maxZ))
            .normalize();
        Vec3 distBlockXYz = playerVec.subtract(Vec3.createVectorHelper(aabb.maxX, aabb.maxY, aabb.minZ))
            .normalize();

        Vec3 distBlockxYZ = playerVec.subtract(Vec3.createVectorHelper(aabb.minX, aabb.maxY, aabb.maxZ))
            .normalize();
        Vec3 distBlockXyz = playerVec.subtract(Vec3.createVectorHelper(aabb.maxX, aabb.minY, aabb.minZ))
            .normalize();

        Vec3 distBlockxYz = playerVec.subtract(Vec3.createVectorHelper(aabb.minX, aabb.maxY, aabb.minZ))
            .normalize();
        Vec3 distBlockXyZ = playerVec.subtract(Vec3.createVectorHelper(aabb.maxX, aabb.minY, aabb.maxZ))
            .normalize();
        // To ensure that the unit vector is actually within the space, we project the vectors onto it, and make the
        // total length that length
        ...
        unit.xCoord *= currentLongestProj * 0.99;
        unit.yCoord *= currentLongestProj * 0.99;
        unit.zCoord *= currentLongestProj * 0.99;
        // print(unit);
        // print(distBlockxyz);
        // print(distBlockXYZ);
        // print(distBlockXyz);
         ...
         * GL11.glDisable(GL11.GL_TEXTURE_2D);
         * GL11.glDepthMask(false);
         * //drawOutlinedBoundingBox(AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(evt.target.blockX+depth,evt.
         * target.blockY+depth,evt.target.blockZ+depth,evt.target.blockX+1-depth,evt.target.blockY+1-depth,evt.target.
         * blockZ+1-depth).expand(0.002F, 0.002F, 0.002F).getOffsetBoundingBox(-var8, -var10, -var12));
         * //Setup the GL stuff for the outline
         * GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
         * GL11.glColor4f(1.0F, 0.0F, 0.0F, 0.4F);
         * GL11.glLineWidth(4.0F);
         ...
         * drawLine(zeroVec,unit.addVector(-(xOffset - player.lastTickPosX), -(yOffset - player.lastTickPosY), -(zOffset
         * - player.lastTickPosZ)));
