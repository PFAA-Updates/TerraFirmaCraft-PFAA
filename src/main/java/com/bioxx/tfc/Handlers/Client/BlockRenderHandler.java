...
public class BlockRenderHandler implements ISimpleBlockRenderingHandler {
    ...
    public boolean renderWorldBlock(IBlockAccess world, int i, int j, int k, Block block, int modelId,
         ...
         * return RenderLeaves.renderLeaves(block, i, j, k, var6, var7, var8, (RenderBlocks)renderer,
         * ModLoader.getMinecraftInstance().isFancyGraphicsEnabled(), true);
         * }
         */
    ...
    public void renderInventoryBlock(Block block, int metadata, int modelID, RenderBlocks renderer) {
        ...
        if (modelID == TFCBlocks.peatGrassRenderId) {
            for (int i = 0; i < 6; i++) icons[i] = TFCBlocks.peat.getBlockTextureFromSide(i);
            renderInvBlock(block, renderer, icons);
        } else if (modelID == TFCBlocks.grassRenderId) {
            for (int i = 0; i < 6; i++) {
                if (block == TFCBlocks.dirt) icons[i] = TFCBlocks.dirt.getBlockTextureFromSide(i);
                else icons[i] = TFCBlocks.dirt2.getBlockTextureFromSide(i);
            }
            renderInvBlock(block, renderer, icons);
        } else if (modelID == TFCBlocks.clayGrassRenderId) {
            for (int i = 0; i < 6; i++) {
                if (block == TFCBlocks.clay) icons[i] = TFCBlocks.clay.getBlockTextureFromSide(i);
                else icons[i] = TFCBlocks.clay2.getBlockTextureFromSide(i);
            }
            renderInvBlock(block, renderer, icons);
        }
    }
