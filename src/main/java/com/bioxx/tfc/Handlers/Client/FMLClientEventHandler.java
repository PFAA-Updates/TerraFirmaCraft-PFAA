...
public class FMLClientEventHandler {
    ...
    public void renderMealIngredInGui(GuiContainer gui, EntityPlayer player) {
        ...
        if (slot != null && slot.func_111238_b()) {
            if (slot.getHasStack() && slot.getStack()
                ...
                for (int i = 0; i < fg.length; i++) {
                    Item food = FoodRegistry.getInstance()
                        .getFood(fg[i]);
                    if (food == null) continue; // We need to continue the loop for when a middle slot was left empty
                    int x = mouseX + 19;
                    int y = mouseY + 11;
     ...
     * {
     * return (x >= slot.xDisplayPosition - 1) && (x < slot.xDisplayPosition + 16 + 1) && (y >= slot.yDisplayPosition -
     * 1) && (y < slot.yDisplayPosition + 16 + 1);
     * }
     */
}
