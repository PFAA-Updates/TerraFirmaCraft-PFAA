...
public class EntityDamageHandler {
    ...
    public void onEntityHurt(LivingHurtEvent event) {
        ...
        if (event.source == DamageSource.onFire) {
            ...
            event.ammount = 25;
        } else if ("player".equals(event.source.damageType) || "mob".equals(event.source.damageType)
    ...
    protected int applyArmorCalculations(EntityLivingBase entity, DamageSource source, float originalDamage) {
        ...
        EntityArmorCalcEvent eventPre = new EntityArmorCalcEvent(
            entity,
            originalDamage,
        ...
        if (!source.isUnblockable() && armor != null) {
            ...
            EntityArmorCalcEvent eventPost = new EntityArmorCalcEvent(
                entity,
                damage,
            ...
            entity.func_110142_aN()
                .func_94547_a(source, hasHealth, eventPost.incomingDamage);
        }
        return 0;
    }
    ...
    private EnumDamageType getDamageType(DamageSource source) {
        ...
        if (source.getSourceOfDamage() instanceof EntityPlayer) {
            ...
            if (player.getCurrentEquippedItem() != null && player.getCurrentEquippedItem()
                ...
                return ((ICausesDamage) player.getCurrentEquippedItem()
                    .getItem()).getDamageType();
            }
        }

        if (source.getSourceOfDamage() instanceof EntityLiving) {
            EntityLiving el = (EntityLiving) source.getSourceOfDamage();
            if (el.getHeldItem() != null && el.getHeldItem()
                .getItem() instanceof ICausesDamage) {
                return ((ICausesDamage) el.getHeldItem()
                    .getItem()).getDamageType();
            }
        }
    ...
    private int getRandomSlot(Random rand) {
        ...
        if (chance < 10) return 3;// Helm
        else if (chance < 20) return 0;// Feet
        else if (chance < 80) return 2;// Chest
        else return 1;// Legs
    }
    ...
    public void onAttackEntity(AttackEntityEvent event) {
        ...
        if (stack != null && stack.getItem()
            .onLeftClickEntity(stack, player, target)) return;
        ...
        if (target.canAttackWithItem()) {
            if (!target.hitByEntity(target)) {
                ...
                if (player.isPotionActive(Potion.weakness))
                    damageAmount -= 2 << player.getActivePotionEffect(Potion.weakness)
                        .getAmplifier();
                ...
                if (damageAmount > 0 || enchantmentDamage > 0) {
                    ...
                    boolean entityAttacked = target
                        .attackEntityFrom(DamageSource.causePlayerDamage(player), damageAmount);
