...
public class EntityLivingHandler {
                ...
                if (spawnProtectionTimer < TFC_Time.getTotalTicks()) {
                    ...
                    for (int i = -2; i < 3; i++) {
                        for (int k = -2; k < 3; k++) {
                            ...
                            TFC_Core.getCDM(player.worldObj)
                                .addProtection(lastChunkX + i, lastChunkZ + k, TFCOptions.protectionGain);
                        }
                    }
                ...
                if (pi != null && pi.playerUUID.equals(player.getUniqueID())) {
                    ...
                    if (player.inventory.getCurrentItem() != null) {
                        if (player.inventory.getCurrentItem()
                            ...
                            .getItem() instanceof ItemFoodTFC) {
                                pi.guishowFoodRestoreAmount = true;
                                pi.guiFoodRestoreAmount = Food.getWeight(player.inventory.getCurrentItem());
                            } else pi.guishowFoodRestoreAmount = false;
                    } else pi.guishowFoodRestoreAmount = false;
                }
    ...
    public void handleFOV(FOVUpdateEvent event) {
        ...
        if (player.isUsingItem() && player.getItemInUse()
    ...
    public void handleItemPickup(EntityItemPickupEvent event) {
        ...
        if (player.inventory instanceof InventoryPlayerTFC) {
            ...
            // Back slot contains a quiver, handle picking up arrows and javelins.
                ...
                if (item.getItem() instanceof ItemArrow) {
                    ...
                    if (is != null) event.item.setEntityItemStack(is);
                    else {
                        is = item;
                        is.stackSize = 0;
                    ...
                    for (int i = 0; i < 9; i++) {
                        if (player.inventory.getStackInSlot(i) != null && player.inventory.getStackInSlot(i)
                            .getItem() instanceof ItemJavelin) foundJav = true;
                    }

                    // If there is already a javelin on the hotbar, attempt to put the picked up javelin into the
                    // quiver.
                    if (foundJav) {
                        ItemStack is = quiver.addItem(backItem, item);
                        if (is == null) {
        ...
        if (item.getItem() == TFCItems.looseRock) player.triggerAchievement(TFC_Achievements.achLooseRock);
        else if (item.getItem() instanceof ItemOreSmall) player.triggerAchievement(TFC_Achievements.achSmallOre);
        else if (item.getItem() instanceof ItemBloom) player.triggerAchievement(TFC_Achievements.achIronAge);
        else if (item.getItem()
            .equals(TFCItems.gemDiamond)) player.triggerAchievement(TFC_Achievements.achDiamond);
        else if (item.getItem()
            .equals(TFCItems.onion) && TFCOptions.onionsAreGross)
            player.triggerAchievement(TFC_Achievements.achRutabaga);
        else if (item.getItem()
            .equals(TFCItems.oreChunk)
    ...
    public void onEntityDeath(LivingDeathEvent event) {
        ...
        if (event.entity.dimension == 1) event.entity.travelToDimension(0);
    }
        ...
        if (!event.entity.worldObj.isRemote && event.recentlyHit
            ...
            if (event.source.getSourceOfDamage() instanceof EntityPlayer || event.source.isProjectile()) {
                ...
                if (event.source.getSourceOfDamage() instanceof EntityPlayer)
                    ...
                    if (proj.shootingEntity instanceof EntityPlayer) p = (EntityPlayer) proj.shootingEntity;
                }
                for (EntityItem ei : event.drops) {
                    ...
                    if (is.getItem() instanceof IFood) {
                        ...
                        float newWeight = oldWeight * (TFC_Core.getSkillStats(p)
                            .getSkillMultiplier(Global.SKILL_BUTCHERING) + 0.01f);
                        while (newWeight >= Global.FOOD_MIN_DROP_WEIGHT) {
                            float fw = Helper.roundNumber(Math.min(Global.FOOD_MAX_WEIGHT, newWeight), 10);
                            if (fw < Global.FOOD_MAX_WEIGHT) newWeight = 0;
                            newWeight -= fw;

                            ItemStack result = ItemFoodTFC.createTag(new ItemStack(is.getItem(), 1), fw);

                            if (sweetMod != 0) Food.setSweetMod(result, sweetMod);
                            if (sourMod != 0) Food.setSourMod(result, sourMod);
                            if (saltyMod != 0) Food.setSaltyMod(result, saltyMod);
                            if (bitterMod != 0) Food.setBitterMod(result, bitterMod);
                            if (umamiMod != 0) Food.setSavoryMod(result, umamiMod);
                ...
                if (foundFood && p != null) {
                    TFC_Core.getSkillStats(p)
                        .increaseSkill(Global.SKILL_BUTCHERING, 1);
                }
            }
        }
