...
public class ItemNBTPacket extends AbstractPacket {
    ...
    public ItemNBTPacket(NBTTagCompound nbt, List<String> acceptedTagNames) {
        ...
        for (String tagName : tagNames) tags.setTag(tagName, nbt.getTag(tagName));
    }
    ...
    public ItemNBTPacket addAcceptedTag(String name) {
        if (!removeNames.contains(name) && !tagNames.contains(name)) tagNames.add(name);
        return this;
    }
    ...
    public ItemNBTPacket addRemoveTag(String name) {
        if (!removeNames.contains(name)) removeNames.add(name);
        return this;
    }
    ...
    public void encodeInto(ChannelHandlerContext ctx, ByteBuf buffer) {
        ...
        for (String tagName : removeNames) tags.removeTag(tagName);

        try {
            pb.writeNBTTagCompoundToBuffer(tags);

            pb.writeInt(tagNames.size());
            for (String tagName : tagNames) pb.writeStringToBuffer(tagName);

            pb.writeInt(removeNames.size());
            for (String tagName : removeNames) pb.writeStringToBuffer(tagName);
        } catch (Exception e) {
    ...
    public void decodeInto(ChannelHandlerContext ctx, ByteBuf buffer) {
        ...
        try {
            ...
            int size = pb.readInt();
            for (int i = 0; i < size; ++i) tagNames.add(pb.readStringFromBuffer(256));

            size = pb.readInt();
            for (int i = 0; i < size; ++i) removeNames.add(pb.readStringFromBuffer(256));
        } catch (Exception e) {
            TerraFirmaCraft.LOG.catching(e);
        }
    }
    ...
    public void handleClientSide(EntityPlayer player) {
        ...
        if (stack != null) {
            ...
            for (String tagName : tagNames) stackNBT.setTag(tagName, tags.getTag(tagName));
            for (String tagName : removeNames) stackNBT.removeTag(tagName);
            player.inventory.getCurrentItem()
                .setTagCompound(stackNBT);
        }
    }
