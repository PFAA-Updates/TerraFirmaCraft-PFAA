...
public class EntityFallingBlockTFC extends Entity implements IEntityAdditionalSpawnData {
        ...
        if (this.block.getMaterial() == Material.air) {
            ...
            if (!this.worldObj.isRemote) {
                ...
                if (this.onGround) {
                    ...
                    } else if (worldObj.getBlock(i, j, k)
                ...
                if (this.onGround) {
                    ...
                    if (this.worldObj.getBlock(i, j, k) != Blocks.piston_extension) {
                        ...
                        if (canPlaceEntityOnSide(
                            ...
                            1,
                            (Entity) null,
                            (ItemStack) null) && !BlockFalling.func_149831_e(this.worldObj, i, j - 1, k)) {
                            ...
                            if (this.tileEntityData != null && this.block instanceof ITileEntityProvider) {
                                ...
                                if (tileentity != null) {
                                    ...
                                    Iterator iterator = this.tileEntityData.func_150296_c()
                                        .iterator();
                            ...
                            this.entityDropItem(
                                new ItemStack(this.block, 1, this.block.damageDropped(this.blockMeta)),
                                0.0F);
                        }
                    }
                } else if (this.aliveTimer > 100 && !this.worldObj.isRemote && (j < 1 || j > 256)
                    || this.aliveTimer > 600) {
    ...
    public boolean canPlaceEntityOnSide(World world, Block fallingBlock, int x, int y, int z, boolean skipEntityCheck,
        int side, Entity thisEntity, ItemStack is) {
    ...
    public boolean canReplace(World world, int x, int y, int z) {
        ...
        else if (b instanceof BlockOre && TFCOptions.enableCaveInsDestroyOre) return world.setBlockToAir(x, y, z);
        return false;
    }
    ...
    protected void fall(float fallDistance) {
        if (this.hurtEntities) {
            ...
            if (height > 0) {
                ...
                DamageSource damagesource = new DamageSource("caveIn").setDamageBypassesArmor()
                    .setDamageIsAbsolute();
                ...
                while (iterator.hasNext()) {
                    ...
                    entity.attackEntityFrom(
                        damagesource,
                        Math.min(MathHelper.floor_float(height * this.damage), this.maxDamage));
                }
            }
        }
    }
