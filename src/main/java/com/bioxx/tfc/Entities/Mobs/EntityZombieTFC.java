...
public class EntityZombieTFC extends EntityZombie implements ICausesDamage, IInnateArmor {
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.attackDamage)
            .setBaseValue(TFC_MobData.ZOMBIE_DAMAGE);
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(TFC_MobData.ZOMBIE_HEALTH);// MaxHealth
    }
    ...
    protected void dropRareDrop(int par1) {
        switch (this.rand.nextInt(3)) {
            case 0:
                ...
                if (r0.nextInt(3) == 0) this.dropItem(TFCItems.wroughtIronIngot, 1);
                break;
            case 1:
    ...
    protected void addRandomArmor() {
        ...
        if (this.rand.nextFloat() < (this.worldObj.difficultySetting == EnumDifficulty.HARD ? 0.05F : 0.01F)) {
            ...
            if (var1 == 0) this.setCurrentItemOrArmor(0, new ItemStack(TFCItems.bronzePick));
            else this.setCurrentItemOrArmor(0, new ItemStack(TFCItems.bronzeShovel));
        }
    }
    ...
    public void handleHealthUpdate(byte par1) {
        if (par1 == 16) this.worldObj.playSoundEffect(
            this.posX + 0.5D,
            this.posY + 0.5D,
            this.posZ + 0.5D,
            "mob.zombie.remedy",
    ...
    public boolean attackEntityFrom(DamageSource ds, float par2) {
        ...
        if (this.isEntityInvulnerable()) {
            ...
            if (this.getHealth() <= 0.0F) {
                ...
                if ((ds == DamageSource.anvil || ds == DamageSource.fallingBlock)
                ...
                if (flag) {
                    ...
                    if (ds != DamageSource.drown) this.setBeenAttacked();

                    if (entity != null) {
                        double d0 = entity.posX - this.posX;
                        double d1;

                        for (d1 = entity.posZ - this.posZ; d0 * d0 + d1 * d1
                ...
                if (this.getHealth() <= 0.0F) {
                    if (flag) this.playSound(this.getDeathSound(), this.getSoundVolume(), this.getSoundPitch());
                    this.onDeath(ds);
                } else if (flag) {
    ...
    private void summonAid(DamageSource ds) {
        ...
        SummonAidEvent summonAid = ForgeEventFactory.fireZombieSummonAid(
            this,
            worldObj,
            i,
            j,
        ...
        if (summonAid.getResult() == Result.DENY) {
            ...
            || entitylivingbase != null && this.worldObj.difficultySetting == EnumDifficulty.HARD
                && this.rand.nextFloat() < this.getEntityAttribute(field_110186_bp)
                    .getAttributeValue()) {
                        ...
                        for (int l = 0; l < 50; ++l) {
                            ...
                            if (World.doesBlockHaveSolidTopSurface(this.worldObj, i1, j1 - 1, k1)
                                ...
                                && TFC_Core.getCDM(worldObj)
                                    .getData(i1 >> 4, k1 >> 4)
                                    .getSpawnProtectionWithUpdate() <= 0) {
                                entityzombie.setPosition(i1, j1, k1);

                                if (this.worldObj.checkNoEntityCollision(entityzombie.boundingBox)
                                    && this.worldObj.getCollidingBoundingBoxes(entityzombie, entityzombie.boundingBox)
                                        .isEmpty()
                                    ...
                                    this.getEntityAttribute(field_110186_bp)
                                        .applyModifier(
                                            new AttributeModifier(
                                                ...
                                                0));
                                    entityzombie.getEntityAttribute(field_110186_bp)
                                        .applyModifier(
                                            new AttributeModifier(
