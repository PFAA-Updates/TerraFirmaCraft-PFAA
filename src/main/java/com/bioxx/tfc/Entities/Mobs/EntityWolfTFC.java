...
public class EntityWolfTFC extends EntityWolf implements IAnimal, IInnateArmor, ICausesDamage {
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(TFC_MobData.WOLF_HEALTH);// MaxHealth
    }

    @Override
    public boolean attackEntityAsMob(Entity par1Entity) {
        int damage = (int) (TFC_MobData.WOLF_DAMAGE * getStrengthMod()
            * getAggressionMod()
            * (getSizeMod() / 2 + 0.5F));
    ...
    protected void dropFewItems(boolean par1, int par2) {
        ...
        this.entityDropItem(
            new ItemStack(TFCItems.hide, 1, Math.max(0, Math.min(2, (int) (sizeMod * ageMod * 0.9)))),
            0);
    ...
    public float getTailRotation() {
        ...
        if (this.isAngry()) return 1.5393804F;
        else if (this.getOwner() != null)
            return (0.55F - ((this.getMaxHealth() - this.dataWatcher.getWatchableObjectFloat(18)) / scale) * 0.02F)
                * (float) Math.PI;
        else return (float) Math.PI / 5F;
    }
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (itemstack != null) {
            if (this.isBreedingItemTFC(itemstack) && checkFamiliarity(InteractionEnum.BREED, player)
                ...
                return true;
            } else if (itemstack.getItem() instanceof ItemCustomNameTag && itemstack.hasTagCompound()
    ...
    public boolean isFood(ItemStack item) {
        return item != null && (item.getItem() == TFCItems.beefRaw || item.getItem() == TFCItems.chickenRaw
            || item.getItem() == TFCItems.fishRaw
            || item.getItem() == TFCItems.horseMeatRaw
            || item.getItem() == TFCItems.muttonRaw
            || item.getItem() == TFCItems.porkchopRaw
    ...
    public void onLivingUpdate() {
        ...
        if (this.getLeashed()) {
            ...
            // Wolves who have been given a bone, are tied to a fence, and are not angry/targeting another animal must
            // sit
            ...
            // Animals who are tied to a player, or are tied to a fence and are angry/targeting another animal are
            // allowed to stand up and move about
            else if (leashedTo instanceof EntityPlayer || leashedTo instanceof EntityLeashKnot && this.isAngry()) {
                this.aiSit.setSitting(false);
                this.setSitting(false);
            }

            if (!wasRoped) wasRoped = true;
        }
        ...
        if (super.isInLove()) setInLove(true);
    ...
    public void setTamed(boolean par1) {
        if (this.familiarity > 80 && !this.isTamed()) {
            ...
            this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
                .setBaseValue(TFC_MobData.WOLF_HEALTH);
            float h = (float) (healthRatio * this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
                .getBaseValue());
            this.setHealth(h);
        }
    }
