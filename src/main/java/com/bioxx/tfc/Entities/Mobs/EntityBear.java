...
public class EntityBear extends EntityTameable implements ICausesDamage, IAnimal, IInnateArmor {
    ...
    private static final int FAMILIARITY_CAP = 80; // Adult bears cap out at 80 since it is currently impossible to get
                                                   // baby bears.
    ...
    public EntityBear(World par1World) {
        ...
        obedienceMod = (float) Math.sqrt(
            (rand.nextInt(rand.nextInt(DEGREE_OF_DIVERSION * 10) + 1) * (rand.nextBoolean() ? 1 : -1) * 0.01f
                + 1f / aggressionMod));
        sex = rand.nextInt(2);
        if (getGender() == GenderEnum.MALE) tasks.addTask(6, new EntityAIMate(this, moveSpeed));
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(TFC_MobData.BEAR_HEALTH);// MaxHealth
    }
    ...
    public boolean canMateWith(EntityAnimal par1EntityAnimal) {
        ...
        if (!(par1EntityAnimal instanceof EntityBear)) return false;
    ...
    protected String getDeathSound() {
        if (!isChild()) return TFC_Sounds.BEARDEATH;
        else return TFC_Sounds.BEARCUBCRY;
    }
    ...
    protected String getHurtSound() {
        if (!isChild()) return TFC_Sounds.BEARHURT;
        else return TFC_Sounds.BEARCUBCRY;
    }
    ...
    protected String getLivingSound() {
        if (isAdult() && worldObj.rand.nextInt(100) < 5) return TFC_Sounds.BEARCRY;
        else if (isChild() && worldObj.rand.nextInt(100) < 5) return TFC_Sounds.BEARCUBCRY;
    ...
    public void handleFamiliarityUpdate() {
        ...
        if (lastFamiliarityUpdate < totalDays) {
            if (familiarizedToday && familiarity < 100) {
                ...
                float familiarityChange = 3 * obedienceMod / aggressionMod; // Changed from 6 to 3 so bears are harder
                                                                            // to tame by default. -Kitty
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (itemstack != null && itemstack.getItem() instanceof ItemCustomNameTag
