...
public class EntitySheepTFC extends EntitySheep implements IShearable, IAnimal {
     ...
     */
    public static final float[][] FLEECE_COLOR_TABLE = new float[][] { { 1.0F, 1.0F, 1.0F }, { 0.95F, 0.7F, 0.2F },
        { 0.9F, 0.5F, 0.85F }, { 0.6F, 0.7F, 0.95F }, { 0.9F, 0.9F, 0.2F }, { 0.5F, 0.8F, 0.1F }, { 0.95F, 0.7F, 0.8F },
        { 0.3F, 0.3F, 0.3F }, { 0.6F, 0.6F, 0.6F }, { 0.3F, 0.6F, 0.7F }, { 0.7F, 0.4F, 0.9F }, { 0.2F, 0.4F, 0.8F },
    ...
    public int combineColors(EntityAnimal parent, int mateColor) {
        ...
        this.colorCrafting.getStackInSlot(0)
            .setItemDamage(parent1Color);
        this.colorCrafting.getStackInSlot(1)
            .setItemDamage(parent2Color);
        ItemStack itemstack = CraftingManager.getInstance()
            .findMatchingRecipe(this.colorCrafting, ((EntitySheep) parent).worldObj);
        int babyColor;
    ...
    public EntityAgeable createChildTFC(EntityAgeable eAgeable) {
        ...
        data.add(
            eAgeable.getEntityData()
                .getFloat("MateSize"));
        data.add(
            eAgeable.getEntityData()
                .getFloat("MateStrength"));
        data.add(
            eAgeable.getEntityData()
                .getFloat("MateAggro"));
        data.add(
    ...
    protected void dropFewItems(boolean par1, int par2) {
        ...
        if (!this.getSheared()) this.entityDropItem(
            new ItemStack(TFCItems.sheepSkin, 1, Math.max(0, Math.min(2, (int) (ageMod * sizeMod)))),
            0.0F);
        else this
            .entityDropItem(new ItemStack(TFCItems.hide, 1, Math.max(0, Math.min(2, (int) (ageMod * sizeMod)))), 0.0F);
    ...
    protected void entityInit() {
        ...
        this.dataWatcher.addObject(24, String.valueOf("0")); // Time of conception, stored as a string since we can't do
                                                             // long
    }
    ...
    public boolean interact(EntityPlayer player) {
        if (!worldObj.isRemote) {
            ...
            if (player.getHeldItem() != null && player.getHeldItem()
                ...
                && !getSheared()
                && this.checkFamiliarity(InteractionEnum.SHEAR, player)
                && isAdult()) {
                if (!familiarizedToday && this.familiarity <= FAMILIARITY_CAP) {
                    familiarizedToday = true;
                    this.getLookHelper()
                        .setLookPositionWithEntity(player, 0, 0);
                    this.playLivingSound();
                }
                setSheared(true);
                this.entityDropItem(new ItemStack(TFCItems.wool, 1), 0.0F);
                if (!player.capabilities.isCreativeMode) player.getHeldItem()
                    .damageItem(1, player);
            }
        }
    ...
    public void onLivingUpdate() {
        if (this.worldObj.isRemote) this.sheepTimer = Math.max(0, this.sheepTimer - 1);
    ...
    public void setGrowingAge(int par1) {
        if (!TFC_Core.preventEntityDataUpdate) this.dataWatcher.updateObject(12, Integer.valueOf(par1));
    }
