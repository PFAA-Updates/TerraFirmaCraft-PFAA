...
public class EntityFishTFC extends EntitySquid {
    ...
    public boolean isInWater() {
        return this.worldObj.handleMaterialAcceleration(
            this.boundingBox.expand(0.0D, -0.0200000238418579D, 0.0D),
            Material.water,
            this);
    }
    ...
    public void onLivingUpdate() {
        ...
        if (!worldObj.isRemote) {
            ...
            if (hooked && this.riddenByEntity instanceof EntityFishHookTFC
                ...
                if (Vec3.createVectorHelper(fh.pullX, fh.pullY, fh.pullZ)
                    .lengthVector() != 0) {
                    Vec3 dirVec = Vec3.createVectorHelper(fh.pullX, fh.pullY, fh.pullZ)
                        .normalize();
                ...
                if (pullX != 0 && TFC_Core.isWater(
                    worldObj
                        .getBlock((int) (this.posX - (pullX / Math.abs(pullX))), (int) this.posY, (int) this.posZ))) {
                    randX = rand.nextDouble() * 0.5 + 0.16;
                } else {
                    randX = 0;
                }

                if (pullZ != 0 && TFC_Core.isWater(
                    worldObj
                        .getBlock((int) this.posX, (int) this.posY, (int) (this.posZ - (pullZ / Math.abs(pullZ)))))) {
                ...
                if (pullX != 0 && TFC_Core.isWater(
                    worldObj
                        .getBlock((int) this.posX, (int) (this.posY - (pullY / (Math.abs(pullY)))), (int) this.posZ))) {
                    randY = 1 - (randX + randZ);
                } else {
                    randY = 0;
                }
                ...
                Vec3 fishVec = Vec3.createVectorHelper(
                    -(pullX * randX) * fishStrength * energyStrength,
            ...
            if (hooked && this.riddenByEntity instanceof EntityFishHookTFC) {
                ...
                for (int i = 0; i < 1; i++) {
                    if (fh.isTooFarFromPlayer(posX, posY, posZ)) {
                        Vec3 dirVec = fh
                            .getTooFarAdjustedVec(Vec3.createVectorHelper(motionX, motionY, motionZ), posX, posY, posZ);
            ...
            this.currentRenderYaw = xzMotion != 0
                ? (float) (Math.atan2(-renderMotionX, -renderMotionZ) * 180.0D / Math.PI)
                : currentRenderYaw;
    ...
    protected void updateEntityActionState() {
        ...
        if (this.entityAge > 100) {
            ...
            || this.randomMotionVecX == 0.0F && this.randomMotionVecY == 0.0F
    ...
    private int[] getRandomDestination(double x, double y, double z) {
        ...
        boolean needsNewLocation = this.rand.nextInt(25) < 3
        ...
        while (needsNewLocation && numAttempts < 255) {
            ...
            destX = (int) x
                + this.rand.nextInt(10) * (this.rand.nextBoolean() ? (validDirs[1] ? -1 : validDirs[0] ? 1 : 0)
                    : validDirs[0] ? 1 : validDirs[1] ? -1 : 0);
            destY = (int) y
                + this.rand.nextInt(3) * (this.rand.nextBoolean() ? (validDirs[3] ? -1 : validDirs[2] ? 1 : 0)
                    : validDirs[2] ? 1 : validDirs[3] ? -1 : 0);
            destZ = (int) z
                + this.rand.nextInt(10) * (this.rand.nextBoolean() ? (validDirs[5] ? -1 : validDirs[4] ? 1 : 0)
                    : validDirs[4] ? 1 : validDirs[5] ? -1 : 0);
