...
public class EntityChickenTFC extends EntityChicken implements IAnimal {
    ...
    public EntityChickenTFC(World world, double posX, double posY, double posZ, NBTTagCompound genes) {
        ...
        sizeMod = (rand.nextInt(DEGREE_OF_DIVERSION + 1) * (rand.nextBoolean() ? 1 : -1) / 10f + 1F)
            * (1.0F - 0.1F * sex)
            * (float) Math.sqrt((motherSize + fatherSize) / 1.9F);
    ...
    // This should only be called when spawning baby chickens with a spawn egg, so both size values come from the animal
    // clicked on.
    @Override
    ...
    public void roosterCrow() {
        if ((TFC_Time.getTotalTicks() - 15) % TFC_Time.DAY_LENGTH == 0 && getGender() == GenderEnum.MALE
            && isAdult()
