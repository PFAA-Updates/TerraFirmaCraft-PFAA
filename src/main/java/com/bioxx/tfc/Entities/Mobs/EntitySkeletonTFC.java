...
public class EntitySkeletonTFC extends EntityMob implements IRangedAttackMob, ICausesDamage, IInnateArmor {
    ...
    public void setCombatTask() {
        ...
        if (itemstack != null && (itemstack.getItem() == TFCItems.bow || itemstack.getItem() instanceof ItemJavelin)) {
             ...
             * {
             * this.func_110148_a(SharedMonsterAttributes.field_111264_e).func_111128_a(((ItemJavelin)itemstack.getItem(
             * )).weaponDamage);
             * }
             */
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(TFC_MobData.SKELETON_HEALTH);// MaxHealth
    ...
    public void onEntityUpdate() {
        ...
        if (this.isBurning()) this.attackEntityFrom(DamageSource.onFire, 50);
    }
    ...
    public void onLivingUpdate() {
        ...
        if (this.worldObj.isRemote && this.getSkeletonType() == 1) this.setSize(0.72F, 2.34F);
    ...
    public void onDeath(DamageSource par1DamageSource) {
        ...
        if (par1DamageSource.getSourceOfDamage() instanceof EntityArrow
            ...
            if (d0 * d0 + d1 * d1 >= 2500.0D) entityplayer.triggerAchievement(AchievementList.snipeSkeleton);
        }
    }
    ...
    protected void dropFewItems(boolean par1, int par2) {
        ...
        if (this.getSkeletonType() == 1) {
            if (this.getHeldItem() != null && this.getHeldItem()
                .getItem() instanceof ItemJavelin && this.rand.nextFloat() < 0.03f)
                this.dropItem(getHeldItem().getItem(), 1);
        } else {
            if (this.getHeldItem() != null && this.getHeldItem()
                .getItem() instanceof ItemCustomBow) {
    ...
    protected void dropEquipment(boolean forceDrop, int dropChance) {
        for (int j = 0; j < this.getLastActiveItems().length; ++j) {
            ...
            if (itemstack != null && (forceDrop || flag1)
                && this.rand.nextFloat() - dropChance * 0.01F < this.equipmentDropChances[j]) {
    ...
    protected void addRandomArmor() {
        ...
        if (this.getSkeletonType() == 0) {
            ...
            if (this.rand.nextFloat() < 0.095F) ++i;
            if (this.rand.nextFloat() < 0.095F) ++i;
            if (this.rand.nextFloat() < 0.095F) ++i;

            if (i == 0) this.setCurrentItemOrArmor(0, new ItemStack(TFCItems.sedStoneJavelin));
            else if (i == 1) this.setCurrentItemOrArmor(0, new ItemStack(TFCItems.igExStoneJavelin));
            else if (i == 2) this.setCurrentItemOrArmor(0, new ItemStack(TFCItems.copperJavelin));
            else if (i == 3) this.setCurrentItemOrArmor(0, new ItemStack(TFCItems.bronzeJavelin));
            else if (i == 4) this.setCurrentItemOrArmor(0, new ItemStack(TFCItems.wroughtIronJavelin));
        }
    }
    ...
    public static Item getArmorItemForSlot(int par0, int par1) {
        switch (par0) {
            case 4:
                if (par1 == 0) return TFCItems.leatherHelmet;
                else if (par1 == 1) return TFCItems.copperHelmet;
                else if (par1 == 2) return TFCItems.bronzeHelmet;
                else if (par1 == 3) return TFCItems.wroughtIronHelmet;
                else if (par1 == 4) return TFCItems.steelHelmet;
                break;
            case 3:
                if (par1 == 0) return TFCItems.leatherChestplate;
                else if (par1 == 1) return TFCItems.copperChestplate;
                else if (par1 == 2) return TFCItems.bronzeChestplate;
                else if (par1 == 3) return TFCItems.wroughtIronChestplate;
                else if (par1 == 4) return TFCItems.steelChestplate;
                break;
            case 2:
                if (par1 == 0) return TFCItems.leatherLeggings;
                else if (par1 == 1) return TFCItems.copperGreaves;
                else if (par1 == 2) return TFCItems.bronzeGreaves;
                else if (par1 == 3) return TFCItems.wroughtIronGreaves;
                else if (par1 == 4) return TFCItems.steelGreaves;
                break;
            case 1:
                if (par1 == 0) return TFCItems.leatherBoots;
                else if (par1 == 1) return TFCItems.copperBoots;
                else if (par1 == 2) return TFCItems.bronzeBoots;
                else if (par1 == 3) return TFCItems.wroughtIronBoots;
                else if (par1 == 4) return TFCItems.steelBoots;
                break;
            default:
                return null;
        }
        ...
        if (this.rand.nextFloat() < ARMOR_PROBABILITY[this.worldObj.difficultySetting.getDifficultyId()]) {
            ...
            for (int j = 3; j >= 0; --j) {
                ...
                if (itemstack == null) {
                    ...
                    if (item != null) this.setCurrentItemOrArmor(j + 1, new ItemStack(item));
                }
            }
        }
    }
    ...
    public IEntityLivingData onSpawnWithEgg(IEntityLivingData par1EntityLivingData) {
        ...
        this.setCanPickUpLoot(
            this.rand.nextFloat()
                < 0.55F * this.worldObj.func_147462_b/* getLocationTensionFactor */(this.posX, this.posY, this.posZ));
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase par1EntityLiving, float par2) {
        ...
        if (projectile != null) {
            ...
            projectile.setDamage(
                projectile.getDamage() * 1.0F + this.rand.nextGaussian() * 0.25D
                    + this.worldObj.difficultySetting.getDifficultyId() * 0.11F);

            if (var3 > 0) projectile.setDamage(projectile.getDamage() + var3 * 0.5D);
            if (var4 > 0) projectile.setKnockbackStrength(var4);
    ...
    public boolean getCanSpawnHere() {
        ...
        if (b == TFCBlocks.leaves || b == TFCBlocks.leaves2 || b == TFCBlocks.thatch) return false;
