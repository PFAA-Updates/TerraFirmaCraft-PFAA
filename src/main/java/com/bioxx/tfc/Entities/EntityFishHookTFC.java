...
public class EntityFishHookTFC extends EntityFishHook {
    ...
    public EntityFishHookTFC(World par1World, EntityPlayer par2EntityPlayer) {
        ...
        this.setLocationAndAngles(
            ...
            par2EntityPlayer.posY + 1.62D - par2EntityPlayer.yOffset,
    ...
    public void onUpdate() {
        ...
        if (this.fishPosRotationIncrements > 0) {
            ...
            if (!this.worldObj.isRemote) {
                ...
                if (this.field_146042_b.isDead || !this.field_146042_b.isEntityAlive()
                    || itemstack == null
                    || !(itemstack.getItem() instanceof ItemCustomFishingRod)
                    || this.getDistanceSqToEntity(this.field_146042_b) > 2500.0D) {
                    this.setDead();
            ...
            if (movingobjectposition != null) vec31 = Vec3.createVectorHelper(
            ...
            if (movingobjectposition != null) {
                 ...
                 * {
                 * //if (movingobjectposition.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this,
                 * this.field_146042_b), 0.0F))
            ...
            if (!this.inGround) {
                ...
                if (this.onGround || this.isCollidedHorizontally) f2 = 0.5F;

                byte b0 = 5;
                double d6 = 0.0D;

                for (int k = 0; k < b0; ++k) {
                    double d7 = this.boundingBox.minY + (this.boundingBox.maxY - this.boundingBox.minY) * (k + 0) / b0
                        - 0.125D
                        + 0.125D;
                    double d8 = this.boundingBox.minY + (this.boundingBox.maxY - this.boundingBox.minY) * (k + 1) / b0
                        - 0.125D
                        + 0.125D;
                    ...
                    if (this.worldObj.isAABBInMaterial(axisalignedbb1, Material.water)) d6 += 1.0D / b0;
                }
                ...
                if (distance > this.maxDistance && maxDistance != -1) {
                    Vec3 distVec = Vec3.createVectorHelper(
                        this.posX - this.field_146042_b.posX,
                        this.posY - this.field_146042_b.posY,
                        this.posZ - this.field_146042_b.posZ);
         ...
         * Vec3 distVec = Vec3.createVectorHelper(this.posX - this.field_146042_b.posX,this.posY -
         * this.field_146042_b.posY,this.posZ - this.field_146042_b.posZ);
         * double distRatio = maxDistance / distance;
         * this.posX = this.field_146042_b.posX + (distVec.xCoord * distRatio);
         * this.posY = this.field_146042_b.posY + (distVec.yCoord * distRatio);
         * this.posZ = this.field_146042_b.posZ + (distVec.zCoord * distRatio);
    ...
    public Vec3 applyEntityForce(Vec3 entityForce, double x, double y, double z) {
        ...
        if (is != null && is.getItem() instanceof ItemCustomFishingRod) {
            ...
            if (this.reelCounter > 2) {
                is.stackTagCompound.setInteger(
                    "tension",
                    (int) (((forceRatio) - 29) + (Math.pow(entityForce.lengthVector() / 0.2, 3) * 2)) * 100);
            } else {
                this.reelCounter++;
            }
        }
        ...
        if (lineTension >= MAX_LINE_TENSION) {
            this.field_146042_b.getCurrentEquippedItem()
                .damageItem(20, field_146042_b);
        ...
        // this.field_146042_b.addChatMessage(new ChatComponentText("tension: " + lineTension + ", force ratio: " +
        // forceRatio +", distance: " + this.getDistanceToEntity(field_146042_b) + ", max: "+this.maxDistance));
        return Vec3.createVectorHelper(
            netForceVec.xCoord,
            (worldObj.isAirBlock((int) x, (int) y + 1, (int) z) && netForceVec.yCoord > 0 ? 0 : netForceVec.yCoord),
            netForceVec.zCoord);
    }

    public Vec3 getNormalDirectionOfPlayer(double x, double y, double z) {
        Vec3 dirVec = Vec3.createVectorHelper(
            this.field_146042_b.posX - x,
            this.field_146042_b.posY - y,
            this.field_146042_b.posZ - z);
        return dirVec.normalize();
    }

    public Vec3 getTooFarAdjustedVec(Vec3 motionVec, double x, double y, double z) {
        Vec3 playerMotion = Vec3
            .createVectorHelper(this.field_146042_b.motionX, this.field_146042_b.motionY, this.field_146042_b.motionZ);
        double subractedRatio = Math.max(
            1 - (this.maxDistance / this.field_146042_b
                .getDistance(x + playerMotion.xCoord, y + playerMotion.yCoord, z + playerMotion.zCoord)),
            0);
        return Vec3.createVectorHelper(
            (this.field_146042_b.posX + playerMotion.xCoord - (motionVec.xCoord + x)) * subractedRatio,
            (this.field_146042_b.posY + playerMotion.yCoord - (motionVec.yCoord + y)) * subractedRatio,
            (this.field_146042_b.posZ + playerMotion.zCoord - (motionVec.zCoord + z)) * subractedRatio);
    }
    ...
    public int getAverageFishPopFromChunks() {
        if (this.worldObj.isRemote) {
            ...
            int chunksVisited = 0;
            int totalFish = TFC_Core.getCDM(worldObj)
                .getFishPop(lastChunkX, lastChunkZ);
            if (totalFish > 0) {
                chunksVisited++;
            } else {
                return 0;
            ...
            for (int radius = 1; radius < 5 && chunksVisited < maxChunksVisitable; radius++) {
                for (int i = -radius; i <= radius; i++) {
                    for (int k = -radius; k <= radius; k += (Math.abs(i) == radius ? 1 : radius * 2)) {
                        int tempFish = TFC_Core.getCDM(worldObj)
                            .getFishPop(lastChunkX + i, lastChunkZ + k);
                        if (tempFish > 0) {
                            chunksVisited++;
                            totalFish += tempFish;
                        }
    ...
    public int func_146034_e/* catchFish */() {
        if (this.worldObj.isRemote) {
            ...
            TFC_Core.getCDM(worldObj)
                .catchFish(lastChunkX, lastChunkZ);
            int chunksVisited = 1;
            for (int radius = 1; radius < 5 && chunksVisited < maxChunksVisitable; radius++) {
                for (int i = -radius; i <= radius; i++) {
                    for (int k = -radius; k <= radius; k += (Math.abs(i) == radius ? 1 : radius * 2)) {
                        if (TFC_Core.getCDM(worldObj)
                            .catchFish(lastChunkX + i, lastChunkZ + k)) {
                            chunksVisited++;
                        }
                    }
                }
    ...
    public void setDead() {
        ...
        this.lineTension = 0;
        this.maxDistance = -1;
        if (this.field_146042_b != null) this.field_146042_b.fishEntity = null;
    }
}
