...
public class EntityStand extends EntityLiving {
    ...
    protected void entityInit() {
        ...
        this.dataWatcher
            .addObject(start + EntityStand.defaultEquipableLength + EntityStand.defaultArmorLength, new Float(1));
        this.dataWatcher.addObject(
            start + EntityStand.defaultEquipableLength + EntityStand.defaultArmorLength + 1,
            Integer.valueOf(0));
    }
    ...
    public void syncData() {
        if (dataWatcher != null) {
            if (this.worldObj.isRemote) {
                ...
                rotation = this.dataWatcher.getWatchableObjectFloat(
                    start + EntityStand.defaultEquipableLength + EntityStand.defaultArmorLength);
                woodType = this.dataWatcher.getWatchableObjectInt(
                    start + EntityStand.defaultEquipableLength + EntityStand.defaultArmorLength + 1);
            } else {
                int start = 20;
    ...
    public void onUpdate() {
        if (this.worldObj.getEntitiesWithinAABB(getClass(), this.boundingBox)
            .size() > 1) {
            this.setDead();
        }
        ...
        tempZ = this.posZ;
        super.onUpdate();
        if (this.worldObj.isRemote) setSize(0.125f, 2f);
        else setSize(0.1F, 2);
        ...
        // this.getLookHelper().setLookPosition(this.posX + lookX, this.posY + (double)this.getEyeHeight(), this.posZ +
        // lookZ, 10.0F, (float)this.getVerticalFaceSpeed());
    ...
    public boolean interact(EntityPlayer ep) {
        if (!worldObj.isRemote) {
            ...
            if (is != null) {
                ...
                double angleTan = hitVec.yCoord
                    / Math.sqrt(hitVec.xCoord * hitVec.xCoord + hitVec.zCoord * hitVec.zCoord);
