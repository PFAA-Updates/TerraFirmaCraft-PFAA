...
public class EntityAIMateTFC extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (!theAnimal.getInLove() || !theAnimal.isAdult()) {
            ...
            if (targetMate != null) {
                if (targetMate.getGender() == theAnimal.getGender() || theAnimal.isPregnant()
                    || targetMate.isPregnant()) {
                    return false;
                }
            }
    ...
    public boolean continueExecuting() {
        return targetMate.getEntity()
            .isEntityAlive() && targetMate.getInLove()
            && matingCounter < 60
            && theAnimal.getInLove()
    ...
    public void updateTask() {
        theAnimal.getEntity()
            ...
            .setLookPositionWithEntity(
                targetMate.getEntity(),
                10F,
                theAnimal.getEntity()
                    .getVerticalFaceSpeed());
        theAnimal.getEntity()
            .getNavigator()
            .tryMoveToEntityLiving(targetMate.getEntity(), speed);
        matingCounter++;

        if (matingCounter >= 60 && theAnimal.getEntity()
            .getDistanceSqToEntity(this.targetMate.getEntity()) < 9.0D) theAnimal.mate(targetMate);
    }

    private IAnimal getLocalMate() {
        float f = 8F;
        List list = theWorld
            .getEntitiesWithinAABB(theAnimal.getClass(), theAnimal.getEntity().boundingBox.expand(f, f, f));

        for (Iterator iterator = list.iterator(); iterator.hasNext();) {
            Entity entity = (Entity) iterator.next();
            if (entity instanceof IAnimal) {
                IAnimal entityanimal = (IAnimal) entity;
                if (theAnimal.canMateWith(entityanimal)) return entityanimal;
            }
        }
