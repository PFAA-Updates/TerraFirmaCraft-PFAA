...
public class EntityAIFindNest extends EntityAIBase {
    ...
    // To prevent chickens from trying to sit in unreachable nests. See below in updateTask, if the chicken doesnt move
    // >0.5 m in 40 ticks, it
    ...
    public boolean shouldExecute() {
        if (theCreature instanceof EntityChickenTFC && !(theCreature instanceof EntityPheasantTFC)) {
            return ((EntityChickenTFC) theCreature).isAdult() && ((EntityChickenTFC) theCreature).getFamiliarity() >= 15
                && this.theWorld.getBlock((int) theCreature.posX, (int) theCreature.posY, (int) theCreature.posZ)
                    != TFCBlocks.nestBox
                && this.theWorld.getBlock((int) theCreature.posX, (int) theCreature.posY - 1, (int) theCreature.posZ)
                    != TFCBlocks.nestBox
    ...
    public boolean continueExecuting() {
        ...
        return this.currentTick <= this.maxSittingTicks && this.movement <= 60
            && this
                .isSittableBlock(this.theCreature.worldObj, this.sitableBlockX, this.sitableBlockY, this.sitableBlockZ);
    }
    ...
    protected boolean getNearbySitableBlockDistance() {
        ...
        for (int j = (int) this.theCreature.posX - 16; j < this.theCreature.posX + 16.0D; ++j) {
            for (int k = (int) this.theCreature.posZ - 16; k < this.theCreature.posZ + 16.0D; ++k) {
                for (int l = i; l < i + 4; l++) {
                    if (this.isSittableBlock(this.theCreature.worldObj, j, l, k)
                        && this.theCreature.worldObj.isAirBlock(j, l + 1, k)) {
    ...
    public void startExecuting() {
        this.theCreature.getNavigator()
            .tryMoveToXYZ(
                this.sitableBlockX + 0.5D,
                this.sitableBlockY + 1,
                this.sitableBlockZ + 0.5D,
                this.field_75404_b);
        this.currentTick = 0;
        this.movement = 0;
        ...
        this.end = false;
        this.maxSittingTicks = this.theCreature.getRNG()
            .nextInt(
                this.theCreature.getRNG()
                    .nextInt(1200) + 1200)
            + 1200;
    }
        ...
        if (this.theCreature.getDistanceSq(this.sitableBlockX, this.sitableBlockY + 1, this.sitableBlockZ) > 1.0D) {
            this.theCreature.getNavigator()
                .tryMoveToXYZ(
                    ...
                    this.field_75404_b);
            ++this.movement;
            this.compoundDistance += this.theCreature.getDistance(
                this.theCreature.lastTickPosX,
                this.theCreature.lastTickPosY,
                this.theCreature.lastTickPosZ);
            if (this.currentTick - 40 > this.lastCheckedTick) {
                ArrayList<EntityChickenTFC> crowd = (ArrayList<EntityChickenTFC>) theCreature.worldObj
                    .getEntitiesWithinAABB(EntityChickenTFC.class, theCreature.boundingBox.expand(24, 2, 24));
                ArrayList<EntityChickenTFC> invalid = new ArrayList<EntityChickenTFC>();
                for (EntityChickenTFC chicken : crowd) {
                    if (chicken.getGender()
                        .equals(GenderEnum.MALE) || chicken.isChild()) {
                        invalid.add(chicken);
                    }
                }
                ...
                if (this.compoundDistance < 0.5 || crowd.size() >= 10) {
                    failureDepressionMap.put(
                        (this.sitableBlockX + "," + this.sitableBlockY + "," + this.sitableBlockZ),
                        TFC_Time.getTotalTicks() + 24000);
                    this.end = true;
                } else {
    ...
    protected boolean isSittableBlock(World world, int x, int y, int z) {
        if (failureDepressionMap.containsKey((x + "," + y + "," + z))) {
            ...
            if (time > TFC_Time.getTotalTicks()) return false;
            else failureDepressionMap.remove(new int[] { x, y, z });
        }
        ...
        if (block == TFCBlocks.nestBox) {
            ...
            if (!tileentitynest.hasBird() || tileentitynest.getBird() == theCreature) return true;
        }
