...
public class EntityAIPanicTFC extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (this.theEntityCreature.getAITarget() == null && !this.theEntityCreature.isBurning()
            ...
            Vec3 attackedVec = this.theEntityCreature instanceof IAnimal
                ? ((IAnimal) this.theEntityCreature).getAttackedVec()
                : null;
            ...
            if (vec3 == null) {
                ...
                if (alertHerd && this.theEntityCreature instanceof IAnimal) {
                    List list = this.theEntityCreature.worldObj.getEntitiesWithinAABB(
                        this.theEntityCreature.getClass(),
    ...
    public Vec3 updateAttackVec(IAnimal theCreature, Vec3 attackedVec) {
        if (theCreature.getFearSource() != null && TFC_Core.getEntityPos(theEntityCreature)
            .distanceTo(attackedVec) > this.theEntityCreature.getDistanceToEntity(theCreature.getFearSource())) {
            Vec3 newVec = Vec3.createVectorHelper(
                theCreature.getFearSource().posX,
                theCreature.getFearSource().posY,
                theCreature.getFearSource().posZ);
