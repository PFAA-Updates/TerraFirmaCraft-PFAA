...
public class EntityProjectileTFC extends EntityArrow implements ICausesDamage {
    ...
    public EntityProjectileTFC(World world, EntityLivingBase shooter, EntityLivingBase victim, float force,
    ...
    public void onCollideWithPlayer(EntityPlayer player) {
        if (!this.worldObj.isRemote) {
            ...
            if (inground) {
                ...
                if (duraBuff != 0) AnvilManager.setDurabilityBuff(is, duraBuff);
                if (damageBuff != 0) AnvilManager.setDamageBuff(is, damageBuff);
                ...
                if (itemstack.stackSize <= 0) flag = true;
                else if (this.canBePickedUp == 1 && !player.inventory.addItemStackToInventory(itemstack)) flag = false;
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote && this.isDead) {
            ...
            if (this.ticksExisted < 1200 && maxDamage > this.damageTaken) this.isDead = false;
        }
    }
