...
public class BlockSulfur extends BlockTerra implements ICustomCollision {
    ...
    private int itemMeta = Arrays.asList(Global.POWDER)
        .indexOf("Sulfur Powder");
    ...
    public void registerBlockIcons(IIconRegister registerer) {
        for (int i = 0; i < 4; i++) icons[i] = registerer.registerIcon(Reference.MOD_ID + ":" + "ores/Sulfur" + i);
    }
    ...
    public void onNeighborBlockChange(World world, int i, int j, int k, Block l) {
        int num = 0;
        if (world.getBlock(i, j, k + 1)
            .isSideSolid(world, i, j, k + 1, ForgeDirection.NORTH)) num++;
        if (world.getBlock(i, j, k - 1)
            .isSideSolid(world, i, j, k - 1, ForgeDirection.SOUTH)) num++;
        if (world.getBlock(i + 1, j, k)
            .isSideSolid(world, i + 1, j, k, ForgeDirection.WEST)) num++;
        if (world.getBlock(i - 1, j, k)
            .isSideSolid(world, i - 1, j, k, ForgeDirection.EAST)) num++;
        if (world.getBlock(i, j + 1, k)
            .isSideSolid(world, i, j + 1, k, ForgeDirection.DOWN)) num++;
        if (world.getBlock(i, j - 1, k)
            .isSideSolid(world, i, j - 1, k, ForgeDirection.UP)) num++;
        if (num == 0) {
