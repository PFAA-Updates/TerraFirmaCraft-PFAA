...
public class BlockLogNatural extends BlockTerra {
        ...
        if (!world.isRemote) {
            if (!world.getBlock(x, y - 1, z)
                ...
                if (noLogsNearby(world, x + 1, y, z) && noLogsNearby(world, x - 1, y, z)
                    && noLogsNearby(world, x, y, z + 1)
                    && noLogsNearby(world, x, y, z - 1)
                    && noLogsNearby(world, x + 1, y, z + 1)
                    && noLogsNearby(world, x + 1, y, z - 1)
                    && noLogsNearby(world, x - 1, y, z + 1)
                    && noLogsNearby(world, x - 1, y, z - 1)
                    && noLogsNearby(world, x + 1, y - 1, z)
                    && noLogsNearby(world, x - 1, y - 1, z)
                    && noLogsNearby(world, x, y - 1, z + 1)
                    && noLogsNearby(world, x, y - 1, z - 1)
                    && noLogsNearby(world, x + 1, y - 1, z + 1)
                    && noLogsNearby(world, x + 1, y - 1, z - 1)
                    && noLogsNearby(world, x - 1, y - 1, z + 1)
                    && noLogsNearby(world, x - 1, y - 1, z - 1)) world.setBlock(x, y, z, Blocks.air, 0, 0x2);
            }
        }
    }
    ...
    public IIcon getIcon(int side, int meta) {
        if (side == 0 || side == 1) return innerIcons[meta];
        ...
        if (!world.isRemote) {
            if (equip != null) {
                ...
                if (isAxe) {
                    ...
                    } else equip.damageItem(damage, entityplayer);

                    int smallStack = logs % 16;
                    dropBlockAsItem(world, x, y, z, new ItemStack(TFCItems.logs, smallStack, damageDropped(meta)));
                    logs -= smallStack;

                    // In theory this section should never be triggered since the full stacks are dropped higher up the
                    // tree, but keeping it here just in case.
                    while (logs > 0) {
            ...
            } else world.setBlock(x, y, z, this, meta, 0x2);
        }
    }
     ...
     * {
     * //TODO Rewrite the treecap algorithm using a list of coords instead of the ugly array. Shoudl also use a maxmium
    ...
    private void scanLogs(World world, int i, int j, int k, int meta, boolean[][][] checkArray, byte x, byte y, byte z,
        ItemStack stack) {
        if (y >= 0 && j + y < 256) {
            int offsetX = 0;
            int offsetY = 0;
            ...
            if (stack != null) {
                if (damage + stack.getItemDamage() <= stack.getMaxDamage()) {
                    ...
                    if (!isStone || world.rand.nextInt(10) != 0) logs++;
                    if (logs >= 16) {
                        dropBlockAsItem(
                            world,
                            i + x,
                            j + y,
                            k + z,
                            new ItemStack(TFCItems.logs, 16, damageDropped(meta)));
                        logs -= 16;
                    }
