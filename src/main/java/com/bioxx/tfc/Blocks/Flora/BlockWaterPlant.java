...
public class BlockWaterPlant extends BlockSand implements ITileEntityProvider {
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        if (!TFC_Core.isSaltWater(world.getBlock(x, y + 1, z))) return new ArrayList<ItemStack>();

        ArrayList<ItemStack> ret = new ArrayList<ItemStack>();
        ret.add(getSeaWeed(world.rand));
        TEWaterPlant te = (TEWaterPlant) world.getTileEntity(x, y, z);
        if (te != null) ret.add(new ItemStack(te.getBlockFromType(), 1, metadata));
        return ret;
    }
    ...
    private ItemStack getSeaWeed(Random r) {
        return ItemFoodTFC
            .createTag(new ItemStack(TFCItems.seaWeed, 1, 0), Helper.roundNumber(2 + r.nextFloat() * 5, 10));
    }
    ...
    public void registerBlockIcons(IIconRegister registerer) {
        seaWeed = registerer.registerIcon(seaweed);// registerer.registerIcon(Reference.ModID + ":" + "plants/Sea
                                                   // Grass");
        pondWeed = registerer.registerIcon(pondweed);// registerer.registerIcon(Reference.ModID + ":" + "plants/Pond
                                                     // Grass");
