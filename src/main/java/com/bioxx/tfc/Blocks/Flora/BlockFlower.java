...
public class BlockFlower extends BlockTerra {
    ...
    public BlockFlower() {
        ...
        flowerNames = new String[] { "flower_dandelion", "flower_nasturtium", "flower_meads_milkweed",
    ...
    public boolean canGrowConditions(World world, int x, int y, int z, int flowerMeta) {
        ...
        if (flowerMeta == 3 && bioTemperature > 20 && rain > 500 && evt < 2) {
            return true;
        } else if (bioTemperature > 5 && rain > 250) return true;
        return false;
    }
    ...
    public void getSubBlocks(Item item, CreativeTabs tabs, List list) {
        ...
        if (addToCreative) {
            for (int i = 0; i < flowerNames.length; i++) list.add(new ItemStack(item, 1, i));
        }
    }
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        ...
        return (world.isAirBlock(x, y, z) || block.getMaterial()
            .isReplaceable()) && this.canThisPlantGrowOnThisBlock(block);
    }
