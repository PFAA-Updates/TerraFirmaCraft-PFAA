...
public class BlockFruitWood extends BlockTerraContainer {
    ...
    public void registerBlockIcons(IIconRegister registerer) {
        for (int i = 0; i < 9; i++) icons[i] = registerer
            .registerIcon(Reference.MOD_ID + ":" + "wood/fruit trees/" + Global.FRUIT_META_NAMES[i] + " Wood");
    }
    ...
    public void harvestBlock(World world, EntityPlayer entityplayer, int i, int j, int k, int l) {
        ...
        if (isAxeorSaw) {
            ...
            if (world.getBlock(i, j + 1, k) == this || world.getBlock(i, j - 1, k) == this) {
                ...
                if (TFC_Core.isGrass(world.getBlock(i, j + y - 1, k))
                    || TFC_Core.isDirt(world.getBlock(i, j + y - 1, k))) {
            ...
            } else if (world.getBlock(i + 1, j, k) == this || world.getBlock(i - 1, j, k) == this
                || world.getBlock(i, j, k + 1) == this
                || world.getBlock(i, j, k - 1) == this) {
                    Random r = new Random();
                    if (r.nextInt(100) > 50 && isAxeorSaw) {
                        if (l < 8 && (world.getBlock(i + 1, j, k) == TFCBlocks.fruitTreeLeaves2
    ...
    public void onNeighborBlockChange(World world, int i, int j, int k, Block block) {
        ...
        for (int h = -1; h <= 1; h++) {
            for (int g = -1; g <= 1; g++) {
                for (int f = -1; f <= 1; f++) {
                    if (world.getBlock(i + h, j + g, k + f) == this
                        && world.getBlockMetadata(i + h, j + g, k + f) == world.getBlockMetadata(i, j, k)) check = true;
                }
            }
        }
        if (!check) world.setBlockToAir(i, j, k);
    }
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int i, int j, int k) {
        if (world.getBlock(i, j - 1, k) == this || world.getBlock(i, j - 1, k)
            .isOpaqueCube()) return AxisAlignedBB.getBoundingBox(i + 0.3, j, k + 0.3, i + 0.7, j + 1, k + 0.7);
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess world, int i, int j, int k) {
        if (world.getBlock(i, j - 1, k) == this || world.getBlock(i, j - 1, k)
            .isOpaqueCube()) this.setBlockBounds(0.3f, 0, 0.3f, 0.7f, 1, 0.7f);
        else this.setBlockBounds(0, 0.4f, 0, 1, 0.6f, 1);
    }
    ...
    public void updateTick(World world, int i, int j, int k, Random rand) {
         ...
         * }
         * }
         * else if (te.birthTimeWood + branchGrowTime < TFC_Time.getTotalDays() &&te.height == 2 && te.isTrunk &&
         * rand.nextInt(16 / t) == 0 &&
         * world.getBlock(i, j + 1, k) != this)
         * {
         * int r = rand.nextInt(4);
         * if (r == 0 && world.blockExists(i + 1, j, k) && (world.isAirBlock(i + 1, j, k) || world.getBlock(i + 1, j, k)
         * == TFCBlocks.fruitTreeLeaves))
         * {
         ...
         * }
         * }
         * else if (r == 1 && world.blockExists(i, j, k - 1) && (world.isAirBlock(i, j, k - 1) || world.getBlock(i, j, k
         * - 1) == TFCBlocks.fruitTreeLeaves))
         * {
         ...
         * }
         * }
         * else if (r == 2 && world.blockExists(i - 1, j, k) && (world.isAirBlock(i - 1, j, k) || world.getBlock(i - 1,
         * j, k) == TFCBlocks.fruitTreeLeaves))
         * {
         * world.setBlock(i - 1, j, k, this, world.getBlockMetadata(i, j, k), 0x2);
         * if (world.getTileEntity(i - 1, j, k) instanceof TEFruitTreeWood)
         * {
         ...
         * }
         * }
         * else if (r == 3 && world.blockExists(i, j, k + 1) && (world.isAirBlock(i, j, k + 1) || world.getBlock(i, j, k
         * + 1) == TFCBlocks.fruitTreeLeaves))
         * {
         * world.setBlock(i, j, k + 1, this, world.getBlockMetadata(i, j, k), 0x2);
         * if (world.getTileEntity(i, j, k + 1) instanceof TEFruitTreeWood)
         ...
         * }
         * else if (te.birthTimeWood + 1 < TFC_Time.getTotalDays() && rand.nextInt(leafGrowthRate) == 0 &&
         * world.getBlock(i, j + 2, k) != this)
         * {
         * if (world.isAirBlock(i, j + 1, k) && world.isAirBlock(i, j + 2, k) && BlockFruitLeaves.canStay(world, i, j +
         * 1, k)) //above
         * world.setBlock(i, j + 1, k, TFCBlocks.fruitTreeLeaves, world.getBlockMetadata(i, j, k), 0x2);
         * else if (world.blockExists(i + 1, j, k) && world.isAirBlock(i + 1, j, k) && world.isAirBlock(i + 1, j + 1, k)
         * && BlockFruitLeaves.canStay(world, i + 1, j, k)) //+x
         * world.setBlock(i + 1, j, k, TFCBlocks.fruitTreeLeaves, world.getBlockMetadata(i, j, k), 0x2);
         * else if (world.blockExists(i - 1, j, k) && world.isAirBlock(i - 1, j, k) && world.isAirBlock(i - 1, j + 1, k)
         * && BlockFruitLeaves.canStay(world, i - 1, j, k)) //-x
         * world.setBlock(i - 1, j, k, TFCBlocks.fruitTreeLeaves, world.getBlockMetadata(i, j, k), 0x2);
         * else if (world.blockExists(i, j, k + 1) && world.isAirBlock(i, j, k + 1) && world.isAirBlock(i, j + 1, k + 1)
         * && BlockFruitLeaves.canStay(world, i, j, k + 1)) //+z
         * world.setBlock(i, j, k + 1, TFCBlocks.fruitTreeLeaves, world.getBlockMetadata(i, j, k), 0x2);
         * else if (world.blockExists(i, j, k - 1) && world.isAirBlock(i, j, k - 1) && world.isAirBlock(i, j + 1, k - 1)
         * && BlockFruitLeaves.canStay(world, i, j, k - 1)) //-z
         * world.setBlock(i, j, k - 1, TFCBlocks.fruitTreeLeaves, world.getBlockMetadata(i, j, k), 0x2);
         * else if (world.blockExists(i + 1, j, k - 1) && world.isAirBlock(i + 1, j, k - 1) && world.isAirBlock(i + 1, j
         * + 1, k - 1) && BlockFruitLeaves.canStay(world, i + 1, j, k - 1)) //+x/-z
