...
public class BlockSapling extends BlockTerraContainer {
    ...
    public void registerBlockIcons(IIconRegister registerer) {
        for (int i = 0; i < woodNames.length; i++) this.icons[i] = registerer
            .registerIcon(Reference.MOD_ID + ":" + "wood/trees/" + this.woodNames[i] + " Sapling");
    ...
    // Set the sapling growth timer the moment it is planted, instead of the first random tick it gets after being
    // planted.
    @Override
    public void onBlockAdded(World world, int i, int j, int k) {
        int meta = world.getBlockMetadata(i, j, k);
        float growSpeed = 1;
        if (meta == 1 || meta == 11) growSpeed = 1.2f;
        else if (meta == 5 || meta == 0 || meta == 13) growSpeed = 1.4f;
        else if (meta == 9 || meta == 14 || meta == 15) growSpeed = 1.6f;

        if (world.getTileEntity(i, j, k) instanceof TESapling) {
            TESapling te = (TESapling) world.getTileEntity(i, j, k);

            // Set the growTime tick timestamp to be 7-11.2 days times config multiplier from now, plus up to an extra
            // day.
            if (te != null && te.growTime == 0) te.growTime = (long) (TFC_Time.getTotalTicks()
                + (TFC_Time.DAY_LENGTH * 7 * growSpeed * TFCOptions.saplingTimerMultiplier)
                + (world.rand.nextFloat() * TFC_Time.DAY_LENGTH));
        }
    }
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        return (world.isAirBlock(x, y, z) || world.getBlock(x, y, z)
            .getMaterial()
            .isReplaceable()) && this.canThisPlantGrowOnThisBlockID(world.getBlock(x, y - 1, z));
    }
