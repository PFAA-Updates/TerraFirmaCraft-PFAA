...
public abstract class BlockCustomLiquid extends BlockDynamicLiquid implements IFluidBlock {
    ...
    public int colorMultiplier(IBlockAccess par1IBlockAccess, int par2, int par3, int par4) {
        if (this.blockMaterial != Material.water) return 16777215;
        else return 0x354d35;
    }
    ...
    private void checkForHarden(World world, int x, int y, int z) {
        if (world.getBlock(x, y, z) == this) if (this.blockMaterial == Material.lava) {
            boolean flag = false;

            if (flag || world.getBlock(x, y, z - 1)
                .getMaterial() == Material.water) flag = true;

            if (flag || world.getBlock(x, y, z + 1)
                .getMaterial() == Material.water) flag = true;

            if (flag || world.getBlock(x - 1, y, z)
                .getMaterial() == Material.water) flag = true;

            if (flag || world.getBlock(x + 1, y, z)
                .getMaterial() == Material.water) flag = true;

            if (flag || world.getBlock(x, y + 1, z)
                .getMaterial() == Material.water) flag = true;

            if (flag) {
                int l = world.getBlockMetadata(x, y, z);

                if (l == 0) setBlockforLava(world, x, y, z, 0);
                else if (l <= 4) setBlockforLava(world, x, y, z, 1);
    ...
    public boolean canSustainPlant(IBlockAccess world, int x, int y, int z, ForgeDirection direction,
        ...
        if (plant instanceof BlockCustomLilyPad
            && ((BlockCustomLilyPad) plant).canThisPlantGrowOnThisBlock(this, meta)) {
            return true;
        }
        ...
        switch (plantType) {
            ...
            case Plains:
                return this == TFCBlocks.grass || this == TFCBlocks.grass2
                    || this == TFCBlocks.dirt
                    || this == TFCBlocks.dirt2;
            case Water:
                return world.getBlock(x, y, z)
                    .getMaterial() == Material.water && world.getBlockMetadata(x, y, z) == 0;
            case Beach:
                boolean isBeach = TFC_Core.isGround(this);
                boolean hasWater = world.getBlock(x - 1, y, z)
    ...
    public void setBlockforLava(World world, int x, int y, int z, int typeOfLava) {
        DataLayer rockLayer3 = TFC_Climate.getCacheManager(world)
            .getRockLayerAt(x, y, 2);
        // int blockId = rockLayer3.data1;
        int meta = rockLayer3.data2;
        Random rand = new Random();
        boolean felsicLava = true;

        if (this == TFCBlocks.stoneIgIn && (meta == 2 || meta == 1)) felsicLava = false;
        else if (this == TFCBlocks.stoneIgEx && (meta == 1 || meta == 2)) felsicLava = false;
        if (typeOfLava == 0 || typeOfLava == 2) // non flowing rock
        {
            if (felsicLava) {
                if (rand.nextInt(10) == 0 && typeOfLava == 0) world.setBlock(x, y, z, Blocks.obsidian);
                else {
            ...
            if (felsicLava) world.setBlockMetadataWithNotify(x, y, z, 0, 0);
            else world.setBlockMetadataWithNotify(x, y, z, 1, 0);
        }
    }

    @Override
    public int tickRate(World world) {
        if (this.getMaterial() == Material.lava) return 10;
        return 3;
    }
    ...
    public void registerBlockIcons(IIconRegister register) {
        if (this.blockMaterial == Material.lava) {
            this.getFluid()
                .setIcons(register.registerIcon("lava_still"), register.registerIcon("lava_flow"));
        } else {
            this.getFluid()
                .setIcons(register.registerIcon("water_still"), register.registerIcon("water_flow"));
        }
    ...
    public boolean canStay(World world, int x, int y, int z) {
        Block block = world.getBlock(x, y, z);
        if (block == TFCBlocks.thatch || block == TFCBlocks.barrel
            || block == TFCBlocks.vessel
            || block == TFCBlocks.berryBush
            || block == TFCBlocks.smokeRack
            || block instanceof BlockCustomDoor
            || block == TFCBlocks.ingotPile) return false;
        else return super.func_149807_p(world, x, y, z);
    }
    ...
    protected int getLiquidHeight(World world, int x, int y, int z, int count) {
        ...
        if (liquidHeight < 0) {
            ...
            // Source Block
            if (liquidHeight == 0) this.sourceBlockCount++;
            else if (liquidHeight >= 8) liquidHeight = 0;
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
            ...
            if (this.sourceBlockCount >= 2 && this.blockMaterial == Material.water && !TFCOptions.enableFiniteWater) {
                ...
                if (world.getBlock(x, y - 1, z)
                    ...
                    || world.getBlock(x, y - 1, z)
                        .getMaterial() == this.blockMaterial && world.getBlockMetadata(x, y - 1, z) == 0) {
                    newHeight = 0;
                }
            }

            if (this.blockMaterial == Material.lava && meta < 8
                && newHeight < 8
                && newHeight > meta
                && rand.nextInt(4) != 0) {
                tickRate *= 4;
        ...
        {
            if (this.blockMaterial == Material.lava && world.getBlock(x, y - 1, z)
                .getMaterial() == Material.water) {
    ...
    private boolean canReplace(World world, int x, int y, int z) {
        Material material = world.getBlock(x, y, z)
            .getMaterial();
        if (material == this.blockMaterial || material == Material.lava) return false;
        else return !this.canStay(world, x, y, z);
    }
    ...
    private boolean[] getFlowDirections(World world, int x, int y, int z) {
        ...
        for (side = 0; side < 4; ++side) {
            ...
            else if (side == 2) newZ = z - 1;
            else if (side == 3) ++newZ;

            if (!this.canStay(world, newX, y, newZ) && (world.getBlock(newX, y, newZ)
                .getMaterial() != this.blockMaterial || world.getBlockMetadata(newX, y, newZ) != 0)) {
    ...
    private int getFlowPriorities(World world, int x, int y, int z, int distance, int side) {
        ...
        for (int side2 = 0; side2 < 4; ++side2) {
            if ((side2 != 0 || side != 1) && (side2 != 1 || side != 0)
                && (side2 != 2 || side != 3)
                && (side2 != 3 || side != 2)) {
                int xCoord = x;
                int zCoord = z;

                if (side2 == 0) xCoord = x - 1;
                else if (side2 == 1) ++xCoord;
                else if (side2 == 2) zCoord = z - 1;
                else if (side2 == 3) ++zCoord;

                if (!this.canStay(world, xCoord, y, zCoord) && (world.getBlock(xCoord, y, zCoord)
                    .getMaterial() != this.blockMaterial || world.getBlockMetadata(xCoord, y, zCoord) != 0)) {
    ...
    protected int getMetaData(World world, int x, int y, int z) {
        return world.getBlock(x, y, z)
            .getMaterial() == this.blockMaterial ? world.getBlockMetadata(x, y, z) : -1;
    }
}
