...
public class BlockCharcoal extends BlockTerra {
    ...
    public void onBlockHarvested(World world, int x, int y, int z, int side, EntityPlayer entityplayer) {
        if (!world.isRemote) {
            if (entityplayer.capabilities.isCreativeMode) {
                ...
                if (equip != null) {
                    if (equip.getItem() instanceof ItemCustomShovel) isShovel = true;
                }

                if (isShovel) {
                    int top = 0;
                    while (world.getBlock(x, y + top + 1, z) == this) ++top;

                    dropBlockAsItem(world, x, y, z, new ItemStack(TFCItems.coal, 1, 1));
                    if (side - 1 > 0) {
                        if (world.getBlock(x, y + 1, z) == this) {
                            int m1 = world.getBlockMetadata(x, y + top, z);
                            if (m1 - 1 > 0) world.setBlockMetadataWithNotify(x, y + top, z, m1 - 1, 2);
                            else world.setBlockToAir(x, y + top, z);
                ...
                if (side == 0) world.setBlockToAir(x, y, z);
            }
        }
    }
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
        if (!world.isRemote) {
            if (world.isAirBlock(x, y - 1, z)) {
                ...
                if (world.getBlock(x, y - 1, z) == this) combineCharcoalDown(world, x, y, z);

                if (world.getBlock(x, y + 1, z) == this) combineCharcoalUp(world, x, y, z);
            }
        }
    }
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        if (md == 8) return AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + 1, z + 1);
