...
public class BlockSmoke extends BlockTerra {
    ...
    public void onBlockAdded(World world, int x, int y, int z) {
        if (!world.isRemote) {
            ...
            if (meta < 15) {
                if ((isSurrounded(world, x, y + 1, z) || world.rand.nextInt(5) != 0 || meta < 8)
                    && addSmoke(world, x, y + 1, z, meta)) {
    ...
    private void verify(World world, int x, int y, int z) {
        if (!world.isRemote) {
            ...
            if (thisMeta == 0) return;
            boolean hasBase = false;
            if (hasBase(world, x, y - 1, z, thisMeta - 1) || hasBase(world, x, y - 1, z - 1, thisMeta - 1)
                || hasBase(world, x, y - 1, z + 1, thisMeta - 1)
                || hasBase(world, x - 1, y - 1, z, thisMeta - 1)
            ...
            if (!hasBase) world.setBlockToAir(x, y, z);
        }
    }
