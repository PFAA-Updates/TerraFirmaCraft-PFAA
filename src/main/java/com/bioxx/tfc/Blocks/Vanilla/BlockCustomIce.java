...
public class BlockCustomIce extends BlockIce {
    ...
    public boolean canSustainPlant(IBlockAccess world, int x, int y, int z, ForgeDirection direction,
        ...
        if (plant == TFCBlocks.cactus && this == TFCBlocks.cactus) return true;

        if (plant == TFCBlocks.reeds && this == TFCBlocks.reeds) return true;

        int meta = world.getBlockMetadata(x, y, z);
        if (plantable instanceof BlockCustomLilyPad
            && ((BlockCustomLilyPad) plant).canThisPlantGrowOnThisBlock(this, meta)) return true;
    ...
    public int getLightOpacity(IBlockAccess world, int x, int y, int z) {
        int meta = world.getBlockMetadata(x, y, z);
        if (meta == 0) return 9;// sea ice is cloudy
