...
public class BlockCustomSnow extends BlockTerra {
    ...
    public boolean canPlaceBlockAt(World world, int i, int j, int k) {
        ...
        if (block == TFCBlocks.leaves || block == TFCBlocks.leaves2 || block == TFCBlocks.thatch) return true;
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        return AxisAlignedBB
            .getBoundingBox(x + this.minX, y + this.minY, z + this.minZ, x + this.maxX, y + f, z + this.maxZ);
    }
    ...
    public void updateTick(World world, int x, int y, int z, Random r) {
        ...
        if (world.getSavedLightValue(EnumSkyBlock.Block, x, y, z) > 11) {
            if (r.nextInt(5) == 0) {
                if (meta > 0) world.setBlockMetadataWithNotify(x, y, z, meta - 1, 2);
                else world.setBlock(x, y, z, Blocks.air, 0, 0x2);
            }
        }
        ...
        {
            if (r.nextInt(20) == 0) {
                int max = (world.getBlock(x, y - 1, z)
                    .getMaterial() == Material.leaves) ? 3 : 7;
    ...
    private boolean canAddSnowCheckNeighbors(World world, int x, int y, int z, int meta) {
        ...
        else if (block == TFCBlocks.leaves || block == TFCBlocks.leaves2) // 4 levels if adjacent to leaves (instead of
                                                                          // just one level)
            return meta < 3;
    ...
    private boolean canAddSnow(World world, int x, int y, int z, int meta) {
        if (!canAddSnowCheckNeighbors(world, x + 1, y, z, meta)) return false;
        if (!canAddSnowCheckNeighbors(world, x - 1, y, z, meta)) return false;
        if (!canAddSnowCheckNeighbors(world, x, y, z + 1, meta)) return false;
