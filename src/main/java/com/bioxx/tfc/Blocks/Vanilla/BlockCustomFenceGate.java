...
public class BlockCustomFenceGate extends BlockFenceGate implements ITileEntityProvider, IMultipleBlock {
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        if (open) return null; // Gate is open so it has no bounding box.
        else return (direction != 2 && direction != 0
            ? AxisAlignedBB.getBoundingBox(x + 0.375F, y, z, x + 0.625F, y + 1.5F, z + 1)
    ...
    public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer,
        ...
        if (te.getOpen()) te.setOpen(false);
        else te.setOpen(true);
