...
public class BlockTFCFence extends BlockFence {
    ...
    public void registerBlockIcons(IIconRegister iconRegisterer) {
        for (int i = 0; i < woodNames.length; i++) {
            ...
            iconsPostTop[i] = iconRegisterer
                .registerIcon(Reference.MOD_ID + ":" + "wood/" + woodNames[i] + " Fence Top");
        }
    }
    ...
    public IIcon getIcon(int par1, int par2) {
        if (par1 == 1) return iconsPostTop[par2];
    ...
    public boolean canConnectFenceTo(IBlockAccess bAccess, int x, int y, int z) {
        ...
        if (TFCBlocks.canFenceConnectTo(block)) return true;
        else return block != this && block.getMaterial()
