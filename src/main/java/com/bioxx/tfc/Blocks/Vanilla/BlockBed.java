...
public class BlockBed extends BlockDirectional {
    ...
    public boolean isBed(IBlockAccess world, int x, int y, int z, EntityLivingBase player) {
        World w = (World) world;
        if (!w.isRemote && player != null) ((EntityPlayer) player).sleepTimer = 50;
    ...
    public IIcon getIcon(int par1, int par2) {
        if (par1 == 0) {
            ...
            return (i1 != 1 || l != 2) && (i1 != 0 || l != 3)
                ? (l != 5 && l != 4 ? this.bedTopIcons[i1] : this.bedSideIcons[i1])
                : this.bedEndIcons[i1];
        }
    }
    ...
    public void registerBlockIcons(IIconRegister par1IconRegister) {
        this.bedTopIcons = new IIcon[] { par1IconRegister.registerIcon(Reference.MOD_ID + ":straw bed_feet_top"),
            par1IconRegister.registerIcon(Reference.MOD_ID + ":straw bed_head_top") };
        this.bedEndIcons = new IIcon[] { par1IconRegister.registerIcon(Reference.MOD_ID + ":straw bed_feet_end"),
            par1IconRegister.registerIcon(Reference.MOD_ID + ":straw bed_head_end") };
        this.bedSideIcons = new IIcon[] { par1IconRegister.registerIcon(Reference.MOD_ID + ":straw bed_feet_side"),
            par1IconRegister.registerIcon(Reference.MOD_ID + ":straw bed_head_side") };
    }
    ...
    public void dropBlockAsItemWithChance(World par1World, int par2, int par3, int par4, int par5, float par6,
        int par7) {
        if (!isBlockHeadOfBed(par5)) super.dropBlockAsItemWithChance(par1World, par2, par3, par4, par5, par6, 0);
    }
    ...
    public void onBlockHarvested(World par1World, int par2, int par3, int par4, int par5,
        ...
        if (par6EntityPlayer.capabilities.isCreativeMode && isBlockHeadOfBed(par5)) {
            ...
            if (par1World.getBlock(par2, par3, par4) == this) par1World.setBlockToAir(par2, par3, par4);
        }
    }
}
