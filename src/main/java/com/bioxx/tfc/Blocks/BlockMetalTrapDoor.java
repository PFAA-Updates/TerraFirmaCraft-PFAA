...
public class BlockMetalTrapDoor extends BlockTerraContainer {
    ...
    public IIcon[] icons;
    public static String[] metalNames = { "Bismuth", "Bismuth Bronze", "Black Bronze", "Black Steel", "Blue Steel",
        "Brass", "Bronze", "Copper", "Gold", "Wrought Iron", "Lead", "Nickel", "Pig Iron", "Platinum", "Red Steel",
        "Rose Gold", "Silver", "Steel", "Sterling Silver", "Tin", "Zinc"/* ,"Unknown" */ }; // There is no trapdoor
                                                                                            // anvil recipe involving
                                                                                            // unknown ingots.
    ...
    public void getSubBlocks(Item par1, CreativeTabs par2CreativeTabs, List list) {
        for (int i = 0; i < metalNames.length; i++) {
            // Only add trap doors where both pieces are the same metal to reduce clutter from the creative menu and
            // NEI. Other combinations can still be created.
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess access, int x, int y, int z) {
        if (access.getTileEntity(x, y, z) != null && access.getTileEntity(x, y, z) instanceof TEMetalTrapDoor)
            this.setBlockBoundsForBlockRender(
                access.getBlockMetadata(x, y, z),
                ((TEMetalTrapDoor) access.getTileEntity(x, y, z)).data);
    }
    ...
    public IIcon getIcon(IBlockAccess access, int i, int j, int k, int meta) {
        ...
        if (te != null && te.sheetStack != null) return icons[te.sheetStack.getItemDamage() & 31];
        else return TFC_Textures.invisibleTexture;
    }
