...
public class BlockWoodSupport extends BlockTerra {
    ...
    public static ForgeDirection getSupportDirection(World world, int x, int y, int z) {
        ...
        if (r != null) // Fixes NPE
        {
            if (r[2] > r[3]) return ForgeDirection.NORTH;
            if (r[3] > r[2]) return ForgeDirection.SOUTH;
            if (r[5] > r[4]) return ForgeDirection.EAST;
            if (r[4] > r[5]) return ForgeDirection.WEST;
        }
    ...
    public static int[] getSupportsInRangeDir(World world, int x, int y, int z, int range, boolean checkConnection) {
        ...
        boolean foundNH = true;
        int s = 0;
        boolean foundSV = false;
        boolean foundSH = true;
        int e = 0;
        boolean foundEV = false;
        boolean foundEH = true;
        int w = 0;
        boolean foundWV = false;
        boolean foundWH = true;
        ...
        for (int i = 1; i <= range; i++) {
            if (!foundEV) {
                if (!checkConnection) {
                    if (world.isAirBlock(x + i, y, z) || TFCBlocks.isBlockVSupport(world.getBlock(x + i, y, z))) e++;
                    else clearEastPath = false;
                } else if (checkConnection && !TFCBlocks.isBlockHSupport(world.getBlock(x + i, y, z))
                    && !TFCBlocks.isBlockVSupport(world.getBlock(x + i, y, z))) foundEH = false;
                else e++;
                if (clearEastPath && TFCBlocks.isBlockVSupport(world.getBlock(x + i, y, z)) && (e >= 0 || i == 1)) {
                    if (scanVert(world, x + i, y, z)) foundEV = true;
                    else e -= 50;
                }
            }
            if (!foundWV) {
                if (!checkConnection) {
                    if (world.isAirBlock(x - i, y, z) || TFCBlocks.isBlockVSupport(world.getBlock(x - i, y, z))) w++;
                    else clearWestPath = false;
                } else if (checkConnection && !TFCBlocks.isBlockHSupport(world.getBlock(x - i, y, z))
                    && !TFCBlocks.isBlockVSupport(world.getBlock(x - i, y, z))) foundWH = false;
                else w++;
                if (clearWestPath && TFCBlocks.isBlockVSupport(world.getBlock(x - i, y, z)) && (w >= 0 || i == 1)) {
                    if (scanVert(world, x - i, y, z)) foundWV = true;
                    else w -= 50;
                }
            }
            if (!foundSV) {
                if (!checkConnection) {
                    if (world.isAirBlock(x, y, z + i) || TFCBlocks.isBlockVSupport(world.getBlock(x, y, z + i))) s++;
                    else clearSouthPath = false;

                } else if (checkConnection && !TFCBlocks.isBlockHSupport(world.getBlock(x, y, z + i))
                    && !TFCBlocks.isBlockVSupport(world.getBlock(x, y, z + i))) foundSH = false;
                else s++;
                if (clearSouthPath && TFCBlocks.isBlockVSupport(world.getBlock(x, y, z + i)) && (s >= 0 || i == 1)) {
                    if (scanVert(world, x, y, z + i)) foundSV = true;
                }
            }
            if (!foundNV) {
                if (!checkConnection) {
                    if (world.isAirBlock(x, y, z - i) || TFCBlocks.isBlockVSupport(world.getBlock(x, y, z - i))) n++;
                    else clearNorthPath = false;
                } else if (checkConnection && !TFCBlocks.isBlockHSupport(world.getBlock(x, y, z - i))
                    && !TFCBlocks.isBlockVSupport(world.getBlock(x, y, z - i))) foundNH = false;
                else n++;
                if (clearNorthPath && TFCBlocks.isBlockVSupport(world.getBlock(x, y, z - i)) && (n >= 0 || i == 1)) {
                    if (scanVert(world, x, y, z - i)) foundNV = true;
                }
            }
        }

        if (foundEV && foundEH && foundWV && foundWH) return new int[] { 0, 0, 0, 0, w, e };
        if (foundSV && foundSH && foundNV && foundNH) return new int[] { 0, 0, n, s, 0, 0 };
        return null;
    }

    private static boolean scanVert(World world, int x, int y, int z) {
        int out = 1;
        while (TFCBlocks.isBlockVSupport(world.getBlock(x, y - out, z))) out++;
    ...
    public static int isNextToSupport(World world, int x, int y, int z) {
        if (TFCBlocks.isBlockVSupport(world.getBlock(x + 1, y, z))
            || TFCBlocks.isBlockHSupport(world.getBlock(x + 1, y, z))) return 5;
        if (TFCBlocks.isBlockVSupport(world.getBlock(x - 1, y, z))
            || TFCBlocks.isBlockHSupport(world.getBlock(x - 1, y, z))) return 4;
        if (TFCBlocks.isBlockVSupport(world.getBlock(x, y, z + 1))
            || TFCBlocks.isBlockHSupport(world.getBlock(x, y, z + 1))) return 3;
        if (TFCBlocks.isBlockVSupport(world.getBlock(x, y, z - 1))
            || TFCBlocks.isBlockHSupport(world.getBlock(x, y, z - 1))) return 2;
        return 0;
    }
    ...
    private AxisAlignedBB getCollisionBoundingBoxFromPoolIBlockAccess(IBlockAccess blockAccess, int x, int y, int z) {
        ...
        if (isHorizontal) {
            minY = 0.5;
            maxY = 1;
            if (TFCBlocks.isBlockVSupport(blockAccess.getBlock(x + 1, y, z))
                || TFCBlocks.isBlockHSupport(blockAccess.getBlock(x + 1, y, z))) maxX = 1;
            if (TFCBlocks.isBlockVSupport(blockAccess.getBlock(x - 1, y, z))
                || TFCBlocks.isBlockHSupport(blockAccess.getBlock(x - 1, y, z))) minX = 0;
            if (TFCBlocks.isBlockVSupport(blockAccess.getBlock(x, y, z + 1))
                || TFCBlocks.isBlockHSupport(blockAccess.getBlock(x, y, z + 1))) maxZ = 1;
            if (TFCBlocks.isBlockVSupport(blockAccess.getBlock(x, y, z - 1))
                || TFCBlocks.isBlockHSupport(blockAccess.getBlock(x, y, z - 1))) minZ = 0;
            /*
            ...
            /*
             * if(TFCBlocks.isBlockVSupport(blockAccess.getBlock(x+1, y, z)) ||
             * TFCBlocks.isBlockHSupport(blockAccess.getBlock(x+1, y, z)))
             * maxX = 1;
             * if(TFCBlocks.isBlockVSupport(blockAccess.getBlock(x-1, y, z)) ||
             * TFCBlocks.isBlockHSupport(blockAccess.getBlock(x-1, y, z)))
             * minX = 0;
             * if(TFCBlocks.isBlockVSupport(blockAccess.getBlock(x, y, z+1)) ||
             * TFCBlocks.isBlockHSupport(blockAccess.getBlock(x, y, z+1)))
             * maxZ = 1;
             * if(TFCBlocks.isBlockVSupport(blockAccess.getBlock(x, y, z-1)) ||
             * TFCBlocks.isBlockHSupport(blockAccess.getBlock(x, y, z-1)))
             * minZ = 0;
    ...
    public int getRenderType() {
        if (TFCBlocks.isBlockVSupport(this)) return TFCBlocks.woodSupportRenderIdV;
        else return TFCBlocks.woodSupportRenderIdH;
    }
    ...
    public AxisAlignedBB getSelectedBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        if (isHorizontal) {
            ...
            maxY = 1;
            if (TFCBlocks.isBlockVSupport(world.getBlock(x + 1, y, z))
                || TFCBlocks.isBlockHSupport(world.getBlock(x + 1, y, z))) maxX = 1;
            if (TFCBlocks.isBlockVSupport(world.getBlock(x - 1, y, z))
                || TFCBlocks.isBlockHSupport(world.getBlock(x - 1, y, z))) minX = 0;
            if (TFCBlocks.isBlockVSupport(world.getBlock(x, y, z + 1))
                || TFCBlocks.isBlockHSupport(world.getBlock(x, y, z + 1))) maxZ = 1;
            if (TFCBlocks.isBlockVSupport(world.getBlock(x, y, z - 1))
                || TFCBlocks.isBlockHSupport(world.getBlock(x, y, z - 1))) minZ = 0;
            if (TFCBlocks.isBlockVSupport(world.getBlock(x, y - 1, z))) minY = 0;
        } else {
            minY = 0;
            maxY = 1;
            if (TFCBlocks.isBlockVSupport(world.getBlock(x + 1, y, z))
    ...
    public void harvestBlock(World world, EntityPlayer entityplayer, int i, int j, int k, int l) {
        ...
        else dropBlockAsItem(world, i, j, k, new ItemStack(this, 1, l));
    }
    ...
    public void onNeighborBlockChange(World world, int i, int j, int k, Block l) {
        ...
        if (isVertical && !isOtherHorizontal)// Vertical Beam
        {
            ...
            if (!world.getBlock(i, j - 1, k)
                .isOpaqueCube() && !TFCBlocks.isBlockVSupport(world.getBlock(i, j - 1, k))) {
        ...
        } else if (isHorizontal)// Horizontal Beam
        {
            ...
            if (b1) {
                ...
                if (this == TFCBlocks.woodSupportH) world.setBlock(i, j, k, TFCBlocks.woodSupportV, meta, 0x2);
                else if (this == TFCBlocks.woodSupportH2) world.setBlock(i, j, k, TFCBlocks.woodSupportV2, meta, 0x2);
            }
        }
    }
    ...
    public boolean canPlaceBlockOnSide(World world, int x, int y, int z, int side) {
        ...
        if (!TFCBlocks.isBlockVSupport(downBlock)) {
            if (side == 0 && world.isAirBlock(x, y - 1, z)) {
                ...
                if (isNextToSupport(world, x, y, z) != 0 && hasSupportsInRange(world, x, y, z, 5)) return true;
            } else if (side == 3) {
                if (isNextToSupport(world, x, y, z) != 0 && hasSupportsInRange(world, x, y, z, 5)) return true;
            } else if (side == 4) {
                if (isNextToSupport(world, x, y, z) != 0 && hasSupportsInRange(world, x, y, z, 5)) return true;
            ...
            if (side == 1 && world.isAirBlock(x, y, z)) return true;
            else if (side == 2 && (TFCBlocks.isBlockVSupport(world.getBlock(x, y, z - 1)) || world.getBlock(x, y, z - 1)
                .isOpaqueCube()) && world.isAirBlock(x, y, z - 1)) return true;
            else if (side == 3 && (TFCBlocks.isBlockVSupport(world.getBlock(x, y, z + 1)) || world.getBlock(x, y, z + 1)
                .isOpaqueCube()) && world.isAirBlock(x, y, z + 1)) return true;
            else if (side == 4 && (TFCBlocks.isBlockVSupport(world.getBlock(x - 1, y, z)) || world.getBlock(x - 1, y, z)
                .isOpaqueCube()) && world.isAirBlock(x - 1, y, z)) return true;
            else if (side == 5 && (TFCBlocks.isBlockVSupport(world.getBlock(x + 1, y, z)) || world.getBlock(x + 1, y, z)
                .isOpaqueCube()) && world.isAirBlock(x + 1, y, z)) return true;
        }
