...
public class BlockOre extends BlockCollapsible {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityplayer, int par6, float par7,
        ...
        if (TFCOptions.enableDebugMode && world.isRemote) {
            ...
            TerraFirmaCraft.LOG.info(
                "Meta = " + (new StringBuilder()).append(getUnlocalizedName())
                    .append(":")
                    .append(metadata)
                    .toString());
    ...
    public int[] getDropBlock(World world, int x, int y, int z) {
        ...
        DataLayer dl = TFC_Climate.getCacheManager(world)
            .getRockLayerAt(x, z, TFC_Core.getRockLayerFromHeight(world, x, y, z));

        if (dl != null) {
            BlockStone stone = null;
            if (dl.block instanceof BlockStone) stone = (BlockStone) dl.block;
    ...
    public boolean removedByPlayer(World world, EntityPlayer player, int x, int y, int z) {
        if (!world.isRemote) {
            ...
            if (player == null || dropOres) {
                if (isCoal) itemstack = new ItemStack(TFCItems.coal, 1 + world.rand.nextInt(2));
                else {
                    TEOre te = (TEOre) world.getTileEntity(x, y, z);
                    int ore = getOreGrade(te, meta);
                    itemstack = new ItemStack(TFCItems.oreChunk, 1, damageDropped(ore));
                }
            } else if (hasHammer && !isCoal) itemstack = new ItemStack(TFCItems.smallOreChunk, 1, meta);
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        if (!world.isRemote) scanVisible(world, x, y, z);
    }
    ...
    public void scanVisible(World world, int x, int y, int z) {
        if (!world.isRemote) {
            ...
            if (!(te instanceof TEOre)) {
                TerraFirmaCraft.LOG
                    .warn("TE was not ore on call of BlockOre.scanVisible({}, {}, {}, {})", world, x, y, z);
                return;
            }

            if ((((TEOre) te).extraData & 8) == 0 && y < 255 && y > 0) {
                if (world.blockExists(x, y - 1, z) && world.blockExists(x, y + 1, z)
