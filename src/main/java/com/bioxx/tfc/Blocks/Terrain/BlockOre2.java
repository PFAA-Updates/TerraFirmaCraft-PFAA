...
public class BlockOre2 extends BlockOre {
    ...
    public BlockOre2(Material mat) {
        super(mat);
        blockNames = Global.ORE_MINERAL_NAMES;
    }

    @Override
    public int damageDropped(int dmg) {
        return dmg + Global.ORE_METAL_NAMES.length;
    }
    ...
    public ItemStack kimberliteGemSpawn() {
        ...
        Random random = new Random();
        if (random.nextInt(50) == 0) quality = 1;
        else if (random.nextInt(75) == 0) quality = 2;
        else if (random.nextInt(150) == 0) quality = 3;
        else if (random.nextInt(300) == 0) quality = 4;
     ...
     * if(true)
     * {
     * ItemChisel.CreateSlab(world, i, j, k, this.blockID, (byte) world.getBlockMetadata(i, j, k), side,
     * mod_TFC_Core.stoneMinedSlabs.blockID);
