...
public class BlockGrass extends BlockTerra {
    ...
    public IIcon getIcon(int side, int meta) {
        if (side == 1) return grassTopTexture;
        else if (side == 0) return TFC_Textures.invisibleTexture;
        else return iconGrassSideOverlay;
    }
    ...
    private boolean isSnow(IBlockAccess access, int x, int y, int z) {
        Material material = access.getBlock(x, y, z)
            .getMaterial();
    ...
    public void updateTick(World world, int i, int j, int k, Random rand) {
        if (!world.isRemote) {
            ...
            } else if (world.getBlock(i, j + 1, k)
                .isSideSolid(world, i, j + 1, k, ForgeDirection.DOWN)) {
                    ...
                    if (TFC_Core.isGrass(this) && !TFC_Core.isDryGrass(this)
                        && world.getBlock(i, j + 1, k)
                            .getMaterial() != Material.water
                        && world.isAirBlock(i, j + 1, k)) {
                        if (rand.nextInt((int) ((16800 - rain) / 4)) == 0 && temp > 20) world
                            .setBlock(i, j + 1, k, TFCBlocks.tallGrass, (world.rand.nextInt(30) == 0 ? 1 : 0), 0x2); // 1/30
                                                                                                                     // chance
                                                                                                                     // to
                                                                                                                     // spawn
                                                                                                                     // fern
                    ...
                    } else if (TFC_Core.isGrass(this) && TFC_Core.isDryGrass(this)
                        && (nearWater || rain >= 500)
        ...
        for (int var6 = 0; var6 < 4; ++var6) {
            ...
            if (world.blockExists(x, Global.SEALEVEL, z)) {
                ...
                if (y > 0) {
                    ...
                    if (TFC_Core.isDirt(id) && rand.nextInt(10) == 0) world.setBlock(
                        x,
                        y,
                        z,
                        TFC_Core.getTypeForGrassWithRainByBlock(id, TFC_Climate.getRainfall(world, x, y, z)),
                        meta,
                        0x2);
    ...
    public int getTopSolidBlock(World world, int xCoord, int zCoord) {
        ...
        for (zCoord &= 15; y > 0; --y) {
            ...
            if (block.isOpaqueCube() && material.isOpaque()
                ...
                && (block.isSideSolid(world, x, y, z, ForgeDirection.UP)
                    || block.isSideSolid(world, x, y, z, ForgeDirection.DOWN))) {
                return y;
            }
        }
    ...
    public void onEntityWalking(World world, int x, int y, int z, Entity entity) {
        if (!world.isRemote && this != TFCBlocks.clayGrass2
            && this != TFCBlocks.clayGrass
            && this != TFCBlocks.peatGrass) {
            Random r = new Random();
            if (BlockCollapsible.canFallBelow(world, x, y - 1, z) && r.nextInt(10) == 0
                && !BlockCollapsible.isNearSupport(world, x, y, z, 4, 0)) {
                int meta = world.getBlockMetadata(x, y, z);
    ...
    public Item getItemDropped(int metadata, Random rand, int fortune) {
        return TFC_Core.getTypeForDirtFromGrass(this)
            .getItemDropped(metadata, rand, fortune);
    }
