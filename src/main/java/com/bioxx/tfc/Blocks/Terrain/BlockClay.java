...
public class BlockClay extends BlockTerra {
    ...
    public void registerBlockIcons(IIconRegister registerer) {
        ...
        for (int i = 0; i < count; i++) dirtTexture[i] = registerer
            .registerIcon(Reference.MOD_ID + ":" + "clay/Clay " + Global.STONE_ALL[i + textureOffset]);
    }
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        ...
        for (int i = 0; i < count; i++) ret.add(new ItemStack(item, 1, 0));
        return ret;
    }

    /*
     * public void getCollidingBoundingBoxes(World world, int i, int j, int k, AxisAlignedBB par5AxisAlignedBB,
