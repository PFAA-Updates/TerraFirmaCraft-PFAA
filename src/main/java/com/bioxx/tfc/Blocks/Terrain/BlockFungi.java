...
public class BlockFungi extends BlockMushroom {
    ...
    public void registerBlockIcons(IIconRegister register) {
        ...
        for (int i = 0; i < this.icons.length; ++i) {
            ...
            this.icons[i] = register
                .registerIcon((icons.length > 2 ? Reference.MOD_ID + ":plants/" : "") + Global.FUNGI_META_NAMES[i]);
        }
    }
    ...
    public boolean canBlockStay(World world, int x, int y, int z) {
        if (y >= 0 && y < 256) {
            ...
            return var5 == Blocks.mycelium
                || world.getFullBlockLightValue(x, y, z) < 13 && this.canThisPlantGrowOnThisBlock(var5);
        } else {
            return false;
        }
    }
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        ...
        return (world.isAirBlock(x, y, z) || var5.getMaterial()
            .isReplaceable()) && this.canThisPlantGrowOnThisBlock(var5) && this.canBlockStay(world, x, y, z);
    }
    ...
    public boolean func_149884_c/* fertilizeMushroom */(World world, int x, int y, int z, Random rnd) {
        ...
        if (this == block && meta == 0) bigGen = new WorldGenBigMushroom(0);
        else if (this == block && meta == 1) bigGen = new WorldGenBigMushroom(1);
