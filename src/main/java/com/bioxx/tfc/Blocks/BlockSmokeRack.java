...
public class BlockSmokeRack extends BlockTerraContainer {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityplayer, int side, float hitX,
        ...
        if (!world.isRemote) {
            ...
            if ((meta & 1) == 0 && hitZ < 0.5) {
                if (te.getStackInSlot(0) == null && yarn != null
                    && isItemValid(entityplayer.inventory.getCurrentItem())) {
                    te.setInventorySlotContents(
                        0,
                ...
                if (te.getStackInSlot(1) == null && yarn != null
                    && isItemValid(entityplayer.inventory.getCurrentItem())) {
                    te.setInventorySlotContents(
                        1,
                        entityplayer.inventory.getCurrentItem()
    ...
    private boolean isItemValid(ItemStack is) {
        ...
        if (is.getItem() instanceof ItemFoodMeat) {
            ...
            if (!Food.isCooked(is)) return true;
        }
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
        ...
        if (!world.isRemote) {
            if ((meta & 1) == 0) {
                if (!isValidNeighbor(world, x, y, z - 1, ForgeDirection.NORTH)
                    || !isValidNeighbor(world, x, y, z + 1, ForgeDirection.SOUTH)) {
                    TFC_Core.destroyBlock(world, x, y, z);
                }
            } else {
                if (!isValidNeighbor(world, x - 1, y, z, ForgeDirection.WEST)
                    || !isValidNeighbor(world, x + 1, y, z, ForgeDirection.EAST)) {
                    TFC_Core.destroyBlock(world, x, y, z);
                }
            }
