...
public class BlockChestTFC extends BlockTerraContainer {
    ...
    public int getDamageValue(World world, int x, int y, int z) {
        ...
        if (te instanceof TEChest) return ((TEChest) te).type;
        return 0;
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7,
        ...
        if (world.isRemote) {
            ...
            if (iinventory != null) player.openGui(TerraFirmaCraft.instance, 29, world, x, y, z);
    ...
    private ForgeDirection getAdjacentChestDirection(IBlockAccess world, int x, int y, int z, int type) {
        ForgeDirection[] dirs = { ForgeDirection.NORTH, ForgeDirection.SOUTH, ForgeDirection.EAST,
            ForgeDirection.WEST };
        for (ForgeDirection dir : dirs) {
            if (isChestOfType(world, x + dir.offsetX, y, z + dir.offsetZ, type)) return dir;
        }
    ...
    public boolean canReplace(World world, int x, int y, int z, int side, ItemStack itemStack) {
        ...
        for (ForgeDirection dir : dirs) {
            if (isChestOfType(world, x + dir.offsetX, y, z + dir.offsetZ, type)) {
                if (adjDirection != ForgeDirection.UNKNOWN) return false;
        ...
        ForgeDirection doubleDirection = getAdjacentChestDirection(
            world,
            x + adjDirection.offsetX,
            y,
            z + adjDirection.offsetZ,
            type);
