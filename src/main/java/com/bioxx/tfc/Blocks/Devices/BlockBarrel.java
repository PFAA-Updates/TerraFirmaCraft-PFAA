...
public class BlockBarrel extends BlockTerraContainer {
    ...
    public IIcon getIcon(int side, int meta) {
        ...
        if (side >= 10) {
            side -= 10;
            if (side == 0 || side == 1) return TFC_Textures.invisibleTexture;
            else return blockIcon;
        }
        if (meta < 16) return TFCBlocks.planks.getIcon(side, meta);
    ...
    public void getSubBlocks(Item par1, CreativeTabs par2CreativeTabs, List par3List) {
        for (int i = 0; i < woodNames.length; i++) par3List.add(new ItemStack(this, 1, i));
    }
        ...
        if (!world.isRemote) {
            ...
            if (te != null && te.shouldDropItem
                ...
                if (te.getSealed()) {
                    ...
                    for (int s = 0; s < te.getSizeInventory(); ++s) te.setInventorySlotContents(s, null);
                } else {
                    ItemStack is = new ItemStack(Item.getItemFromBlock(this), 1, te.barrelType);
                    EntityItem ei = new EntityItem(world, x, y, z, is);
                    world.spawnEntityInWorld(ei);
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
        if (world.getTileEntity(x, y, z) instanceof TEBarrel) {
            ...
            if (this == TFCBlocks.barrel && te != null && te.getGunPowderCount() >= 12 && te.getSealed()) {
                ...
                if (world.getBlock(x - 1, y, z) instanceof BlockFire) fireNearby = true;
                if (world.getBlock(x + 1, y, z) instanceof BlockFire) fireNearby = true;
                if (world.getBlock(x, y, z - 1) instanceof BlockFire) fireNearby = true;
                if (world.getBlock(x, y, z + 1) instanceof BlockFire) fireNearby = true;
    ...
    public NBTTagCompound writeBarrelToNBT(TEBarrel te) {
        ...
        if (te.fluid != null) te.fluid.writeToNBT(fluidNBT);
        ...
        if (nbttaglist.tagCount() > 0) nbt.setTag("Items", nbttaglist);
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (world.isRemote) {
            ...
            if (world.getTileEntity(x, y, z) instanceof TEBarrel) {
                ...
                if (this == TFCBlocks.barrel && te.getSealed()
                    && te.getGunPowderCount() >= 12
                    && player.getCurrentEquippedItem() != null
                    && player.getCurrentEquippedItem()
                        .getItem() instanceof ItemFlintAndSteel) {
                    spawnPowderKeg(world, x, y, z, te, false);
                    world.setBlockToAir(x, y, z);
                    return true;
                }
                ...
                if (!handleInteraction(player, te)) {
                    ...
                    else player.openGui(TerraFirmaCraft.instance, 36, world, x, y, z);
                    return true;
                } else return true;
            }
        }
    ...
    protected boolean handleInteraction(EntityPlayer player, TEBarrel te) {
        if (!te.getSealed() && te.getInvCount() <= 1 && !te.getWorldObj().isRemote) {
            ...
            if ((FluidContainerRegistry.isFilledContainer(equippedItem)
                ...
                if (equippedItem.stackSize == 0 && (is.getMaxStackSize() == 1 || !player.inventory.hasItemStack(is))) // put
                                                                                                                      // buckets
                                                                                                                      // in
                                                                                                                      // the
                                                                                                                      // slot
                    ...
                    if (!player.inventory.addItemStackToInventory(is)) player.dropPlayerItemWithRandomChoice(is, false); // if
                                                                                                                         // the
                                                                                                                         // new
                                                                                                                         // item
                                                                                                                         // dosn't
                ...
                || equippedItem.getItem() instanceof IFluidContainerItem) {
                    ...
                    if (equippedItem.stackSize == 0
                        && (is.getMaxStackSize() == 1 || !player.inventory.hasItemStack(is))) // put buckets in the slot
                                                                                              // you used them from.
                        player.inventory.setInventorySlotContents(player.inventory.currentItem, is);
                    else {
                    ...
                    return true;
                } else if (equippedItem.getItem() instanceof ItemBarrels
                    || equippedItem.getItem() instanceof ItemLargeVessel) {
                        ItemStack is = equippedItem.copy();
                        is.stackSize = 1;
                        if (equippedItem.hasTagCompound()) {
                            if (equippedItem.getTagCompound()
                                .hasKey("fluidNBT")
                                && !equippedItem.getTagCompound()
                                    .hasKey("Items")
                                && te.getFluidLevel() < te.getMaxLiquid()) {
                                FluidStack fs = FluidStack.loadFluidStackFromNBT(
                                    equippedItem.getTagCompound()
                                        .getCompoundTag("fluidNBT"));
                                if (te.addLiquid(fs)) {
                                    if (fs.amount == 0) {
                                        equippedItem.getTagCompound()
                                            .removeTag("Sealed");
                                        equippedItem.getTagCompound()
                                            .removeTag("fluidNBT");
                                        if (equippedItem.getTagCompound()
                                            .hasNoTags()) equippedItem.setTagCompound(null);
                                    } else {
                                        fs.writeToNBT(
                                            equippedItem.getTagCompound()
                                                .getCompoundTag("fluidNBT"));
                            ...
                            if (te.getFluidStack() != null) {
                                ...
                                if (is.getItem() instanceof ItemBarrels) {
                                    ...
                                    if (is.getItemDamage() == 0) return false;

                                    FluidStack fs = te.getFluidStack()
                                        .copy();
                                    if (fs.amount > 5000) {
                                        fs.amount = 5000;
                                        te.drainLiquid(5000);
    ...
    public int getDamageValue(World world, int x, int y, int z) {
        ...
        if (te instanceof TEBarrel) return ((TEBarrel) te).barrelType;
        return 0;
    }
