...
public class BlockOilLamp extends BlockTerraContainer {
    ...
    public IIcon getIcon(int side, int meta) {
        int m = meta & 7;
        if (side == 0 || side == 1) {
            if (m == 0) return TFC_Textures.sheetGold;
            else if (m == 1) return TFC_Textures.sheetPlatinum;
            else if (m == 2) return TFC_Textures.sheetRoseGold;
            else if (m == 3) return TFC_Textures.sheetSilver;
            else if (m == 4) return TFC_Textures.sheetSterlingSilver;
            else if (m == 5) return TFC_Textures.sheetBlueSteel;
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (!world.isRemote) {
            ...
            if (!isLampLit(meta)) {
                ...
                if (te != null) {
                    te.updateLampFuel(false); // Update lamp fuel, but don't burn fuel for time passed while lamp was
                                              // off before turning the lamp on.
                    if (te.isFuelValid())
                ...
                TEOilLamp te = (TEOilLamp) world.getTileEntity(x, y, z);
                if (te != null) {
                    te.updateLampFuel(true); // Update lamp fuel and burn fuel for time passed before turning the lamp
                                             // off.
                }
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block b) {
        if (!World.doesBlockHaveSolidTopSurface(world, x, y - 1, z)) TFC_Core.setBlockToAirWithDrops(world, x, y, z);
    }
