...
public class BlockEarlyBloomery extends BlockTerraContainer implements ICustomCollision {
    ...
    private boolean isNorthStackValid(World world, int x, int y, int z) {
        return (world.getBlock(x, y, z)
            ...
            && world.getBlock(x, y, z)
                .isNormalCube()
            || TFC_Core.isSouthFaceSolid(world, x, y, z); // Since its the North Block, we need to make sure the South
                                                          // side facing the stack is solid
    }
    ...
    private boolean isSouthStackValid(World world, int x, int y, int z) {
        return (world.getBlock(x, y, z)
            ...
            && world.getBlock(x, y, z)
                .isNormalCube()
            || TFC_Core.isNorthFaceSolid(world, x, y, z);// Since its the South Block, we need to make sure the North
                                                         // side facing the stack is solid
    }
    ...
    private boolean isEastStackValid(World world, int x, int y, int z) {
        return (world.getBlock(x, y, z)
            ...
            && world.getBlock(x, y, z)
                .isNormalCube()
            || TFC_Core.isWestFaceSolid(world, x, y, z);// Since its the East Block, we need to make sure the West side
                                                        // facing the stack is solid
    }
    ...
    private boolean isWestStackValid(World world, int x, int y, int z) {
        return (world.getBlock(x, y, z)
            ...
            && world.getBlock(x, y, z)
                .isNormalCube()
            || TFC_Core.isEastFaceSolid(world, x, y, z); // Since its the West Block, we need to make sure the East side
                                                         // facing the stack is solid
    }
    ...
    private boolean checkHorizontal(World world, int x, int y, int z, boolean flip) {
        ...
        boolean l = false;
        boolean r = false;
        if ((world.getBlock(x - map[0], y, z - map[1])
            .getMaterial() == Material.rock
            || world.getBlock(x - map[0], y, z - map[1])
                .getMaterial() == Material.iron)
            && world.getBlock(x - map[0], y, z - map[1])
                .isOpaqueCube())
            l = true;
    ...
    private boolean checkVertical(World world, int x, int y, int z, boolean flip) {
        ...
        if (!b && world.getBlock(x, y - 1, z) == TFCBlocks.detailed
            ...
            switch (dir) {
                case 0:
                    if (TFC_Core.isNorthFaceSolid(world, x, y - 1, z) && TFC_Core.isEastFaceSolid(world, x, y - 1, z)
                        && TFC_Core.isWestFaceSolid(world, x, y - 1, z)) b = true;
                    break;
                case 1:
                    if (TFC_Core.isEastFaceSolid(world, x, y - 1, z) && TFC_Core.isNorthFaceSolid(world, x, y - 1, z)
                        && TFC_Core.isSouthFaceSolid(world, x, y - 1, z)) b = true;
                    break;
                case 2:
                    if (TFC_Core.isSouthFaceSolid(world, x, y - 1, z) && TFC_Core.isEastFaceSolid(world, x, y - 1, z)
                        && TFC_Core.isWestFaceSolid(world, x, y - 1, z)) b = true;
                    break;
                case 3:
                    if (TFC_Core.isWestFaceSolid(world, x, y - 1, z) && TFC_Core.isNorthFaceSolid(world, x, y - 1, z)
                        && TFC_Core.isSouthFaceSolid(world, x, y - 1, z)) b = true;
                    break;
            }
    ...
    public static int flipDir(int dir) {
        int out = 0;
        if (dir - 2 >= 0) out = dir - 2;
        else if (dir + 2 < 4) out = dir + 2;
        return out;
    }
    ...
    public void addCollisionBoxesToList(World world, int x, int y, int z, List list) {
        ...
        if (!BlockEarlyBloomery.isOpen(meta)) {
            if (dir == 0) list.add(new Object[] { AxisAlignedBB.getBoundingBox(0.0F, 0.0F, 0.0f, 1.0f, 1.0F, f) });
            else if (dir == 1)
            ...
            else if (dir == 3) list.add(new Object[] { AxisAlignedBB.getBoundingBox(0.0F, 0.0F, 0.0F, f, 1.0F, 1.0F) });
        } else {
            if (dir == 0) {
