...
public class BlockBlastFurnace extends BlockTerraContainer {
    ...
    public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int par6, float par7,
        ...
        if (!world.isRemote) {
            if (!canBlockStay(world, i, j, k)) {
                ...
                if (te.isValid) {
                    if (equippedItem != null && (equippedItem.getItem() == TFCItems.fireStarter
                        || equippedItem.getItem() == TFCItems.flintSteel))
                        if (te.canLight()) entityplayer.getCurrentEquippedItem()
    ...
    public boolean checkStackAt(World world, int x, int y, int z) {
        return checkBlock(world, x + 1, y, z, x, z) && checkBlock(world, x - 1, y, z, x, z)
            && checkBlock(world, x, y, z + 1, x, z)
            && checkBlock(world, x, y, z - 1, x, z)
            && (world.isAirBlock(x, y, z) || world.getBlock(x, y, z) == TFCBlocks.molten);
    }

    public boolean checkBlock(World world, int x, int y, int z, int stackX, int stackZ) {
        if (!world.blockExists(x, y, z) || world.getBlock(x, y, z) != TFCBlocks.fireBrick) return false;

        int count = 0;
        int xCoord = x - 1;
        int zCoord = z;
        if (world.blockExists(xCoord, y, zCoord) && !(xCoord == stackX && zCoord == stackZ)
            && world.getBlock(xCoord, y, zCoord) == TFCBlocks.metalSheet
            && world.getTileEntity(xCoord, y, zCoord) instanceof TEMetalSheet) {
            TEMetalSheet te = (TEMetalSheet) world.getTileEntity(xCoord, y, zCoord);
            if (!te.westExists() || !isValidMetalSheet(te)) return false;
            count++;
        }
        xCoord = x + 1;
        zCoord = z;
        if (world.blockExists(xCoord, y, zCoord) && !(xCoord == stackX && zCoord == stackZ)
            ...
            && world.getTileEntity(xCoord, y, zCoord) instanceof TEMetalSheet) {
            TEMetalSheet te = (TEMetalSheet) world.getTileEntity(xCoord, y, zCoord);
            if (!te.eastExists() || !isValidMetalSheet(te)) return false;
            count++;
        }
        xCoord = x;
        zCoord = z - 1;
        if (world.blockExists(xCoord, y, zCoord) && !(xCoord == stackX && zCoord == stackZ)
            ...
            && world.getTileEntity(xCoord, y, zCoord) instanceof TEMetalSheet) {
            TEMetalSheet te = (TEMetalSheet) world.getTileEntity(xCoord, y, zCoord);
            if (!te.southExists() || !isValidMetalSheet(te)) return false;
            count++;
        }
        xCoord = x;
        zCoord = z + 1;
        if (world.blockExists(xCoord, y, zCoord) && !(xCoord == stackX && zCoord == stackZ)
            ...
            && world.getTileEntity(xCoord, y, zCoord) instanceof TEMetalSheet) {
            TEMetalSheet te = (TEMetalSheet) world.getTileEntity(xCoord, y, zCoord);
            if (!te.northExists() || !isValidMetalSheet(te)) return false;
            count++;
        }
        return count >= 3;
    }
    ...
    public boolean isValidMetalSheet(TEMetalSheet te) {
        if (te != null) {
            ...
            if (sheet != null && (sheet.getItem() == TFCItems.wroughtIronSheet || sheet.getItem() == TFCItems.steelSheet
                || sheet.getItem() == TFCItems.blackSteelSheet
                || sheet.getItem() == TFCItems.blueSteelSheet
                || sheet.getItem() == TFCItems.redSteelSheet)) return true;
        }
        return false;
    }
    ...
    public IIcon getIcon(int i, int j) {
        ...
        if (i == 0 || i == 1) {
            if (lit == 1) return textureSide[1];
            else return textureSide[0];
        } else {
            if (lit == 1) return textureOn;
            else return textureOff;
        }
    }
    ...
    public boolean removedByPlayer(World world, EntityPlayer player, int i, int j, int k) {
        if (!world.isRemote) {
            ...
            if (world.getBlock(i, j, k) == TFCBlocks.molten) world.setBlockToAir(i, j, k);
            if (world.getBlock(i, j + 1, k) == TFCBlocks.molten) world.setBlockToAir(i, j + 1, k);
            if (world.getBlock(i, j + 2, k) == TFCBlocks.molten) world.setBlockToAir(i, j + 2, k);
            if (world.getBlock(i, j + 3, k) == TFCBlocks.molten) world.setBlockToAir(i, j + 3, k);
            if (world.getBlock(i, j + 4, k) == TFCBlocks.molten) world.setBlockToAir(i, j + 4, k);
            world.setBlockToAir(i, j, k);
        }
        return true;
    }
