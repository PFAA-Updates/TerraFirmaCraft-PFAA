...
public class BlockPottery extends BlockTerraContainer {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (!world.isRemote) {
            ...
            if (te.isLit() || player.inventory.getCurrentItem() != null && (player.inventory.getCurrentItem()
                .getItem() == TFCItems.flintSteel
                || player.inventory.getCurrentItem()
                    .getItem() == TFCItems.fireStarter))
                return false;

            if (player.inventory.getCurrentItem() != null && player.inventory.getCurrentItem()
                .getItem() == TFCItems.straw && !player.isSneaking()) {
                te.addStraw(player.inventory.getCurrentItem(), player);
                return true;
            } else if (player.inventory.getCurrentItem() != null && player.inventory.getCurrentItem()
                .getItem() instanceof ItemLogs && !player.isSneaking() && te.straw == 8) {
                    if (te.addLog(player.inventory.getCurrentItem(), player)) {
                        world.playSoundEffect(
                            x + 0.5,
                            y + 0.5,
                            z + 0.5,
                            TFCBlocks.logNatural.stepSound.func_150496_b(),
                            (TFCBlocks.logNatural.stepSound.getVolume() + 1.0F) / 2.0F,
                            TFCBlocks.logNatural.stepSound.getPitch() * 0.8F);
                        return true;
                    }
                } else if (player.inventory.getCurrentItem() != null && !(player.inventory.getCurrentItem()
                    .getItem() instanceof ItemPotteryBase)
                    || te.getStackInSlot(0) != null && te.getStackInSlot(1) != null
                    ...
                    || player.inventory.getCurrentItem() == null) {
                        if (te.wood > 0) {
                            ...
                            if (te.getStackInSlot(0) != null && te.getStackInSlot(0)
                                .getItem() instanceof ItemBlock || hitX < 0.5 && hitZ < 0.5) te.ejectItem(0);
                            else if (hitX > 0.5 && hitZ < 0.5) te.ejectItem(1);
                            else if (hitX < 0.5 && hitZ > 0.5) te.ejectItem(2);
                            else if (hitX > 0.5 && hitZ > 0.5) te.ejectItem(3);
                            world.markBlockForUpdate(te.xCoord, te.yCoord, te.zCoord);
                        }
                    }
        }
