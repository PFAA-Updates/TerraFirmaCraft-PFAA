...
public class BlockLargeVessel extends BlockBarrel {
    ...
    public IIcon getIcon(int side, int meta) {
        if (meta == 1) {
            if (side == 1) return ceramicIcons[0];
            else if (side == 0) return ceramicIcons[2];
            else return ceramicIcons[1];
        }
        if (side == 1) return clayIcons[0];
        else if (side == 0) return clayIcons[2];
        else return clayIcons[1];
    }
        ...
        if (world.isRemote) {
            ...
            if (world.getTileEntity(x, y, z) != null) {
                ...
                if (!handleInteraction(player, te)) {
                    if (te.getInvCount() == 0) player.openGui(TerraFirmaCraft.instance, 46, world, x, y, z);
                    else player.openGui(TerraFirmaCraft.instance, 47, world, x, y, z);
                    return true;
                } else return true;
            }
        }
