...
public class BlockAnvil extends BlockTerraContainer {
    ...
    public void getSubBlocks(Item par1, CreativeTabs par2CreativeTabs, List par3List) {
        if (this == TFCBlocks.anvil) {
            for (int i = 1; i < 8; i++) par3List.add(new ItemStack(this, 1, i));
        }

        if (this == TFCBlocks.anvil2) {
            for (int i = 0; i < 3; i++) par3List.add(new ItemStack(this, 1, i));
        }
    }
    ...
    @Override
    public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float hitX,
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World par1World, int par2, int par3, int par4) {
        ...
        if (te instanceof TEAnvil) {
            ...
            if (teAnvil.anvilTier != AnvilReq.STONE.Tier || this == TFCBlocks.anvil2) {
                if (direction == 0) return AxisAlignedBB.getBoundingBox(
                    par2 + 0.2,
                    (double) par3 + 0,
                    (double) par4 + 0,
                    par2 + 0.8,
        ...
        if (te.anvilTier != AnvilReq.STONE.Tier) {
            if (direction == 0) {
                ...
                return AxisAlignedBB
                    .getBoundingBox(x + 0.2, (double) y + 0, (double) z + 0, x + 0.8, y + 0.6, (double) z + 1);
            } else {
                this.setBlockBounds(0, 0, 0.2f, 1, 0.6f, 0.8f);
                return AxisAlignedBB
                    .getBoundingBox((double) x + 0, (double) y + 0, z + 0.2, (double) x + 1, y + 0.6, z + 0.8);
            }
        } else {
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess bAccess, int x, int y, int z) {
        ...
        if (te.anvilTier != AnvilReq.STONE.Tier) {
            if (direction == 0) this.setBlockBounds(0.2f, 0, 0, 0.8f, 0.6f, 1);
            else this.setBlockBounds(0, 0, 0.2f, 1, 0.6f, 0.8f);
        } else {
            this.setBlockBounds(0, 0, 0, 1, 0.9F, 1);
        }
    }
    ...
    public IIcon getIcon(int i, int j) {
        ...
        if (j == 0 && this == TFCBlocks.anvil) {
            ...
            if (i == 0 || i == 1) return textureMapTop[meta];
            else return textureMapSide[meta];
        }
    }
    ...
    public void dropBlockAsItemWithChance(World par1World, int x, int y, int z, int meta, float par6, int par7) {
        if (!par1World.isRemote) this.dropBlockAsItem(par1World, x, y, z, new ItemStack(this, 1, meta));
    }

    @Override
    protected void dropBlockAsItem(World par1World, int par2, int par3, int par4, ItemStack is) {
        if (!par1World.isRemote && par1World.getGameRules()
            .getGameRuleBooleanValue("doTileDrops")) {
            if (is.getItemDamage() == 0 && this == TFCBlocks.anvil) return;
            float f = 0.7F;
    ...
    public void onBlockPlacedBy(World world, int i, int j, int k, EntityLivingBase entityliving, ItemStack is) {
        ...
        if (this == TFCBlocks.anvil) te.anvilTier = AnvilReq.getReqFromInt(meta).Tier;
        else if (this == TFCBlocks.anvil2) te.anvilTier = AnvilReq.getReqFromInt2(meta).Tier;
    }
        ...
        if (var5 != null) {
            for (int var6 = 0; var6 < var5.getSizeInventory(); ++var6) {
                ...
                if (var7 != null) {
                    ...
                    for (float var10 = world.rand.nextFloat() * 0.8F + 0.1F; var7.stackSize > 0; world
                        .spawnEntityInWorld(var12)) {
                        int var11 = world.rand.nextInt(21) + 10;

                        if (var11 > var7.stackSize) var11 = var7.stackSize;
                        var7.stackSize -= var11;
                        var12 = new EntityItem(
                            world,
                            x + var8,
                            y + var9,
                            z + var10,
                        ...
                        if (var7.hasTagCompound()) var12.getEntityItem()
                            .setTagCompound(
    ...
    public void registerBlockIcons(IIconRegister registerer) {
        ...
        for (int i = (anvilId == 0 ? 1 : 0); i < (anvilId == 0 ? 8 : 3); i++) {
            textureMapTop[i] = registerer
                .registerIcon(Reference.MOD_ID + ":" + "devices/Anvil_" + (i + anvilId) + "_Top");
            textureMapSide[i] = registerer
                .registerIcon(Reference.MOD_ID + ":" + "devices/Anvil_" + (i + anvilId) + "_Side");
        }
