...
public class BlockDetailed extends BlockPartial {
    ...
    public boolean isSideSolid(IBlockAccess world, int x, int y, int z, ForgeDirection side) {
        ...
        for (int subX = startX; subX < endX; ++subX)
            for (int subY = startY; subY < endY; ++subY) for (int subZ = startZ; subZ < endZ; ++subZ)
                if (!te.getBlockExists(subX, subY, subZ) && --transpCount < 0) return false;
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityplayer, int side, float hitX,
        ...
        PlayerInfo pi = PlayerManagerTFC.getInstance()
            .getPlayerInfoFromPlayer(entityplayer);
        for (int i = 0; i < 9; i++) {
        ...
        if (entityplayer.getCurrentEquippedItem() != null && entityplayer.getCurrentEquippedItem()
            .getItem() instanceof ItemChisel && hasHammer && world.isRemote && pi.lockMatches(x, y, z)) {
            TEDetailed te = (TEDetailed) world.getTileEntity(x, y, z);
            lockX = x;
            lockY = y;
            lockZ = z;
    ...
    public boolean onBlockActivatedServer(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        float hitY, float hitZ) {
        int mode = 0;
        PlayerInfo pi = PlayerManagerTFC.getInstance()
        ...
        if (mode == 1) {
            ...
            if (xSelected < 4 && ySelected < 4 && zSelected < 4) for (int subX = 0; subX < 4; subX++)
                for (int subZ = 0; subZ < 4; subZ++) for (int subY = 0; subY < 4; subY++) {
                    index = (subX * 8 + subZ) * 8 + subY;
                    deleteBox(world, x, y, z, player, te, index, hasChisel, hasHammer);
                }
            if (xSelected > 3 && ySelected < 4 && zSelected < 4) for (int subX = 4; subX < 8; subX++)
                for (int subZ = 0; subZ < 4; subZ++) for (int subY = 0; subY < 4; subY++) {
                    index = (subX * 8 + subZ) * 8 + subY;
                    deleteBox(world, x, y, z, player, te, index, hasChisel, hasHammer);
                }
            if (xSelected > 3 && ySelected < 4 && zSelected > 3) for (int subX = 4; subX < 8; subX++)
                for (int subZ = 4; subZ < 8; subZ++) for (int subY = 0; subY < 4; subY++) {
                    index = (subX * 8 + subZ) * 8 + subY;
                    deleteBox(world, x, y, z, player, te, index, hasChisel, hasHammer);
                }
            if (xSelected < 4 && ySelected < 4 && zSelected > 3) for (int subX = 0; subX < 4; subX++)
                for (int subZ = 4; subZ < 8; subZ++) for (int subY = 0; subY < 4; subY++) {
                    index = (subX * 8 + subZ) * 8 + subY;
                    deleteBox(world, x, y, z, player, te, index, hasChisel, hasHammer);
                }
            if (xSelected < 4 && ySelected > 3 && zSelected < 4) for (int subX = 0; subX < 4; subX++)
                for (int subZ = 0; subZ < 4; subZ++) for (int subY = 4; subY < 8; subY++) {
                    index = (subX * 8 + subZ) * 8 + subY;
                    deleteBox(world, x, y, z, player, te, index, hasChisel, hasHammer);
                }
            if (xSelected > 3 && ySelected > 3 && zSelected < 4) for (int subX = 4; subX < 8; subX++)
                for (int subZ = 0; subZ < 4; subZ++) for (int subY = 4; subY < 8; subY++) {
                    index = (subX * 8 + subZ) * 8 + subY;
                    deleteBox(world, x, y, z, player, te, index, hasChisel, hasHammer);
                }
            if (xSelected > 3 && ySelected > 3 && zSelected > 3) for (int subX = 4; subX < 8; subX++)
                for (int subZ = 4; subZ < 8; subZ++) for (int subY = 4; subY < 8; subY++) {
                    index = (subX * 8 + subZ) * 8 + subY;
                    deleteBox(world, x, y, z, player, te, index, hasChisel, hasHammer);
                }
            if (xSelected < 4 && ySelected > 3 && zSelected > 3) for (int subX = 0; subX < 4; subX++)
                for (int subZ = 4; subZ < 8; subZ++) for (int subY = 4; subY < 8; subY++) {
                    index = (subX * 8 + subZ) * 8 + subY;
                    deleteBox(world, x, y, z, player, te, index, hasChisel, hasHammer);
                }
    ...
    public void deleteBox(World world, int x, int y, int z, EntityPlayer player, TEDetailed te, int index,
        int hasChisel, int hasHammer) {
        te.data.clear(index);
    ...
    @Override
    public void addCollisionBoxesToList(World world, int i, int j, int k, AxisAlignedBB aabb, List list,
        Entity entity) {
    ...
    public Object[] rayTraceBound(AxisAlignedBB bound, int i, int j, int k, Vec3 player, Vec3 view) {
        ...
        if (!isVecInsideYZBounds(bound, minX)) minX = null;
        if (!isVecInsideYZBounds(bound, maxX)) maxX = null;
        if (!isVecInsideXZBounds(bound, minY)) minY = null;
        if (!isVecInsideXZBounds(bound, maxY)) maxY = null;
        if (!isVecInsideXYBounds(bound, minZ)) minZ = null;
        if (!isVecInsideXYBounds(bound, maxZ)) maxZ = null;
        ...
        byte side = -1;
        if (tracedBound == minX) side = 4;
        if (tracedBound == maxX) side = 5;
        if (tracedBound == minY) side = 0;
        if (tracedBound == maxY) side = 1;
        if (tracedBound == minZ) side = 2;
        if (tracedBound == maxZ) side = 3;
    ...
    private boolean isVecInsideYZBounds(AxisAlignedBB bound, Vec3 vec3) {
        ...
        else return vec3.yCoord >= bound.minY && vec3.yCoord <= bound.maxY
    ...
    private boolean isVecInsideXZBounds(AxisAlignedBB bound, Vec3 vec3) {
        if (vec3 == null) return false;
        else return vec3.xCoord >= bound.minX && vec3.xCoord <= bound.maxX
            && vec3.zCoord >= bound.minZ
            && vec3.zCoord <= bound.maxZ;
    }
    ...
    public int getFlammability(IBlockAccess world, int x, int y, int z, ForgeDirection face) {
        TEDetailed te = (TEDetailed) world.getTileEntity(x, y, z);
        if (te.typeID >= 0) return Blocks.fire.getFlammability(Block.getBlockById(te.typeID));
        else return 0;
    }
    ...
    public int getFireSpreadSpeed(IBlockAccess world, int x, int y, int z, ForgeDirection face) {
        TEDetailed te = (TEDetailed) world.getTileEntity(x, y, z);
        if (te.typeID >= 0) return Blocks.fire.getEncouragement(Block.getBlockById(te.typeID));
        else return 0;
    }
