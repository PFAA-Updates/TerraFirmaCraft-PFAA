...
public class BlockIngotPile extends BlockTerraContainer {
        ...
        if (world.isRemote) {
            ...
            if ((TEIngotPile) world.getTileEntity(x, y, z) != null) {
                ...
                if (!entityplayer.isSneaking() && tileentityingotpile.getStackInSlot(0) != null) {
                    if (tileentityingotpile.getStackInSlot(0).stackSize > 0) tileentityingotpile.injectContents(0, -1);

                    world.spawnEntityInWorld(
                        new EntityItem(
                            world,
                            tileentityingotpile.xCoord,
                            tileentityingotpile.yCoord + 1,
                            tileentityingotpile.zCoord,
                            new ItemStack(
                                tileentityingotpile.getStackInSlot(0)
                                    .getItem(),
                                1,
                                tileentityingotpile.getStackInSlot(0)
                                    .getItemDamage())));
                    ...
                    if (tileentityingotpile.getStackInSlot(0).stackSize < 1) world.setBlockToAir(x, y, z);
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        if (te != null && te.getStackInSlot(0) != null) return AxisAlignedBB.getBoundingBox(
            x,
            (double) y + 0,
            (double) z + 0,
            (double) x + 1,
            y + ((te.getStackInSlot(0).stackSize + 7) / 8) * 0.125,
            (double) z + 1);
        // else

        return AxisAlignedBB
            .getBoundingBox(x, (double) y + 0, (double) z + 0, (double) x + 1, y + 0.25, (double) z + 1);
    }
    ...
    public AxisAlignedBB getSelectedBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        if (te.getStackInSlot(0) != null) return AxisAlignedBB.getBoundingBox(
            x,
            (double) y + 0,
            (double) z + 0,
            (double) x + 1,
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess bAccess, int x, int y, int z) {
        ...
        else this.setBlockBounds(0f, 0f, 0f, 0f, 0.25f, 0f);
    }
        ...
        if (te != null) {
            for (int var6 = 0; var6 < te.getSizeInventory(); ++var6) {
                ...
                if (var7 != null) {
                    ...
                    for (float var10 = this.random.nextFloat() * 0.8F + 0.1F; var7.stackSize > 0; world
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
        if (!world.isRemote) {
            if (!world.isSideSolid(x, y - 1, z, ForgeDirection.UP)
                && world.getTileEntity(x, y, z) instanceof TEIngotPile) {
                ...
                if (world.getBlock(x, y - 1, z) == this && world.getTileEntity(x, y - 1, z) instanceof TEIngotPile) {
                    ...
                    if (ingot == lowerIngot) combineIngotsDown(world, x, y, z);
                } else if (world.getBlock(x, y + 1, z) == this
                    && world.getTileEntity(x, y + 1, z) instanceof TEIngotPile) {
                        ...
                        if (ingot == upperIngot) combineIngotsUp(world, x, y, z);
                    } else {
