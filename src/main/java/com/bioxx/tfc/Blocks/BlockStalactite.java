...
public class BlockStalactite extends BlockTerra {
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess access, int i, int j, int k) {
        ...
        r = new Random(i + (i * k));
        if (isStalac) {
            float height = TFC_Core.isRawStone(access.getBlock(i, j + 1, k)) ? 1
                : TFC_Core.isRawStone(access.getBlock(i, j + 2, k)) ? 2 : 3;
            f = 0.1f + r.nextFloat() * 0.025f;
            float width = height * f;
            if (height == 3) height = 0.5f + r.nextFloat() * 0.5f;
            else height = 1;
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int i, int j, int k) {
        ...
        r = new Random(i + (i * k));
        if (isStalac) {
            float height = TFC_Core.isRawStone(world.getBlock(i, j + 1, k)) ? 1
                : TFC_Core.isRawStone(world.getBlock(i, j + 2, k)) ? 2 : 3;
            f = 0.1f + r.nextFloat() * 0.025f;
            float width = height * f;
            if (height == 3) height = 0.5f + r.nextFloat() * 0.5f;
            else height = 1;
        ...
        return AxisAlignedBB
            .getBoundingBox(i + this.minX, j + this.minY, k + this.minZ, i + this.maxX, j + this.maxY, k + this.maxZ);
    }
    ...
    public IIcon getIcon(IBlockAccess access, int i, int j, int k, int meta) {
        ...
        boolean isStalag = isStalagmite(access.getBlockMetadata(i, j, k));
        if (isStalac) {
            if (TFC_Core.isRawStone(access.getBlock(i, j + 1, k))) return access.getBlock(i, j + 1, k)
                .getIcon(0, access.getBlockMetadata(i, j + 1, k));
            else if (TFC_Core.isRawStone(access.getBlock(i, j + 2, k))) return access.getBlock(i, j + 2, k)
                .getIcon(0, access.getBlockMetadata(i, j + 2, k));
            else if (TFC_Core.isRawStone(access.getBlock(i, j + 3, k))) return access.getBlock(i, j + 3, k)
                .getIcon(0, access.getBlockMetadata(i, j + 3, k));
        } else if (isStalag) {
            if (TFC_Core.isRawStone(access.getBlock(i, j - 1, k))) return access.getBlock(i, j - 1, k)
                .getIcon(0, access.getBlockMetadata(i, j - 1, k));
            else if (TFC_Core.isRawStone(access.getBlock(i, j - 2, k))) return access.getBlock(i, j - 2, k)
    ...
    public boolean canBlockStay(World world, int i, int j, int k) {
        ...
        int h = 0;
        if (isStalac) {
            if (TFC_Core.isRawStone(world.getBlock(i, j + 1, k))) h = 1;
            else if (TFC_Core.isRawStone(world.getBlock(i, j + 2, k))) h = 2;
            else if (TFC_Core.isRawStone(world.getBlock(i, j + 3, k))) h = 3;
            for (int y = h; y > 0; y--) {
