        ...
        try {
            ...
            for (int x = -sizeHalf; x < sizeHalf; x++) {
                for (int z = -sizeHalf; z < sizeHalf; z++) {
                    count++;
                    int ph = TFC_Climate.getCacheManager(world)
                        .getDrainageLayerAt(xCoord + x * skipSize, zCoord + z * skipSize).data1;
                    int g = ph * 50;
            ...
            for (int x = -sizeHalf; x < sizeHalf; x++) {
                for (int z = -sizeHalf; z < sizeHalf; z++) {
                    ...
                    DataLayer dl = TFC_Climate.getCacheManager(world)
                        .getDrainageLayerAt(xCoord + x * skipSize, zCoord + z * skipSize);
        ...
        try {
            ...
            for (int x = -sizeHalf; x < sizeHalf; x++) {
                for (int z = -sizeHalf; z < sizeHalf; z++) {
                    count++;
                    int temp = (int) (255
                        * (TFC_Climate.getBioTemperature(world, xCoord + x * skipSize, zCoord + z * skipSize) / 50));
            ...
            for (int x = -size / 2; x < size / 2; x++) {
                for (int z = -size / 2; z < size / 2; z++) {
                    ...
                    graphics.setColor(
                        Color.getColor(
                            "",
                            ((TFCBiome) world.getWorldChunkManager()
                                .getBiomeGenAt(x * skipSize + xCoord, z * skipSize + zCoord)).getBiomeColor()));
