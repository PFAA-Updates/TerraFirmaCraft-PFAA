...
public class GenCommand extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] params) {
        ...
        if (params.length == 1) {
            if (params[0].equalsIgnoreCase("fruittree")) {
                ...
                if (!fruitGen.generate(
                    sender.getEntityWorld(),
                    sender.getEntityWorld().rand,
                    (int) player.posX,
                    (int) player.posY,
                    (int) player.posZ)) TFC_Core.sendInfoMessage(player, new ChatComponentText("Generation Failed"));
            }
            ...
            if (params[0].equals("fissure")) {
                ...
                } else TFC_Core.sendInfoMessage(player, new ChatComponentText("Invalid Tree"));
            }
    ...
    public int getTree(String tree) {
        if ("oak".equalsIgnoreCase(tree)) return 0;
        else if ("aspen".equalsIgnoreCase(tree)) return 1;
        else if ("birch".equalsIgnoreCase(tree)) return 2;
        else if ("chestnut".equalsIgnoreCase(tree)) return 3;
        else if ("douglasfir".equalsIgnoreCase(tree)) return 4;
        else if ("hickory".equalsIgnoreCase(tree)) return 5;
        else if ("maple".equalsIgnoreCase(tree)) return 6;
        else if ("ash".equalsIgnoreCase(tree)) return 7;
        else if ("pine".equalsIgnoreCase(tree)) return 8;
        else if ("sequoia".equalsIgnoreCase(tree)) return 9;
        else if ("spruce".equalsIgnoreCase(tree)) return 10;
        else if ("sycamore".equalsIgnoreCase(tree)) return 11;
        else if ("whitecedar".equalsIgnoreCase(tree)) return 12;
        else if ("whiteelm".equalsIgnoreCase(tree)) return 13;
        else if ("willow".equalsIgnoreCase(tree)) return 14;
        else if ("kapok".equalsIgnoreCase(tree)) return 15;
        else if ("acacia".equalsIgnoreCase(tree)) return 16;
        else return -1;
    }
}
