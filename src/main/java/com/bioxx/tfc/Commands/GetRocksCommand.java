...
public class GetRocksCommand extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] params) {
        ...
        throw new PlayerNotFoundException(
            "Rock Layer 1: " + t0s + "   Rock Layer 2: " + t1s + "   Rock Layer 3: " + t2s);
    }

    public static int getSoilMetaFromStone(Block inType, int inMeta) {
        if (inType == TFCBlocks.stoneIgIn) return inMeta;
        else if (inType == TFCBlocks.stoneSed) return inMeta + 3;
        else if (inType == TFCBlocks.stoneIgEx) return inMeta + 11;
        else return inMeta + 15;
    }
