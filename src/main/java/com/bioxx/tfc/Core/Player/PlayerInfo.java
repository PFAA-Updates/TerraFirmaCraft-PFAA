...
public class PlayerInfo {
    ...
    public void switchHoeMode(EntityPlayer player) {
        ...
        SkillRank agRank = TFC_Core.getSkillStats(player)
        ...
        if (lastChange + 3 < TFC_Time.getTotalTicks()) {
            ...
            if (player.getCurrentEquippedItem() != null && (player.getCurrentEquippedItem()
                .getItem() == TFCItems.igInHoe
                || player.getCurrentEquippedItem()
                    .getItem() == TFCItems.igExHoe
                || player.getCurrentEquippedItem()
                    .getItem() == TFCItems.sedHoe
                || player.getCurrentEquippedItem()
            ...
            if (hoeMode == MODE_NUTRIENT
                && (!isMetalHoe || isMetalHoe && agRank != SkillRank.Expert && agRank != SkillRank.Master)) hoeMode++;
    ...
    public void switchChiselMode() {
        if (lastChange + 3 < TFC_Time.getTotalTicks()) {
            ...
            if (chiselMode == ChiselManager.getInstance()
                .getSize() - 1) {
                chiselMode = 0;
            } else {
                chiselMode++;
    ...
    public boolean lockMatches(int x, int y, int z) {
        return (lockX == -9999999 || lockX == x) && (lockY == -9999999 || lockY == y)
            && (lockZ == -9999999 || lockZ == z);
    }
}
