...
public class BodyTempStats {
    ...
    public void onUpdate(EntityPlayer player) {
        if (TFC_Time.getTotalTicks() - this.tempTimer >= 20) {
            this.tempTimer += 20;
            if (heatStorage > temperatureLevel) heatStorage--;
    ...
    private void applyTemperature(EntityPlayer player) {
        ...
        if (rand.nextInt(2000 - getBaseBodyTempMod(player)) < 100 && food.stomachLevel >= 500) temperatureLevel++;
        if ((player.isSprinting() || player.swingProgress != 0)
            && rand.nextInt(1000 - (getBaseBodyTempMod(player)) / 2) < 100) temperatureLevel++;
        ...
        extraFoodConsumed = 0; // (temperatureLevel <0 && heatStorage <= 0 &&
                               // rand.nextInt(350)<100)?temperatureLevel/-10:0;
        extraWaterConsumed = 0; // (temperatureLevel >0 && heatStorage <= 0 &&
                                // rand.nextInt(350)<100)?temperatureLevel/10:0;
    ...
    public int applyTemperatureFromEnvironment(EntityPlayer player) {
        ...
        if (temperature <= 10) {
            ...
            if (rand.nextInt(1200 + modifier) < 10) return -1;
        }
        // if it's warm
            ...
            if (temperatureLevel <= -1) {
                if (rand.nextInt(1200 - Math.min(1199, (int) (temperature - 10) * 15)) < 100) return 1;
            } else if (temperature >= 1) {
                if (rand.nextInt(1200 - Math.min(1199, (int) (temperature - 10) * 15)) < 100) return -1;
            }
        } else if (temperature > 20) {
            if (temperatureLevel <= 1) {
                if (rand.nextInt(1200 - Math.min(1199, (int) (temperature - 20) * 10)) < 100) return 1;
            } else if (temperature > 1) {
                if (rand.nextInt(1200 - Math.min(1199, (int) (temperature - 20) * 10)) < 100) return -1;
            }
        }
                ...
                for (int k = z - 7; k < z + 7; k++) {
                    if (player.worldObj.blockExists(i, j, k)) {
                        ...
                        if (player.worldObj.getBlock(i, j, k) == Blocks.lava
                            ...
                            // returnAmount += (rand.nextInt(2000 - 198*(10-( (int)player.getDistance(i, j, k) ))
                            // )<10?1:0);
                            // Lava averages 700-1200 C = 950 C, assume source is lava.
                            double tempValue = 950;

                            // if there is a firepit, use it's heat instead.
                            if (te instanceof TEFireEntity) tempValue = ((TEFireEntity) te).fireTemp;

                            // Just to make sure it's not 0
                            double distanceSq = player.getDistanceSq(i, j, k) + 0.05;

                            // radiation isn't perfect, so I don't know what a good numerator is, but it decreases with
                            // the square of the distance.
                            // We can assume that the temperature of the actual heat source is when the player is
                            // directly touching it, which we have assigned to
                            // a distanceSq of 0.05, therefore, the heat from such a heat source is = to the heat value
                            // * 0.05 divided by the distance squared
