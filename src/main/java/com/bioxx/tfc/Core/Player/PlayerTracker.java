...
public class PlayerTracker {
    ...
    public void onPlayerLoggedIn(PlayerLoggedInEvent event) {
        ...
        // TerraFirmaCraft.log.info("------"+event.player.getDisplayName()+" : "+
        // event.player.getUniqueID().toString()+"--------");

        PlayerManagerTFC.getInstance().players
            .add(new PlayerInfo(event.player.getCommandSenderName(), event.player.getUniqueID()));
    ...
    public void onPlayerRespawn(PlayerRespawnEvent event) {
        ...
        PlayerInfo pi = PlayerManagerTFC.getInstance()
            .getPlayerInfoFromPlayer(event.player);
        if (pi.tempSkills != null) TFC_Core.setSkillStats(event.player, pi.tempSkills);

        // Load the item in the back slot if keepInventory was set to true.
        if (pi.tempEquipment != null && event.player.worldObj.getGameRules()
    ...
    public void onPlayerTossEvent(ItemTossEvent event) {
        if (event.entityItem == null) event.setCanceled(true);
    }
}
