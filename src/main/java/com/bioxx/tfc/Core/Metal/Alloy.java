...
public class Alloy {
    ...
    public Alloy matches(List<AlloyMetal> a) {
        ...
        else return new Alloy(this.outputType, amount);
    }
    ...
    public float getPercentForMetal(Metal m) {
        ...
        while (iter.hasNext()) {
            ...
            if (amc.metalType == m) return amc.metal;
        }
        return 0;
    }
    ...
    public void toPacket(DataOutputStream dos) {
        try {
            if (outputType != null) dos.writeUTF(outputType.name);
            else dos.writeUTF("Unknown");
    ...
    public Alloy fromPacket(DataInputStream dis) {
        try {
            ...
            for (int i = 0; i < size; i++) {
                AlloyMetal am = new AlloyMetal(
                    MetalRegistry.instance.getMetalFromString(dis.readUTF()),
                    dis.readFloat());
    ...
    public Alloy fromNBT(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < nbtlist.tagCount(); i++) {
            ...
            AlloyMetal am = new AlloyMetal(
                MetalRegistry.instance.getMetalFromString(nbt1.getString("metalType")),
