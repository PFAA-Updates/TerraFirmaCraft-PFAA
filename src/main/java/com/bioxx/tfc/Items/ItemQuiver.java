...
public class ItemQuiver extends ItemTerra implements IEquipable {
    ...
    public int getQuiverArrows(ItemStack item) {
        ...
        for (ItemStack i : inventory) if (i != null && i.getItem() instanceof ItemArrow) n += i.stackSize;
    ...
    public int getQuiverJavelins(ItemStack item) {
        int n = 0;
        ItemStack[] inventory = loadInventory(item);
        for (ItemStack i : inventory) if (i != null && i.getItem() instanceof ItemJavelin) n += i.stackSize;
    ...
    public List[] getQuiverJavelinTypes(ItemStack item) {
        ...
        for (ItemStack i : inventory) {
            if (i != null && i.getItem() instanceof ItemJavelin) {
                String s = i.getItem()
                    .getItemStackDisplayName(i);
                if (!list.contains(s)) list.add(s);
                int n = list.indexOf(s);
                if (listNum.size() == n) listNum.add(0);
    ...
    public void addInformation(ItemStack is, EntityPlayer player, List arraylist, boolean flag) {
        ...
        if (TFC_Core.showShiftInformation()) {
            ...
            // arraylist.add(EnumChatFormatting.ITALIC + TFC_Core.translate("gui.Armor.Pierce") + ": " +
            // EnumChatFormatting.AQUA + ArmorType.getPiercingAR());
            // arraylist.add(EnumChatFormatting.ITALIC + TFC_Core.translate("gui.Armor.Slash") + ": " +
            // EnumChatFormatting.AQUA + ArmorType.getSlashingAR());
            // arraylist.add(EnumChatFormatting.ITALIC + TFC_Core.translate("gui.Armor.Crush") + ": " +
            // EnumChatFormatting.AQUA + ArmorType.getCrushingAR());
            // arraylist.add("");
            arraylist.add(EnumChatFormatting.WHITE + TFC_Core.translate("gui.Advanced") + ":");
            arraylist.add(
                EnumChatFormatting.ITALIC + TFC_Core.translate("gui.Bow.Arrows")
                    + ": "
                    + EnumChatFormatting.YELLOW
                    + getQuiverArrows(is));
            arraylist.add(
                EnumChatFormatting.ITALIC + TFC_Core.translate("gui.Bow.Javelins")
                    + ": "
                    + EnumChatFormatting.YELLOW
                    + getQuiverJavelins(is));
            ...
            if (is.hasTagCompound()) {
                ...
                if (stackTagCompound.hasKey("creator")) arraylist.add(
                    EnumChatFormatting.ITALIC + TFC_Core.translate("gui.Armor.ForgedBy")
                        + " "
                        + stackTagCompound.getString("creator"));
            }
        } else arraylist.add(
            EnumChatFormatting.DARK_GRAY + TFC_Core.translate("gui.Advanced")
                + ": ("
                + TFC_Core.translate("gui.Hold")
                + " "
                + EnumChatFormatting.GRAY
                + TFC_Core.translate("gui.Shift")
    ...
    public ItemStack consumeAmmo(ItemStack quiver, EnumAmmo ammoType, boolean shouldConsume) {
        ...
        for (int i = 0; i < inventory.length; i++) {
            if (inventory[i] != null && inventory[i].getItem() instanceof IQuiverAmmo
                && ((IQuiverAmmo) inventory[i].getItem()).getAmmoType() == ammoType) {
                ItemStack out = inventory[i].copy();
                out.stackSize = 1;
                if (shouldConsume) inventory[i].stackSize--;
    ...
    public ItemStack[] loadInventory(ItemStack quiver) {
        ...
        if (nbt != null && nbt.hasKey("Items")) {
            ...
            for (int i = 0; i < nbttaglist.tagCount(); i++) {
                ...
                if (byte0 >= 0 && byte0 < 8) inventory[byte0] = ItemStack.loadItemStackFromNBT(nbttagcompound1);
            }
        }
        return inventory;
    }
    ...
    public void saveInventory(ItemStack quiver, ItemStack[] inventory) {
        ...
        if (quiver != null) {
            if (!quiver.hasTagCompound()) quiver.setTagCompound(new NBTTagCompound());
            quiver.getTagCompound()
