...
public class ItemIngot extends ItemTerra implements ISmeltable {
    ...
    public boolean isPlaceable(ItemStack is) {
        ...
        return !(id == TFCItems.weakSteelIngot || id == TFCItems.highCarbonSteelIngot
            || id == TFCItems.highCarbonBlackSteelIngot
            || id == TFCItems.weakRedSteelIngot
            || id == TFCItems.weakBlueSteelIngot
            || id == TFCItems.highCarbonRedSteelIngot
            || id == TFCItems.highCarbonBlueSteelIngot);
    }
    ...
    public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z, int side,
        ...
        if (entityplayer.isSneaking() && stackTagCompound == null
            && itemstack.getItem()
                .getUnlocalizedName()
                .indexOf("Double") == -1
            ...
            if (!world.isRemote && entityplayer.isSneaking()
                && (world.getBlock(x, y, z) != TFCBlocks.ingotPile || side != 1 && side != 0)) {
                ...
                // TileEntityIngotPile te2 = (TileEntityIngotPile)Minecraft.getMinecraft().theWorld.getTileEntity(x, y,
                // z);
                if (te != null) {
                    te.getBlockType()
                        .onBlockActivated(world, x, y, z, entityplayer, side, hitX, hitY, hitZ);
                    if (te.storage[0] != null && te.contentsMatch(0, itemstack) && te.storage[0].stackSize < 64) {
                        ...
                        if (te.storage[0] != null && !te.contentsMatch(0, itemstack) && te.storage[0].stackSize < 64) {
                            ...
                            if (createPile(itemstack, entityplayer, world, x, y, z, side, dir)) {
                                 ...
                                 * {
                                 * //((TileEntityIngotPile)world.getTileEntity(x,y,z)).setType(MetalRegistry.instance.
                                 * getMetalFromItem(this).Name);
                                 * }
                                 */
                                world.addBlockEvent(x, y, z, TFCBlocks.ingotPile, 0, 0);
                                te.getBlockType()
    ...
    public int getItemStackLimit(ItemStack is) {
        ...
        if (is.hasTagCompound()) {
            ...
            if (TFC_ItemHeat.hasTemp(is) || tag.hasKey(TEAnvil.ITEM_CRAFTING_VALUE_TAG)
                || tag.hasKey(TEAnvil.ITEM_CRAFTING_RULE_1_TAG)) {
                return 1;
            }
        }
