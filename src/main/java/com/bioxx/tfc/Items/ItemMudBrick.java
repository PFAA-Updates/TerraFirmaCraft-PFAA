...
public class ItemMudBrick extends ItemTerra {
    ...
    public void getSubItems(Item par1, CreativeTabs par2CreativeTabs, List list) {
        for (int i = 0; i < Global.STONE_ALL.length; i++) list.add(new ItemStack(this, 1, i));
        for (int i = 0; i < Global.STONE_ALL.length; i++) list.add(new ItemStack(this, 1, i + 32));
    }
    ...
    public String getUnlocalizedName(ItemStack itemstack) {
        if ((itemstack.getItemDamage() & 32) > 0) return "item.Wet Mud Brick";
    ...
    public int getColorFromItemStack(ItemStack is, int pass) {
        ...
        if (is.getItemDamage() < 32) color -= 0x222222;
        return color;
    }
    ...
    public void addInformation(ItemStack is, EntityPlayer player, List arraylist, boolean flag) {
        ...
        else arraylist.add(EnumChatFormatting.DARK_RED + "Unknown: " + is.getItemDamage());
    }
}
