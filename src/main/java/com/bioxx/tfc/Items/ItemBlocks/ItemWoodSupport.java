...
public class ItemWoodSupport extends ItemTerraBlock {
    ...
    public boolean isValidUnder(World world, int x, int y, int z, int side) {
        ...
        return b == TFCBlocks.woodSupportV || b == TFCBlocks.woodSupportV2
            || b.isSideSolid(world, x, y - 1, z, ForgeDirection.UP);
    }
    ...
    public boolean onItemUse(ItemStack itemstack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (y < 250 && y > 0 && side == 1 && isValidUnder(world, x, y, z, side)) {
            if (!player.isSneaking() && world.isAirBlock(x, y + 1, z)
                && world.isAirBlock(x, y + 2, z)
                && world.isAirBlock(x, y + 3, z)
                && itemstack.stackSize >= 3
                && world.getBlock(x, y, z) != TFCBlocks.woodSupportV
                && world.getBlock(x, y, z) != TFCBlocks.woodSupportV2) {
            ...
            for (dist = 1; dist <= 20 && !shouldGen; dist++) {
                if (!world.getBlock(x, y - dist, z)
                    .isReplaceable(world, x, y - dist, z)) {
                    ...
                    if (world.getBlock(x, y - dist, z)
                        .isSideSolid(world, x, y - dist, z, ForgeDirection.UP)) {
                        shouldGen = true;
                        break;
                    } else {
                        break;
            ...
            if (itemstack.stackSize >= dist - 1) for (int j = dist - 1; j >= 1 && shouldGen; j--) {
                if (world.getBlock(x, y - j, z)
                    .isReplaceable(world, x, y - j, z)) {
            ...
            if (getBlock() == TFCBlocks.woodSupportV2) b = TFCBlocks.woodSupportH2;
            ...
            if (y == 255 && b.getMaterial()
                ...
                if (itemstack.stackSize < total) return false;
                int i1 = this.getMetadata(itemstack.getItemDamage());
                int count = 0;
                while (true) {
                    int j1 = b.onBlockPlaced(
                        world,
                        x + (dir.offsetX * count),
                        y,
                        z + (dir.offsetZ * count),
                    ...
                    if (placeBlockAt(
                        ...
                        world.playSoundEffect(
                            (float) x + dir.offsetX * count + 0.5F,
                            y + 0.5F,
                            (float) z + dir.offsetZ * count + 0.5F,
                            b.stepSound.func_150496_b(),
                            (b.stepSound.getVolume() + 1.0F) / 2.0F,
                            b.stepSound.getPitch() * 0.8F);
                        --itemstack.stackSize;
                    }
                    count++;
    ...
    public boolean placeBlockAt(Block b, ItemStack is, EntityPlayer player, World world, int x, int y, int z,
