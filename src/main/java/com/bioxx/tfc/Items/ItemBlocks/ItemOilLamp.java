...
public class ItemOilLamp extends ItemTerraBlock implements ISmeltable, IFluidContainerItem {
    ...
    public boolean onItemUse(ItemStack is, EntityPlayer player, World world, int x, int y, int z, int side, float hitX,
        ...
        if (mop != null && is.getItemDamage() == 5
            && world.getBlock(mop.blockX, mop.blockY, mop.blockZ) == TFCBlocks.lavaStationary) {
    ...
    public void addInformation(ItemStack is, EntityPlayer player, List arraylist, boolean flag) {
        ...
        if (is.hasTagCompound()) {
            ...
            if (fs != null && fs.getFluid() == TFCFluids.OLIVEOIL) arraylist.add(
                ((fs.amount) * TFCOptions.oilLampFuelMult) + " "
                    + TFC_Core.translate("gui.hoursRemaining")
                    + " ("
                    + Helper.roundNumber((fs.amount / 250f) * 100f, 10)
                    + "%)");
