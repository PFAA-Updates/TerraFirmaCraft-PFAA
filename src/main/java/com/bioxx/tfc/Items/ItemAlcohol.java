...
public class ItemAlcohol extends ItemTerra {
    ...
    public int getColorFromItemStack(ItemStack is, int pass) {
        return pass == 1 ? FluidContainerRegistry.getFluidForFilledItem(is)
            .getFluid()
            .getColor() : super.getColorFromItemStack(is, pass);
    }
    ...
    public ItemStack onEaten(ItemStack is, World world, EntityPlayer player) {
                    ...
                    if (fs.soberTime > TFC_Time.getTotalTicks() + 7000 + levelMod) {
                        ...
                        if (fs.soberTime > TFC_Time.getTotalTicks() + 10000 + levelMod
                            ...
                            player.attackEntityFrom(
                                new DamageSource("alcohol").setDamageBypassesArmor()
                                    .setDamageIsAbsolute(),
                                player.getMaxHealth());
                        }
                    }
        ...
        if (!player.capabilities.isCreativeMode
            ...
            // If we couldn't fit the empty bottle in the inventory, and we drank the last alcohol bottle, put the empty
            // bottle in the empty held slot
            if (is.stackSize == 0) return new ItemStack(TFCItems.glassBottle);
            // If we couldn't fit the empty bottle in the inventory, and there is more alcohol left in the stack, drop
            // the bottle on the ground
            else player.dropPlayerItemWithRandomChoice(new ItemStack(TFCItems.glassBottle), false);
        }
