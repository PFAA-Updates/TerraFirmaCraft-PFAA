...
public class ItemJavelin extends ItemTerraTool implements ICausesDamage, IProjectile, IQuiverAmmo {
    ...
    private int getInventorySlotContainJavelin(EntityPlayer player) {
        for (int j = 0; j < player.inventory.mainInventory.length; ++j) {
            if (player.inventory.mainInventory[j] != null
                && player.inventory.mainInventory[j].getItem() instanceof ItemJavelin) {
                return j;
            }
        }
    ...
    public float getRangedDamage(ItemStack is) {
        if (is != null) return weaponRangeDamage + (weaponRangeDamage * AnvilManager.getDamageBuff(is));
        else return weaponRangeDamage;
    }
