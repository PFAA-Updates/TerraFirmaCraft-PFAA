...
public class ItemCustomBucketMilk extends ItemTerra implements IFood {
    ...
    public void addInformation(ItemStack is, EntityPlayer player, List arraylist, boolean flag) {
        ...
        if (decay > 0) {
            ...
            if (perc > 50) s = EnumChatFormatting.DARK_GRAY + TFC_Core.translate("gui.milk.old");
            if (perc > 70) s = EnumChatFormatting.DARK_GRAY + TFC_Core.translate("gui.milk.sour");

            arraylist.add(s);
        }
        if (TFCOptions.enableDebugMode) arraylist.add(EnumChatFormatting.DARK_GRAY + "Decay: " + decay);
    }
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer entity) {
        ...
        if (mop == null) {
            if (fs.needDrink() && fs.needFood()) entity.setItemInUse(is, this.getMaxItemUseDuration(is));
        }
        return is;
    }
