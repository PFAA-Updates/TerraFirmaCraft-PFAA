...
public class ItemCustomBucket extends ItemTerra {
        ...
        if (mop == null) {
            ...
            if (mop.typeOfHit == MovingObjectType.BLOCK) {
                ...
                if (isEmpty) {
                    ...
                    if (MinecraftForge.EVENT_BUS.post(event) || event.isCanceled()) return is;

                    if (event.getResult() == Event.Result.ALLOW) return event.result;

                    if (TFC_Core.isFreshWater(world.getBlock(x, y, z))) {
                        world.setBlockToAir(x, y, z);
                        if (player.capabilities.isCreativeMode) return is;
            ...
            } else if (this.bucketContents == Blocks.air && mop.entityHit instanceof EntityCowTFC
                && ((EntityCowTFC) mop.entityHit).getGender() == GenderEnum.FEMALE) {
    ...
    public boolean onItemUse(ItemStack is, EntityPlayer player, World world, int x, int y, int z, int side, float hitX,
        ...
        if (!isEmpty && world.getBlock(x, y, z) != Blocks.cauldron
            && world.isAirBlock(x + map[side][0], y + map[side][1], z + map[side][2])) {
