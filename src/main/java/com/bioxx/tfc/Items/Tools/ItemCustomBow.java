...
public class ItemCustomBow extends ItemBow implements ISize {
    ...
    public boolean consumeArrowInQuiver(EntityPlayer player, boolean shouldConsume) {
         ...
         * for(int i = 0; i < 9; i++)
         * {
         * if(player.inventory.getStackInSlot(i) != null && player.inventory.getStackInSlot(i).getItem() instanceof
         * ItemQuiver)
         * {
        ...
        if (quiver != null && quiver.getItem() instanceof ItemQuiver)
            if (((ItemQuiver) quiver.getItem()).consumeAmmo(quiver, EnumAmmo.ARROW, shouldConsume) != null) return true;
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer player) {
        ...
        if (player.capabilities.isCreativeMode || player.inventory.hasItem(TFCItems.arrow)
            || consumeArrowInQuiver(player, false)) player.setItemInUse(is, this.getMaxItemUseDuration(is));
    ...
    public void onPlayerStoppedUsing(ItemStack is, World world, EntityPlayer player, int inUseCount) {
        ...
        // If there was no ammo in the inventory then we need to check if there is a quiver and if there is ammo inside
        // of it.
        if (!hasAmmo) hasAmmoInQuiver = consumeArrowInQuiver(player, false);

        if (hasAmmo || hasAmmoInQuiver) {
            float forceMult = j / getUseSpeed(player);
            // f = (f * f + f * 2.0F) / 3.0F;

            if (forceMult < 0.25D) return;

            if (forceMult > 1.25F) forceMult = 1.25F;

            EntityProjectileTFC entityarrow = new EntityProjectileTFC(world, player, forceMult * 2.0F);
            entityarrow.setDamage(forceMult * 150.0);
            if (forceMult == 1.25F) entityarrow.setIsCritical(true);
            ...
            world.playSoundAtEntity(
                ...
                "random.bow",
                1.0F,
                1.0F / (itemRand.nextFloat() * 0.4F + 1.2F) + forceMult * 0.5F);

            if (flag) entityarrow.canBePickedUp = 2;
            else if (hasAmmo) player.inventory.consumeInventoryItem(TFCItems.arrow);
            else if (hasAmmoInQuiver) consumeArrowInQuiver(player, true);
    ...
    public void registerIcons(IIconRegister par1IconRegister) {
        ...
        for (int i = 0; i < iconArray.length; ++i) iconArray[i] = par1IconRegister
            .registerIcon(Reference.MOD_ID + ":" + this.getIconString() + "_" + bowPullIconNameArray[i]);
