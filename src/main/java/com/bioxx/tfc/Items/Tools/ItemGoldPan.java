...
public class ItemGoldPan extends ItemTerra {
    ...
    public IIcon getIconFromDamage(int i) {
        i = i & 15;
        if (i == 1) return icons[1];
        if (i == 2) return icons[2];
                ...
                if (is.getItemDamage() == 0) {
                    ...
                    if (cd == null) {
                        TFC_Core.sendInfoMessage(
                            player,
                            new ChatComponentText("The ChunkData returned null, please report this to the developer."));
                        return is;
                    }
                    ...
                    if (cd.sluicedAmount < TFCOptions.goldPanLimit || !TFCOptions.enableOverworkingChunks) {
                        if (TFC_Core.isGravel(blockHit)) {
                            ...
                            if (world.rand.nextInt(10) == 0) world.setBlockToAir(x, y, z);
                    ...
                    if (world.getBlock(x, y + 1, z)
                        .getMaterial() == Material.water && bMeta > 0
                        && mop.sideHit == 1) {
                        ...
                        if (uses > 0) {
                            ...
                            uses--;
                            if (uses > 0) is.setItemDamage((is.getItemDamage() & 15) + (uses << 4)); // NOPMD
                            else {
