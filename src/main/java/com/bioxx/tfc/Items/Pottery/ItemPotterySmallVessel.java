...
public class ItemPotterySmallVessel extends ItemPotteryBase implements IBag {
    ...
    private int mergeMetals(Metal mt0, Metal mt1, int m0, int m1) {
        if (mt0 != null && mt1 != null && m0 > 0) {
            if (mt0.name.equals(mt1.name)) return m0 + m1;
        }
        return m0;
    }
    ...
    public int getColor(ItemStack is) {
        if (!is.hasTagCompound() || !is.getTagCompound()
            .hasKey("color")) return -1;
        else return is.getTagCompound()
            .getInteger("color");
    }
    ...
    public ItemStack[] loadBagInventory(ItemStack is) {
        ...
        if (is != null && is.hasTagCompound()
            && is.getTagCompound()
                .hasKey("Items")) {
            NBTTagList nbttaglist = is.getTagCompound()
                .getTagList("Items", 10);
            for (int i = 0; i < nbttaglist.tagCount(); i++) {
                NBTTagCompound nbttagcompound1 = nbttaglist.getCompoundTagAt(i);
                byte byte0 = nbttagcompound1.getByte("Slot");
                if (byte0 >= 0 && byte0 < 4) bag[byte0] = ItemStack.loadItemStackFromNBT(nbttagcompound1);
            }
        } else return null;
    ...
    public boolean onUpdate(ItemStack is, World world, int x, int y, int z) {
        ...
        if (bag != null) {
            ...
            for (ItemStack i : bag) {
                if (i != null && i.stackSize == 0) i = null;
            }
            saveContents(is, bag);
        }
        return true;
    ...
    public void saveContents(ItemStack vessel, ItemStack[] bag) {
        ...
        if (vessel != null) {
            if (!vessel.hasTagCompound()) vessel.setTagCompound(new NBTTagCompound());
            vessel.getTagCompound()
        ...
        if (!entityplayer.isSneaking()) {
            if (itemstack.getItemDamage() == 2) {
                ...
                if (nbt == null) {
                    ...
                    if (TFC_Time.getTotalHours() - temp < 11) entityplayer.openGui(
    ...
    public void addItemInformation(ItemStack is, EntityPlayer player, List<String> arraylist) {
        ...
        if (tag != null) {
            ...
            if (tag.hasKey("TempTimer")) {
                ...
                else arraylist.add(EnumChatFormatting.WHITE + TFC_Core.translate("gui.ItemHeat.Solidified"));
            }
                    ...
                    if (byte0 >= 0 && byte0 < 4) {
                        ...
                        if (itemstack.stackSize > 0) {
                            if (itemstack.getItem() instanceof ItemFoodTFC) {
                                ...
                                String ds = " " + EnumChatFormatting.DARK_GRAY
                                    + Helper.roundNumber(decay / weight * 100, 10)
                                    + "%";
                                if (decay <= 0) ds = "";

                                arraylist.add(
                                    EnumChatFormatting.GOLD.toString() + itemstack.getItem()
                                ...
                                EnumChatFormatting.GOLD.toString() + itemstack.stackSize
                                    + "x "
                                    + itemstack.getItem()
                                        .getItemStackDisplayName(itemstack));
                        }
    ...
    public void addExtraInformation(ItemStack is, EntityPlayer player, List<String> arraylist) {
        if (TFC_Core.showShiftInformation()) {
            ...
            if (tag != null && tag.hasKey("TempTimer")) {
                long total = TFC_Time.getTotalHours();
                long temp = tag.getLong("TempTimer");
                if (total - temp < 11) arraylist.add(TFC_Core.translate("gui.PotteryVesselSmall.Inst0"));
            } else arraylist.add(TFC_Core.translate("gui.PotteryVesselSmall.Inst0"));
        } else {
