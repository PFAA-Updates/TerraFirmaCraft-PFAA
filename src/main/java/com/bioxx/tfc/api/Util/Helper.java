...
public class Helper {
    ...
    public static double getReachDistance(World par1World, EntityLivingBase entity, boolean par3) {
        ...
        if (entity.getHeldItem() != null && (entity.getHeldItem()
            .getItem()) instanceof ISize) {
            var21 *= ((ISize) (entity.getHeldItem()
                .getItem())).getReach(null).multiplier;
        }
    ...
    public static MovingObjectPosition getMovingObjectPositionFromPlayer(World world, EntityLivingBase entity,
        boolean scanFluids) {
        return getMovingObjectPositionFromPlayer(world, entity, scanFluids, 4);
    }

    public static MovingObjectPosition getMovingObjectPositionFromPlayer(World world, EntityLivingBase entity,
        boolean scanFluids, int reach) {
        float var4 = 1.0F;
    ...
    public static int getInteger(Object obj, String srgName, String obfName, String deobfName, boolean useDeobf) {
        Field f = null;
        try {
            if (!useDeobf) f = obj.getClass()
                .getDeclaredField(deobfName);
            else if (usesSRG(obj, srgName)) f = obj.getClass()
                .getDeclaredField(srgName);
            else f = obj.getClass()
                .getDeclaredField(obfName);
