...
public class TFC_ItemHeat {
    ...
    public static String getHeatColor(float temp, float meltTemp) {
        ...
        } else if (temp >= 1500) phrase = "\247f" + TFC_Core.translate("gui.ItemHeat.BrilliantWhite");

        if (temp > meltTemp) phrase = phrase + "\247f - " + TFC_Core.translate("gui.ItemHeat.Liquid");
    ...
    public static String getHeatColorFood(float temp, float meltTemp) {
        if (temp < meltTemp) {
            if (temp < meltTemp * 0.1F) return TFC_Core.translate("gui.FoodHeat.Cold");
            ...
            else return "\2474" + TFC_Core.translate("gui.ItemHeat.VeryHot");
        }
    ...
    public static String getHeatColorTorch(float temp, float meltTemp) {
        if (temp < meltTemp) {
            if (temp > 0 && temp < meltTemp * 0.8F) return TFC_Core.translate("gui.Torch.CatchingFire");
            else if (temp >= meltTemp * 0.8F) return "\2474" + TFC_Core.translate("gui.Torch.Lit");
        }
        return TFC_Core.translate("gui.ClearSlot");
    }
    ...
    public static float getSpecificHeat(ItemStack is) {
        ...
        if (is != null && manager != null) {
            HeatIndex hi = manager.findMatchingIndex(is);
            if (hi != null) return hi.specificHeat;
            else return 1;
        } else return 1;
    }
    ...
    public static boolean hasTemp(ItemStack is) {
        if (is != null) {
            if (is.hasTagCompound() && is.getTagCompound()
                .hasKey("temperature")) return true;
        }
        return false;
    }
        ...
        if (is != null) {
            if (is.hasTagCompound()) {
                ...
                if (hasTemp(is)) {
                    ...
                    if (temp <= 0) comp.removeTag("temperature");
                    if (comp.hasNoTags()) is.stackTagCompound = null;
                }
            }
        }
    }
    ...
    public static void removeTempTag(ItemStack is) {
        if (is.hasTagCompound() && is.getTagCompound()
            .hasKey("temperature")) {
            is.getTagCompound()
                .removeTag("temperature");
        }
        if (is.hasTagCompound() && is.getTagCompound()
            .hasNoTags()) is.stackTagCompound = null;
    }
}
