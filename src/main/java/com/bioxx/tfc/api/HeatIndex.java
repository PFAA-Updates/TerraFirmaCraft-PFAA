...
public class HeatIndex {
    ...
    public HeatIndex(ItemStack in, com.bioxx.tfc.api.HeatRaw raw) {
        input = in;
        specificHeat = raw.specificHeat;
        meltTemp = raw.meltTemp;
    }

    public HeatIndex(ItemStack in, com.bioxx.tfc.api.HeatRaw raw, ItemStack out) {
        this(in, raw);
        output = out;
    }
    ...
    public Item getOutputItem() {
        if (output != null) return output.getItem();
        else return null;
    }

    public int getOutputDamage() {
        if (output != null) return output.getItemDamage();
        else return 0;
    }
    ...
    public ItemStack getOutput(Random r) {
        if (getOutputItem() == null) return null;
        int rand = 0;
    ...
    public ItemStack getOutput(ItemStack in, Random r) {
        ...
        if (is != null && this.keepNBT) {
            if (is.hasTagCompound()) {
                ...
                for (Object o : in.getTagCompound()
                    .func_150296_c()) {
                    NBTBase n = (NBTBase) o;
                ...
                if (com.bioxx.tfc.api.TFC_ItemHeat.hasTemp(is))
                    com.bioxx.tfc.api.TFC_ItemHeat.setTemp(is, com.bioxx.tfc.api.TFC_ItemHeat.getTemp(is) * 0.9f);
            }
        }
        return is;
    }
    ...
    public boolean matches(ItemStack is) {
        if (is != null) {
            boolean b = is.getItem()
                .getHasSubtypes();
            if (is.getItem() != input.getItem()) return false;
            else if (b && input.getItemDamage() != 32767 && is.getItemDamage() != input.getItemDamage()) return false;
        } else return false;
        return true;
    }
}
