...
public class BarrelRecipe {
    ...
    public Boolean matches(ItemStack item, FluidStack fluid) {
        boolean iStack = removesLiquid ? true
            : recipeIS != null && item != null
                && fluid != null
                && recipeFluid != null
                && item.stackSize >= (int) Math.ceil(fluid.amount / recipeFluid.amount);
        boolean fStack = !removesLiquid ? true
            : recipeFluid != null && item != null
                && fluid != null
                && recipeOutFluid != null
                && fluid.amount >= item.stackSize * recipeOutFluid.amount;
    ...
    public String getRecipeName() {
        ...
        if (this.recipeOutIS != null) {
            if (recipeOutIS.stackSize > 1) s += recipeOutIS.stackSize + "x ";
            s += recipeOutIS.getDisplayName();
        }
        if (recipeOutFluid != null && !this.recipeFluid.isFluidEqual(recipeOutFluid)) s = recipeOutFluid.getFluid()
            .getLocalizedName(recipeOutFluid);
        return s;
    }
    ...
    public Stack<ItemStack> getResult(ItemStack inIS, FluidStack inFS, int sealedTime) {
        ...
        if (recipeOutIS != null) {
            ...
            if (remainder > 0) {
                stackList.push(new ItemStack(item, remainder, damage)); // Push this on first, so it doesn't end up in
                                                                        // the input slot.
    ...
    public FluidStack getResultFluid(ItemStack inIS, FluidStack inFS, int sealedTime) {
        if (recipeOutFluid != null) {
            FluidStack fs = null;
            // The FluidStack .copy() method does not make a copy of the NBT tag, which may have been the cause of the
            // quantum entanglement
            if (recipeOutFluid.tag != null) {
                fs = new FluidStack(
                    recipeOutFluid.getFluid(),
                    recipeOutFluid.amount,
                    (NBTTagCompound) recipeOutFluid.tag.copy());
            } else {
