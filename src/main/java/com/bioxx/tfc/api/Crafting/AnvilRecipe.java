...
public class AnvilRecipe {
    ...
    public AnvilRecipe(ItemStack in, ItemStack in2, String p, boolean flux, AnvilReq req, ItemStack result) {
        ...
        this.craftingValue = 70 + new Random(
            TFC_Core.getSuperSeed(AnvilManager.world) + (in != null ? Item.getIdFromItem(in.getItem()) : 0)
                + (result != null ? Item.getIdFromItem(result.getItem()) : 0)).nextInt(craftingBoundDefault);
    }
    ...
    public AnvilRecipe setCraftingBound(int max) {
        craftingValue = 70 + new Random(
            TFC_Core.getSuperSeed(AnvilManager.world) + (input1 != null ? Item.getIdFromItem(input1.getItem()) : 0)
                + (result != null ? Item.getIdFromItem(result.getItem()) : 0)).nextInt(max);
        return this;
    }
    ...
    public boolean matches(AnvilRecipe recipe) {
        if (areItemStacksEqual(input1, recipe.input1) && areItemStacksEqual(input2, recipe.input2)
    ...
    public boolean isComplete(AnvilManager am, AnvilRecipe recipe, int[] rules) {
        ...
        if (areItemStacksEqual(input1, recipe.input1) && areItemStacksEqual(input2, recipe.input2)
            && plan.equals(recipe.plan)
            && pr.rules[0].matches(rules, 0)
            && pr.rules[1].matches(rules, 1)
            && pr.rules[2].matches(rules, 2)
            && craftingValue == recipe.craftingValue
            && AnvilReq.matches(anvilreq, recipe.anvilreq)) if (this.flux && recipe.flux) return true;
        else if (!this.flux) return true;
        return false;
    }
    ...
    private boolean areItemStacksEqual(ItemStack is1, ItemStack is2) {
        if (is1 != null && is2 != null) {
            ...
            if (is1.getItemDamage() != 32767 && is1.getItemDamage() != is2.getItemDamage()) return false;
    ...
    public ItemStack getCraftingResult(ItemStack input) {
        ...
        if (this.inheritsDamage) is.setItemDamage(input.getItemDamage());
        return is;
    }
    ...
    public float getSkillMult(EntityPlayer p) {
        ...
        for (String s : skillsList) {
            total++;
            skill += TFC_Core.getSkillStats(p)
                .getSkillMultiplier(s);
        }
