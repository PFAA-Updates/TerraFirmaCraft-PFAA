...
public class Food {
    ...
    private static NBTTagCompound getProcTag(ItemStack is) {
        if (is.hasTagCompound() && is.getTagCompound()
            .hasKey(PROCESSING_TAG)) {
            return (NBTTagCompound) is.getTagCompound()
                .getTag(PROCESSING_TAG);
    ...
    public static boolean areEqual(ItemStack is1, ItemStack is2) {
        return isBrined(is1) == isBrined(is2) && isPickled(is1) == isPickled(is2)
    ...
    public static float getCooked(ItemStack is) {
        NBTTagCompound nbt = getProcTag(is);
        if (nbt.hasKey(COOKED_TAG)) return nbt.getFloat(COOKED_TAG);
        else return 0.0F;
    }
    ...
    public static int[] getCookedProfile(ItemStack is) {
        NBTTagCompound nbt = getProcTag(is);
        if (nbt.hasKey(COOKED_PROFILE_TAG)) return nbt.getIntArray(COOKED_PROFILE_TAG);
    ...
    public static int[] getFuelProfile(ItemStack is) {
        NBTTagCompound nbt = getProcTag(is);
        if (nbt.hasKey(FUEL_PROFILE_TAG)) return nbt.getIntArray(FUEL_PROFILE_TAG);
        else return new int[] { 0, 0, 0, 0, 0 };
    }
    ...
    public static void setDecay(ItemStack is, float value) {
        ...
        if (value > getWeight(is)) is.stackSize = 0;
    }

    public static float getDecay(ItemStack is) {
        NBTTagCompound nbt = getNBT(is);
        if (nbt.hasKey(DECAY_TAG)) return nbt.getFloat(DECAY_TAG);
        else return 0.0F;
    }
    ...
    public static int getDecayTimer(ItemStack is) {
        NBTTagCompound nbt = getNBT(is);
        if (nbt.hasKey(DECAY_TIMER_TAG)) return nbt.getInteger(DECAY_TIMER_TAG);
        else return (int) TFC_Time.getTotalHours();
    }

    public static void setWeight(ItemStack is, float value) {
        NBTTagCompound nbt = getNBT(is);
        nbt.setFloat(WEIGHT_TAG, Helper.roundNumber(value, 100));
        if (getDecay(is) > value || value <= 0) is.stackSize = 0;
    }

    public static float getWeight(ItemStack is) {
        NBTTagCompound nbt = getNBT(is);
        if (nbt.hasKey(WEIGHT_TAG)) return nbt.getFloat(WEIGHT_TAG);
        else return 0.0F;
    }
    ...
    public static short getDried(ItemStack is) {
        NBTTagCompound nbt = getProcTag(is);
        if (nbt.hasKey(DRIED_TAG)) return nbt.getShort(DRIED_TAG);
        else return 0;
    }
    ...
    public static short getSmokeCounter(ItemStack is) {
        NBTTagCompound nbt = getProcTag(is);
        if (nbt.hasKey(SMOKE_COUNTER_TAG)) return nbt.getShort(SMOKE_COUNTER_TAG);
        else return 0;
    }
    ...
    public static int getSweetMod(ItemStack is) {
        NBTTagCompound nbt = getNBT(is);
        if (nbt.hasKey(SWEET_MOD_TAG)) return nbt.getInteger(SWEET_MOD_TAG);
        else return 0;
    }
    ...
    public static int getSourMod(ItemStack is) {
        NBTTagCompound nbt = getNBT(is);
        if (nbt.hasKey(SOUR_MOD_TAG)) return nbt.getInteger(SOUR_MOD_TAG);
        else return 0;
    }
    ...
    public static int getSaltyMod(ItemStack is) {
        NBTTagCompound nbt = getNBT(is);
        if (nbt.hasKey(SALTY_MOD_TAG)) return nbt.getInteger(SALTY_MOD_TAG);
        else return 0;
    }
    ...
    public static int getBitterMod(ItemStack is) {
        NBTTagCompound nbt = getNBT(is);
        if (nbt.hasKey(BITTER_MOD_TAG)) return nbt.getInteger(BITTER_MOD_TAG);
        else return 0;
    }
    ...
    public static int getSavoryMod(ItemStack is) {
        NBTTagCompound nbt = getNBT(is);
        if (nbt.hasKey(UMAMI_MOD_TAG)) return nbt.getInteger(UMAMI_MOD_TAG);
        else return 0;
    }
    ...
    public static int getMealSkill(ItemStack is) {
        NBTTagCompound nbt = getNBT(is);
        if (nbt.hasKey(MEAL_SKILL_TAG)) return nbt.getInteger(MEAL_SKILL_TAG);
        else return 0;
    }
    ...
    public static String getInfusion(ItemStack is) {
        NBTTagCompound nbt = getNBT(is);
        if (nbt.hasKey(INFUSION_TAG)) return nbt.getString(INFUSION_TAG);
        else return null;
    }
    ...
    public static int[] getFoodGroups(ItemStack is) {
        NBTTagCompound nbt = getNBT(is);
        if (nbt.hasKey(FOOD_GROUP_TAG)) return nbt.getIntArray(FOOD_GROUP_TAG);
    ...
    public static float getDecayRate(ItemStack is) {
        NBTTagCompound nbt = getNBT(is);
        if (nbt.hasKey(DECAY_RATE_TAG)) return nbt.getFloat(DECAY_RATE_TAG);
        else return 1.0F;
    }
}
