...
public class ChiselMode {
    ...
    public boolean isChiselable(Block block) {
        boolean isChiselable = block == TFCBlocks.planks || block instanceof BlockCobble
    ...
    public int hasChisel(EntityPlayer player) {
        ...
        if (player.inventory.mainInventory[player.inventory.currentItem] != null
            && player.inventory.mainInventory[player.inventory.currentItem].getItem() instanceof ItemChisel) {
    ...
    public static PlayerInfo playerInfo(World world, EntityPlayer player) {
        PlayerInfo pi;
        if (!world.isRemote) {
            pi = PlayerManagerTFC.getInstance()
