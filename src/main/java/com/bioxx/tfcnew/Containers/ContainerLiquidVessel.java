...
public class ContainerLiquidVessel extends ContainerTFC {
    ...
    private void layoutContainer(IInventory playerInventory) {
        ...
        for (row = 0; row < 9; ++row) {
            ...
            else this.addSlotToContainer(new Slot(playerInventory, row, 8 + row * 18, 148));
        }
    ...
    public void detectAndSendChanges() {
        ...
        if (nbt != null) {
            ...
            if (!world.isRemote && m != null && stack != null) {
                if (input != null && input.getItem() == TFCItems.ceramicMold
                    ...
                    TFC_ItemHeat.setTemp(
                        is,
                        (short) (HeatRegistry.getInstance()
                            .getMeltingPoint(is) * 1.5f));
                    ...
                    && input.stackSize == 1
                    && metalAmount > 0
                    && ("Copper".equals(m.name) || "Bronze".equals(m.name)
                        || "Bismuth Bronze".equals(m.name)
                        || "Black Bronze".equals(m.name))) {
                            int amt = -1;
                            if ("Copper".equals(m.name)) amt = 398;
                            else if ("Bronze".equals(m.name)) amt = 399;
                            else if ("Bismuth Bronze".equals(m.name)) amt = 400;
                            else if ("Black Bronze".equals(m.name)) amt = 401;
                    ...
                    if (input != null && input.getItem() instanceof ItemPotteryMold && input.getItemDamage() > 1) {
                        boolean correctMetalFlag = false;
                        if ("Copper".equals(m.name) && (input.getItemDamage() - 2) % 4 == 0) correctMetalFlag = true;
    ...
    public ItemStack transferStackInSlotTFC(EntityPlayer player, int slotNum) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotNum < 1) {
                if (!this.mergeItemStack(slotStack, 1, inventorySlots.size(), true)) return null;
