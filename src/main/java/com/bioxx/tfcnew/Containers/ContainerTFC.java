    ...
    public void detectAndSendChanges() {
        ...
        for (int i = 0; i < this.inventorySlots.size(); ++i) {
            ...
            if (!areItemStacksEqual(itemstack1, itemstack)) {
                if (doItemSaving && i < inventoryItemStacks.size() - 36 && !isLoading) shouldSave = true;
                ...
                if (shouldSave) {
                    ...
                    player.inventory
                        .setInventorySlotContents(bagsSlotNum, (ItemStack) inventoryItemStacks.get(slotNum));
                    for (int j = 0; j < this.crafters.size(); ++j) ((ICrafting) this.crafters.get(j))
                        .sendSlotContents(this, slotNum, (ItemStack) inventoryItemStacks.get(slotNum));
                }
        ...
        if (shouldReload && !isLoading) reloadContainer();
