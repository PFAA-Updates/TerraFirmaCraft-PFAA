...
public class ContainerCreativeTFC extends Container {
        ...
        for (i = 0; i < 5; ++i) {
            for (int j = 0; j < 9; ++j) {
                this.addSlotToContainer(
                    new Slot(GuiContainerCreativeTFC.getInventory(), i * 9 + j, 9 + j * 18, 18 + i * 18));
            }
        }
        ...
        for (int k = 0; k < 5; ++k) {
            for (int l = 0; l < 9; ++l) {
                ...
                if (i1 >= 0 && i1 < this.itemList.size()) {
                    GuiContainerCreativeTFC.getInventory()
                        .setInventorySlotContents(l + k * 9, (ItemStack) this.itemList.get(i1));
                } else {
                    GuiContainerCreativeTFC.getInventory()
                        .setInventorySlotContents(l + k * 9, (ItemStack) null);
                }
            }
        }
    }
    ...
    public boolean canDragIntoSlot(Slot par1Slot) {
        return par1Slot.inventory instanceof InventoryPlayer
            || par1Slot.yDisplayPosition > 90 && par1Slot.xDisplayPosition <= 162;
    }
}
