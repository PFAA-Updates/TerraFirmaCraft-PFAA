        ...
        for (var4 = 0; var4 < this.numRows; ++var4) {
            for (var5 = 0; var5 < 9; ++var5) {
                this.addSlotToContainer(
                    new SlotChest(lowerChestInventory, var5 + var4 * 9, 8 + var5 * 18, 18 + var4 * 18)
    ...
    public ItemStack transferStackInSlotTFC(EntityPlayer player, int slotNum) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotNum < chestSlotCount) {
                if (!this.mergeItemStack(slotStack, chestSlotCount, this.inventorySlots.size(), true)) return null;
            }
            // From inventory to chest
            else {
                if (!this.mergeItemStack(slotStack, 0, chestSlotCount, false)) return null;
            }
