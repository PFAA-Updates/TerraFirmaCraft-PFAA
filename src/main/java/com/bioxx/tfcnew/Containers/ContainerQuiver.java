...
public class ContainerQuiver extends ContainerTFC {
    ...
    public ContainerQuiver(InventoryPlayer playerinv, World world, int x, int y, int z) {
        ...
        if (!world.isRemote) loadBagInventory();
    ...
    protected void layoutContainer(IInventory playerInventory, int xSize, int ySize) {
        ...
        for (row = 0; row < 9; ++row) {
            ...
            else this.addSlotToContainer(new Slot(playerInventory, row, 8 + row * 18, 112));
        }
    ...
    public void loadBagInventory() {
        if (player.inventory.getStackInSlot(bagsSlotNum) != null && player.inventory.getStackInSlot(bagsSlotNum)
            .hasTagCompound()) {
            NBTTagList nbttaglist = player.inventory.getStackInSlot(bagsSlotNum)
                .getTagCompound()
                .getTagList("Items", 10);
        ...
        if (player.inventory.getStackInSlot(bagsSlotNum) != null && player.inventory.getStackInSlot(bagsSlotNum)
            ...
            if (nbttaglist != null) {
                for (int i = 0; i < nbttaglist.tagCount(); i++) {
                    NBTTagCompound nbttagcompound1 = nbttaglist.getCompoundTagAt(i);
                    byte byte0 = nbttagcompound1.getByte("Slot");
                    if (byte0 == slot) return ItemStack.loadItemStackFromNBT(nbttagcompound1);
                }
            }
        }
        return null;
    ...
    public ItemStack transferStackInSlotTFC(EntityPlayer player, int slotNum) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotNum < 8) {
                if (!this.mergeItemStack(slotStack, 8, inventorySlots.size(), true)) return null;
            } else {
                if (!this.mergeItemStack(slotStack, 0, 8, false)) return null;
            }
