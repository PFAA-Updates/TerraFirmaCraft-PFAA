...
public class ContainerGrill extends ContainerTFC {
    ...
    public ItemStack transferStackInSlotTFC(EntityPlayer player, int slotNum) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotNum < 6) {
                if (!this.mergeItemStack(slotStack, 6, this.inventorySlots.size(), true)) return null;
            } else {
                if (!this.mergeItemStack(slotStack, 0, 6, false)) return null;
            }
    ...
    public void detectAndSendChanges() {
        ...
        if (this.fire != null) firetemp = this.fire.fireTemp;
        else firetemp = 0;
    }

    @Override
    public void updateProgressBar(int par1, int par2) {
        if (this.fire != null && par1 == 0) this.fire.fireTemp = par2;
    }
}
