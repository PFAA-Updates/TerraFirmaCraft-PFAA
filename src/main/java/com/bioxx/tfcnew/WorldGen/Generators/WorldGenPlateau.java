...
public class WorldGenPlateau {
    ...
    public boolean generate(World par1World, Random rand, int x, int y, int z, int radiusStart, int radiusTop,
        int height, int centers, int radiusJitter, int taperChance) {
        // template call// (new WorldGenPlateau()).generate(/*currentWorld*/ , /*randomGenerator*/ ,/*x*/ ,/*y*/ ,/*z*/
        // ,/*radiusStart*/ ,/*radiusTop*/ ,
        ...
        // radiusJitter=2; //this makes the radius of a disk shift in or out by X amount, makes it less smooth, 1 is
        // off, 2+ is more chaotic
        // taperChance=5; // chance that the radius will be decreased this layer, height/(radiusStart-radiusTOP) will
        // give you a good starting number for this
        ...
        for (int centerRun = 0; centerRun <= centers; centerRun++) // make the seperate towers
        {
            ...
            for (int run = 0; run <= height; run++) // make the stack
            {
                ...
                if (radiusTop <= radius && rand.nextInt(taperChance) == 1) radius--;
            }
        }
        return true;
    }
