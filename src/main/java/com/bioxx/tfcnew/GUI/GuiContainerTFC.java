...
public class GuiContainerTFC extends GuiContainer {
    ...
    protected void setDrawInventory(boolean b) {
        if (!drawInventory && b) ySize += PlayerInventory.invYSize;
        else if (drawInventory && !b) ySize -= PlayerInventory.invYSize;
        drawInventory = b;
    }
    ...
    protected void drawGui(ResourceLocation rl) {
        ...
        if (drawInventory) PlayerInventory.drawInventory(this, width, height, this.getShiftedYSize());
    }
    ...
    protected void drawHoveringTextZLevel(List par1List, int par2, int par3, FontRenderer font, float z) {
        if (!par1List.isEmpty()) {
            ...
            if (par1List.size() > 1) k1 += 2 + (par1List.size() - 1) * 10;
