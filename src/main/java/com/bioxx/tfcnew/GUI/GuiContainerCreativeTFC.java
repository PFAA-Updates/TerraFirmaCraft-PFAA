...
public class GuiContainerCreativeTFC extends InventoryEffectRenderer {
    ...
    /**
     * Used to back up the ContainerCreativeTFC's inventory slots before filling it with the player's inventory slots
     * for
     * the inventory tab.
     */
        ...
        if (par1Slot == null && selectedTabIndex != CreativeTabs.tabInventory.getTabIndex() && par4 != 5) {
            ...
            if (inventoryplayer.getItemStack() != null) {
                ...
                if (par3 == 1) {
                    ...
                    if (inventoryplayer.getItemStack().stackSize == 0) inventoryplayer.setItemStack((ItemStack) null);
                }
            }
        } else {
            int l;
            ...
            if (par1Slot == this.zeroSlot && flag) {
                for (l = 0; l < this.mc.thePlayer.inventoryContainer.getInventory()
                    .size(); ++l) {
                ...
                if (selectedTabIndex == CreativeTabs.tabInventory.getTabIndex()) {
                    if (par1Slot == this.zeroSlot) {
                        ...
                        itemstack1 = par1Slot.decrStackSize(
                            par3 == 0 ? 1
                                : par1Slot.getStack()
                                    .getMaxStackSize());
                        ...
                        this.mc.playerController
                            .sendPacketDropItem/* func_78752_a */(this.mc.thePlayer.inventory.getItemStack());
                        this.mc.thePlayer.inventory.setItemStack((ItemStack) null);
                    } else {
                        this.mc.thePlayer.inventoryContainer.slotClick(
                            par1Slot == null ? par2
                                : SlotCreativeInventoryTFC.getSlot((SlotCreativeInventoryTFC) par1Slot).slotNumber,
                            par3,
                            par4,
                            this.mc.thePlayer);
                    ...
                    if (itemstack != null && itemstack2 != null
                        && itemstack.isItemEqual(itemstack2)
                        && ItemStack.areItemStackTagsEqual(itemstack, itemstack2)) // Forge: Bugfix, Compare NBT data,
                                                                                   // allow for deletion of enchanted
                                                                                   // books, MC-12770
                    {
                        if (par3 == 0) {
                            if (flag) itemstack.stackSize = itemstack.getMaxStackSize();
                            else if (itemstack.stackSize < itemstack.getMaxStackSize()) ++itemstack.stackSize;
                    ...
                    this.inventorySlots
                        .slotClick(par1Slot == null ? par2 : par1Slot.slotNumber, par3, par4, this.mc.thePlayer);

                    if (Container.func_94532_c(par3) == 2) {
                        for (l = 0; l < 9; ++l) this.mc.playerController.sendSlotPacket(
                            this.inventorySlots.getSlot(45 + l)
                                .getStack(),
                            36 + l);
                    } else if (par1Slot != null) {
                        itemstack1 = this.inventorySlots.getSlot(par1Slot.slotNumber)
                            .getStack();
    ...
    protected void keyTyped(char par1, int par2) {
        if (!CreativeTabs.creativeTabArray[selectedTabIndex].hasSearchBar()) {
            ...
            if (!this.checkHotbarKeys(par2)) {
                if (this.searchField.textboxKeyTyped(par1, par2)) this.updateCreativeSearch();
                else super.keyTyped(par1, par2);
            }
        }
    }
    ...
    protected void mouseClicked(int par1, int par2, int par3) {
        if (par3 == 0) {
            ...
            for (int k1 = 0; k1 < j1; ++k1) {
                ...
                if (this.switchTab(creativetabs, l, i1)) return;
            }
        }
    ...
    private void setCurrentCreativeTab(CreativeTabs par1CreativeTabs) {
        if (par1CreativeTabs == null) return;
        ...
        if (par1CreativeTabs == CreativeTabs.tabInventory) {
            ...
            if (this.backupContainerSlots == null) this.backupContainerSlots = containercreative.inventorySlots;

            containercreative.inventorySlots = new ArrayList<Slot>();
            for (int j = 0; j < container.inventorySlots.size(); ++j) {
                SlotCreativeInventoryTFC slotcreativeinventory = new SlotCreativeInventoryTFC(
                    this,
                    (Slot) container.inventorySlots.get(j),
                    j);
                ...
                } else if (j == 50)// 50 should be the back slot for armor
                {
                    ...
                    if (j >= 36) slotcreativeinventory.yDisplayPosition = 112;
                    else slotcreativeinventory.yDisplayPosition = 54 + i1 * 18;
                }
            }
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        if (this.zeroSlot != null && selectedTabIndex == CreativeTabs.tabInventory.getTabIndex()
            && this.func_146978_c/* isPointInRegion */(
                this.zeroSlot.xDisplayPosition,
                this.zeroSlot.yDisplayPosition,
                16,
                16,
                par1,
                par2)) {
    ...
    protected void renderToolTip(ItemStack par1ItemStack, int par2, int par3) {
        if (selectedTabIndex == CreativeTabs.tabAllSearch.getTabIndex()) {
            ...
            CreativeTabs creativetabs = par1ItemStack.getItem()
                .getCreativeTab();
            ...
            if (creativetabs == null && par1ItemStack.getItem() == Items.enchanted_book) {
                ...
                if (map.size() == 1) {
                    Enchantment enchantment = Enchantment.enchantmentsList[((Integer) map.keySet()
                        .iterator()
                        .next()).intValue()];
                    CreativeTabs[] acreativetabs = CreativeTabs.creativeTabArray;
            ...
            if (creativetabs != null) list.add(
                1,
                EnumChatFormatting.BOLD.toString() + EnumChatFormatting.BLUE
                    + I18n.format(creativetabs.getTranslatedTabLabel()));

            for (int i1 = 0; i1 < list.size(); ++i1) {
                if (i1 == 0) list.set(i1, par1ItemStack.getRarity().rarityColor + (String) list.get(i1));
                else list.set(i1, EnumChatFormatting.GRAY + (String) list.get(i1));
            }
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (creativetabs.shouldHidePlayerInventory()) this.drawTexturedModalRect(
            i1,
            k + (int) ((l - k - 17) * this.currentScroll),
        ...
        if (creativetabs.getTabPage() != tabPage)
            if (creativetabs != CreativeTabs.tabAllSearch && creativetabs != CreativeTabs.tabInventory) return;

        this.renderCreativeTab(creativetabs);

        if (creativetabs == CreativeTabs.tabInventory) GuiInventoryTFC.drawPlayerModel(
            this.guiLeft + 43,
            this.guiTop + 45,
            20,
            this.guiLeft + 43 - par2,
            this.guiTop + 45 - 30 - par3,
            this.mc.thePlayer);
    }
    ...
    protected boolean switchTab(CreativeTabs par1CreativeTabs, int par2, int par3) {
        ...
        if (k == 5) l = this.xSize - 28 + 2;
        else if (k > 0) l += k;

        int i1;

        if (par1CreativeTabs.isTabInFirstRow()) i1 = b0 - 32;
        else i1 = b0 + this.ySize;
    ...
    protected void renderCreativeTab(CreativeTabs par1CreativeTabs) {
        ...
        if (i == 5) l = this.guiLeft + this.xSize - 28;
        else if (i > 0) l += i;
    ...
    protected void actionPerformed(GuiButton par1GuiButton) {
        ...
        if (par1GuiButton.id == 1) this.mc.displayGuiScreen(new GuiStats(this, this.mc.thePlayer.getStatFileWriter()));

        if (par1GuiButton.id == 101) tabPage = Math.max(tabPage - 1, 0);
        else if (par1GuiButton.id == 102) tabPage = Math.min(tabPage + 1, maxPages);
    }
