...
public class GuiBarrel extends GuiContainerTFC {
    ...
    public static final ResourceLocation TEXTURE = new ResourceLocation(
        Reference.MOD_ID,
        Reference.ASSET_PATH_GUI + "gui_barrel.png");
    ...
    public void updateScreen() {
        ...
        if (barrelTE.getInvCount() > 0) {
            if (guiTab == 0) ((GuiButton) buttonList.get(4)).visible = false;// Turn off Liquid
            else if (guiTab == 1) ((GuiButton) buttonList.get(1)).visible = false;// Turn off Liquid
        } else {
            if (guiTab == 0) ((GuiButton) buttonList.get(4)).visible = true;// Turn on Liquid
            else if (guiTab == 1) ((GuiButton) buttonList.get(1)).visible = true;// Turn on Liquid
        }
        if (barrelTE.getFluidLevel() > 0) {
            if (guiTab == 0) ((GuiButton) buttonList.get(3)).visible = false;// Turn off Solid
            else if (guiTab == 1) ((GuiButton) buttonList.get(0)).visible = false;// Turn off Solid
        } else {
            if (guiTab == 0) ((GuiButton) buttonList.get(3)).visible = true;// Turn on Solid
            else if (guiTab == 1) ((GuiButton) buttonList.get(0)).visible = true;// Turn on Solid
        }
    ...
    public void createButtons() {
        buttonList.clear();
        if (guiTab == 0) {
            if (!barrelTE.getSealed()) buttonList
                .add(new GuiButton(0, guiLeft + 38, guiTop + 50, 50, 20, TFC_Core.translate("gui.Barrel.Seal")));
            else buttonList
                .add(new GuiButton(0, guiLeft + 38, guiTop + 50, 50, 20, TFC_Core.translate("gui.Barrel.Unseal")));
            buttonList.add(new GuiButton(1, guiLeft + 88, guiTop + 50, 50, 20, TFC_Core.translate("gui.Barrel.Empty")));
            if (barrelTE.mode == TEBarrel.MODE_IN) buttonList.add(
                new GuiBarrelTabButton(
                    2,
                    guiLeft + 39,
                    guiTop + 29,
                    16,
                    16,
                    this,
                    TFC_Core.translate("gui.Barrel.ToggleOn"),
                    0,
                    204,
                    16,
                    16));
            else if (barrelTE.mode == TEBarrel.MODE_OUT) buttonList.add(
                new GuiBarrelTabButton(
                    2,
                    guiLeft + 39,
                    guiTop + 29,
                    ...
                    16,
                    this,
                    TFC_Core.translate("gui.Barrel.ToggleOff"),
                    0,
                    188,
                    16,
                    16));
            buttonList.add(
                new GuiBarrelTabButton(
                    ...
                    15,
                    this,
                    TFC_Textures.guiSolidStorage,
                    TFC_Core.translate("gui.Barrel.Solid")));
            buttonList.add(
                new GuiBarrelTabButton(
                    4,
                    guiLeft + 5,
                    ...
                    15,
                    this,
                    TFC_Textures.guiLiquidStorage,
                    TFC_Core.translate("gui.Barrel.Liquid")));
            ...
            if (!barrelTE.getSealed()) buttonList
                .add(new GuiButton(2, guiLeft + 6, guiTop + 33, 44, 20, TFC_Core.translate("gui.Barrel.Seal")));
            else buttonList
                .add(new GuiButton(2, guiLeft + 6, guiTop + 33, 44, 20, TFC_Core.translate("gui.Barrel.Unseal")));
        }
    }
        ...
        public void drawButton(Minecraft par1Minecraft, int xPos, int yPos) {
            if (this.visible) {
                ...
                this.drawCenteredString(
                    fontrenderer,
                    barrelTE.mode == 0 ? TFC_Core.translate("gui.Barrel.ToggleOn")
                        : TFC_Core.translate("gui.Barrel.ToggleOff"),
                    this.xPosition + this.width / 2,
    ...
    public class GuiBarrelTabButton extends GuiButton {
        ...
        public GuiBarrelTabButton(int index, int xPos, int yPos, int width, int height, GuiBarrel gui, IIcon icon,
            String s) {
        ...
        public GuiBarrelTabButton(int index, int xPos, int yPos, int width, int height, GuiBarrel gui, String s, int xp,
            int yp, int xs, int ys) {
            super(index, xPos, yPos, width, height, s);
            screen = gui;
            this.xPos = xp;
            this.yPos = yp;
    ...
    protected void actionPerformed(GuiButton guibutton) {
        if (guiTab == 0) {
            if (guibutton.id == 0) {
                if (!barrelTE.getSealed()) barrelTE.actionSeal(0, player);
                else barrelTE.actionUnSeal(0, player);
            } else if (guibutton.id == 1) barrelTE.actionEmpty();
            ...
            if (guibutton.id == 1 && barrelTE.getInvCount() == 0) barrelTE.actionSwitchTab(0, player);
            else if (guibutton.id == 2) {
                if (!barrelTE.getSealed()) barrelTE.actionSeal(1, player);
                else barrelTE.actionUnSeal(1, player);
                createButtons();
            }
        }
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int mouseX, int mouseY) {
        ...
        if (guiTab == 0) {
            ...
            if (barrelTE != null && barrelTE.fluid != null) {
                ...
                IIcon liquidIcon = barrelTE.fluid.getFluid()
                    .getIcon(barrelTE.fluid);
                TFC_Core.bindTexture(TextureMap.locationBlocksTexture);
                int color = barrelTE.fluid.getFluid()
                    .getColor(barrelTE.fluid);
                GL11.glColor4ub(
                    (byte) ((color >> 16) & 255),
                    (byte) ((color >> 8) & 255),
                    (byte) (color & 255),
            ...
            // Draw Fluid Name
            if (barrelTE.getFluidStack() != null) drawCenteredString(
                this.fontRendererObj,
                barrelTE.fluid.getFluid()
                    .getLocalizedName(barrelTE.getFluidStack()),
                guiLeft + 88,
                guiTop + 7,
                0x555555);
            ...
            if (barrelTE.sealtime != 0) {
                drawCenteredString(
                    this.fontRendererObj,
                    TFC_Time.getDateStringFromHours(barrelTE.sealtime),
                    guiLeft + 88,
                    guiTop + 17,
                    0x555555);
            }
            ...
            if (barrelTE.recipe != null) {
                if (!(barrelTE.recipe instanceof BarrelBriningRecipe)) {
                    drawCenteredString(
                        this.fontRendererObj,
                        TFC_Core.translate("gui.Output") + ": " + barrelTE.recipe.getRecipeName(),
                        guiLeft + 88,
                        guiTop + 72,
                        0x555555);
                } else if (barrelTE.getSealed() && barrelTE.getFluidStack() != null
                    && barrelTE.getFluidStack()
                        .getFluid() == TFCFluids.BRINE) {
                            if (inStack != null && inStack.getItem() instanceof IFood
                                ...
                                drawCenteredString(
                                    this.fontRendererObj,
                                    TFC_Core.translate("gui.barrel.brining"),
                                    guiLeft + 88,
                                    guiTop + 72,
                                    0x555555);
                            }
                        }
            } else if (barrelTE.recipe == null && barrelTE.getSealed()
                && barrelTE.getFluidStack() != null
                && inStack != null
                && inStack.getItem() instanceof IFood
                && barrelTE.getFluidStack()
                    .getFluid() == TFCFluids.VINEGAR
                && !Food.isPickled(inStack)
                && Food.getWeight(inStack) / barrelTE.getFluidStack().amount
                    <= Global.FOOD_MAX_WEIGHT / barrelTE.getMaxLiquid()) {
                        if ((((IFood) inStack.getItem()).getFoodGroup() == EnumFoodGroup.Fruit
                            ...
                            drawCenteredString(
                                this.fontRendererObj,
                                TFC_Core.translate("gui.barrel.pickling"),
                                guiLeft + 88,
                                guiTop + 72,
                                0x555555);
                        }
                        ...
                        BarrelPreservativeRecipe preservative = BarrelManager.getInstance()
                            .findMatchingPreservativeRepice(
                                barrelTE,
                                inStack,
                                barrelTE.getFluidStack(),
                                barrelTE.getSealed());
                        if (preservative != null) {
                            drawCenteredString(
                                this.fontRendererObj,
                                TFC_Core.translate(preservative.getPreservingString()),
                                guiLeft + 88,
                                guiTop + 72,
                                0x555555);
                        }
