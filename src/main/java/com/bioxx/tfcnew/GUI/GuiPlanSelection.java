...
public class GuiPlanSelection extends GuiContainerTFC {
    ...
    public static ResourceLocation texture = new ResourceLocation(
        Reference.MOD_ID,
        Reference.ASSET_PATH_GUI + "gui_plan.png");
        ...
        for (Object[] o : plans) {
            ...
            buttonList.add(
                ...
                new GuiPlanButton(
                    ...
                    a.getCraftingResult(),
                    this,
                    TFC_Core.translate("gui.plans." + p)));
            index--;
            if (xOffset + 36 < xSize) xOffset += 18;
            else {
                xOffset = 5;
                yOffset += 18;
            }
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
        drawGui(texture);
        if (anvilTE.getStackInSlot(TEAnvil.INPUT1_SLOT) != null) drawCenteredString(
            this.fontRendererObj,
            "Plans: " + anvilTE.getStackInSlot(TEAnvil.INPUT1_SLOT)
                .getDisplayName(),
            guiLeft + xSize / 2,
            guiTop + 5,
            0x000000);
        ...
        for (Object p : plans) {
            AnvilRecipe ar = manager.findMatchingRecipe(
                new AnvilRecipe(
                    ...
                    anvilTE.anvilItemStacks[TEAnvil.INPUT2_SLOT],
                    (String) p,
                    AnvilReq.getReqFromInt(anvilTE.anvilTier),
                    null));

            ar = handleMatchingRecipe(ar);
            if (ar != null) planList.add(new Object[] { (String) p, ar });
        }
        return planList;
    ...
    public AnvilRecipe handleMatchingRecipe(AnvilRecipe ar) {
        if (ar != null) if (anvilTE.anvilItemStacks[TEAnvil.INPUT1_SLOT] != null
            && anvilTE.anvilItemStacks[TEAnvil.INPUT1_SLOT].getItem() == TFCItems.bloom
            && ar.getCraftingResult()
                .getItem() == TFCItems.bloom) {
                    if (anvilTE.anvilItemStacks[TEAnvil.INPUT1_SLOT].getItemDamage() <= 100) return null;
                }
        return ar;
    }
