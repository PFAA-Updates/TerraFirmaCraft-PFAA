...
public class ItemGrill extends ItemTerraBlock {
    ...
    public boolean onItemUse(ItemStack itemstack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (!world.isRemote) {
            if (side == 1 && world.isAirBlock(x, y + 1, z)) {
                ...
                if (hitX < 0.2) hinge = 0;
                else if (hitZ < 0.2) hinge = 1;
                else if (hitX > 0.8) hinge = 2;
                else if (hitZ > 0.8) hinge = 3;
                else hinge = 0; // Default
                ...
                } else return false; // don't delete misplaced Grill
    ...
    public boolean checkSides(World world, int x, int y, int z) {
        ...
        if (world.getBlock(x - 1, y, z)
            .isSideSolid(world, x - 1, y, z, ForgeDirection.WEST))// Check the East Block if the West Side is solid
        {
            count++;
        }
        if (world.getBlock(x + 1, y, z)
            .isSideSolid(world, x + 1, y, z, ForgeDirection.EAST))// Check the West Block if the East Side is solid
        {
            count++;
        }
        if (world.getBlock(x, y, z - 1)
            .isSideSolid(world, x, y, z - 1, ForgeDirection.SOUTH))// Check the North Block if the South Side is solid
        {
            count++;
        }
        if (world.getBlock(x, y, z + 1)
            .isSideSolid(world, x, y, z + 1, ForgeDirection.NORTH))// Check the South Block if the North Side is solid
        {
            count++;
        }
        return count >= 2;
