...
public class ItemSoil extends ItemTerraBlock {
    ...
    public void addInformation(ItemStack is, EntityPlayer player, List arraylist, boolean flag) {
        ...
        if (TFC_Core.isPeat(b) || TFC_Core.isPeatGrass(b)) return;

        int dam = is.getItemDamage();
        if (b == TFCBlocks.dirt2 || b == TFCBlocks.sand2
            || b == TFCBlocks.clay2
        ...
        if (dam < Global.STONE_ALL.length) arraylist.add(EnumChatFormatting.DARK_GRAY + Global.STONE_ALL[dam]);
        else arraylist.add(EnumChatFormatting.DARK_RED + "Unknown");
    }
    ...
    public IIcon getIconFromDamage(int damage) {
        ...
        } else return super.getIconFromDamage(damage);
    }
