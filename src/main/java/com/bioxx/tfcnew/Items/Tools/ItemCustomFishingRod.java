...
public class ItemCustomFishingRod extends ItemFishingRod implements ISize {
    ...
    public boolean onEntitySwing(EntityLivingBase entityLiving, ItemStack is) {
        if (is.stackTagCompound != null && is.stackTagCompound.hasKey("swing")
            && is.stackTagCompound.getBoolean("swing")) {
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer player) {
        if (is.stackTagCompound != null && is.stackTagCompound.hasKey("tickReeledIn")) {
            ...
            if (TFC_Time.getTotalTicks() <= tickReeledIn + 20) return is;
        }
    ...
    public void registerIcons(IIconRegister par1IconRegister) {
        ...
        for (int i = 0; i < castIconArray.length; ++i) castIconArray[i] = par1IconRegister
            .registerIcon(Reference.MOD_ID + ":" + this.getIconString() + "_cast_" + i);
        for (int i = 0; i < uncastIconArray.length; ++i) uncastIconArray[i] = par1IconRegister
            .registerIcon(Reference.MOD_ID + ":" + this.getIconString() + "_uncast_" + i);
    ...
    public IIcon getItemIconForUseDuration(int par1, boolean cast) {
        ...
        if (cast) return castIconArray[par1];
    ...
    public IIcon getIcon(ItemStack is, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
        boolean cast = player.fishEntity != null;// is.hasTagCompound() && is.stackTagCompound.hasKey("fishing") &&
                                                 // is.stackTagCompound.getBoolean("fishing");
        ...
        if (!cast) {
            ...
            return getItemIconForUseDuration(
                Math.min(originalTex + (texShift == 10 ? 1 : 0), castIconArray.length - 1),
                cast);
        }
    }
    ...
    public EnumAction getItemUseAction(ItemStack is) {
        if (is.stackTagCompound != null && is.stackTagCompound.hasKey("fishing")
            && is.stackTagCompound.getBoolean("fishing")) {
