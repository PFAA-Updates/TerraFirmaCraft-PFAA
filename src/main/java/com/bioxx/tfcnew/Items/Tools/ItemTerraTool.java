...
public class ItemTerraTool extends ItemTool implements ISize {
    ...
    public void addInformation(ItemStack is, EntityPlayer player, List arraylist, boolean flag) {
        ...
        if (is.getItem() instanceof ICausesDamage) arraylist.add(
            EnumChatFormatting.AQUA + TFC_Core.translate(
                ((ICausesDamage) this).getDamageType()
                    .toString()));
    ...
    public static void addSmithingBonusInformation(ItemStack is, List<String> arraylist) {
        if (AnvilManager.getDurabilityBuff(is) > 0) arraylist.add(
            TFC_Core.translate("gui.SmithingBonus") + " : +"
                + Helper.roundNumber(AnvilManager.getDurabilityBuff(is) * 100, 10)
                + "%");
    }
    ...
    public void registerIcons(IIconRegister registerer) {
        ...
        if (TFC_Textures.brokenItem == null)
            TFC_Textures.brokenItem = registerer.registerIcon(Reference.MOD_ID + ":" + "tools/Broken Item");
