        ...
        if (!world.isRemote && id != TFCBlocks.toolRack) {
            ...
            for (int i = 0; i < 36 && hasBowl == -1; i++) {
                if (entityplayer.inventory.mainInventory[i] != null
                    && entityplayer.inventory.mainInventory[i].getItem() == TFCItems.potteryBowl
                    && entityplayer.inventory.mainInventory[i].getItemDamage() == 1) hasBowl = i;
            }

            Material mat = world.getBlock(x, y, z)
                .getMaterial();

            if (side == 1 && id.isSideSolid(world, x, y, z, ForgeDirection.UP)
                && !TFC_Core.isSoil(id)
                && !TFC_Core.isWater(id)
