...
public class ItemUnfinishedArmor extends ItemTerra implements ISmeltable {
    ...
    public void addExtraInformation(ItemStack is, EntityPlayer player, List<String> arraylist) {
        if (is.getItemDamage() == 0) arraylist.add(TFC_Core.translate("gui.stage1"));
        else arraylist.add(TFC_Core.translate("gui.stage2"));
    }

    @Override
    public String getItemStackDisplayName(ItemStack itemstack) {
        return new StringBuilder().append(super.getItemStackDisplayName(itemstack))
            .toString();
    }
    ...
    public short getMetalReturnAmount(ItemStack is) {
        if (is.getItemDamage() == 1) return metalAmount2;
        return metalAmount;
    }
