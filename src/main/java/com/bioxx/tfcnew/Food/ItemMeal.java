...
public class ItemMeal extends ItemTerra implements IFood {
    ...
    public ItemMeal() {
        ...
        this.metaNames = new String[] { "Meal0", "Meal1", "Meal2", "Meal3", "Meal4", "Meal5", "Meal6", "Meal7", "Meal8",
            "Meal9", "Meal10", };
    ...
    protected void addFGInformation(ItemStack is, List<String> arraylist) {
        ...
        for (int i = 0; i < fg.length; i++) {
            if (fg[i] != -1) arraylist.add(localize(fg[i]));
        }
    }

    protected String localize(int id) {
        return ItemFoodTFC.getFoodGroupColor(
            FoodRegistry.getInstance()
                .getFoodGroup(id))
            + TFC_Core.translate(
                FoodRegistry.getInstance()
                    .getFood(id)
    ...
    public ItemStack onEaten(ItemStack is, World world, EntityPlayer player) {
        ...
        if (!world.isRemote) {
            ...
            if (is.hasTagCompound()) {
                ...
                for (int i = 0; i < fg.length; i++) {
                    if (fg[i] != -1) foodstats.addNutrition(
                        FoodRegistry.getInstance()
                            .getFoodGroup(fg[i]),
                        eatAmount * nWeights[i] * 2.5f);
                }
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer player) {
        ...
        if (foodstats.needFood()) player.setItemInUse(is, this.getMaxItemUseDuration(is));
