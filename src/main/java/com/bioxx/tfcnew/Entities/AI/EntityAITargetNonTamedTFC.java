...
public class EntityAITargetNonTamedTFC extends EntityAITargetNonTamed {
    ...
    public EntityAITargetNonTamedTFC(EntityTameable entity, Class targetClass, int targetChance,
    ...
    public boolean shouldExecute() {
        if (entityTameable instanceof IAnimal) {
            ...
            if (this.targetClass == EntityPlayer.class
                && animal.checkFamiliarity(InteractionEnum.TOLERATEPLAYER, null)) {
                return false;
            } else if (familiarity > 0 && this.taskOwner.getRNG()
                .nextInt(familiarity) != 0) return false; // The more familiar the animal, the less likely it will
                                                          // attack.
        }
