...
public class EntityHorseTFC extends EntityHorse implements IInvBasic, IAnimal {
    ...
    private static final IEntitySelector HORSE_BREEDING_SELECTOR = new com.bioxx.tfc.Entities.Mobs.EntityHorseBredSelector();
    private static final IAttribute HORSE_JUMP_STRENGTH = new RangedAttribute("horse.jumpStrengthTFC", 0.7D, 0.0D, 2.0D)
        .setDescription("Jump StrengthTFC")
        .setShouldWatch(true);
    ...
    public EntityHorseTFC(World par1World) {
        ...
        timeOfConception = 0;
        sex = rand.nextInt(2);
        sizeMod = (float) Math.sqrt(
            ((rand.nextInt(rand.nextInt((DEGREE_OF_DIVERSION + 1) * 10) + 1) * (rand.nextBoolean() ? 1 : -1)) * 0.01f
                + 1F) * (1.0F - DIMORPHISM * sex));
        strengthMod = (float) Math.sqrt(
            ((rand.nextInt(rand.nextInt(DEGREE_OF_DIVERSION * 10) + 1) * (rand.nextBoolean() ? 1 : -1)) * 0.01f
                + sizeMod));
        aggressionMod = (float) Math.sqrt(
            ((rand.nextInt(rand.nextInt(DEGREE_OF_DIVERSION * 10) + 1) * (rand.nextBoolean() ? 1 : -1)) * 0.01f + 1));
        obedienceMod = (float) Math.sqrt(
            ((rand.nextInt(rand.nextInt(DEGREE_OF_DIVERSION * 10) + 1) * (rand.nextBoolean() ? 1 : -1)) * 0.01f
                + 1f / aggressionMod));
        ...
        this.tasks.addTask(
            3,
            new EntityAIAvoidEntityTFC(this, com.bioxx.tfc.Entities.Mobs.EntityWolfTFC.class, 8f, 0.5F, 0.7F));
        this.tasks.addTask(
            3,
            new EntityAIAvoidEntityTFC(this, com.bioxx.tfc.Entities.Mobs.EntityBear.class, 16f, 0.25F, 0.3F));
         ...
         * We want spawned animals to be adults, so we set their birthdays far enough back in time such that they reach
         * adulthood now.
         */
    ...
    public EntityHorseTFC(World par1World, IAnimal mother, List<Float> data, int type, int variant) {
        ...
        aggressionMod = (float) Math
            .sqrt(aggressionMod * aggressionMod * (float) Math.sqrt((mother.getAggressionMod() + fatherAggro) * 0.5F));
    ...
    protected void applyEntityAttributes() {
        ...
        this.getAttributeMap()
            .registerAttribute(HORSE_JUMP_STRENGTH);
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(1250 * sizeMod * strengthMod);// MaxHealth
        this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
            .setBaseValue(0.22499999403953552D * strengthMod * obedienceMod / (sizeMod * sizeMod));
    ...
    public boolean canMateWith(IAnimal animal) {
        return animal.getGender() != this.getGender() && this.isAdult()
            && animal.isAdult()
    ...
    public void clearLeashed(boolean par1, boolean par2) {
        ...
        if (entity instanceof EntityPlayer) {
            ...
            if (entity.isSneaking()) super.clearLeashed(par1, true);
        } else {
    ...
    public EntityAgeable createChildTFC(EntityAgeable eAgeable) {
        ...
        double healthSum = this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .getBaseValue() + this.mateMaxHealth
            + this.calcMaxHealth();
        baby.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(healthSum / 3.0D);

        // Average Mom + Dad + Random Jump Strength
        double jumpSum = this.getEntityAttribute(HORSE_JUMP_STRENGTH)
            .getBaseValue() + this.mateJumpStrength
            + this.calcJumpStrength();
        baby.getEntityAttribute(HORSE_JUMP_STRENGTH)
            .setBaseValue(jumpSum / 3.0D);

        // Average Mom + Dad + Random Move Speed
        double speedSum = this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
            .getBaseValue() + this.mateMoveSpeed
            + this.calcMoveSpeed();
        baby.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
            .setBaseValue(speedSum / 3.0D);

        baby.setHealth(
            (float) baby.getEntityAttribute(SharedMonsterAttributes.maxHealth)
                .getBaseValue());
        return baby;
    }
    ...
    protected void entityInit() {
        ...
        this.dataWatcher.addObject(25, String.valueOf("0")); // Time of conception, stored as a string since we can't do
                                                             // long
    ...
    public void familiarize(EntityPlayer ep) {
        ...
        if ((this.riddenByEntity == null || !this.riddenByEntity.equals(ep)) && !familiarizedToday
            && stack != null
            && this.isFood(stack)
            && canFamiliarize()) {
            if (!ep.capabilities.isCreativeMode) {
                ep.inventory.setInventorySlotContents(
            ...
            this.getLookHelper()
                .setLookPositionWithEntity(ep, 0, 0);
    ...
    private boolean getBreedable() {
        return this.riddenByEntity == null && this.ridingEntity == null
            && this.isTame()
            && this.isAdultHorse()
            && !this.func_110222_cv()/* Not a Mule, Zombie or Skeleton */
    ...
    protected EntityHorseTFC getClosestHorse(Entity entity, double range) {
        ...
        List<Entity> list = this.worldObj.getEntitiesWithinAABBExcludingEntity(
            entity,
            entity.boundingBox.addCoord(range, range, range),
            HORSE_BREEDING_SELECTOR);
        Iterator<Entity> iterator = list.iterator();

        while (iterator.hasNext()) {
            EntityHorseTFC nearbyHorse = (EntityHorseTFC) iterator.next();
    ...
    public boolean getLeashed() {
        if (super.getLeashed() && getLeashedToEntity() instanceof EntityPlayer
            && ((EntityPlayer) getLeashedToEntity()).inventory.getCurrentItem() == null
            && func_110174_bM()/* maximumHomeDistance */ != -1) {
            return false;
        }
    ...
    public int increaseTemper(int temper) {
        ...
        int j = MathHelper
            .clamp_int(this.getTemper() + (int) (temper * obedienceMod * (1f / aggressionMod)), 0, this.getMaxTemper());
        this.setTemper(j);
        return j;
    }
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (itemstack != null && this.isBreedingItemTFC(itemstack)
            && checkFamiliarity(InteractionEnum.BREED, player)
            && this.getGrowingAge() == 0
            && !super.isInLove()
            && (this.familiarizedToday || !canFamiliarize())) {
            if (!player.capabilities.isCreativeMode) player.inventory.setInventorySlotContents(
                player.inventory.currentItem,
                ((ItemFoodTFC) itemstack.getItem()).onConsumedByEntity(player.getHeldItem(), worldObj, this));
            this.hunger += 24000;
            this.setInLove(true);
            return true;
        } else if (itemstack != null && itemstack.getItem() instanceof ItemCustomNameTag
            && itemstack.hasTagCompound()
            && itemstack.stackTagCompound.hasKey("ItemName")) {
                ...
                if (itemstack != null) {
                    ...
                    if (this.func_110229_cs()/* Donkey or Mule */ && !this.isChested()
                        && itemstack.getItem() == Item.getItemFromBlock(TFCBlocks.chest)) {
    ...
    public boolean isFood(ItemStack item) {
        return item != null && (item.getItem() == TFCItems.wheatGrain || item.getItem() == TFCItems.oatGrain
            || item.getItem() == TFCItems.riceGrain
            || item.getItem() == TFCItems.barleyGrain
            || item.getItem() == TFCItems.ryeGrain
            || item.getItem() == TFCItems.maizeEar);
    }
    ...
    public void mate(IAnimal otherAnimal) {
        ...
        mateMaxHealth = ((EntityLivingBase) otherAnimal).getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .getBaseValue();
        mateJumpStrength = ((EntityLivingBase) otherAnimal).getEntityAttribute(HORSE_JUMP_STRENGTH)
            .getBaseValue();
        mateMoveSpeed = ((EntityLivingBase) otherAnimal).getEntityAttribute(SharedMonsterAttributes.movementSpeed)
            .getBaseValue();
    }
    ...
    private void mountHorse(EntityPlayer player) {
        ...
        if (!this.worldObj.isRemote && checkFamiliarity(InteractionEnum.MOUNT, player)) player.mountEntity(this);
    }

    @Override
    public void onLivingUpdate() {
        // Handle Hunger ticking
        if (hunger > 168000) hunger = 168000;
        if (hunger > 0) hunger--;
        ...
        syncData();
        if (isAdult()) setGrowingAge(0);
        else setGrowingAge(-1);
    ...
    public void openGUI(EntityPlayer player) {
        Entity temp = player.ridingEntity; // Trick the player into thinking they are riding the horse so we can track
                                           // that specific horse
        ...
        if (temp == null) // Shift-right click while not riding to open inventory
        {
            ...
            if (player instanceof EntityPlayerMP) {
                ...
                playerMP.playerNetServerHandler.sendPacket(new S1BPacketEntityAttach(0, playerMP, this)); // Sets
                                                                                                          // ridingEntity
                                                                                                          // to this for
                                                                                                          // playerMP
                openHorseContainer(player);
                playerMP.playerNetServerHandler.sendPacket(new S1BPacketEntityAttach(0, playerMP, temp)); // Resets
                                                                                                          // ridingEntity
                                                                                                          // for
                                                                                                          // playerMP
            } else {
    ...
    public void updateChestSaddle() {
        ...
        if (animalchest != null) {
            ...
            for (int j = 0; j < i; ++j) {
                ...
                if (itemstack != null) this.horseChest.setInventorySlotContents(j, itemstack.copy());
            }
            animalchest = null;
        }
    ...
    private void updateSaddle() {
        if (!this.worldObj.isRemote) {
            ...
            if (this.getHorseType() == 0) this.func_146086_d(this.horseChest.getStackInSlot(1));
        }
    }
    ...
    public void writeEntityToNBT(NBTTagCompound nbttc) {
        ...
        if (this.horseChest.getStackInSlot(1) != null) {
            nbttc.setTag(
                "ArmorItem",
                this.horseChest.getStackInSlot(1)
        ...
        if (this.horseChest.getStackInSlot(0) != null) {
            nbttc.setTag(
                "SaddleItem",
                this.horseChest.getStackInSlot(0)
                    .writeToNBT(new NBTTagCompound()));
        }
    }
}
