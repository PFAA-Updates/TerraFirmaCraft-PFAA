...
public abstract class SyncingOption {
    ...
    public SyncingOption(String name, Class<?> clazz, Configuration cfg, String cat)
    ...
    public void apply(boolean enabled) throws IllegalAccessException {
        ...
        {
            boolean result = enabled ? CraftingManager.getInstance()
                .getRecipeList()
                .addAll(getRecipes())
                : CraftingManager.getInstance()
                    .getRecipeList()
                    .removeAll(getRecipes());
            if (TFCOptions.enableDebugMode) TerraFirmaCraft.LOG
