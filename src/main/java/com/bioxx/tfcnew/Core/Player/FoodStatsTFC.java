...
public class FoodStatsTFC {
    ...
    private boolean updateStats = !TFCOptions.enableDebugMode; // Replace with true to allow stat depleting with debug
                                                               // mode enabled
    ...
    private ResourceLocation wastedBlur = new ResourceLocation("shaders/post/blur.json");// new
                                                                                         // ResourceLocation("shaders/post/blurPhosphor.json");
    ...
    public void onUpdate(EntityPlayer player) {
        if (!player.worldObj.isRemote) {
            ...
            float temp = TFC_Climate
                .getHeightAdjustedTemp(player.worldObj, (int) player.posX, (int) player.posY, (int) player.posZ);

            float tempWaterMod = temp;
            if (tempWaterMod >= 25) tempWaterMod = (tempWaterMod - 25) * 0.2f;
            else tempWaterMod = 0;
            ...
            if (TFC_Time.getTotalTicks() - this.foodTimer >= TFC_Time.HOUR_LENGTH && !player.capabilities.isCreativeMode
                ...
                if (this.stomachLevel <= 0) {
                    ...
                    if (this.satVeg) this
                        .addNutrition(EnumFoodGroup.Vegetable, this.satisfaction * ((1 - this.nutrVeg) / 100), false);
                    if (this.satFruit)
            ...
            if (TFC_Time.getTotalTicks() - this.foodHealTimer >= TFC_Time.HOUR_LENGTH / 2) {
                ...
                /*
                 * else if (this.stomachLevel <= 0 && getNutritionHealthModifier() < 0.85f &&
                 * !TFC_Core.isPlayerInDebugMode(player) && player.getSleepTimer() == 0)
                 * {
            ...
            if (!player.capabilities.isCreativeMode && updateStats) {
                for (; waterTimer < TFC_Time.getTotalTicks(); waterTimer++) {
                    ...
                    if (!TFC_Core.isPlayerInDebugMode(player) && waterLevel == 0 && temp > 35) player.attackEntityFrom(
                        new DamageSource("heatStroke").setDamageBypassesArmor()
                            .setDamageIsAbsolute(),
                        2);
                }
            }
    ...
    public void clientUpdate() {
        if (Minecraft.getMinecraft().entityRenderer instanceof EntityRendererTFC) {
            ...
            if ((erTFC.getCurrentShaderLocation() == null || !erTFC.getCurrentShaderLocation()
                .equals(wastedBlur)) && soberTime > TFC_Time.getTotalTicks() + 8000) {
                erTFC.setManualShader(wastedBlur);
            } else if ((erTFC.getCurrentShaderLocation() == null || !erTFC.getCurrentShaderLocation()
                .equals(drunkBlur)) && soberTime > TFC_Time.getTotalTicks() + 4000
                && soberTime <= TFC_Time.getTotalTicks() + 8000) {
    ...
    public void setFoodLevel(float par1) {
        if (par1 != this.stomachLevel) sendUpdate = true;
    ...
    public void setSatisfaction(float par1, int[] fg) {
        ...
        for (int i = 0; i < fg.length; i++) {
            if (fg[i] != -1) {
                EnumFoodGroup efg = FoodRegistry.getInstance()
                    .getFoodGroup(fg[i]);
                switch (efg) {
                    case Protein:
                        satProtein = true;
    ...
    public long getPlayerFoodSeed() {
        if (nameSeed == Long.MIN_VALUE) {
            long seed = 0;
            byte[] nameBytes = player.getCommandSenderName()
                .getBytes();
            for (byte b : nameBytes) seed += b;
    ...
    /***
     * @return Returns an array containing taste preferences in the following order: Sweetness, Sourness, Saltiness,
     *         Bitterness, Savoriness
     */
    public int[] getPrefTaste() {
        Random r = new Random(getPlayerFoodSeed());
        return new int[] { 20 + r.nextInt(70), 20 + r.nextInt(70), 20 + r.nextInt(70), 20 + r.nextInt(70),
            20 + r.nextInt(70) };
    }
    ...
    public static boolean reduceFood(ItemStack is, float amount) {
        if (is.hasTagCompound()) {
            ...
            if (decay >= 0 && (weight - decay) - amount <= 0) return true;
            else if (decay <= 0 && weight - amount <= 0) return true;
            else {
    ...
    public void addNutrition(EnumFoodGroup fg, float foodAmt, boolean shouldDoMath) {
        ...
        if (shouldDoMath) amount = foodAmt / 5f / 50f;// converts it to 5% if it is 5oz of food
        switch (fg) {
            case Dairy:
    ...
    public void consumeAlcohol() {
        ...
        else soberTime += player.worldObj.rand.nextInt(1000) + 400;
        sendUpdate = true;
    }
}
