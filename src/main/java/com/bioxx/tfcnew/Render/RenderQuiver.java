...
public class RenderQuiver {
    ...
    private static final ResourceLocation QUIVER_TEXTURE = new ResourceLocation(
        Reference.MOD_ID,
    ...
    public void doRender(EntityLivingBase entity, ItemStack item) {
        ...
        if (item != null) {
            ...
            if (entity instanceof EntitySkeletonTFC) this.quiver.render(entity, 16);
            else this.quiver.render(entity, (((ItemQuiver) item.getItem()).getQuiverArrows(item)) / 8);
        }
        GL11.glPopMatrix();
    }
