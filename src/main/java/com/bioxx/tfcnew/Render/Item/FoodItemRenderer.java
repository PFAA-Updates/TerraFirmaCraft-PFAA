    ...
    public void renderItem(ItemRenderType type, ItemStack is, Object... data) {
        ...
        if (is.getItem() instanceof IFood && is.hasTagCompound()) {
            ...
            if (type == ItemRenderType.INVENTORY) {
                ...
                if (weightPerc <= 1) // Only draw bars if the weight is within the max weight. Food created using the
                                     // blank createTag (weight = 999) will not have the bars.
                {
                    if (((IFood) is.getItem()).renderDecay()) {
                        if (decayPerc < 0.10) {
                            decayTop = decayTop * 10;
                            renderQuad(1, 13, 13 - decayTop, 1, 0x00ff00);
                        } else renderQuad(1, 13, 13 - decayTop, 1, 0xff0000);
                    }
