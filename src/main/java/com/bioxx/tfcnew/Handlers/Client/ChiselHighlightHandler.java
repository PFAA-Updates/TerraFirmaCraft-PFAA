    ...
    public void drawBlockHighlightEvent(DrawBlockHighlightEvent evt) {
        ...
        if (evt.currentItem != null && evt.currentItem.getItem() instanceof ItemChisel) {
            ...
            if (hasHammer) {
                ...
                int mode = PlayerManagerTFC.getInstance()
                    .getClientPlayer().chiselMode;
                ...
                if (mode > -1) {
                    ...
                    ChiselManager.getInstance()
                        .setDivision(mode, target.sideHit);
                    int divX = ChiselManager.getInstance()
                        .getDivX(mode, id);
                    int divY = ChiselManager.getInstance()
                        .getDivY(mode, id);
                    int divZ = ChiselManager.getInstance()
                        .getDivZ(mode, id);
                    ...
                    if (divX > 0) {
                        ...
                        // drawOutlinedBoundingBox(AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(target.blockX+depth,target.blockY+depth,target.blockZ+depth,target.blockX+1-depth,target.blockY+1-depth,target.blockZ+1-depth).expand(0.002F,
                 ...
                 * int extraY2 = 1;
                 * int extraZ2 = 1;
                 * TileEntityPartial tep = (TileEntityPartial) world.getTileEntity(target.blockX, target.blockY,
                 * target.blockZ);
                 * if (tep != null)
                 * {
                 ...
                 * GL11.glDisable(GL11.GL_TEXTURE_2D);
                 * GL11.glDepthMask(true);
                 * drawOutlinedBoundingBox(AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(minX, minY, minZ, maxX,
                 * maxY, maxZ).expand(0.002F, 0.002F, 0.002F).getOffsetBoundingBox(-var8, -var10, -var12));
                 * GL11.glDepthMask(true);
                 * GL11.glEnable(GL11.GL_TEXTURE_2D);
                 * GL11.glDisable(GL11.GL_BLEND);
                 * }
