...
public class RenderOverlayHandler {
    ...
    public static ResourceLocation tfcicons = new ResourceLocation(
        Reference.MOD_ID,
        Reference.ASSET_PATH_GUI + "icons.png");
    private FontRenderer fontrenderer;
    public int recordTimer;
    private final Field _recordPlayingUpFor = ReflectionHelper
        .findField(GuiIngame.class, "recordPlayingUpFor", "field_73845_h");
    ...
    public void renderText(RenderGameOverlayEvent.Chat event) {
        ...
        if (Minecraft.getMinecraft().playerController.isInCreativeMode()) event.posY += 4;
        else event.posY -= 12;
    }
    ...
    public void render(RenderGameOverlayEvent.Pre event) {
        ...
        if (quiver instanceof ItemQuiver) {
            ...
            if (leftSide) {
                ...
                fontrenderer
                    .drawString(Integer.toString(getQuiverJavelins()), xPos + 19, yPos + 21, Color.white.getRGB());
            } else {
                ...
                fontrenderer.drawString(
                    Integer.toString(getQuiverArrows()),
                    xPos - arrowOffset,
                    yPos + 4,
                    Color.white.getRGB());
                fontrenderer.drawString(
                    Integer.toString(getQuiverJavelins()),
                    xPos - javOffset,
                    yPos + 21,
                    Color.white.getRGB());
            }
        ...
        if (mc.playerController.gameIsSurvivalOrAdventure()) {
            ...
            // Draw experience bar when not riding anything, riding a non-living entity such as a boat/minecart, or
            // riding a pig.
            ...
            if (player.ridingEntity instanceof EntityLivingBase) {
                ...
                double mountMaxHealth = mount.getEntityAttribute(SharedMonsterAttributes.maxHealth)
                    .getBaseValue();
                ...
                mc.fontRenderer.drawString(
                    mountHealthString,
                    mid + 47 - (mc.fontRenderer.getStringWidth(mountHealthString) / 2),
                    armorRowHeight + 2,
                    Color.white.getRGB());
    ...
    protected void renderDismountOverlay(Minecraft mc, int midpoint, int height, float partialTicks) {
        if (recordTimer == 0) {
            try {
                ...
                _recordPlayingUpFor.setInt(mc.ingameGUI, 0); // Reset the vanilla timer so the vanilla overlay doesn't
                                                             // appear
        ...
        if (recordTimer > 0) {
            ...
            if (opacity > 0) {
                try {
                    ...
                    mc.fontRenderer.drawString(
                        recordPlaying,
                        -mc.fontRenderer.getStringWidth(recordPlaying) / 2,
                        -12,
                        0xFFFFFF | (opacity << 24));
    ...
    public void renderText(RenderGameOverlayEvent.Text event) {
        ...
        if (mc.gameSettings.showDebugInfo || TFCOptions.enableDebugMode) {
            ...
            DataLayer evt = TFC_Climate.getCacheManager(mc.theWorld)
                .getEVTLayerAt(xCoord, zCoord);
            event.left.add(
                String.format(
                    "rain: %.0f, temp: %.2f, average bio temp: %.2f, evt: %.3f",
                    new Object[] { TFC_Climate.getRainfall(mc.theWorld, xCoord, yCoord, zCoord),
                        ...
                        TFC_Climate.getBioTemperatureHeight(mc.theWorld, xCoord, yCoord, zCoord), evt.floatdata1 }));

            if (TFCOptions.enableDebugMode) {
                event.left.add(
                    "Stability: " + TFC_Climate.getStability(mc.theWorld, xCoord, zCoord)
                        + ", Drainage: "
                        + TFC_Climate.getCacheManager(mc.theWorld)
                            .getDrainageLayerAt(xCoord, zCoord)
                            .getName()
                        + ", pH: "
                        + TFC_Climate.getCacheManager(mc.theWorld)
                            .getPHLayerAt(xCoord, zCoord)
                            .getName());
                event.left.add(
                    "Rock Layers: " + TFC_Climate.getCacheManager(mc.theWorld)
                        ...
                        + TFC_Climate.getCacheManager(mc.theWorld)
                            ...
                            .getName());
                event.left.add(
                    "Tree Layers: " + TFC_Climate.getCacheManager(mc.theWorld)
                        .getTreeLayerAt(xCoord, zCoord, 0)
                        .getName()
                        + ", "
                        + TFC_Climate.getCacheManager(mc.theWorld)
                            .getTreeLayerAt(xCoord, zCoord, 1)
                            .getName()
                        + ", "
                        + TFC_Climate.getCacheManager(mc.theWorld)
                            .getTreeLayerAt(xCoord, zCoord, 2)
                            .getName());
            }
        }
    }
