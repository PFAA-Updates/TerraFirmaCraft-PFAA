...
public class BlockBerryBush extends BlockTerraContainer {
    ...
    public BlockBerryBush() {
        ...
        metaNames = new String[] { "Wintergreen", "Blueberry", "Raspberry", "Strawberry", "Blackberry", "Bunchberry",
            "Cranberry", "Snowberry", "Elderberry", "Gooseberry", "Cloudberry" };
    ...
    public void getSubBlocks(Item item, CreativeTabs tabs, List list) {
        for (int i = 0; i < metaNames.length; i++) list.add(new ItemStack(item, 1, i));
    }
        ...
        if (!world.isRemote) {
            ...
            if (te != null && te.hasFruit) {
                ...
                dropBlockAsItem(
                    ...
                    y,
                    z,
                    ItemFoodTFC.createTag(fi.getOutput(), Helper.roundNumber(3 + world.rand.nextFloat() * 5, 10)));
            }
        }
    }
    ...
    private void lifeCycle(World world, int x, int y, int z) {
        if (!world.isRemote) {
            ...
            if (te instanceof TEBerryBush) tebb = (TEBerryBush) world.getTileEntity(x, y, z);
            if (tebb != null) {
                FloraIndex floraIndex = FloraManager.getInstance()
                    .findMatchingIndex(getType(world.getBlockMetadata(x, y, z)));
                float temp = TFC_Climate.getHeightAdjustedTemp(world, x, y, z);

                if (temp >= floraIndex.minTemp && temp < floraIndex.maxTemp) {
                    if (!tebb.hasFruit && floraIndex.inHarvest(TFC_Time.getSeasonAdjustedMonth(z))
                        && TFC_Time.getMonthsSinceDay(tebb.dayHarvested) > 0) {
                        tebb.hasFruit = true;
    ...
    public IIcon getIcon(IBlockAccess access, int x, int y, int z, int side) {
        ...
        if (te != null && te.hasFruit) return iconsBerries[meta];
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        ...
        if (meta == RASPBERRY || meta == BLACKBERRY) {
            if (entity instanceof EntityLivingBase) entity.attackEntityFrom(DamageSource.cactus, 5);
        }
    }
