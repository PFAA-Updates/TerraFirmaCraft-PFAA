...
public class BlockFruitLeaves extends BlockTerraContainer {
    ...
    public IIcon getIcon(int side, int meta) {
        if (TerraFirmaCraft.proxy.getGraphicsLevel()) return icons[meta & 7];
        else return iconsOpaque[meta & 7];
    }
    ...
    public void registerBlockIcons(IIconRegister iconRegisterer) {
        for (int i = 0; i < 9; i++) {
            ...
            // iconsDead[i] = iconRegisterer.registerIcon(Reference.ModID + ":" + "wood/fruit trees/" + WoodNames[i] + "
            // Leaves");
            // iconsDeadOpaque[i] = iconRegisterer.registerIcon(Reference.ModID + ":" + "wood/fruit trees/" +
            // WoodNames[i] + " Leaves Opaque");
            ...
            if (te != null) {
                if (fi2 != null) {
                    if (temp >= fi2.minTemp && temp < fi2.maxTemp) {
                        if (fi2.inHarvest(TFC_Time.getSeasonAdjustedMonth(z)) && !te.hasFruit
                ...
                if (rand.nextInt(100) > 50) world.markBlockForUpdate(x, y, z);
            }
        }
    }
            ...
            for (int j = 0; j >= -1; j--) {
                for (int k = 1; k >= -1; k--) {
                    if (world.getBlock(i + x, j + y, k + z) == TFCBlocks.fruitTreeWood
                        && world.getBlock(i + x, j + y + 1, k + z) != TFCBlocks.fruitTreeWood) // Only branches or the
                                                                                               // top of the trunk
                        return true;
                }
            }
    ...
    public void onBlockClicked(World world, int x, int y, int z, EntityPlayer entityplayer) {
        if (!world.isRemote) {
            ...
            if (fi != null && (fi.inHarvest(TFC_Time.getSeasonAdjustedMonth(z))
                || fi.inHarvest((TFC_Time.getSeasonAdjustedMonth(z) + 11) % 12) && (meta & 8) == 8)) {
                TEFruitLeaves te = (TEFruitLeaves) world.getTileEntity(x, y, z);
                if (te != null && te.hasFruit) {
                    ...
                    dropBlockAsItem(
                        ...
                        z,
                        ItemFoodTFC
                            .createTag(fi.getOutput(), Helper.roundNumber(4 + (world.rand.nextFloat() * 12), 10)));
                }
            }
        }
    }
