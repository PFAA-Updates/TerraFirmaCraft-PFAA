...
public class BlockCustomLeaves extends BlockLeaves implements IShearable {
    ...
    public boolean shouldSideBeRendered(IBlockAccess world, int x, int y, int z, int side) {
        ...
        else if (side == 1 && this.maxY < 1.0D) return true;
        else if (side == 2 && this.minZ > 0.0D) return true;
        else if (side == 3 && this.maxZ < 1.0D) return true;
        else if (side == 4 && this.minX > 0.0D) return true;
        else if (side == 5 && this.maxX < 1.0D) return true;
    ...
    public void onNeighborBlockChange(World world, int xOrig, int yOrig, int zOrig, Block b) {
        if (!world.isRemote) {
                ...
                for (int xd = -searchRadius; xd <= searchRadius; ++xd) {
                    ...
                    for (int yd = -searchY; yd <= searchY; ++yd) {
                        ...
                        for (int zd = -searchZ; zd <= searchZ; ++zd) {
                            ...
                            else this.adjacentTreeBlocks[xd + center][yd + center][zd + center] = -1;
                        }
                    }
                }
            ...
            if (res < 0) {
                if (world.getChunkFromBlockCoords(xOrig, zOrig) != null) this.destroyLeaves(world, xOrig, yOrig, zOrig);
            }
        }
    }
    ...
    private void removeLeaves(World world, int x, int y, int z) {
        ...
        if (world.rand.nextInt(100) < 30) dropBlockAsItem(world, x, y, z, new ItemStack(TFCItems.stick, 1));
        world.setBlockToAir(x, y, z);
    }
    ...
    public void harvestBlock(World world, EntityPlayer entityplayer, int i, int j, int k, int meta) {
        if (!world.isRemote) {
                        ...
                        for (int z = -1; z < 2; z++) {
                            for (int y = -1; y < 2; y++) {
                                if (world.getBlock(i + x, j + y, k + z)
                                    ...
                                    if (itemstack.stackSize == 0) entityplayer.inventory
            ...
            if (world.rand.nextInt(100) < 28) dropBlockAsItem(world, i, j, k, new ItemStack(TFCItems.stick, 1));
            else if (world.rand.nextInt(100) < 6 && TFCOptions.enableSaplingDrops) dropSapling(world, i, j, k, meta);
    ...
    public IIcon getIcon(int side, int meta) {
        if (meta > woodNames.length - 1) meta = 0;
        if (TerraFirmaCraft.proxy.getGraphicsLevel()) return this.icons[meta];
        else return this.iconsOpaque[meta];
    }
