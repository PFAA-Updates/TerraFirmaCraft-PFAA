...
public class BlockFarmland extends BlockContainer {
    ...
    public void registerBlockIcons(IIconRegister registerer) {
        ...
        for (int i = 0; i < count; i++) dirtTexture[i] = registerer
            .registerIcon(Reference.MOD_ID + ":" + "farmland/Farmland " + Global.STONE_ALL[i + textureOffset]);
    }
    ...
    public IIcon getIcon(IBlockAccess access, int x, int y, int z, int side) {
        ...
        if (meta < 0 || meta >= dirtTexture.length) meta = 0;
        if (side == 1)// top
            return dirtTexture[meta];
        else return this.dirtBlock.getIcon(side, meta);
    }
    ...
    public IIcon getIcon(int side, int meta) {
        if (meta < 0 || meta >= dirtTexture.length) meta = 0;
        if (side == ForgeDirection.UP.ordinal()) return dirtTexture[meta];
        else return this.dirtBlock.getIcon(0, meta);
    }
            ...
            for (int y = j; y <= j + 1; ++y) {
                for (int z = k - 4; z <= k + 4; ++z) {
                    if (world.blockExists(x, y, z)) {
                        ...
                        if (TFC_Core.isFreshWater(b)) return true;
                    }
                }
            }
        }
    ...
    public static boolean isSaltWaterNearby(World world, int i, int j, int k) {
        for (int x = i - 4; x <= i + 4; ++x) {
            for (int y = j; y <= j + 1; ++y) {
                for (int z = k - 4; z <= k + 4; ++z) {
                    Block b = world.getBlock(x, y, z);
                    if (TFC_Core.isSaltWater(b)) return true;
                }
            }
        }
        return false;
    ...
    public boolean canSustainPlant(IBlockAccess world, int x, int y, int z, ForgeDirection direction,
        ...
        if (plant == Blocks.pumpkin_stem || plant == Blocks.melon_stem) return false;
