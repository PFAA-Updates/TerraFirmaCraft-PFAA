...
public class BlockLiquidStatic extends BlockLiquid implements IFluidBlock {
        ...
        if (!world.isRemote) {
            ...
            if (world.getBlock(x, y, z) == TFCBlocks.freshWaterStationary && world.isAirBlock(x, y + 1, z)
                ...
                if (rand.nextInt(100) < 25 && world.getBlockLightValue(x, y, z) < 7) {
                    outerloop: for (int x1 = x - 3; x1 < x + 3; x1++) {
                        for (int z1 = z - 3; z1 < z + 3; z1++) {
                            if (TFC_Core.isGrass(world.getBlock(x1, y, z1))
                                || world.getBlock(x1, y - 1, z1) == TFCBlocks.waterPlant) {
            ...
            if (this.getMaterial() == Material.lava) {
                if (world.getBlock(x, y + 1, z)
                    ...
                    if (world.getBlock(i, j, k)
                        ...
                        } else if (world.isSideSolid(i, j + 1, k, ForgeDirection.DOWN)
                            || world.isSideSolid(i, j - 1, k, ForgeDirection.UP)
                            || world.isSideSolid(i - 1, j, k, ForgeDirection.EAST)
                            || world.isSideSolid(i + 1, j, k, ForgeDirection.WEST)
                            || world.isSideSolid(i, j, k + 1, ForgeDirection.NORTH)
                            || world.isSideSolid(i, j, k - 1, ForgeDirection.SOUTH)) {
    ...
    public void registerBlockIcons(IIconRegister registerer) {
        ...
        else icons = new IIcon[] { registerer.registerIcon("water_still"), registerer.registerIcon("water_flow") };
        this.getFluid()
            .setIcons(icons[0], icons[1]);
    }
