...
public class BlockSluice extends BlockContainer {
    ...
    public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int par6, float par7,
        ...
        if (world.isRemote) return true;
            ...
            if (!isBlockFootOfBed(meta)) if ((TESluice) world.getTileEntity(i, j, k) != null) {
                ...
                if (is != null && is.getItem() == TFCItems.goldPan && is.getItemDamage() != 0) {
                    ...
                    if (tileentitysluice.soilAmount > 50) tileentitysluice.soilAmount = 50;
                    is.setItemDamage(0);
                    entityplayer.inventory.setInventorySlotContents(entityplayer.inventory.currentItem, is);
                    /*
                     * The line below was uncommented before the smp inclusion. If this section isn't working it's
                     * because the above line never worked properly.
                     * Apparently I was doing the same thing twice, so I'm not sure if one of these wasn't working.
                     */
                    // mc.thePlayer.inventory.setInventorySlotContents(mc.thePlayer.inventory.currentItem, new
                    // ItemStack(mod_TFC_Core.terraGoldPan,1,0));
                    return true;
                }
    ...
    public IIcon getIcon(int side, int meta) {
        if ((meta & 4) != 0 && side == 1) return TFCFluids.SALTWATER.getFlowingIcon();
        else return TFCBlocks.woodSupportH.getIcon(side, 8);
    }
    ...
    public int colorMultiplier(IBlockAccess world, int x, int y, int z) {
        if ((world.getBlockMetadata(x, y, z) & 4) == 0) return 16777215;
    ...
    public Item getItemDropped(int i, Random random, int j) {
        if (!isBlockFootOfBed(i)) return TFCItems.sluiceItem;
        else return null;
    }
    ...
    public void onBlockPlacedBy(World world, int i, int j, int k, EntityLivingBase entityliving, ItemStack is) {
        ...
        if (world.getBlock(i, j, k) == this) world.setBlock(i + byte0, j, k + byte1, this, l + 8, 3);
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess par1IBlockAccess, int i, int j, int k) {
        ...
        if (BlockSluice.isBlockFootOfBed(meta)) setBlockBounds(0, 0, 0, 1, 0.5f, 1);
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int i, int j, int k) {
        int meta = world.getBlockMetadata(i, j, k);
        if (BlockSluice.isBlockFootOfBed(meta)) return AxisAlignedBB.getBoundingBox(i, j, k, i + 1, j + 0.5f, k + 1);
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        ...
        boolean stay = canStay(world, x, y, z, false, dir) && canStay(world, x + offset[0], y, z + offset[1], true, dir)
            && (block.isAir(world, x, y, z) || block.getMaterial()
                .isReplaceable());
    ...
    public boolean canPlace(World world, int i, int j, int k, int dir) {
        ...
        boolean stay = canStay(world, i, j, k, false, dir) && canStay(world, i + offset[0], j, k + offset[1], true, dir)
            && (topBlock.isAir(world, i, j, k) || topBlock.getMaterial()
                .isReplaceable())
            && (footBlock.isAir(world, i + offset[0], j, k + offset[1]) || footBlock.getMaterial()
                .isReplaceable());
    ...
    private boolean canStay(World world, int i, int j, int k, boolean foot, int dir) {
        int l = dir;
        if (l == 0) if (!foot && (!world.getBlock(i + 1, j, k)
            .isNormalCube()
            || !world.getBlock(i - 1, j, k)
                .isNormalCube()
            || !world.getBlock(i, j, k - 1)
                .isNormalCube()
            || !world.getBlock(i, j - 1, k)
                .isNormalCube()
            || world.getBlock(i, j + 2, k)
                .isNormalCube()))
            return false;
        else if (foot && (!world.getBlock(i + 1, j, k)
            .isNormalCube()
            || !world.getBlock(i - 1, j, k)
                .isNormalCube()
            || !world.getBlock(i, j - 1, k)
            ...
            || world.getBlock(i, j + 2, k)
                .isNormalCube()))
            return false;
        if (l == 1) if (!foot && (!world.getBlock(i, j, k + 1)
            .isNormalCube()
            || !world.getBlock(i, j, k - 1)
                .isNormalCube()
            || !world.getBlock(i + 1, j, k)
            ...
            || world.getBlock(i, j + 2, k)
                .isNormalCube()))
            return false;
        else if (foot && (!world.getBlock(i, j, k + 1)
            .isNormalCube()
            || !world.getBlock(i, j, k - 1)
                .isNormalCube()
            || !world.getBlock(i, j - 1, k)
            ...
            || world.getBlock(i, j + 2, k)
                .isNormalCube()))
            return false;
        if (l == 2) if (!foot && (!world.getBlock(i + 1, j, k)
            .isNormalCube()
            || !world.getBlock(i - 1, j, k)
                .isNormalCube()
            || !world.getBlock(i, j, k + 1)
            ...
            || world.getBlock(i, j + 2, k)
                .isNormalCube()))
            return false;
        if (l == 3) if (!foot && (!world.getBlock(i, j, k + 1)
            .isNormalCube()
            || !world.getBlock(i, j, k - 1)
                .isNormalCube()
            || !world.getBlock(i - 1, j, k)
    ...
    public void onNeighborBlockChange(World world, int i, int j, int k, Block block) {
        ...
        if (isBlockFootOfBed(i1)) {
            if (world.getBlock(i - HEAD_FOOT_BLOCKMAP[j1][0], j, k - HEAD_FOOT_BLOCKMAP[j1][1]) != this
                || !canStay(world, i, j, k, true, j1)) world.setBlockToAir(i, j, k);
        } else if (world.getBlock(i + HEAD_FOOT_BLOCKMAP[j1][0], j, k + HEAD_FOOT_BLOCKMAP[j1][1]) != this
            || !canStay(world, i, j, k, false, j1)) {
                world.setBlockToAir(i, j, k);
                if (!world.isRemote) dropBlockAsItem(world, i, j, k, i1, 0);
            }
    }
