...
public class BlockLeatherRack extends BlockTerraContainer {
    ...
    public BlockLeatherRack() {
        ...
        this.setBlockBounds(0, 0, 0, 1, 0.001f, 1); // Very flat, but still has a hitbox that can be highlighted with
                                                    // the cursor.
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityplayer, int side, float hitX,
        ...
        if (!world.isRemote && world.getTileEntity(x, y, z) instanceof TELeatherRack) // Instanceof check to avoid TE
                                                                                      // casting crashes.
        {
            TELeatherRack te = (TELeatherRack) world.getTileEntity(x, y, z);
            ItemStack equipped = entityplayer.getCurrentEquippedItem();

            if (te.workedArea != -1 && equipped != null && equipped.getItem() instanceof IKnife) // The player is trying
                                                                                                 // to scrape a piece
                                                                                                 // off the hide.
            {
                int coord = (int) Math.floor(hitX / 0.25f) + (int) Math.floor(hitZ / 0.25f) * 4; // Get location of spot
                                                                                                 // being scraped.
            ...
            } else // Any other situation where the block is clicked, but the player is not properly trying to scrape a
                   // hide.
            {
                world.setBlockToAir(x, y, z); // Spawns any existing hides from the block as entities, and destroys the
                                              // block and TE.
            }
        }

        if (!canBlockStay(world, x, y, z)) // For when players attempt to scrape a leather rack that should not exist
                                           // (cheated in, no log underneath, etc).
        {
            world.setBlockToAir(x, y, z);
        }

        return false; // Always return false because a block is not being placed? Not sure if this is correct logic.
                      // -Kitty
    }
    ...
    public boolean canBlockStay(World world, int x, int y, int z) {
        ...
        return world.getBlock(x, y - 1, z)
            .getMaterial() == Material.wood; // Leather racks without a log underneath should not exist.
    }

    @Override
    public void onNeighborBlockChange(World world, int i, int j, int k, Block par5) {
        super.onNeighborBlockChange(world, i, j, k, par5);
        if (!canBlockStay(world, i, j, k)) world.setBlock(i, j, k, Blocks.air, 0, 2);
    }

    @Override
    public void onBlockPreDestroy(World world, int i, int j, int k, int meta) {
        if (!world.isRemote && world.getTileEntity(i, j, k) instanceof TELeatherRack) // Instanceof check to avoid TE
                                                                                      // casting crashes.
        {
    ...
    public Item getItemDropped(int metadata, Random rand, int fortune) {
        return null; // The block itself has no item to drop, the hide stored in the leather rack is dropped/spawned
                     // elsewhere.
    }
