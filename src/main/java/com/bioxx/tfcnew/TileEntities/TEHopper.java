...
public class TEHopper extends NetworkTileEntity implements IHopper {
    ...
    public boolean isUseableByPlayer(EntityPlayer p) {
        return this.worldObj.getTileEntity(this.xCoord, this.yCoord, this.zCoord) != this ? false
            : p.getDistanceSq(this.xCoord + 0.5D, this.yCoord + 0.5D, this.zCoord + 0.5D) <= 64.0D;
    }
    ...
    public void updateEntity() {
        if (this.worldObj.isRemote) {
            if (pressCooldown > 0) --this.pressCooldown;
            else this.pressBlock = null;
            ...
            if (pressCooldown > 0) {
                --this.pressCooldown;
                if (pressCooldown % 20 == 0) press();
            } else if (pressCooldown == 0 && pressBlock != null) {
                for (int i = 0; i < storage.length; i++) {
                    if (storage[i] == null || ItemStack.areItemStacksEqual(storage[i], pressBlock)
                        && storage[i].stackSize < storage[i].getMaxStackSize()) {
                        if (storage[i] == null) storage[i] = pressBlock;
                        else storage[i].stackSize++;
            ...
            if (blockAbove != null && this.hasPressableItem() > 0) {
                if (pressBlock != null && !(blockAbove instanceof BlockCobble || blockAbove instanceof BlockGravel
                    || blockAbove instanceof BlockSand
                    || blockAbove instanceof BlockDirt)) {
    ...
    private void press() {
        ...
        if (item != null) {
            if (item.stackSize > 0) Food.setWeight(item, Food.getWeight(item) - 0.64f);// 0.64 per cycle leads to 250mB
                                                                                       // per stack of olives
     ...
     * else
     * {
     * EntityItem entityitem = searchForLooseInput(hopper.getWorldObj(), hopper.getXPos(), hopper.getYPos() + 1.0D,
     * hopper.getZPos());
     * if (entityitem != null)
     * {
     ...
     * {
     * return !inv.isItemValidForSlot(slot, is) ? false : !(inv instanceof ISidedInventory) ||
     * ((ISidedInventory)inv).canInsertItem(slot, is, side);
     * }
     */
     ...
     * return searchForOutputInventory(this.getWorldObj(), this.xCoord + Facing.offsetsXForSide[i], this.yCoord +
     * Facing.offsetsYForSide[i], this.zCoord + Facing.offsetsZForSide[i]);
     * }
     */

    public static IInventory getInputInventory(IHopper hopper) {
        return searchForOutputInventory(
            hopper.getWorldObj(),
            hopper.getXPos(),
            hopper.getYPos() + 1.0D,
            hopper.getZPos());
