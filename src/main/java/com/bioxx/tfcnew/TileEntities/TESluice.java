...
public class TESluice extends TileEntity implements IInventory {
    ...
    public ItemStack decrStackSize(int i, int j) {
        if (sluiceItemStacks[i] != null) {
            ...
            if (sluiceItemStacks[i].stackSize == 0) sluiceItemStacks[i] = null;
            return itemstack1;
        } else {
            return null;
        }
    ...
    private void ejectItem(ItemStack is) {
        ...
        EntityItem entityitem = new EntityItem(
            ...
            yCoord + f1,
            zCoord + f2,
            new ItemStack(is.getItem(), is.stackSize, is.getItemDamage()));
        float f3 = 0.05F;
    ...
    public int getFirstFreeSlot() {
        for (int i = 0; i < this.getSizeInventory(); i++) {
            if (this.getStackInSlot(i) == null) return i;
        }
        return -1;
    }
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            List list = worldObj.getEntitiesWithinAABB(
                EntityItem.class,
                AxisAlignedBB.getBoundingBox(xCoord, yCoord, zCoord, xCoord + 1, yCoord + 1.1f, zCoord + 1));

            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                EntityItem entity = (EntityItem) iterator.next();
                Item item = entity.getEntityItem()
                    .getItem();
                if (item == Item.getItemFromBlock(TFCBlocks.gravel) || item == Item.getItemFromBlock(TFCBlocks.gravel2)
                    || item == Item.getItemFromBlock(TFCBlocks.sand)
                    || item == Item.getItemFromBlock(TFCBlocks.sand2)) {
                    int stackSize = entity.getEntityItem().stackSize;
                    int accept = (50 + 19 - soilAmount) / 20;
                    if (stackSize <= accept) {
                        soilAmount += 20 * stackSize;
                        entity.setDead();
                        if (soilAmount > 50) soilAmount = 50;
                        if (item == Item.getItemFromBlock(TFCBlocks.gravel)
                            || item == Item.getItemFromBlock(TFCBlocks.gravel2)) soilType = 2;
                        else soilType = 1;
                    }
                }
            }
            ...
            if (soilAmount > 0 && waterInput && waterOutput) {
                ...
                ChunkData cd = TFC_Core.getCDM(worldObj)
                    .getData(xCoord >> 4, zCoord >> 4);
                ...
                while (processTimeRemaining > 100 && soilAmount > 0) {
                    float gemMod = 1;
                    float oreMod = 1;
                    if (soilType == 1) gemMod = 0.65f;
                    else if (soilType == 2) oreMod = 0.6f;

                    ArrayList<ItemStack> items = new ArrayList<ItemStack>();
                    if (random.nextInt((int) (200 * oreMod)) == 0 && !coreSampleStacks.isEmpty()) addToInventory(
                        coreSampleStacks.get(random.nextInt(coreSampleStacks.size()))
                            .copy());
                        ...
                        if (r == 0) addToInventory(new ItemStack(TFCItems.gemDiamond, 1, 3));
                        else if (r < 15) addToInventory(new ItemStack(TFCItems.gemDiamond, 1, 2));
                        else if (r < 25) addToInventory(new ItemStack(TFCItems.gemDiamond, 1, 1));
                        else if (r < 50) addToInventory(new ItemStack(TFCItems.gemDiamond, 1, 0));
                    }
                    cd.sluicedAmount++;
            ...
            if (soilAmount == 0) processTimeRemaining = 0;
        }
        /*********************************************************
         ********************* Server Only End
         *********************************************************/
        ...
        boolean isOutputAir = worldObj
            .isAirBlock(xCoord + dir.getOpposite().offsetX * 2, yCoord - 1, zCoord + dir.getOpposite().offsetZ * 2);
        boolean isOutputWater = TFC_Core.isWater(
            worldObj
                .getBlock(xCoord + dir.getOpposite().offsetX * 2, yCoord - 1, zCoord + dir.getOpposite().offsetZ * 2));
        boolean isWaterDepth7 = worldObj.getBlockMetadata(xCoord + dir.offsetX, yCoord + 1, zCoord + dir.offsetZ) == 7;
        int meta2 = worldObj
            .getBlockMetadata(xCoord + dir.getOpposite().offsetX, yCoord, zCoord + dir.getOpposite().offsetZ);
        if (isInputWater && isWaterDepth7 && !isFlowing && (isOutputAir || isOutputWater)) {
            ...
            if ((worldObj
                .getBlockMetadata(xCoord + dir.getOpposite().offsetX, yCoord, zCoord + dir.getOpposite().offsetZ) & 4)
                == 0) {
            ...
            // Set output water
            worldObj.setBlock(
                xCoord + dir.getOpposite().offsetX * 2,
                yCoord - 1,
                zCoord + dir.getOpposite().offsetZ * 2,
                water);
        }
        if ((!isInputWater || !isWaterDepth7 || !isOutputAir && !isOutputWater) && isFlowing) {
            ...
            // Set output water
            if (!isOutputAir && isOutputWater) worldObj.setBlockToAir(
                xCoord + dir.getOpposite().offsetX * 2,
                yCoord - 1,
                zCoord + dir.getOpposite().offsetZ * 2);
        }
