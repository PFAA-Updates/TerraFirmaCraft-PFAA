...
public class TEPottery extends NetworkTileEntity implements IInventory {
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote && wood == 8) {
            ...
            if (burnStart == 0 && isFireNear()) startPitFire();

            // Make sure to keep the fire going throughout the length of the burn
            if (blockAbove != Blocks.fire
                && TFC_Time.getTotalTicks() - burnStart <= TFC_Time.HOUR_LENGTH * TFCOptions.pitKilnBurnTime) {
                if ((blockAbove.isAir(worldObj, xCoord, yCoord + 1, zCoord)
                    || worldObj.getBlock(xCoord, yCoord + 1, zCoord)
                        .getMaterial()
                        .getCanBurn())
                    && isValid()) worldObj.setBlock(xCoord, yCoord + 1, zCoord, Blocks.fire);
                else {
                    wood = 0;
                    inventory[4] = null;
                    inventory[5] = null;
            ...
            if (blockAbove == Blocks.fire
                && TFC_Time.getTotalTicks() >= burnStart + (TFCOptions.pitKilnBurnTime * TFC_Time.HOUR_LENGTH)) {
                worldObj.setBlockToAir(xCoord, yCoord + 1, zCoord);
                if (inventory[0] != null) {
                    inventory[0] = KilnCraftingManager.getInstance()
                        .findCompleteRecipe(new KilnRecipe(inventory[0], 0))
                        .copy();
                    if (inventory[0].getItem() instanceof ItemPotteryBase) ((ItemPotteryBase) inventory[0].getItem())
                        .onDoneCooking(worldObj, inventory[0], Alloy.EnumTier.TierI);
                }
                if (inventory[1] != null) {
                    inventory[1] = KilnCraftingManager.getInstance()
                        .findCompleteRecipe(new KilnRecipe(inventory[1], 0))
                        .copy();
                    if (inventory[1].getItem() instanceof ItemPotteryBase) ((ItemPotteryBase) inventory[1].getItem())
                        .onDoneCooking(worldObj, inventory[1], Alloy.EnumTier.TierI);
                }
                if (inventory[2] != null) {
                    inventory[2] = KilnCraftingManager.getInstance()
                        .findCompleteRecipe(new KilnRecipe(inventory[2], 0))
                        .copy();
                    if (inventory[2].getItem() instanceof ItemPotteryBase) ((ItemPotteryBase) inventory[2].getItem())
                        .onDoneCooking(worldObj, inventory[2], Alloy.EnumTier.TierI);
                }
                if (inventory[3] != null) {
                    inventory[3] = KilnCraftingManager.getInstance()
                        .findCompleteRecipe(new KilnRecipe(inventory[3], 0))
                        .copy();
                    if (inventory[3].getItem() instanceof ItemPotteryBase) ((ItemPotteryBase) inventory[3].getItem())
                        .onDoneCooking(worldObj, inventory[3], Alloy.EnumTier.TierI);
                }
    ...
    private Boolean isFireNear() {
        ...
        for (int x = -1; x <= 1; x++) {
            for (int z = -1; z <= 1; z++) {
                if (worldObj.blockExists(xCoord + x, yCoord + 1, zCoord + z)
                    && worldObj.getBlock(xCoord + x, yCoord + 1, zCoord + z) == Blocks.fire) foundFire = true;
            }
        }
    ...
    public boolean isLit() {
        return TFC_Time.getTotalTicks() > burnStart
            && TFC_Time.getTotalTicks() - burnStart < TFC_Time.HOUR_LENGTH * TFCOptions.pitKilnBurnTime;
    }
        ...
        if (inventory[0] == null && inventory[1] == null && inventory[2] == null && inventory[3] == null) {
            ...
            if (m > 0) {
                entityitem = new EntityItem(
                    ...
                    yCoord + 0.5,
                    zCoord + 0.5,
                    new ItemStack(TFCItems.straw, m));
                entityitem.lifespan = 48000;
                worldObj.spawnEntityInWorld(entityitem);
            }
    ...
    public void createInitNBT(NBTTagCompound nbt) {
        /*
         * Normally this would be the wrong approach to sending init data. However for the pottery since we have to sync
         * a few items
