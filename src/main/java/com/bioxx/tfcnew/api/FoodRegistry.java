...
public class FoodRegistry {
    ...
    public Item getFood(int id) {
        if (proteinMap.containsKey(id)) return proteinMap.get(id);
        if (vegetableMap.containsKey(id)) return vegetableMap.get(id);
        if (fruitMap.containsKey(id)) return fruitMap.get(id);
        if (grainMap.containsKey(id)) return grainMap.get(id);
        if (dairyMap.containsKey(id)) return dairyMap.get(id);
        return null;
    }

    public EnumFoodGroup getFoodGroup(int id) {
        if (proteinMap.containsKey(id)) return EnumFoodGroup.Protein;
        if (vegetableMap.containsKey(id)) return EnumFoodGroup.Vegetable;
        if (fruitMap.containsKey(id)) return EnumFoodGroup.Fruit;
        if (grainMap.containsKey(id)) return EnumFoodGroup.Grain;
        if (dairyMap.containsKey(id)) return EnumFoodGroup.Dairy;
