...
public class TEFireEntity extends NetworkTileEntity {
    ...
    public void receiveAirFromBellows() {
        if (airFromBellows < AIRTOADD * 3) airFromBellows += AIRTOADD;
        if (airFromBellows > AIRTOADD * 3) airFromBellows = AIRTOADD * 3;
    }

    public void keepTempToRange() {
        if (fireTemp > getMaxTemp()) fireTemp = getMaxTemp();
        else if (fireTemp < 0) fireTemp = 0;
    }
    ...
    protected float handleTemp() {
        if (fuelTimeLeft > 0) {
            fuelTimeLeft--;
            if (airFromBellows > 0) fuelTimeLeft--;
        } else if (fuelTimeLeft < 0) fuelTimeLeft = 0;

        if (fuelTimeLeft > 0) return fuelBurnTemp + airFromBellows;
        else return 0;
    }

    public void handleAirReduction() {
        if (airFromBellows > 0) airFromBellows--;
    }

    public void handleTempFlux(float desiredTemp) {
        if (fireTemp < desiredTemp) {
            if (airFromBellows == 0) fireTemp++;
            else fireTemp += 2;
        } else if (fireTemp > desiredTemp) {
            if (desiredTemp == 0) {
                if (airFromBellows == 0) fireTemp -= 1;
                else fireTemp -= 0.5;
            }
        }
