...
public class BarrelLiquidToLiquidRecipe extends BarrelRecipe {
    ...
    public Boolean matches(ItemStack item, FluidStack fluid) {
        ...
        if (recipeFluid != null && recipeFluid.isFluidEqual(fluid)
            && itemLiquid != null
            && itemLiquid.isFluidEqual(inputfluid)) {
            // Make sure that when we combine the liquids that there is enough room in the barrel for the new liquid to
            // fit
            if (10000 - fluid.amount < itemLiquid.amount) return false;
    ...
    public Stack<ItemStack> getResult(ItemStack inIS, FluidStack inFS, int sealedTime) {
        ...
        if (inIS != null) result.push(
            inIS.getItem()
                .getContainerItem(inIS));
